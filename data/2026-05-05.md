...
TL;DR: 本文提出SDRS框架，将合成数据生成从开环随机采样转变为闭环实验设计。通过析因设计和ANOVA分解，系统审计下游模型的因子敏感性，识别覆盖失败（Type I）和虚假依赖（Type II）两类缺陷，并针对性地生成合成数据来修复。实验证明该方法在多个任务上显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有合成数据pipeline采用开环范式（open-loop），将合成数据视为廉价真实数据的替代品，随机采样生成器输出空间，期望覆盖下游模型的失败模式。这从根本上误用了合成数据最独特的性质——场景因子可控且独立变化的能力。论文旨在将统计实验设计（DoE）引入合成数据生成，实现闭环（closed-loop）的、以模型需求为导向的合成数据策略。

Method: 提出SDRS（Synthetic Designed Experiments for Representational Sufficiency）框架。将下游模型视为黑盒系统，合成生成器视为实验装置。采用分数析因设计（fractional factorial designs）高效探索因子空间，通过ANOVA分解审计模型的因子敏感性轮廓。将模型失败分类为两类：Type I gap（对某些因子水平覆盖不足）和Type II gap（依赖虚假的混杂因素）。基于审计结果针对性生成合成数据。还提出因子间不变性惩罚可传递敏感性的转移现象。

Result: 三项实验验证：(1) dSprites控制诊断实验，植入偏差后审计正确识别两类gap，针对性数据将准确率从49.9%提升至79.0%；(2) 程序化场景稠密分割任务，检测到背景复杂度捷径，针对性数据将mIoU从0.948提升至0.998；(3) 纠缠检测实验，ANOVA审计能识别不完美生成器中因子间的交叉污染。此外发现因子间不变性惩罚可导致敏感性转移。

Conclusion: 将统计实验设计引入合成数据生成是有效且实用的范式转变。SDRS能够系统诊断并修复下游模型的因子级缺陷，远超随机采样的效率。同时揭示了因子间敏感性转移这一开放问题，为表示级修正提出了新挑战。
...
TL;DR: 提出了一种融合四阶非线性PDE与物理雾霾形成模型的混合图像去雾方法，通过暗通道先验估计大气参数并引导四阶PDE演化，在有效抑制雾霾的同时保持结构细节，实验验证了该方法在视觉质量和细节保留上的竞争力。


<details>
  <summary>Details</summary>
Motivation: 真实场景中图像去雾面临场景深度未知、大气散射不确定以及缺乏真实参考图像的挑战，是一个典型的逆问题。现有方法在这些条件下难以兼顾去雾效果与结构细节保持。

Method: 采用混合去雾模型：1) 利用暗通道先验估计大气参数并生成引导图像；2) 基于物理雾霾形成模型，演化四阶电报型PDE，包含边缘自适应扩散系数和由透射图加权的保真项；3) 使用相对误差范数作为PDE收敛判据。

Result: 与暗通道先验、改进暗通道先验及基于变分的单图去雾方法相比，该方法提供了可比的视觉质量，并能有效保持结构细节。有参考时使用MSE和SSIM评估，无参考时使用FADE、对比度恢复指数、平均梯度和熵进行评估。

Conclusion: 提出的混合PDE去雾方法在视觉质量和结构细节保持方面具有竞争力，四阶扩散有效抑制雾霾，双曲型公式改善了数值稳定性和收敛行为。
...
TL;DR: 提出生理因果探针（PCP）新范式及Interv-rPPG框架，通过主动干预视频色度分量验证rPPG假设的物理真实性，有效抵抗运动与光照噪声，提升跨域泛化性能。


<details>
  <summary>Details</summary>
Motivation: 现有自监督rPPG方法陷入相关陷阱，倾向于学习数据中占主导的高能周期信号（如运动或照明噪声），而非微弱的真实rPPG信号，导致模型泛化能力差。

Method: 提出PCP范式，核心是从被动相关学习转向主动精确干预：基于rPPG假设对视频进行干预，验证干预后变化是否符合物理预期。实现框架Interv-rPPG包含两个组件：PhysMambaFormer（rPPG提取器，提出假设）和可控生理信号编辑器（基于假设在色度域进行精确干预）。通过"归零可证伪性"和"公理等变性"验证假设的物理真实性。编辑器通过干预视频的低频色度分量实现精确编辑。

Result: 在VIPL-HR和MMPD等挑战性数据集上，域内和跨域性能均得到提升；在复杂跨数据集设置中超越有监督基线；在干净数据集上因干预机制引入轻微残余色度噪声而保持竞争力。诊断分析表明PCP有效抵抗运动与光照伪影。

Conclusion: PCP范式通过因果干预而非被动相关学习，有效解决了自监督rPPG中的相关陷阱问题，显著提升了模型在复杂场景下的泛化能力和抗干扰能力。
...
TL;DR: 本文对换脸技术进行了全面的综述和基准测试，将现有方法归类为五大范式，并提出了一个高质量基准数据集CASIA FaceSwapping及标准化评估协议，通过大量实验揭示了当前技术的性能特征与局限性。


<details>
  <summary>Details</summary>
Motivation: 现有换脸方法在不同范式间碎片化严重，缺乏标准化数据集和评估协议导致评价高度不一致，同时先前综述多聚焦于广义的深度伪造生成或检测，未将换脸作为独立问题深入研究。

Method: 1) 对现有换脸方法进行结构化综述，划分为五种主要范式并系统分析其设计原则、优势与局限；2) 构建了CASIA FaceSwapping高质量基准数据集，具有均衡的人口统计分布和明确的属性变化；3) 建立标准化评估协议，对代表性方法进行广泛实验。

Result: 通过对代表性方法的广泛实验，获得了对当前换脸技术性能特征和局限性的新见解。

Conclusion: 本文提供了一个统一的视角和原则化的评估框架，以促进更鲁棒、更可控的换脸方法的发展。
...
TL;DR: LiteVLA-H是一个仅256M参数的紧凑型VLA系统，专为无人机在边缘设备(NVIDIA Jetson AGX Orin)上部署设计，通过双速率调度实现快速动作输出（50.65ms, 19.74Hz）和慢速语义输出（149.90-164.57ms），并采用知识保留微调策略避免语义能力退化。


<details>
  <summary>Details</summary>
Motivation: VLA模型在机械臂操作中展现了强大的语义基础和任务泛化能力，但空中部署（无人机）面临严格的机载计算和通信约束，需要低延迟的闭环引导，现有VLA模型难以满足这一需求。

Method: 提出LiteVLA-H，一个256M参数的紧凑VLA系统，在NVIDIA Jetson AGX Orin上运行。核心发现是紧凑边缘场景下端到端延迟主要由多模态预填充主导而非解码额外token的边际成本，据此设计调度器实现快速外环引导模式(50.65ms)和慢速语义模式(149.90-164.57ms)的双速率操作。使用知识保留微调方法混合反应式飞行数据、空中语义数据和通用描述/VQA监督来专门化模型。

Result: 动作分支在边缘推理速率上超过AnywhereVLA、FutureVLA和ReMem-VLA等最新架构，达到50.65ms(19.74Hz)的动作延迟和149.90-164.57ms(6.08-6.67Hz)的语义输出，同时保留周期性语义感知能力。

Conclusion: LiteVLA-H证明了紧凑VLA模型在无人机边缘部署中的可行性，通过利用多模态预填充主导延迟的特性设计调度策略，以及知识保留微调方法，实现了快速反应式控制与语义理解的双重能力平衡。
...
TL;DR: 本文提出基于选择性相关性的知识蒸馏（SCKD）框架，用于从可穿戴鞋垫传感器数据中估计地面反作用力（GRF），通过提取时序特征的相关性图进行知识迁移，在保持高精度的同时降低计算资源需求，实现了便携式、资源高效的步态分析。


<details>
  <summary>Details</summary>
Motivation: 传统GRF测量依赖实验室内的测力台，设备昂贵且不便携；可穿戴鞋垫传感器虽便携但易受噪声干扰导致精度低；深度学习方法精度高但计算量大，难以在便携设备上实现实时分析。因此需要一种既轻量又准确的GRF估计方法。

Method: 提出SCKD（Selective Correlation Based Knowledge Distillation）框架，利用时序特征选择性提取相关性图进行知识蒸馏，将大型教师模型的知识迁移至轻量学生模型，增强可解释性并缓解高维数据处理问题。

Result: 在多种行走速度和时间窗口设置下，与现有方法相比，SCKD生成的紧凑模型在从鞋垫传感器数据估计GRF时表现更优，验证了方法的有效性。

Conclusion: SCKD为可穿戴步态分析提供了一种可靠且资源高效的解决方案，在便携设备上实现了高精度GRF估计，具有在医疗、康复、运动等领域应用的潜力。
...
### [7] [Skeleton-Based Posture Classification to Promote Safer Walker-Assisted Gait in Older Adults](https://arxiv.org/abs/2605.00890)
*Sergio D. Sierra M.,Monica Sinha,Marcela Múnera,Carlos A. Cifuentes*

Main category: cs.CV

TL;DR: 本研究评估了多种机器学习模型（几何方法、XGBoost、SVM及深度学习架构）在智能助行器场景下对步态分类（助行器选择、站坐识别、姿态分类）的效果，其中XGBoost和几何方法表现最佳，XGBoost在二分类任务中达到近99%的训练准确率。


<details>
  <summary>Details</summary>
Motivation: 老年人跌倒是严重的公共卫生问题，本研究旨在通过机器学习提升智能助行器的人机交互能力，以实现有效的跌倒预防。

Method: 对比了几何方法、XGBoost、SVM以及多种深度学习架构（包括4层CNN和编解码器CNN），分别用于助行器选择、站坐识别和姿态分类任务。

Result: XGBoost在二分类任务中表现最佳：助行器选择准确率99.84%，站坐识别99.69%；姿态分类中几何方法对8种姿态达89.9%，XGBoost对17种姿态达99.24%（训练阶段）；深度学习模型在二分类中准确率超98%。

Conclusion: 机器学习方法（特别是XGBoost和几何方法）在智能助行器的步态分类中表现优异，具有增强人机交互和跌倒预防的潜力。
...
### [8] [X2SAM: Any Segmentation in Images and Videos](https://arxiv.org/abs/2605.00891)
*Hao Wang,Limeng Qiao,Chi Zhang,Lin Ma,Guanglu Wan,Xiangyuan Lan,Xiaodan Liang*

Main category: cs.CV

TL;DR: X2SAM是一个统一的分割多模态大语言模型，将图像和视频的任意分割能力统一到单一框架中，通过LLM与Mask Memory模块结合，支持文本和视觉提示下的多种分割任务，并引入视频视觉定位(V-VGD)基准评估模型性能。


<details>
  <summary>Details</summary>
Motivation: 现有MLLMs具备图像级理解但像素级感知有限；SAM等分割模型依赖低级视觉提示无法理解复杂指令；现有分割MLLMs通常专门针对图像或视频，不支持统一的文本和视觉提示接口。

Method: 将LLM与Mask Memory模块耦合，后者存储引导视觉特征以实现时间一致性的视频掩码生成；采用统一的联合训练策略，在异构图像和视频数据集上协同训练。

Result: X2SAM在视频分割任务上表现强劲，在图像分割基准上保持竞争力，并保留通用的图像和视频对话能力。

Conclusion: 统一架构能有效桥接图像与视频分割，单一框架支持多种分割任务和交互方式，同时保持对话能力。
...
### [9] [When To Adapt? Adapting the Model or Data in Federated Medical Imaging](https://arxiv.org/abs/2605.00892)
*Chamani Shiranthika,Parvaneh Saeedi*

Main category: cs.CV

TL;DR: 本文系统比较了联邦医学影像中两类主流域适应策略——数据端协调（harmonization）与模型端个性化（personalization），发现在六种医学影像任务中，协调策略在表观差异主导的域偏移（如CXR分类）中更有效，而个性化策略在结构差异主导的域偏移（如结肠息肉分割）中表现更好；当域间差异较小时两者性能相近，为联邦医学影像中的适应策略选择提供了实用指南。


<details>
  <summary>Details</summary>
Motivation: 联邦学习中客户端间的域异质性严重限制模型性能，现有两类主流策略（模型侧个性化和数据侧协调）虽被广泛使用，但缺乏系统的比较研究，导致实际应用中难以选择合适策略。

Method: 在六种医学影像设置（结肠息肉、皮肤病变、乳腺肿瘤分割，以及结核CXR、脑肿瘤、乳腺肿瘤分类）中，涵盖多种域偏移类型，在一个统一框架下系统评估了多种最先进的协调和个性化方法。

Result: 发现一个条件性权衡：当域变异主要基于表观时协调更有效，当差异是结构性时个性化表现更好；当客户端间变异有限时两种策略性能相似。

Conclusion: 联邦医学影像中适应策略的有效性取决于域偏移的类型和大小，而非仅取决于策略本身；提供了策略选择的实用指南，并指出未来混合范式的发展方向。
...
### [10] [Retrieval-Guided Generation for Safer Histopathology Image Captioning](https://arxiv.org/abs/2605.00893)
...
TL;DR: 本文提出检索引导生成（RGG）用于病理图像描述，通过汇总视觉相似病例的专家文本而非从头生成，在ARCH数据集上语义对齐（余弦相似度~0.60 vs MedGemma的~0.47）和病理学术语保留方面优于纯生成模型，同时揭示了概念混合、继承过度标注等失败模式，具有更高的透明度和可审计性。


<details>
  <summary>Details</summary>
Motivation: 生成式视觉-语言模型在医学图像描述中容易出现幻觉、过度诊断和事实不一致问题，这在病理学领域尤为严重，因此需要更安全可靠的替代方案。

Method: 采用检索引导生成（RGG）方法，即检索与输入图像视觉相似病例的专家文本，对检索到的文本进行汇总形成最终描述，而非从零生成。

Result: 在ARCH组织病理学数据集上，RGG的余弦相似度约0.60，显著优于MedGemma的约0.47，且置信区间不重叠。病理学家的定性评估显示RGG更好地保留了形态学术语，减少了无支持的诊断，但也发现了概念混合和继承过度标注等失败模式。

Conclusion: 检索引导生成作为一种更透明、可靠且便于审计的方法，相较于完全生成式方法在病理图像描述中具有显著优势。
...
### [11] [When Less Is More: Simplicity Beats Complexity for Physics-Constrained InSAR Phase Unwrapping](https://arxiv.org/abs/2605.00896)
*Prabhjot Singh,Manmeet Singh*

Main category: cs.CV

TL;DR: 本文通过大规模消融实验证明，在InSAR相位解缠这一受物理约束的地球物理回归任务中，简单的U-Net（7.76M参数）在精度（R²=0.834, RMSE=1.01cm）和推理速度（2.92ms）上全面超越基于注意力机制的复杂模型（11.37M参数），揭示了"复杂度惩罚"现象：注意力机制会注入非物理的高频伪影，违背弹性形变的光滑性约束。


<details>
  <summary>Details</summary>
Motivation: InSAR相位解缠是火山和地震监测中的计算瓶颈，当前行业趋势盲目采用高复杂度计算机视觉架构（如注意力机制），但未验证其在受物理约束的地球物理回归任务中的适用性。本文旨在填补这一空白，探究简单架构是否比复杂模型更适用于此类平滑场回归问题。

Method: 在LiCSAR基准数据集上（20个帧、39,724个图块、6.51亿像素）进行首次大规模架构消融研究。通过对比vanilla U-Net（7.76M参数）与基于注意力的模型（11.37M参数）的性能，并结合功率谱密度（PSD）分析从物理角度解释差异。最后评估推理延迟以满足早期预警系统亚100ms的实时性要求。

Result: Vanilla U-Net以7.76M参数取得R²=0.834、RMSE=1.01cm，在R²上比注意力模型高34%、RMSE上低51%。PSD分析显示注意力模型在>0.3 cycles/pixel频率段注入非物理的高频伪影，违反了弹性形变的光滑性约束。U-Net推理延迟2.92ms，是唯一满足亚100ms实时性要求的候选模型，相比注意力模型实现2.5倍加速。

Conclusion: 对于平滑场回归任务，卷积的局部性优于现代复杂架构，倡导在遥感机器学习中采用物理引导的简洁设计。该工作弥合了"发表到实践"的鸿沟，为InSAR相位解缠等地球物理应用提供了可实际部署的高效方案。
...
### [12] [LatentDiff: Scaling Semantic Dataset Comparison to Millions of Images](https://arxiv.org/abs/2605.00899)
*James Flora,Kowshik Thopalli,Akshay R. Kulkarni,Weng-Keen Wong,Shusen Liu*

Main category: cs.CV

TL;DR: 提出LatentDiff框架，在预训练视觉编码器的潜空间中进行语义数据集比较，通过稀疏自编码器散度检验和密度比估计，以更低计算成本识别可解释的语义差异；同时引入Noisy-Diff基准测试，实验表明LatentDiff在少至<1%的图像存在语义差异时仍保持高精度。


<details>
  <summary>Details</summary>
Motivation: 现有基于标题的语义数据集比较方法计算成本高昂，且难以应对真实世界中稀疏分布偏移的场景，亟需一种更高效、鲁棒的语义差异检测方法。

Method: LatentDiff结合稀疏自编码器散度检验与密度比估计，直接在预训练视觉编码器的潜空间中操作，无需依赖文本标题；引入Noisy-Diff基准测试用于评估稀疏分布偏移下的性能。

Result: LatentDiff在语义差异检测精度上优于现有方法，且对仅5%到<1%图像存在语义差异的极端稀疏设置保持鲁棒性，计算成本显著低于基于标题的替代方法。

Conclusion: 在潜空间中进行语义数据集比较是高效且可解释的路径，LatentDiff为大规模数据集分析提供了实用工具。
...
### [13] [Validation of Whole-Slide Foundation Models for Image Retrieval in TCGA Data](https://arxiv.org/abs/2605.00902)
*Tianhao Lei,Parsa Esmaeilkhani,Saghir Alfasly,Wataru Uegami,Judy C. Boughey,Matthew P. Goetz,Krishna R. Kalari,H. R. Tizhoosh*

Main category: cs.CV

TL;DR: 本文在TCGA的9,387张全切片图像上系统比较了十种检索流程（包括四种全切片基础模型、基于注意力的多实例学习聚合器以及五种采样密度下的块级检索），发现性能差异主要源于器官和诊断类型而非架构选择。TITAN模型表现最佳但优势微弱，ABMIL和基于块的方法在Top-1和Top-3准确率上相当。性能主要由块级特征表示驱动，聚合带来的收益有限。最佳模型在TCGA上的检索准确率仅约68%±21%，部分亚型在所有方法上准确率为0%，揭示了形态学表示的固有局限性。


<details>
  <summary>Details</summary>
Motivation: 基础模型在计算病理学中正在重塑格局，但它们在整张切片图像检索中的价值相对于强基线（基于块的检索和监督聚合方法）仍不明确，需要系统性基准测试。

Method: 在TCGA 9,387张诊断切片（涵盖17个器官、60种诊断）上，采用留一患者留出评估，比较了四种预训练的全切片基础模型、基于注意力的多实例学习（ABMIL）聚合器以及五种采样密度下的块级检索共十种流程。

Result: 性能差异主要来自器官和诊断类型而非架构；TITAN整体最佳但优势微弱；ABMIL和块级方法在Top-1和Top-3准确率上相当；无模型持续占优；形态学上独特的实体接近天花板性能，而罕见、异质和密切相关的亚型仍具挑战；误分类与已知存在观察者间差异的器官一致；性能主要由块级特征驱动，聚合收益有限；最佳模型仅约68%±21%准确率，部分亚型准确率为0%。

Conclusion: 不存在普遍最优的架构，应支持按器官的基准测试、诊断感知或集成策略、更强的特征表示以及多模态检索框架。形态学表示的局限性要求取得实质性进展后才能实现可靠的临床部署。
...
### [14] [Generalized Category Discovery under Domain Shifts: From Vision to Vision-Language Models](https://arxiv.org/abs/2605.00906)
*Hongjun Wang,Po Hu,Kai Han*

Main category: cs.CV

TL;DR: 本文研究域偏移下的广义类别发现（GCD）问题，提出三种适配不同基础模型的方法：HiLo（自监督视觉模型）、HLPrompt（HiLo+语义感知空间提示）、VLPrompt（视觉-语言模型+因子化文本提示与跨模态一致性正则化），在合成噪声和真实多域偏移场景中均优于现有基线。


<details>
  <summary>Details</summary>
Motivation: 现有GCD方法假设数据来自单一域，但真实无标签数据常同时存在域偏移和语义偏移，亟需研究域偏移下的GCD问题。

Method: 提出三种框架：(i) HiLo通过多级特征提取和互信息最小化解耦域与语义特征，结合PatchMix增强和课程采样；(ii) HLPrompt在HiLo基础上引入语义感知空间提示微调抑制背景和域噪声；(iii) VLPrompt利用视觉-语言模型，采用因子化文本提示和跨模态一致性正则化。三者共享核心设计但适配不同基础模型。

Result: 在合成噪声和真实多域偏移数据集上，三种方法均持续优于强基线方法。

Conclusion: 解耦域与语义特征并适配不同基础模型是应对域偏移下GCD的有效策略，不同框架适用于不同部署场景。
...
TL;DR: TRIP-Evaluate是一个面向交通运输领域大模型的多模态开放基准测试，包含837个按角色-任务-知识分类的文本、图像和点云评测项，支持从整体精度到具体故障模式的细粒度诊断，揭示了大模型在工程计算、规则推理和多模态场景理解等方面的不足。


<details>
  <summary>Details</summary>
Motivation: 现有通用基准无法验证大模型在交通运输领域正确应用法规、执行工程计算或理解交通场景的能力，而少量公开的交通基准范围狭窄且缺乏对文本、图像、点云数据的细粒度诊断支持。

Method: 构建了包含837个评测项的TRIP-Evaluate基准，采用角色-任务-知识分类体系覆盖车辆、交通管理、出行者及规划与设计功能；每个项目标注能力、模态和难度标签；标准化了项目构建、质量控制、提示、解码和评分流程。

Result: 当前版本包含596个文本项、198个图像项和43个点云项。多模型评测显示文本性能在提升，但在多步工程计算、规则约束推理、多模态场景理解和点云理解方面仍存在显著不足。
...
TL;DR: 本文提出一个面向工业RAG系统的OCR基准测试（InduOCRBench），涵盖11种复杂文档类型。评估发现，现有SOTA OCR模型在工业场景下性能明显下降，且高OCR字符准确率并不保证良好的下游RAG效果，结构和语义错误会导致检索和生成失败。


<details>
  <summary>Details</summary>
Motivation: 现有OCR基准依赖字符级指标（如WER/CER），无法充分衡量真实工业条件下RAG系统的实际效果，亟需一个更贴近工业RAG应用场景的OCR评估基准。

Method: 构建涵盖11种挑战性文档类型的基准（包括极端布局、高分辨率页面、复杂/水印背景、非标准阅读顺序的历史文档、视觉装饰文本、表格和数学公式等），在受控的OCR-first RAG流水线下评估近期SOTA OCR模型。

Result: SOTA模型在工业文档上性能显著退化；高字符级准确率不必然带来强RAG性能；结构性和语义错误即使在WER/CER较低时也会导致检索失败；这种不匹配具有类别依赖性，在检索和生成两侧均有体现，且在不同OCR-first流水线选择上保持稳定。

Conclusion: 现有OCR评估指标不足以反映工业RAG系统的真实需求，需要更关注结构和语义保真度的评估方法。
...
TL;DR: 本文提出了一种面向对象的分析流程，用于解析地理定位模型所依赖的视觉证据。通过将归因图分解为可解释的对象级元素，实验表明归因引导的图像裁剪比随机裁剪更能保留模型预测信息，为理解模型决策提供了新的视角。


<details>
  <summary>Details</summary>
Motivation: 现有归因方法（如Grad-CAM）只能生成模糊的区域热力图，难以将模型预测与具体对象或可感知的模式联系起来，因此需要一种方法将归因图分解为可解释、可感知的实体单元。

Method: 提出对象中心的分析流程：从归因图提取显著区域，将其分割为类似对象的元素；通过删除和插入测试评估这些元素的预测相关性，并将归因引导的裁剪与具有相似覆盖面积的随机裁剪进行比较。

Result: 在三国家基准数据集上的实验表明，归因引导的裁剪始终比随机裁剪保留更多模型预测所需的信息。

Conclusion: 归因图可以分解为可解释、可感知的元素，这为地理定位模型的对象级分析迈出了重要一步。
...
TL;DR: SAMamba3D是一个参数高效框架，通过将冻结的SAM编码器与Mamba体积上下文建模和跨尺度特征交互结合，实现了通用化的三维孔隙尺度图像分割，在不同岩石类型、流体模式和扫描条件下均达到或超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有三维孔隙尺度图像分割方法通常针对特定数据集，当岩石类型、流体模式、扫描仪或采集条件变化时需要重新训练或大量微调，缺乏泛化能力。

Method: 提出SAMamba3D框架，核心包括：1) 保持大部分SAM编码器参数冻结以适应2D边界先验；2) 结合基于Mamba的体积上下文建模；3) 渐进式跨尺度特征交互机制，实现通用化三维分割。

Result: 在砂岩和碳酸盐岩数据集（不同流体、润湿性和扫描条件下），SAMamba3D匹配或超越了当前三维基线方法，同时减少了对特定案例重新训练的需求。

Conclusion: SAMamba3D能够生成保持物理意义描述符（流体饱和度、连通性、界面形态）的分割图像，实现更可靠、快速的大规模三维多相图像分析。
...
TL;DR: WildTableBench是一个针对真实世界表格图像的多模态问答基准，包含402张高信息密度表格图像和928个手动标注问题。评估21个前沿模型发现仅一个模型准确率超过50%，其余模型介于4.1%-49.9%，诊断分析揭示模型在结构感知和推理方面存在持续弱点。


<details>
  <summary>Details</summary>
Motivation: 现有表格理解评估多基于结构化文本或干净渲染图像，缺乏对真实世界表格图像视觉复杂性的探索。

Method: 构建WildTableBench基准，包含从在线论坛和网站收集的402张真实表格图像及928个涵盖5大类17个子类型的手工标注问题，评估21个前沿多模态基础模型。

Result: 仅一个模型准确率超过50%，其余模型介于4.1%-49.9%；诊断分析揭示模型在结构感知和推理方面存在持续弱点。

Conclusion: WildTableBench可作为表格图像理解的有价值诊断基准，为当前模型能力提供洞察。
...
TL;DR: 本文提出一种物理感知的人体运动生成框架，通过显式建模人体-物体、人体-场景及内部身体动力学等全谱系作用力，并引入连续距离力模型来泛化接触建模至任意表面，在动态场景中生成物理合理的人体运动，显著提升了物理合理性与泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有方法因接触建模有限（通常局限于手部）而生成物理不真实的运动，缺乏对人-物、人-场景及内部身体动力学的全面建模。

Method: 提出物理感知的人体运动生成框架，显式建模全谱系人体相关作用力；引入软物理约束以维持力与力矩平衡；提出连续距离力模型，将接触建模泛化至任意表面，支持静态环境与动态物体的交互。

Result: 实验表明该方法显著提升了物理合理性，对复杂场景具有良好泛化性，树立了物理一致性人体运动生成的新基准。

Conclusion: 通过全面建模作用力与新型连续距离接触模型，有效解决了动态场景下物理合理的人体运动合成问题，优于现有方法。
...
TL;DR: 本文提出WILD和WILD SAM两种方法，通过伪标签去噪和混合训练策略，显著提升恶劣天气下自动驾驶目标检测器的性能，在Four Seasons数据集上AP提升最高达13%。


<details>
  <summary>Details</summary>
Motivation: 现有目标检测器在恶劣天气下性能严重下降，造成自动驾驶安全关键性的领域偏移问题。现有方法依赖合成数据训练，限制了真实世界适用性；而伪标签方法因恶劣天气下生成的标签噪声大，尚未被天气领域自适应方法采用。

Method: 1) WILD (Weather-Induced pseudo Label Denoising) 框架：对恶劣天气下真实数据生成的噪声伪标签进行过滤去噪。2) WILD SAM：提出混合训练方法，结合伪标签去噪与基于仿真的训练方案，同时利用目标恶劣天气域的真实数据。

Result: 在Four Seasons数据集（雨雪场景）上验证，所提框架相较于基线提升平均精度(AP)最高达13%，显著缩小了天气引起的性能差距。

Conclusion: WILD和WILD SAM有效解决了恶劣天气下的领域漂移问题，通过伪标签去噪与混合训练策略提升了检测器的真实世界适用性。
...
TL;DR: 提出OsteoOpt++，一种基于图像到决策规划循环的个性化下颌骨重建方法，通过贝叶斯优化搜索六个临床可控变量来最大化供体-下颌骨对合度，在通用病例中对合度提升最高达29个百分点（329%相对提升），在患者特异性病例中提升达26个百分点。


<details>
  <summary>Details</summary>
Motivation: 血管化骨移植下颌骨重建存在供体-宿主骨不愈合问题，当前虚拟手术规划仅生成几何方案而非主动促进骨愈合的构型。

Method: 术前CT通过模板-患者配准和CT衍生更新构建个性化数字孪生，采用期望改进加获取规则的贝叶斯优化，搜索六个临床可控的切割平面和供体定位变量，目标函数基于对合度驱动并引入安全因子正则化变体。

Result: 在3个通用病例中，相比常规手术方法，周期平均供体-下颌骨对合度提升最高29个百分点（329%相对提升）；在3+1例患者特异性病例中，相比术后第5天构型提升最高26个百分点。11个建模参数的10%敏感性分析显示目标函数变化不超过3-4%，纵向病例显示预测对合度与一年期骨形成之间的Dice重叠度为0.70和0.76。

Conclusion: 为外科医生提供术前、图像驱动的切割平面方向和供体放置推荐，预测可改善手术室当前配置的愈合条件。
...
TL;DR: 提出Disciplined Diffusion (DDiffusion)，一种通过语义检索和局部编辑来防御文本到图像扩散模型中NSFW内容生成的方法，避免传统二值过滤器的对抗攻击和高误报问题。


<details>
  <summary>Details</summary>
Motivation: 现有安全过滤器依赖文本分类器或图像检查器进行二值化的允许/阻止决策，容易受到对抗性攻击（如关键词替换绕过检测），且高误报率损害良性用户体验。

Method: DDiffusion包含两个核心组件：1) 语义检索机制，通过概念分布而非脆弱的成对相似性评估提示词；2) 扩散过程中的定位方法，选择性地编辑生成图像中的有害区域，而非整体屏蔽。

Result: 返回局部消毒后的图像，抑制恶意内容的同时保留良性提示的生成保真度，避免暴露二值化允许/拒绝信号。

Conclusion: DDiffusion通过非二值化的局部编辑策略，有效防御NSFW内容生成，同时提升对对抗攻击的鲁棒性和良性用户体验。
...
TL;DR: 本文提出 ScribbleEdit，一个大规模合成数据集，通过结合自然语言指令和自由手绘涂鸦来弥合现有图像编辑模型在空间精度与语义细节控制上的差距。实验表明，在该数据集上微调扩散模型和自回归多模态模型，能显著提升其对抽象涂鸦输入的编辑能力。


<details>
  <summary>Details</summary>
Motivation: 现有图像编辑模型难以同时理解用户提供的精确空间布局（涂鸦）和具体语义细节（文本指令），因为缺乏同时包含这两种模态的专业训练数据。

Method: 通过合成pipeline自动生成源-目标图像对（基于inpainting），并配以人工绘制的涂鸦和VLM生成的文本指令，构建ScribbleEdit大规模合成数据集；在该数据集上微调扩散模型和自回归统一多模态图像编辑模型。

Result: 现成模型难以处理抽象涂鸦输入，而在ScribbleEdit上微调后，模型生成空间对齐且语义一致的编辑结果的能力显著提升。

Conclusion: ScribbleEdit数据集能有效弥合涂鸦空间控制与文本语义控制之间的鸿沟，提升模型对多模态联合输入的编辑能力。
...
TL;DR: SCOUT是一个上下文感知的概念融合框架，用于病理学报告生成，通过渐进式条件化图像表示，整合局部组织模式、全切片上下文和专家语义描述，在多个数据集上达到了最优的BLEU和METEOR分数。


<details>
  <summary>Details</summary>
Motivation: 现有病理学基础模型虽然能够生成流畅的报告，但缺乏临床基础，无法准确反映病理学家观察到的关键诊断概念及其关系，主要原因在于难以整合从细胞级细粒度模式到切片级组织结构再到高级诊断概念的异质性视觉证据，同时保持可解释性和临床一致性。

Method: 提出SCOUT框架，包含深度感知的上下文调制和自适应多模态融合机制。该方法在统一学习范式中整合局部组织学模式、全切片上下文和专家精选的语义描述符，使视觉特征在编码过程中被动态优化，并在文本生成阶段保持不同表征尺度间的互补性。

Result: 使用CONCH1.5特征，在TCGA-BRCA、MICCAI REG和HistAI三个数据集上，SCOUT在所有数据集上均取得了最优的BLEU-1到BLEU-4以及METEOR分数，并在TCGA-BRCA和MICCAI REG上取得了最优ROUGE-L分数。在TCGA-BRCA上达到BLEU-1/2/3/4为0.436/0.303/0.202/0.156，METEOR为0.204；在REG 2025上分别达到0.865/0.834/0.805/0.780和0.568。

Conclusion: 渐进式上下文条件化策略能够有效提升有基础依据的病理学报告生成质量，证明了多尺度语义融合在此任务中的有效性。
...
TL;DR: 本文提出了一种基于对比学习的零样本动作识别（ZSAR）方法，通过联合嵌入空间对齐视频与自然语言描述，并设计自动负采样程序增强训练数据，在UCF-101和Kinetics-400数据集上取得了最先进的结果。


<details>
  <summary>Details</summary>
Motivation: 零样本动作识别中存在语义鸿沟（文本域与视觉域表征不一致）和域偏移（训练集与测试集分布差异）两个核心问题，现有联合嵌入空间方法虽有前景但性能仍有提升空间。

Method: 1) 构建视频与句子的联合嵌入空间，通过对比学习对齐视频与对应的自然语言描述；2) 设计自动负采样程序，生成视觉外观与不相关描述的未配对数据以扩充训练集。

Result: 在UCF-101和Kinetics-400数据集上，多种分割配置下均达到最先进水平。

Conclusion: 基于对比学习的联合嵌入空间结合自动负采样策略能有效缓解语义鸿沟和域偏移问题，显著提升零样本动作识别性能。
...
TL;DR: CADFit是一个基于混合优化的CAD重建框架，能够从网格等几何输入中恢复复杂、可编辑的CAD构造序列，通过IoU驱动的优化和丰富的操作集（拉伸、旋转、圆角、倒角）实现，在多个基准上优于现有方法，并支持从图像端到端重建。


<details>
  <summary>Details</summary>
Motivation: 现有CAD重建方法大多局限于难以编辑的格式（如网格、B-rep）或仅支持简单的草图-拉伸流水线和低复杂度数据集，缺少能够从网格恢复复杂、可编辑参数化CAD构造序列的有效方法。

Method: 将CAD重建建模为IoU驱动的结构化CAD程序优化问题，通过增量式拟合和验证参数化操作，利用几何反馈指导优化过程，支持拉伸、旋转、圆角和倒角等多种操作。

Result: 在多个CAD基准上，CADFit在体素交并比和倒角距离上优于现有最先进的网格到CAD方法，并显著降低了重建CAD程序的无效比率，尤其对复杂设计效果显著。
...
TL;DR: 本文提出基于条件分数扩散模型（SBDM）从幅度图像合成相位图，生成k空间数据用于训练深度学习MRI重建模型，解决了因原始k空间数据难以获取导致训练数据不足的问题。实验表明该方法在定量指标和重建保真度上优于平滑相位和GAN等方法。


<details>
  <summary>Details</summary>
Motivation: 临床MRI中原始k空间数据因存储和隐私问题被丢弃，仅保留幅度图像，导致基于深度学习的MRI重建模型训练数据严重不足，阻碍了可泛化重建模型的发展。

Method: 提出基于条件分数扩散模型（SBDM）的生成方法，从幅度图像合成对应的相位图，再与幅度图像结合生成完整的k空间数据，用于训练下游的深度学习MRI加速重建模型。

Result: 基于SBDM合成k空间数据训练的深度学习模型在定量指标和重建保真度上优于使用平滑相位和GAN生成相位图的方法，更接近使用真实k空间数据训练的结果。

Conclusion: 条件分数扩散模型能够从幅度图像生成逼真的相位图，为利用大规模幅度图像数据库训练深度学习MRI重建模型提供了可行方案，有望扩大训练数据规模并提升模型泛化能力。
...
TL;DR: 提出非对称可逆人脸保护框架(ARFP)，通过密钥条件流形绑定、对抗性恢复感知训练和授权可逆恢复三个组件，在保护人脸隐私的同时抵抗逆向恢复攻击，并支持授权恢复和篡改检测。


<details>
  <summary>Details</summary>
Motivation: 现有人脸对抗隐私保护方法依赖输入空间扰动，但对手可以通过学习恢复映射来部分逆转保护，现有防御范式缺乏对可逆性的控制，导致保护效果下降。

Method: ARFP包含三个组件：(1) 密钥条件流形绑定，将保护变换与用户密钥绑定；(2) 对抗性恢复感知训练，引入替代恢复对手提升抗逆能力；(3) 授权可逆恢复，支持使用正确密钥恢复并提供基于随机数的篡改指示。

Result: 在被考虑威胁模型下的实验表明，ARFP在保持授权恢复可用性的同时，提升了对评估中恢复攻击的抵抗能力。

Conclusion: 实验证据表明ARFP在测试场景中展现了密钥敏感的恢复行为和篡改感知能力，为隐私保护的不可逆性提供了新的解决方案。
...
TL;DR: VIAR通过引入隐式平衡层替代VAR的显式深度堆叠，实现常数级训练内存和推理时可调节的每尺度计算量，在ImageNet 256x256上以38.4%参数达到FID 2.16，并可灵活降低峰值内存和提升吞吐量。


<details>
  <summary>Details</summary>
Motivation: VAR基于下一尺度预测的显式深度堆叠导致每尺度计算量固定且高分辨率下内存膨胀，限制了计算效率与部署灵活性。

Method: 在浅层pre/post块之间嵌入隐式平衡层，采用Jacobian-Free反向传播训练（常数级训练内存），推理时通过每尺度迭代旋钮控制计算量。

Result: ImageNet 256x256上FID 2.16, sFID 8.07，仅用VAR 38.4%参数；峰值内存从19.24GB降至8.53GB，吞吐量从15.06提升至32.08 images/s；在零样本修复和类条件编辑中生成更锐利细节和更平滑边界。

Conclusion: 隐式平衡与每尺度计算控制为实用化视觉生成提供了质量-效率可调、内存可控的可行方案。
...
TL;DR: 提出DACG-IR，一种通过显式感知退化特征来动态调制特征表示的统一图像恢复方法，在多种图像恢复任务上达到SOTA。


<details>
  <summary>Details</summary>
Motivation: 统一图像恢复模型在处理多种退化类型时存在任务干扰问题，现有方法缺乏对退化特征的显式感知能力。

Method: 1) 轻量级多尺度退化感知模块提取粗粒度退化信息并生成逐层提示；2) 提示引导编码器/解码器中的注意力温度和输出门控，实现自适应特征提取；3) 空间-通道双门控自适应融合机制细化编码器特征，抑制噪声从浅层向深层传播。

Result: 在单任务、统一模型、恶劣天气去除以及复合退化设置下均优于现有SOTA方法。

Conclusion: 显式退化感知与自适应门控机制能有效缓解任务干扰，提升统一图像恢复模型的泛化能力。
...
TL;DR: 本文研究零样本视觉语言模型（VLM）在非小细胞肺癌（NSCLC）大体肿瘤体积（GTV）勾画中的空间行为，发现解剖位置是驱动模型空间注意力的主导因素，而组织学和分期影响甚微。VoxTell在零样本条件下达到与nnUNet等全监督方法统计上无显著差异的性能（DSC 0.613 vs 0.690），提示评估分割VLM时不仅应关注Dice系数，还应关注其对齐的提示维度。


<details>
  <summary>Details</summary>
Motivation: 零样本视觉语言模型（VLM）为NSCLC的GTV勾画提供了一种可提示的替代方案，但控制其空间行为的提示维度尚不明确。本文旨在通过系统性探针分析揭示哪些提示维度主导了模型的注意力分配。

Method: 在VoxTell模型上，对内部NSCLC肿瘤数据集进行子提示分解（诊断、人口统计学、分期、解剖、通用、无关控制）、属性扰动鲁棒性分析、特异性阶梯分析、跨病例提示交换实验，并以DSC为指标，结合Wilcoxon符号秩检验和Benjamini-Hochberg校正，与微调基线（nnUNet、Ahmed等）和零样本基线进行对比。

Result: 解剖位置是主导因素：63.4%的位置扰动导致灾难性性能下降；提示特异性从通用描述逐步提升至完整描述（诊断单独提示除外）；无关提示正确输出零分割；跨病例交换证实患者特异性条件化（匹配DSC 0.906 vs 不匹配0.406）。组织学和分期替换影响极小。VoxTell零样本DSC为0.613，与nnUNet（0.690，校正p=0.156）和Ahmed等（0.675，校正p=0.679）无统计显著差异，显著优于其他零样本模型。

Conclusion: 分割VLM的评估应不仅依赖Dice系数，还需关注模型所对齐的提示维度。VoxTell在零样本下即可达到与全监督方法相当的性能，其空间行为主要由解剖位置驱动，而非肿瘤的组织学或分期特征。
...
TL;DR: 本文提出了GameScope，目前最大的游戏视频质量数据集，包含4,048个视频样本，覆盖H.264、H.265和AV1三种主流编码器，并标注了平均37个MOS评分及粗粒度质量属性，验证了视觉语言模型在此任务上的优越性能。


<details>
  <summary>Details</summary>
Motivation: 现有游戏视频质量数据集数量有限且存在局限性，缺乏覆盖多种编码器和内容类型的大型多样化主观质量数据集，无法支撑跨编码器一致的视频质量评估模型开发。

Method: 构建了包含4,048个游戏视频样本的数据集，涵盖UGC和PGC内容，使用H.264、H.265和AV1三种编码器，每个样本由平均37人标注MOS评分，并收集了粗粒度质量属性；在此基础上评估了主流VQA方法及视觉语言模型的性能。

Result: 该数据集为目前最大的游戏视频质量数据集，视觉语言模型在所有基准方法中表现最优；这是首个涵盖多编码器、多内容类型并附带质量属性的游戏视频质量评估数据集。

Conclusion: GameScope填补了游戏视频质量评估领域的数据空白，为跨编码器一致性质量评估提供了重要基准，视觉语言模型展现了在该任务上的潜力。
...
TL;DR: 本文提出了一种基于CNN的Transformer架构模型MIMO-ESP，通过将时间轴作为独立轴处理并引入膨胀操作，在视频、交通和降水预测任务中实现了优于现有模型的性能与效率平衡。


<details>
  <summary>Details</summary>
Motivation: 现有RNN模型存在并行化限制和递归堆叠误差；CNN模型因核的局部性难以捕获全局信息，且常将时间轴与图像通道混合导致信息混淆；Transformer模型因自注意力计算复杂度高、训练时间长。需要一种兼顾全局信息捕获与计算效率的时空预测模型。

Method: 提出MIMO-ESP模型，核心包括：(1) 基于CNN构建Transformer架构，以较低复杂度捕获全局信息；(2) 将时间轴作为独立维度处理，避免与通道轴混合；(3) 引入膨胀（dilation）操作，有效联合考虑时空信息。

Result: 在视频预测、交通预测和降水预测三个基准数据集上，MIMO-ESP在保持竞争性效率的同时，性能超越了现有模型。消融实验验证了各模块的有效性。

Conclusion: MIMO-ESP通过CNN化的Transformer架构和独立时间轴处理，有效解决了现有模型在全局信息捕获与计算复杂度之间的权衡问题，为高效时空预测提供了新方案。
...
TL;DR: 本文针对植物叶片疾病分类领域数据集不足的问题，通过构建新数据集并基于DenseNet201架构训练基础模型，实现了比基线模型更优的性能，且该模型在迁移学习中训练更快、更鲁棒、更稳定、所需数据更少。


<details>
  <summary>Details</summary>
Motivation: 植物病害导致农作物大量损失，人工检测耗时费力；尽管ML/CNN方法已取得进展，但公开数据集与训练完备模型所需数据之间存在差距，制约了模型性能。

Method: 识别公开植物叶病分类数据集并开展基准测试；进行数据增强适用性研究；基于发现构建新数据集；采用DenseNet201架构训练Base Model。

Result: 新模型在新数据集上优于基线模型；在植物叶病分类领域的迁移学习实验中，该模型相比通用模型训练更快、更鲁棒、更稳定、所需数据更少。

Conclusion: 通过构建针对性数据集和采用DenseNet201架构，可以有效克服当前领域数据集不足的问题，提升模型性能和迁移学习效率。
...
TL;DR: 本文提出 Chain of Evidence (CoE) 框架，利用视觉语言模型直接在文档截图上进行推理，输出精确的边界框，实现像素级别的可解释迭代检索增强生成，解决了基于文本的 iRAG 系统中粗粒度归因和视觉语义丢失两大瓶颈。


<details>
  <summary>Details</summary>
Motivation: 现有 iRAG 系统基于解析文本运行，存在两个关键瓶颈：(1) 粗粒度归因——用户需在冗长文档中手动定位证据；(2) 视觉语义丢失——将视觉丰富的文档（如幻灯片、含图表的PDF）转为文本会丢弃空间逻辑和布局线索，而这些对于推理至关重要。

Method: 提出 Chain of Evidence (CoE)，一个检索器无关的视觉归因框架。核心思想是利用视觉语言模型直接对检索到的文档候选截图进行推理，绕过格式特定的解析，直接输出精确的边界框，在检索候选集内可视化完整的推理链。

Result: 在 Wiki-CoE（从 2WikiMultiHopQA 衍生的结构化网页数据集）和 SlideVQA（含复杂图表和自由格式布局的演示幻灯片数据集）两个基准上评估。微调的 Qwen3-VL-8B-Instruct 取得了稳健性能，在需要视觉布局理解的场景中显著优于基于文本的基线模型。

Conclusion: CoE 建立了一个检索器无关的、像素级别可解释的 iRAG 解决方案，有效弥合了现有文本驱动系统与视觉丰富文档推理需求之间的鸿沟。
...
TL;DR: SIFT-VTON 利用 SIFT 关键点匹配为扩散模型提供显式几何引导，通过将对应点转换为空间概率分布来监督交叉注意力层，从而在虚拟试穿中更精确地保留衣物细节（如文字和图案），在 VITON-HD 数据集上取得显著改进。


<details>
  <summary>Details</summary>
Motivation: 现有的基于扩散模型的虚拟试穿方法通过交叉注意力机制隐式学习空间对应关系，难以保留精细细节（如文字和图案），需要显式的几何引导来解决这一问题。

Method: 对衣物和人物图像进行 SIFT 关键点匹配并应用领域特定过滤，将对应关系转换为空间概率分布，在训练过程中用于监督交叉注意力层，迫使模型学习精确的空间对齐。

Result: 在 VITON-HD 数据集上，非配对指标显著提升，配对重建指标保持竞争力；定性比较显示文本清晰度和图案对齐效果更好；注意力可视化表明模型对相关衣物细节的关注更加聚焦。

Conclusion: 经典的几何对应方法能够有效增强现代扩散模型在条件合成任务中的表现，显式空间监督有助于提升细节保真度。
...
TL;DR: 针对长尾分布下概念混淆问题，提出CUE方法，通过多标签概念扩展（融合零样本CLIP的实例级视觉线索和LLM的类级语义线索）来保留被破坏的类间关系，在多个长尾基准上取得SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有长尾学习方法主要关注缓解分布偏差，忽视了长尾分布导致的**概念混淆**问题。这一问题源于单标签监督的互斥性：在长尾分布下，互斥性压制了相关类间的特征共享，并放大了头部类的支配地位，从而破坏了类间可区分性。

Method: 提出**CUE（Concept-aware mUlti-label Expansion）**，通过引入多标签概念信号来保留被破坏的类间关系。具体包括：(i) 从零样本CLIP提取**实例级视觉线索**；(ii) 用LLM生成**类级语义线索**；两种线索通过分别加权的**二值逻辑调整（BLA）**辅助损失融合，并与基线Logit-Adjustment（LA）损失联合优化。

Result: 在多个长尾基准数据集上，CUE超越现有最先进方法，实现了平衡且强劲的性能。

Conclusion: 通过多标签概念扩展能有效缓解长尾分布导致的概念混淆问题，恢复被破坏的类间关系，从而提升长尾学习性能。代码已开源。
...
TL;DR: PACE提出了一种新的LiDAR点云压缩框架，通过将祖先上下文聚合重构为非因果主干，并将因果性限制在轻量级、阶段可扩展的预测器中，大幅减少了解码延迟（降低超90%），同时实现了最优的压缩效率，并支持单一模型适应不同的性能-延迟权衡。


<details>
  <summary>Details</summary>
Motivation: 现有基于八叉树的LiDAR点云压缩方法存在两大瓶颈：1) 因果多阶段上下文建模导致解码延迟过高；2) 性能与延迟之间的权衡固定，单一模型无法适应不同场景需求。这些问题的根本原因在于上下文聚合主干与概率预测的紧密耦合。

Method: 将祖先上下文聚合重构为非因果主干（无需逐阶段重复执行），将因果性限制在轻量级、阶段可扩展的预测器中。预测器支持任意数量的预测阶段，无需重新加载参数即可无缝适应不同的性能-延迟权衡。

Result: 在压缩效率上达到了新的最优水平，取得了显著的BD-BR节省，并将自回归模式下的解码延迟降低超过90%。

Conclusion: PACE通过解耦上下文聚合与概率预测，有效解决了现有方法的延迟瓶颈和性能-延迟权衡僵化问题，在实际应用中具有高度吸引力。
...
TL;DR: 本文提出VideoThinker框架，通过因果分析揭示轻量级多模态大模型在RL微调中存在感知偏置问题，并设计两阶段去偏方法（偏置模型训练+CDPO算法）提升轻量模型的视频推理能力，在多个基准上取得SOTA。


<details>
  <summary>Details</summary>
Motivation: RL虽能提升多模态大模型的推理能力，但对部署在边缘设备的轻量模型效果有限。本文旨在解决轻量模型在RL微调中无法发展真正推理能力的问题。

Method: 提出VideoThinker框架，包含两个阶段：(1) Bias Aware Training阶段训练一个专门的"偏置模型"来编码快捷行为；(2) Causal Debiasing Policy Optimization (CDPO)算法，通过排斥目标主动推开偏置模型的错误逻辑，同时拉向正确的通用解。

Result: VideoThinker-R1在相同规模对比中，无需SFT且仅用1%的RL训练数据，在主流基准上平均超越VideoRFT-3B达3.2%，VideoMME上领先7%；跨规模对比中，在MVBench和TempCompass上分别超越更大的Video-UTR-7B达2.1%和3.8%。

Conclusion: 因果启发的去偏框架能有效提升轻量模型的视频推理效率，在数据效率和推理性能上均显著优于现有方法。
...
TL;DR: 本文系统研究了视觉编码器与VLM对齐性能的关系，发现传统选型指标（模型大小、零样本准确率）与VLM性能仅呈弱到中等相关性。通过分析19种预训练视觉编码器，提出以Gromov-Wasserstein距离度量跨模态结构相似性作为选型依据，理论证明其与跨模态映射可学习性的关联，实验验证该指标显著优于现有选型策略。


<details>
  <summary>Details</summary>
Motivation: 当前VLM研究中缺乏对视觉编码器选择原理的系统性理解，常用的选型标准（如模型尺寸、零样本准确率）缺乏理论依据，亟需揭示真正影响VLM对齐性能的编码器关键因素。

Method: 在19种不同来源的预训练视觉编码器上进行系统性实验，提出基于Gromov-Wasserstein距离的跨模态结构相似性度量，从理论上证明该距离与跨模态映射可学习性的关联，并在60+次完整VLM训练中验证该推理指标的有效性。

Result: 传统选型指标（模型大小、零样本准确率）与VLM性能仅呈弱到中等相关性；提出的Gromov-Wasserstein距离指标与VLM最终性能呈现显著更强的相关性，能够在完整训练前有效预测VLM性能。

Conclusion: 跨模态结构相似性是视觉编码器选择的关键因素，Gromov-Wasserstein距离可作为高效、有效的VLM性能预训练评估指标，为VLM架构设计与视觉编码器选型提供了理论指导。
...
TL;DR: 本文提出了一种针对可逆图像隐藏方案的零样本可解释图像隐写分析框架，通过将图像隐藏、揭示和隐写分析统一到一个框架中，并引入残差增强策略，显著提升了跨数据集和跨架构场景下的泛化性能。


<details>
  <summary>Details</summary>
Motivation: 现有隐写分析方法存在两大局限：一是仅将输入图像二分类为隐写图像或封面图像，缺乏可解释性；二是要求训练和测试数据分布相似，难以适用于真实世界的零样本场景。本文旨在克服这些不足，实现对可逆图像隐藏方案的有效检测。

Method: 1) 将图像隐藏、揭示和隐写分析集成到统一框架中，使隐写分析组件具备从隐写图像中恢复秘密信息的能力；2) 提出一种简单有效的残差增强策略，用于生成隐写图像，增强隐写分析器在跨数据集和跨架构场景下的泛化能力。

Result: 在基准数据集上的大量实验表明，所提出的方法在可逆图像隐藏方案的隐写分析任务上显著优于现有技术。

Conclusion: 该框架通过统一建模和残差增强策略，有效解决了零样本设置下隐写分析的泛化性和可解释性问题，为可逆图像隐藏方案的安全评估提供了有力工具。
...
TL;DR: 本文针对视觉-语言模型（VLM）在细粒度感知上的带宽瓶颈，借鉴主动视觉和信息觅食范式，将其形式化为序贯贝叶斯最优实验设计（S-BOED）问题，并提出了一种免训练推理策略。该策略通过平衡空间覆盖与分辨率，在十亿像素级基准上显著提升了当前最优模型的性能，逼近人工标注水平。


<details>
  <summary>Details</summary>
Motivation: 现代VLM的视觉感知受限于"感知带宽瓶颈"：广视野必然牺牲细粒度细节，而复杂推理恰恰依赖于这些细节。传统方法无法在连续超大规模像素空间中同时兼顾视野广度与分辨率精度。

Method: 将克服该瓶颈建模为序贯决策过程，并通过S-BOED框架形式化。在连续十亿像素空间中推导出可处理的近似方案，平衡空间覆盖与分辨率。提出一种免训练推理策略作为S-BOED目标的实际实例化，支持任意优化算法（如贪婪采样、前瞻规划）来近似最优设计。

Result: 在十亿像素级基准上的实验表明，该方法进一步提升了当前最优模型的性能，显著超越标准基线，有效缩小了与人工标注参照之间的差距。

Conclusion: 通过主动视觉与信息觅食启发的序贯决策框架，可以在不重新训练模型的情况下突破VLM的感知带宽瓶颈，为处理超高分辨率视觉推理任务提供了一种灵活且有效的通用模板。
...
TL;DR: CHASE是一种通过显式比较结构化时间解释（竞争假设）来区分安全决策与根本不确定性的选择性预测框架，在部分可观测性场景下优于传统不确定性估计方法，在隐藏连通性推断任务中实现了更优的准确率-覆盖率权衡。


<details>
  <summary>Details</summary>
Motivation: 标准选择性预测方法通常依赖单预测分支输出的置信度进行不确定性估计，但在部分可观测性条件下，局部时间证据可能存在矛盾，导致传统置信度分数产生误导，需要一种能显式处理结构化歧义的选择性预测方法。

Method: 提出CHASE框架，通过显式比较竞争性的结构化时间解释（假设）来决定是否提交决策或弃权；优化一个排序感知的选择器，基于假设间的得分边缘（margin）来全局区分安全决策与根本不确定的决策。

Result: 在隐藏连通性推断任务上（使用巨单层囊泡GUV物理仿真器和真实视频进行零样本迁移），CHASE在80%覆盖率下实现了显著更优的无弃权准确率、三路准确率和歧义对齐弃权率；在非常高歧义区域，三路准确率相对提升8.8%，整体对齐度相对提升11.0%；90%覆盖率下总体风险降低9.9%。

Conclusion: 显式推理竞争假设为部分可观测性下的选择性预测提供了更可靠的决策机制，在结构化歧义场景中实现了优于传统基线的不确定性与准确率平衡。
...
TL;DR: 本文提出AgriKD，一种跨架构知识蒸馏框架，将Vision Transformer（ViT）教师模型的知识迁移至轻量级卷积学生模型，通过输出级、特征级和关系级多目标蒸馏弥补Transformer与CNN之间的表征差距。在多个叶片病害数据集上，学生模型参数减少约172倍，计算成本降低47.57倍，推理延迟减少18-22倍，并在NVIDIA Jetson等边缘设备上实现实时部署。


<details>
  <summary>Details</summary>
Motivation: ViT在叶片病害分类中表现优异，但其高计算代价使其难以部署在资源受限的边缘设备上；现有方法难以将ViT的丰富表征有效迁移至轻量模型。

Method: 提出AgriKD框架，使用ViT作为教师、紧凑型CNN作为学生，在输出级、特征级和关系级三个层面设计蒸馏目标，以弥合Transformer与CNN架构之间的表征差异。

Result: 学生模型参数减少约172倍，计算成本降低47.57倍，推理延迟减少18-22倍；在ONNX、TFLite Float16、TensorRT FP16等运行时格式下性能一致，边缘设备实现实时推理。

Conclusion: AgriKD有效实现了从ViT到轻量CNN的知识迁移，在保持高性能的同时大幅提升效率，适用于资源受限环境下的农业AI应用。
...
TL;DR: VoxAfford通过将冻结的3D VQVAE编码器中的多尺度几何特征注入MLLM输出token，解决了开放词汇3D交互区域检测中输出token缺乏空间信息的问题，在mIoU上提升约8%并实现零样本迁移。


<details>
  <summary>Details</summary>
Motivation: 现有方法使用MLLM的特殊输出token解码为分割掩码，但这些token通过自回归生成建模序列依赖而非空间邻域关系，导致语义丰富但空间信息贫乏，难以准确定位3D交互区域。

Method: 1) 使用冻结的预训练3D VQVAE编码器提取多尺度几何特征；2) 输出token以自身语义为查询，通过交叉注意力从对应体素尺度检索几何模式，并用可学习兼容门控控制注入强度；3) 增强后的token通过语义条件注意力和逐点特征传播生成最终掩码。

Result: 在开放词汇交互区域检测任务中达到SOTA，mIoU提升约8%；真实机器人实验验证了零样本迁移到新物体的能力。

Conclusion: 注入多尺度几何特征能有效弥补MLLM输出token的空间信息不足，提升开放词汇3D交互区域检测性能。
...
TL;DR: VISTA是一个面向视觉语言模型（VLM）的大规模、交互感知的时空理解诊断基准，包含约12K视频-查询对，支持开放集、多实体、多动作的时空理解评估，系统评估了11个SOTA VLM并揭示了传统指标掩盖的时空偏差。


<details>
  <summary>Details</summary>
Motivation: 现有VLMs基准局限于简单单动作视频、封闭属性集和受限实体类型，无法反映真实世界的多实体、多动作自由交互；且缺乏系统性框架来分析模型在互补时空轴上的失败模式。

Method: VISTA将视频分解为可解释实体、关联动作和关系动态，支持多轴诊断；整合多个数据集形成交互感知分类法，构建约12K视频-查询对；在分类法粒度上对11个SOTA VLM进行系统性评估和细粒度性能分解。

Result: 系统性评估了11个SOTA VLM，通过分类法驱动的细粒度诊断揭示了传统聚合指标掩盖的时空理解短板和显著偏差。

Conclusion: VISTA是首个大规模、交互感知的VLM时空理解诊断基准，通过细粒度分类法诊断为模型设计、预训练策略和评估协议提供了指导框架。
...
TL;DR: 提出了一种名为Recall2Predict的端到端可微框架，通过构建结构化的"运动银行"（motion bank）嵌入空间，利用对比学习存储物理可行的轨迹。该方法采用锚点检索层（Anchor Retrieval Layer）动态检索显式运动先验，结合正交初始化查询、双级门控交叉注意力机制和Straight-Through Gumbel-Softmax进行离散轨迹选择，最后通过DETR-style解码器几何精炼。联合优化WTA运动学GMM、潜在多样性惩罚和软最小加权端点损失，在Argoverse 2和Waymo Open Motion数据集上取得竞争性结果。


<details>
  <summary>Details</summary>
Motivation: 运动预测常需在可解释性与预测精度之间权衡。标准锚点架构依赖不透明的潜在查询，极易发生潜在坍缩（latent collapse）；而朴素轨迹采样则限制了多模态多样性。现有方法缺乏既能保持可解释性又能实现高精度多模态预测的解决方案。

Method: 1. 构建"运动银行"：通过对比学习构建物理可行轨迹的结构化嵌入空间。2. 锚点检索层（Anchor Retrieval Layer）：使用正交初始化查询，通过双级门控交叉注意力机制（Dual-Level Gated Cross-Attention）动态检索显式运动先验。3. 离散轨迹选择：采用Straight-Through Gumbel-Softmax估计器保持连续梯度流。4. 几何精炼：DETR-style解码器对检索到的锚点进行细化。5. 联合优化：WTA运动学GMM损失 + 潜在多样性惩罚 + 软最小加权端点损失。

Result: 在Argoverse 2和Waymo Open Motion数据集上取得了竞争性的多模态预测精度，同时消除了标准潜在查询的"黑箱"问题，保持了可解释性。

Conclusion: 通过将解码阶段严格条件化在多样化、可解释的运动基元上，该方法在保持竞争性多模态精度的同时，成功消除了传统潜在查询的不可解释性，为运动预测提供了一种兼顾可解释性与性能的新范式。
...
TL;DR: 针对多模态点云补全中硬投影导致跨模态连接断裂（Cross-Modal Entropy Collapse）的问题，提出SplAttN，用可微高斯泼溅替代硬投影，实现稠密连续图像平面表示，在多个数据集上达到SOTA，并验证了鲁棒的跨模态依赖。


<details>
  <summary>Details</summary>
Motivation: 多模态点云补全的理论机制尚不明确，标准硬投影将稀疏点云投影到图像平面时会产生极度稀疏的支持区域，阻碍视觉先验传播，这种失效模式被称为Cross-Modal Entropy Collapse（跨模态熵坍塌）。

Method: 提出SplAttN，用可微高斯泼溅（Differentiable Gaussian Splatting）替代硬投影，将投影过程重构为连续密度估计，生成稠密连续的图像平面表示，避免了稀疏支持坍塌，有利于梯度流动并提升跨模态连接的可学习性。

Result: 在PCN和ShapeNet-55/34数据集上达到SOTA性能；在KITTI真实世界基准上的反事实评估表明，基线方法退化为单模态模板检索器（对视觉输入移除不敏感），而SplAttN保持了对视觉线索的鲁棒依赖。

Conclusion: SplAttN通过可微高斯泼溅有效建立了跨模态连接，验证了连续密度投影对于多模态点云补全的关键作用。
...
TL;DR: 本文提出决策边界感知生成框架（DBG），针对扩散模型在长尾数据中生成头部到尾部迁移时导致的边界模糊问题，通过生成信息性近边界样本来提升分类器的决策空间可分离性，在多个长尾基准上提升了尾类及整体准确率。


<details>
  <summary>Details</summary>
Motivation: 长尾数据导致分类器偏向头部类而降低尾部类准确率。扩散模型生成增强虽能缓解此问题，但头部到尾部迁移会引入潜在的非局部特征混合，导致决策边界重叠和尾部类分布偏移，造成边界模糊问题。

Method: 提出DBG框架：1) 识别扩散模型生成中的边界模糊问题；2) 通过生成信息性近边界样本，促进近边界表示学习；3) 重新平衡长尾数据集并产生更可分离的决策空间。

Result: 在标准长尾基准数据集上，DBG一致提升了尾类准确率和整体准确率，同时减少了类间重叠。

Conclusion: DBG有效解决了扩散模型在长尾学习中因头部到尾部迁移导致的边界模糊问题，通过近边界样本生成实现了更可分离的决策空间，为长尾学习提供了新的生成式解决方案。
...
TL;DR: 本文提出LIE，一种仅使用LiDAR的高清地图语义构建方法，通过知识蒸馏（教师分支融合学生LiDAR特征和2D强度图）解决LiDAR缺乏密集语义和纹理线索的问题。在nuScenes上比最先进的相机模型mIoU高8.2%，对Argoverse2仅需10%微调即可超越全量训练相机模型，且对长距离和恶劣天气光照具有鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有HD地图构建方法中，相机缺乏深度信息难以准确获取场景几何，而LiDAR虽能提供精确3D测量但缺少密集语义和纹理线索，亟需一种能结合两者优势的单模态解决方案。

Method: 提出知识蒸馏框架LIE：教师分支将学生分支的LiDAR特征与对应的2D强度图瓦片进行融合，通过在线蒸馏方案为地图元素分割提供密集监督信号，实现仅用LiDAR输入的语义地图构建。

Result: LIE在nuScenes上mIoU比最先进的相机模型高8.2%，超越所有单模态方法；在Argoverse2上仅用10%数据微调即可超越全量训练的相机模型；在长距离和恶劣天气光照条件下表现鲁棒。

Conclusion: 仅使用LiDAR的在线知识蒸馏方法可有效弥补LiDAR缺乏语义纹理线索的不足，在HD地图构建任务上优于相机方法，且具有良好的跨数据集迁移能力和鲁棒性。
...
TL;DR: 提出CSGuard，一种基于压缩感知的抗伪造扩散模型水印方案，通过秘密矩阵绑定图像生成与验证，将伪造攻击成功率从100%降至28.12%，同时在良性水印图像上保持100%检测率。


<details>
  <summary>Details</summary>
Motivation: 现有基于潜空间的扩散模型水印方法存在关键漏洞：攻击者可通过逆向水印图像并重新生成（任意提示词）来提取水印，从而对恶意内容进行虚假归属。

Method: 利用压缩感知技术，将水印图像的生成与验证绑定到一个秘密矩阵上。只有拥有该秘密矩阵的用户才能正确嵌入或验证水印，非法用户无法进行伪造攻击。

Result: 伪造攻击成功率从100%降至28.12%；良性水印图像的检测率达100%；不牺牲生成质量和水印完整性。

Conclusion: CSGuard是首个抗伪造的水印方案，能有效防御伪造攻击，同时保持水印的有效性和生成质量。
...
TL;DR: 本文针对Qwen-Image-Edit-2511（60块多模态扩散Transformer）提出三种训练免编辑方法：KVInject（单前向着意力操作）、AttnRouter（按类别路由编辑操作）以及编辑有效注意力子电路的定位分析，实验表明KVInject优于经典MasaCtrl，AttnRouter提升复合指标6.4%，并揭示了编辑有效的层、步长和alpha波段范围。


<details>
  <summary>Details</summary>
Motivation: 经典的两遍式MasaCtrl方法不适用于多模态扩散Transformer（MMDiT），会导致提示不匹配故障模式（复合得分较基线下降31%），因此需要更简单且有效的单前向着意力编辑方法。

Method: (i) KVInject：单前向着意力操作，在局部层/步长波段内将源半部分的键/值投影与噪声半部分进行alpha混合；(ii) AttnRouter：按编辑类别构建路由表，为每种编辑类型分派最优注意力操作；(iii) 通过层、步长和alpha波段的消融实验定位编辑有效的注意力子电路。

Result: KVInject避免了MasaCtrl的提示不匹配故障；AttnRouter在已知类别下将CLIP-T+DINO-I复合指标提升6.4%，自动零样本分类器可缩小98%的差距；编辑有效子电路定位在早期去噪步（S0-7）和alpha在[0.3,0.5]范围；简单K/V缩放从未超越基线，激进变体完全崩溃（复合得分0.084）。

Conclusion: KVInject是比MasaCtrl更简单有效的MMDiT训练免编辑方法，AttnRouter按类别调度操作可进一步提升性能，且编辑有效性集中在早期去噪步和中等alpha范围，UNet领域的简单K/V缩放技巧不适用于MMDiT。
...
TL;DR: 提出CGFformer，一种基于聚类引导的频率Transformer，通过自适应分离模块、双流去噪模块和频率-空间融合模块，解决全色锐化中固定频率滤波器适应性不足和噪声抑制不准确的问题，在多个数据集上取得显著提升。


<details>
  <summary>Details</summary>
Motivation: 现有基于频率的全色锐化方法使用固定频率滤波器，无法适应复杂且空间多样的频率分布；同时现有去噪策略未能充分利用频率分量，难以准确抑制多种噪声类型。

Method: 1）自适应分离模块：通过K-means聚类整合局部特征与非局部信息，实现高低频分量的精确分离；2）双流精炼模块：结合基于Transformer的交叉注意力，联合抑制与频率相关和无关的扰动；3）频率-空间融合模块：增强细节并促进空间-频率交互，重构空间结构。

Result: 在多个基准数据集上的大量实验表明，CGFformer相比现有全色锐化方法取得了显著改进。

Conclusion: CGFformer通过聚类引导的频率Transformer有效解决了频率自适应分离和噪声抑制问题，验证了频率-空间交互在全色锐化任务中的有效性。
...
TL;DR: 本文介绍了首届SF20K竞赛（与ICCV 2025 SLoMO Workshop联合举办）的结果与发现。该竞赛聚焦故事级视频理解，基于业余短片语料构建开放式视频问答任务，包含Main Track（无模型大小限制）和Special Track（参数量<8B）两个赛道。22支队伍共提交286份方案，最佳成绩为主赛道65.7%、特殊赛道48.7%，人类表现上限为91.7%。分析表明：叙事感知的分镜头处理优于均匀帧采样；精心设计的多阶段流水线（小模型）可媲美或超越30倍以上大模型的端到端推理；字幕质量是性能的关键因素。当前长视频问答的主要瓶颈在于信息选择与推理结构，而非原始模型容量，方法与人类叙事理解之间仍存在显著差距。


<details>
  <summary>Details</summary>
Motivation: 推动视频理解从短片段动作识别向故事级别的长视频叙事理解发展，构建一个防止模型依赖记忆流行电影、真正考验多模态理解的开放式问答任务。

Method: 基于SF20K-Test基准（95部电影、979个QA对）进行评估，采用LLM-QA-Eval（基于GPT-4.1-nano的自动评判器）打分。竞赛设置Main Track（无限制）和Special Track（<8B参数）两个赛道，吸引22支队伍286次提交。

Result: 主赛道最佳准确率65.7%，特殊赛道最佳准确率48.7%，人类表现上限91.7%。叙事感知的分镜头处理优于均匀帧采样；小模型多阶段流水线可匹配或超越30倍以上大模型；字幕质量是决定性因素。

Conclusion: 长视频问答的核心瓶颈在于信息选择与推理结构，而非模型参数的原始规模；当前方法与人类水平的叙事理解之间仍存在巨大差距。
...
TL;DR: 本文首次提出3D视觉查询定位（3DVQL）基准，包含2,002个序列、约17万帧和6.4K响应片段，涵盖点云、RGB和深度图像多模态数据，并提出了LaF融合算法在多个基线中取得最优性能。


<details>
  <summary>Details</summary>
Motivation: 现有视觉查询定位研究主要集中在2D视频领域，3D空间中的视觉查询定位尚未得到充分关注，缺乏相应的基准数据集和评估方法。

Method: 构建了3DVQL基准数据集，包含多模态（点云、RGB图像、深度图像）标注；提出了Lift-and-Attention Fusion (LaF)算法用于多模态融合定位。

Result: 实验表明现有方法在不同融合模块上性能差异显著，提出的LaF算法显著优于所有基线模型。

Conclusion: 3DVQL是首个3D多模态视觉查询定位基准，LaF算法为该任务提供了有效的解决方案，代码和数据集将公开发布以推动后续研究。
...
TL;DR: SwiftPie是首个一步式扩散图像个性化工具，通过双分支身份注入机制和掩码引导重缩放策略，在单步扩散过程中实现快速、高质量的主体驱动图像个性化生成，性能与多步方法相当。


<details>
  <summary>Details</summary>
Motivation: 现有图像个性化方法依赖计算密集的微调、迭代优化或多步去噪过程，严重阻碍了在实时应用中的部署和交互能力，亟需一种高效的个性化方案。

Method: 提出双分支身份注入机制将主体身份有效集成到一步扩散模型中，并引入掩码引导重缩放策略增强主体上下文适应性，两者均在单步扩散内完成。

Result: SwiftPie在实现卓越个性化生成速度的同时，在身份保真度和提示对齐方面达到了与多步方法相当的性能。

Conclusion: 该工作为实时、高质量个性化图像生成开辟了新机遇，为交互式视觉合成奠定了基础。
...
TL;DR: 提出了一种无需微调的管道方法，通过粗到细的两遍分解和专家角色分配，利用冻结的视觉语言模型实现交通事故在时间、空间和碰撞类型上的联合定位。在ACCIDENT@CVPR 2026基准上达到ACC^S=0.539，比基准论文的最佳基线高出0.127，每次视频最多调用3次API，总运行成本约20美元。


<details>
  <summary>Details</summary>
Motivation: 真实CCTV交通事故数据是罕见事件，标注事故视频常被禁止，但需要同时在时间、空间和碰撞类型上进行精确定位，现有方法难以在不微调的情况下实现这一联合输出。

Method: 1) 粗到细的两遍分解：第一遍以1fps处理全视频得到粗粒度(时间,位置,碰撞类型)元组；第二遍在±3秒窗口内以5fps细化时间与位置，通过两个确定性置信门控在边界模糊或坐标边缘时回退到粗估计。2) 专家角色分配：Qwen3-VL-Plus负责定位，Gemini 3.1 Flash-Lite负责基于中心裁剪视频片段的碰撞类型判定。

Result: 在ACCIDENT@CVPR 2026基准（2027个真实CCTV视频）上达到ACC^S=0.539（95% CI [0.525, 0.553]），比基准论文最佳基线（0.412）高0.127，比最强单VLM基线Molmo-7B（0.396）高0.143，比朴素基线（0.289）高0.250。每次视频最多3次API调用，API失败时17%回退到物理方法，总运行成本约20美元。

Conclusion: 无需微调的冻结VLM管道通过粗到细分解和专家分工可有效解决CCTV交通事故联合定位这一罕见事件问题，在精度上显著超越现有基线，且成本极低。
...
TL;DR: MIRL是一个解耦框架，通过互信息作为预筛选信号来优化RLVR中的采样预算分配和奖励稀疏性问题，在视觉语言推理任务上以更少的完整轨迹采样实现了更高准确率。


<details>
  <summary>Details</summary>
Motivation: 现有RLVR方法在视觉语言推理中存在两个关键限制：一是大量采样预算浪费在因早期视觉描述错误而注定失败的轨迹上；二是稀疏奖励无法区分失败源于视觉感知阶段还是推理阶段。

Method: 提出MIRL框架，利用生成描述与视觉输入之间的互信息作为廉价的预筛选信号，通过分叉机制将预算分配给高潜力轨迹；同时采用解耦训练为视觉感知优化提供独立的基于互信息的奖励，解决奖励盲目性问题。

Result: 在六个视觉语言推理基准上达到70.22%的平均准确率，仅用10个预样本和top-6选择（比完整轨迹采样少25%）就超越了采样16条完整轨迹的性能。

Conclusion: MIRL通过互信息引导的预算分配和解耦奖励机制，有效提升了RLVR在视觉语言推理中的效率和效果。
...
TL;DR: 本文提出从单张运动模糊图像中提取基础矩阵的新任务，通过建立曝光窗口内不同时间点的对应关系，并改进估计算法以处理时间方向模糊和预测不确定性，在合成与真实数据上验证了有效性。


<details>
  <summary>Details</summary>
Motivation: 现有方法无法从单张运动模糊图像中提取描述3D相机运动的基础矩阵，而模糊图像中的拖尾路径蕴含着相机运动的线索和约束。

Method: 1) 在曝光窗口内建立两个时间实例之间的对应关系；2) 修改基础矩阵估计算法以适应时间方向模糊的对应点；3) 引入拖尾模式预测的不确定性度量，并将其用于估计器的采样过程。

Result: 在合成和真实运动模糊数据集上，该方法能够从单帧图像中估计出编码3D相机运动的基础矩阵，并在下游运动分割任务中展示了实用价值。

Conclusion: 证明了从单张运动模糊图像中推断基础矩阵的可行性，且估计结果在转置意义下唯一（对应时间方向的模糊性），同时该方法可有效应用于运动分割等实际任务。
...
TL;DR: 提出了一种统一的跨域迁移学习框架，通过教师-学生范式和多级知识蒸馏，利用多个异构医学影像数据集提升分割、分类和检测任务的性能。


<details>
  <summary>Details</summary>
Motivation: 现有医学影像分析模型通常针对单一任务和数据集设计，难以利用来自多个异构数据集的知识，泛化能力有限。需要一种可扩展且任务无关的统一框架来提升跨域性能。

Method: 采用教师-学生范式：联合教师模型聚合从多个源数据集学到的域不变表示，任务特定的学生模型通过多级知识蒸馏进行训练。框架最初为分割任务设计，后扩展支持分类和检测任务。

Result: 在多个分割、分类和检测基准数据集上（包括MRI和CT模态），该方法在各类任务和模态中均一致优于强基线方法，表现出更强的分布偏移鲁棒性和泛化能力。

Conclusion: 多数据集知识蒸馏是一种可扩展、任务无关的有效方法，能够在异构医学影像领域提升分割、分类和检测任务的性能。
...
TL;DR: 本文提出随机图像增强的统一视角，将现有方法归类为无条件扩散模型、Ornstein-Uhlenbeck过程和扩散桥三大类连续时间过程，证明它们源于共同的SDE公式，并通过控制实验发现没有始终占优的方法，最后发布了模块化库ItoVision。


<details>
  <summary>Details</summary>
Motivation: 现有随机图像增强方法之间的关系不清晰，需要统一的视角来理解不同方法之间的本质联系和差异。

Method: 提出统一SDE框架，将方法分为三类；通过控制实验（相同架构和训练协议）进行公平比较；构建模块化PyTorch库。

Result: 没有始终占优的方法；识别并解耦了影响性能的具体设计选择。

Conclusion: 不同方法主要在漂移项、扩散项、终端分布和边界条件上有差异，而调度器和采样器是正交的设计选择。
...
TL;DR: 本文提出Omni-Fake，一个统一的多模态深度伪造检测数据集（含120万+样本）及Omni-Fake-R1检测器，通过强化学习融合视觉与听觉线索，实现检测、定位和解释，在多个任务上超越现有基线。


<details>
  <summary>Details</summary>
Motivation: 现有深度伪造检测基准局限于单模态、简化操作或不真实分布，无法评估真实场景下的鲁棒性，亟需一个覆盖多模态、支持联合检测-定位-解释的统一基准。

Method: 构建Omni-Fake数据集（Omni-Fake-Set 100万+训练样本 + Omni-Fake-OOD 20万+分布外样本），覆盖图像、音频、视频、音视频说话人头部四种模态；提出Omni-Fake-R1，基于强化学习的多模态检测器，自适应融合视觉和听觉线索，输出结构化决策、定位和自然语言解释。

Result: 实验表明，Omni-Fake-R1在检测精度、跨模态泛化和可解释性上显著优于现有基线方法。

Conclusion: Omni-Fake为多模态深度伪造检测提供了统一、大规模、高保真的基准，Omni-Fake-R1验证了强化学习驱动的多模态融合在真实场景下的有效性。
...
TL;DR: Act2See框架通过监督微调，让视觉语言模型在推理过程中主动插入视频帧（包括检索现有帧或生成新帧），实现了主动视觉感知，在多个视频推理基准上达到最先进水平。


<details>
  <summary>Details</summary>
Motivation: 现有VLM依赖静态初始帧进行视频推理，无法在推理过程中融入动态信息；增强CoT的方法存在CoT质量次优且缺乏对假设/反事实场景视觉信息合成能力的问题。

Method: 通过前沿VLM生成高质量推理轨迹数据集，轨迹中主动插入帧检索或帧生成的调用指令，经人工标注CoT验证后用于监督微调，使模型在推理时具备主动决定何时搜索或合成视觉证据的涌现能力。

Result: 在VideoEspresso和ViTIB上取得新SOTA，在Video-MME、EgoNormia和VCR-Bench上超越可比或更大模型。

Conclusion: Act2See验证了通过主动视觉感知赋予VLM在推理过程中动态获取视觉信息的能力是一种有效的视频推理范式，推动了该领域的发展。
...
TL;DR: 本文提出TRIMMER，一种基于自监督学习和强化学习的视频摘要框架，通过信息论奖励函数实现高效、可泛化的视频摘要，在无监督/自监督方法中达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有视频摘要方法依赖昂贵的人工标注、跨域泛化能力差、计算成本高，且无监督/弱监督方法在捕获长程时序依赖和语义结构方面表现不足。

Method: TRIMMER采用两阶段框架：先通过自监督学习学习鲁棒表示，再通过强化学习进行时空决策；引入基于熵的度量而非传统相似性目标，捕获高阶时序动态和语义多样性，并直接在所选帧索引上计算奖励以提高计算效率。

Result: 在标准基准上，TRIMMER在无监督和自监督方法中达到SOTA性能，且与领先的有监督方法竞争力相当。

Conclusion: TRIMMER展示了自监督强化学习框架在可扩展和可泛化视频摘要任务上的有效性。
...
TL;DR: 本文提出 Video Active Perception (VAP)，一种无需训练的方法，通过轻量级文本条件视频生成模型模拟主动感知理论，实现高效关键帧选择，在多个长视频QA基准上达到SOTA零样本性能，帧效率提升高达5.6倍。


<details>
  <summary>Details</summary>
Motivation: 现有VLMs在长视频问答中面临关键帧选择困难的问题：均匀采样效率低且性能瓶颈，亟需一种高效且有效的方法来选择与问题相关的关键帧。

Method: 受主动感知理论启发，将关键帧选择视为数据采集过程，利用轻量级文本条件视频生成模型作为先验世界知识，通过计算预测帧与实际帧的差异来筛选信息量最大的帧，无需额外训练。

Result: 在EgoSchema、NExT-QA、ActivityNet-QA、IntentQA和CLEVRER五个长视频/推理QA数据集上达到零样本SOTA；帧效率相比GPT-4o、Gemini 1.5 Pro和LLaVA-OV提升高达5.6倍。

Conclusion: VAP验证了主动感知理论在长视频QA中提升帧选择有效性和效率的潜力，为训练无关的关键帧选择提供了新范式。
...
TL;DR: IMPACT-HOI 是一个混合主动框架，通过构建结构化事件图来标注以自我为中心的程序性视频中的人-物交互，采用信任校准控制器和风险约束执行协议，在用户研究中实现了13.5%的手动标注动作减少、46.67%的事件匹配率和零确认字段违规。


<details>
  <summary>Details</summary>
Motivation: 从人类演示中学习机器人操作需要高质量的结构化监督，现有标注方法效率低且易出错，亟需一种能够平衡自动化与人工确认的混合主动标注框架。

Method: 将标注任务建模为部分指定、起始锚定事件状态的增量解析；设计信任校准控制器，根据标注者行为和经验证据质量在直接查询、人工确认建议和保守补全之间动态选择；采用包含原子回滚的风险约束执行协议，确保人工确认的决策不受冲突的自动化更新影响。

Result: 9名参与者的用户研究表明：手动标注动作减少13.5%，事件匹配率达46.67%，在研究的协议下实现了零确认字段违规。

Conclusion: IMPACT-HOI 通过混合主动策略有效降低了人工标注负担，同时保持了标注质量，为机器人学习从人类演示中获取结构化监督提供了可行方案。
...
TL;DR: 本文提出将Fisher Vector无序编码方法与CNN+Vision Transformer混合架构结合，并针对大规模数据集设计了高斯混合模型（GMM）估计的优化方法，在医学图像分类任务中取得了优于基准的结果。


<details>
  <summary>Details</summary>
Motivation: 现有的无序编码方法（如Fisher Vectors）能提升CNN在数据有限场景下的分类性能，而混合CNN+ViT模型解决了CNN的局部性偏差问题。然而，将这两者结合以同时适用于小规模和大规模数据集的方法在文献中仍较少探索。

Method: 将Fisher Vector（基于GMM估计的无序编码方法）引入混合CNN+ViT架构中；针对大规模数据集中GMM估计计算成本过高的问题，提出一种限制GMM估计成本增长的方法。

Result: 在MedMNIST (v2)所有数据集上超越基准结果；在Clean-CC-CCII和ISIC2018上取得与文献相当的结果。

Conclusion: 所提出的混合架构结合Fisher Vector无序编码方法，能够在不同规模的数据集上有效工作，特别在医学图像分类中展现出优异性能。
...
TL;DR: IMPACT-Scribe是一个利用每次人工修正来改进未来人机协作的密集标注框架，通过不确定性感知边界涂鸦监督、局部提议建模、成本感知查询规划、结构化传播和修正驱动自适应，在密集时序动作标注任务中提高标注质量和边界准确性。


<details>
  <summary>Details</summary>
Motivation: 程序性活动视频的密集时序标注对动作理解和具身智能至关重要，但现有反应式工具导致标注劳动密集型且每次修正被孤立处理，缺乏对标注不确定性和模型可靠性的信息复用。

Method: 提出IMPACT-Scribe框架，包含五个核心组件：不确定性感知边界涂鸦监督、局部提议建模、成本感知查询规划、结构化传播和修正驱动自适应，形成闭环人机协作设计。

Result: 实验和人类研究表明，该闭环设计在每次努力下提高了标注质量，增强了边界准确性，并随时间推移改善了人机交互效果。

Conclusion: IMPACT-Scribe通过将每次修正反馈到系统中，实现了持续改进的密集标注人机协作，代码将公开。
...
TL;DR: TrajRAG是一个检索增强生成框架，通过拓扑-极坐标轨迹表示和分层分块结构，将过往导航片段中的几何-语义经验积累并检索，以增强大模型在零样本目标导航中的推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有零样本目标导航方法依赖大语言模型或视觉-语言模型的常识知识，但这些知识来源于互联网文本而非具身3D经验；同时导航过程中的片段观测通常被丢弃，无法积累终身经验。

Method: 提出TrajRAG框架：1) 拓扑-极坐标轨迹表示，紧凑编码空间布局和语义上下文，去除原始观测冗余；2) 分层分块结构，将相似轨迹组织为统一摘要，实现从粗到细的检索；3) 导航时，候选前沿生成多个轨迹假设，查询TrajRAG中相似历史轨迹以指导大模型选择路径点；4) 新经验持续合并到TrajRAG中。

Result: 在MP3D、HM3D-v1和HM3D-v2数据集上的实验表明，TrajRAG能够有效检索相关的几何-语义经验，并提升零样本目标导航性能。

Conclusion: TrajRAG通过将导航经验以结构化的形式积累和检索，有效弥补了大模型缺乏具身3D经验的不足，实现了终身导航经验的积累。
...
TL;DR: 本文提出了一种面向医学图像分割的公平感知主动学习框架，通过加权熵选择策略调节不确定性，并结合掩码缩放熵将真实认知不确定性与解剖体积变异解耦，有效降低了不同敏感属性组间的性能差距。


<details>
  <summary>Details</summary>
Motivation: 标准不确定性主动学习方法仅关注最大化性能指标，忽略了不同敏感属性组间的性能差异或公平性问题；公平主动学习在分类任务中已有探索，但在医学图像分割领域仍是空白。

Method: 提出公平感知主动学习框架，采用加权熵选择策略，基于标注集上当前组特异性性能估计来调节不确定性；使用掩码缩放熵（仅限感兴趣区域）将真实认知不确定性与解剖体积变异解耦。

Result: 在强偏差和弱偏差设置下，相比随机采样和标准不确定性采样，该方法显著减少了组间性能差异；在最终预算时相比标准熵，强偏差下差距度量减少75%，弱偏差下减少86%，并持续取得最高的公平缩放性能。

Conclusion: 本文是医学图像分割中公平主动学习的首批研究之一，为资源受限环境下训练更公平的模型提供了有效策略。
...
TL;DR: 提出DUNE，一种双分支不可学习集成扰动优化方法，分别在空间和颜色域优化扰动，增强对抗高级防御的鲁棒性，在CIFAR-10和ImageNet上平均测试准确率比12种SOTA方法低14.95%-50.82%。
...
Method: 提出DUNE，采用双分支结构分别在空间域和颜色域独立优化扰动，建立扰动与移位标签的映射；并提出不可学习增强集成策略，在双分支优化中聚合多种预训练模型。

Result: 在CIFAR-10和ImageNet上，DUNE在7种主流防御下平均测试准确率比12种SOTA方案低14.95%至50.82%。

Conclusion: DUNE通过扩展扰动域并采用集成策略，显著提升了不可学习示例对先进防御的鲁棒性。
...
TL;DR: SignVerse-2M是一个包含约200万片段、覆盖25种以上手语的大型多语言姿态原生数据集，通过DWPose将原始视频统一转换为2D姿态序列，为开放世界手语识别、翻译和姿态驱动视频生成提供统一接口。


<details>
  <summary>Details</summary>
Motivation: 现有多数手语资源仅提供原始视频-文本对齐级别的监督，且多在实验室环境下制作，无法直接对接现代姿态驱动的开放世界识别与生成框架：RGB模型对背景/衣着敏感，而姿态模型具有风格不变性；现有姿态引导生成模型（如DWPose）使用统一关键点表示，但手语领域缺乏与之兼容的大规模数据集。

Method: 从公开多语言手语视频资源出发，利用DWPose统一预处理流水线将原始视频转换为2D姿态序列，构建约200万片段、覆盖25种以上手语的语料库；保留真实世界视频的录制条件和说话人多样性，同时通过统一姿态表示减少外观变化；提出SignDW Transformer基线模型。

Result: 构建了SignVerse-2M数据集，验证了其在多语言姿态空间建模中的可行性，展示了与现代表态驱动流水线的兼容性。

Conclusion: 该数据集填补了手语领域缺少姿态原生、面向开放场景的大规模数据资源的空白，为姿态驱动的识别、翻译和生成任务提供统一接口，同时指出了当前局限性和可支持的评估范式。
...
TL;DR: GEASS是一个无需训练的门控模块，通过动态决定模型在每次查询中消费多少自生成字幕来解决VLM中的物体幻觉问题，仅需两次额外前向传播即在多个基准上稳定提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有工作将VLM自生成字幕视为统一正资源，但作者发现直接嵌入字幕反而会降低性能（Qwen2.5-VL-3B在HallusionBench上下降近10个点），原因在于字幕会锚定推理轨迹和词汇选择，且字幕错误存在不对称性（遗漏远多于虚构，但每个虚构影响更大）。因此字幕有用性是逐查询属性而非逐语料属性，需要一种按需选择性使用字幕的方法。

Method: 提出GEASS（Gated Evidence-Aware Selective Steering），一个无需训练的模块，核心组件包括：(1) 通过清洁路径的置信度对字幕进行门控；(2) 依据字幕产生的熵减对其进行加权；(3) 当两条路径（无字幕vs有字幕）存在分歧时提高证据门槛。每次查询仅需两次额外前向传播。

Result: 在POPE和HallusionBench两个基准上，覆盖四个不同的VLM模型，GEASS相比普通推理和对比解码方法均取得一致性提升。

Conclusion: 自生成字幕并非总是有益，其有用性高度依赖于具体查询。通过按查询动态门控和加权字幕信息，GEASS能够在不增加训练成本的前提下有效缓解物体幻觉问题，仅需少量额外计算开销。
...
TL;DR: 提出多尺度高斯-语言地图(GLMap)，通过3D高斯表示与自然语言描述的双模态接口，实现显式几何、多尺度语义与零样本大模型兼容，在导航和空间问答任务上取得优异效果。
...
Method: 提出GLMap，包含三个关键设计：(1) 显式几何；(2) 涵盖实例级和区域级概念的多尺度语义；(3) 每个语义单元联合存储自然语言描述和3D高斯表示的双模态接口。同时提出高斯估计器(Gaussian Estimator)，从稠密点云解析推导高斯参数，无需基于梯度的优化。

Result: 在ObjectNav、InstNav和SQA三个任务上，GLMap有效提升了目标导航和上下文推理能力，并且以零样本方式与基于大模型的方法兼容。

Conclusion: GLMap通过3D高斯与语言描述的双模态语义建图，有效解决了几何-语义权衡和大模型接口问题，为具身导航与推理提供了新的建图范式。
...
TL;DR: 提出ATMask自适应掩码策略，基于层间纹理变化图对高复杂度区域进行选择性掩码，结合大规模CBCT数据集（6,314张扫描），在三项下游任务中实现了比随机掩码和其他SSL方法更高效的表征学习。


<details>
  <summary>Details</summary>
Motivation: 标准MIM的随机掩码策略无法优先覆盖牙科CBCT中具有诊断关键性的区域（如细微病理变化和复杂解剖边界），导致模型无法有效学习关键区域的表征。

Method: 提出ATMask自适应掩码策略：计算层间纹理变化图（inter-slice texture variation map）来识别结构或纹理复杂度高的区域，在预训练时对这些高变化区域进行选择性掩码，迫使模型学习更丰富的上下文表征来推断复杂的3D形态变化。

Result: 在三个下游牙科CBCT任务上，ATMask相比标准随机掩码及其他先进SSL基线方法，实现了更数据高效、更强大的表征学习性能。

Conclusion: ATMask通过关注高纹理复杂度区域的掩码策略，有效提升了牙科CBCT中AI模型的自监督预训练效果，并贡献了首个大规模CBCT预训练数据集（6,314张扫描）。
...
TL;DR: 针对文本到3D生成中存在的宏观拓扑不一致（如Janus问题）和微观几何不连续问题，本文提出了MOC-3D方法，通过语义视角顺序约束模块（利用CLIP先验施加单调性秩约束）和基于流形的特征连续性模块（在SPD流形上度量特征统计分布距离），在ScaleDreamer框架下实现了宏观结构与微观细节的协同优化。


<details>
  <summary>Details</summary>
Motivation: 当前基于SDS和2D扩散先验的文本到3D生成方法存在两个主要问题：一是2D先验的视角偏差导致宏观拓扑不一致（Janus问题），二是高CFG带来的模式搜索模糊和梯度噪声引发微观几何不连续。现有方法未能同时有效解决这两方面挑战。

Method: 基于ScaleDreamer框架，提出MOC-3D方法，包含两个核心模块：1) 语义视角顺序约束模块，利用CLIP先验知识对不同视角的语义分数表示施加单调性秩约束，引导3D对象的全局拓扑结构；2) 基于流形的特征连续性模块，在对称正定（SPD）流形上使用黎曼度量，通过测量特征统计分布在黎曼空间中的距离，从统计意义上促进多视角间微观纹理的平滑演变和连续性。两个模块实现宏微观协同优化。

Result: 该方法能够同时改善宏观结构一致性和微观细节连续性，有效解决Janus问题等拓扑不一致现象以及几何表面不连续问题。

Conclusion: 通过在几何流形和语义视角顺序两个维度上引入约束，MOC-3D为文本到3D生成中的宏观和微观质量问题提供了统一的解决方案，验证了特征统计分布流形建模与语义视角排序约束相结合的有效性。
...
TL;DR: 本文提出ProfileSynth数据集和基于FLAME的侧脸3D重建基线方法，通过几何条件扩散模型生成极端侧脸合成数据，并引入可见性感知的下颌线正则化，实现了从单张侧脸RGB图像进行3D人脸重建，为非侵入式头影测量分析奠定了基础。


<details>
  <summary>Details</summary>
Motivation: 传统正畸头影测量依赖侧位X光片，但频繁X光暴露存在辐射风险。现有从侧脸RGB图像检测 landmarks 的方法仅利用2D特征（如眼睛、嘴巴、耳朵和轮廓），未能充分利用3D面部几何（如面部轮廓和下颌线）进行精确诊断。同时，现有3DMM回归器主要针对近正脸图像开发，在极端侧脸视角（偏航角约90°）下由于大部分面部被遮挡，仅剩边界线索，准确重建面临挑战。

Method: 1) 创建ProfileSynth数据集：在极端偏航范围内采样FLAME形状和姿态参数，利用扩散模型以深度图和法线图为条件生成逼真的侧脸图像；2) 提出侧脸专用FLAME回归基线，包含可见性感知的下颌线正则化。

Result: 建立了"侧脸×3DMM"重建的实用基线，为从侧脸RGB图像进行更准确、非侵入式的头影测量分析提供了有前景的基础。

Conclusion: 本文通过几何条件合成数据和侧脸专用回归方法，弥合了极端侧脸视角下3D人脸重建的空白，为临床正畸中替代X光的非侵入式3D头影测量分析提供了可行方案。
...
TL;DR: 提出基于跨样本语义传播(CSP)的点云自监督学习框架，通过状态空间模型实现批次内样本间的语义状态传播，并引入非对称语义保持蒸馏(SPD)解决批依赖问题，在多个基准数据集上取得最优性能。
...
Method: 提出CSP框架：将批次内样本序列化后输入状态空间模型进行语义状态传播，显式建模跨样本动态依赖；同时引入SPD，通过异构输入机制和语义特征对齐约束，在微调阶段实现语义迁移的结构对齐，消除批依赖导致的不一致性。

Result: 在多个基准数据集上，该方法在性能与语义一致性方面均持续优于现有最先进方法。

Conclusion: 跨样本语义传播机制能有效解决场景级点云自监督学习中的语义不一致问题，所提框架在单场景测试条件下仍能保持结构化语义一致性与鲁棒性。
...
TL;DR: 提出TrajShield，一种无需训练、推理时的防御框架，通过因果干预时间结构语义空间，统一处理显式不安全提示、越狱攻击和时间涌现风险，在T2VSafetyBench上实现平均ASR降低52.44%。


<details>
  <summary>Details</summary>
Motivation: 现有提示层面防御主要继承自文生图安全领域，仅处理输入词汇表面，易受越狱攻击；且T2V生成存在时间涌现风险（看似安全的提示经时间外推产生不安全内容），此前工作未解决。

Method: 将T2V安全重构成时间结构化语义空间中的因果干预：模拟提示隐含轨迹→定位风险因果起源→应用最小侵入式重写以消除风险同时保留安全无关语义。

Result: 在T2VSafetyBench的14个安全类别及多个T2V后端上达到最先进防御性能，平均ASR降低52.44%，同时保持高语义保真度。

Conclusion: TrajShield无需训练即可在推理时统一防御多种安全风险，显著优于现有防御方法。
...
TL;DR: MedScribe将放射学报告生成重新定义为迭代证据收集的序列决策过程，通过大语言模型动态调用病理学诊断工具提取局部体积特征并查询多维检索空间，无需任务特定微调即在CT-RATE和RadChestCT数据集上提升了临床准确性、事实一致性和可解释性。


<details>
  <summary>Details</summary>
Motivation: 现有视觉-语言模型在3D CT影像报告生成中依赖全局嵌入压缩，导致幻觉性发现和有限的解剖学定位能力。

Method: 将报告生成建模为序列决策过程，LLM动态调用病理学专用诊断工具提取局部体积特征，利用结构化特征查询与病理学文本证据对齐的多维检索空间，在合成前显式累积定量证据。

Result: 在CT-RATE和RadChestCT上，无需任务特定微调，即在临床准确性、事实一致性和可解释性上超越现有2D和3D VLM。

Conclusion: 假设驱动的推理方法对于可靠的医学影像报告生成具有重要价值，能有效减少无依据的结论并增强细粒度定位。
...
TL;DR: 本文提出Embody4D，一种面向具身场景的视频到视频世界模型，能够从单目视频合成任意新颖视角，通过3D感知的合成流水线、自适应噪声注入策略和交互感知注意力机制，解决多视图数据稀缺、时空一致性保持和操作细节幻觉等挑战，在合成高保真、视角一致的视频方面达到最优性能。


<details>
  <summary>Details</summary>
Motivation: 现有具身世界模型大多局限于2D表示，缺乏具身空间推理所需的多视角信息，且面临配对多视图数据严重稀缺、生成3D几何体的时空一致性难以维持以及操作细节容易产生幻觉等挑战。

Method: 1) 提出3D感知的组合式合成流水线，构建跨具身形态的机械臂与多样化背景的异构数据集；2) 设计自适应噪声注入策略，利用图像区域间的置信度差异选择性正则化扩散过程以保障时空一致性；3) 引入交互感知注意力机制，显式关注机器人交互区域以保证操作保真度。

Result: 在多种具身场景下实现最优性能，能够合成高保真、视角一致的视频，有效支持下游机器人规划与学习任务。

Conclusion: Embody4D作为鲁棒的具身世界模型，通过解决数据稀缺、时空一致性和操作细节幻觉三大核心问题，为具身空间推理和机器人学习提供了有效的多视图生成框架。
...
### [83] [Cross-Domain Adversarial Augmentation: Stabilizing GANs for Medical and Handwriting Data Scarcity](https://arxiv.org/abs/2605.01815)
*Md. Sohanuzzaman Soad,Mahady Al Hady,S M Rafiuddin Rifat,Sudip Ghose*

Main category: cs.CV

TL;DR: 本文研究了在低资源场景（孟加拉手写字符和胸部X光影像）中，使用DCGAN进行生成式数据增强，通过IS、FID、t-SNE/UMAP评估质量，并验证了在分类器性能上的提升，同时讨论了训练稳定性、数据混合比例、医学图像评估陷阱及隐私风险。


<details>
  <summary>Details</summary>
Motivation: 解决视觉任务中数据稀缺问题，探索GAN在低资源领域的生成式数据增强效果及其对下游分类任务的实用价值。

Method: 使用DCGAN在64×64分辨率下训练，结合梯度惩罚和谱归一化增强稳定性；通过IS、FID、t-SNE/UMAP评估生成样本质量；在真实/混合数据集上训练分类器评估下游效用；进行合成-真实比例消融和样本过滤实验。

Result: 生成式增强提升了样本多样性，并在有限数据条件下持续改进了分类器性能。

Conclusion: 该协议简单可复现，为资源受限的成像任务中应用生成式增强提供了强基线，同时需注意医学图像的评估特殊性和合成数据的隐私风险。
...
TL;DR: 本文提出了一种混合视觉遥测方案，在低比特率视频流基础上，通过选择性传输高细节感兴趣区域(ROI)静态图像，以解决带宽受限机器人系统中视频压缩导致机器感知精度下降的问题。实验基于无人机数据集，对比了纯视频流与混合方案在相同通信预算下的性能。


<details>
  <summary>Details</summary>
Motivation: 带宽受限的机器人监控系统通常依赖单一压缩视频流同时支持场景感知和机器视觉任务，但低比特率视频在保留运动和粗略上下文的同时，丢失了目标识别所需的精细局部细节，导致性能不匹配。

Method: 提出两通道视觉遥测方案：低比特率x265/HEVC视频流提供连续场景理解，同时选择性传输高细节JPEG静态ROI图像用于精细分析。在匹配总通信预算下，对比纯视频流与混合方案性能，使用无人机数据集、两种比特率机制、多种ROI触发策略及目标级分类精炼评估。

Result: 建立了混合传输范式的实验方法论基础，为后续引入JPEG AI作为语义静态图像通道的第二阶段研究提供了可复现的技术栈和评估协议。

Conclusion: 本文不试图证明新静态图像编解码器的优越性，而是确立混合传输范式本身，为在带宽约束下通过选择性高细节ROI增强机器感知提供了可行且可复现的框架。
...
TL;DR: 提出Contextual Latent Steering (CSteer)，一种无需训练的视觉提示引导方法，使通用多模态大模型能够同时指代多个区域，在多个数据集上超越专用指代模型并达到新SOTA。


<details>
  <summary>Details</summary>
Motivation: 现有的多模态大模型在视觉提示引导的区域级感知上表现不佳，尤其是同时指代多个区域或需要全局上下文进行精确视觉指代的场景。

Method: CSteer：一种无需训练的方法，通过预计算隐式表示视觉指代行为的上下文向量（如区域区分和全局上下文关注），在推理时进行表示编辑来引导模型。

Result: 在多个数据集上，使用CSteer的通用多模态大模型在大多数情况下优于专门的指代模型，达到了新的SOTA。

Conclusion: CSteer提供了一种无需微调或修改架构的有效方案，在视觉指代任务上展现出优异性能，为该领域设立了新的基准。
...
TL;DR: GeoSAE通过几何引导的稀疏自编码器，从脑MRI基础模型中提取可解释的阿尔茨海默病生物标志物，仅用2%的嵌入维度即可预测MCI-to-AD转化（AUC 0.746），且特征在不同队列间高度可重复（r=0.97）。


<details>
  <summary>Details</summary>
Motivation: 脑MRI基础模型虽能学习丰富的解剖学表征，但如何解释其中编码的临床信息仍是开放问题。标准稀疏自编码器（SAE）在深层transformer中面临严重的特征坍缩，且在阿尔茨海默病研究中，年龄几乎混淆了所有临床变量，导致朴素标注不可靠。

Method: 提出GeoSAE框架，利用基础模型学习的流形结构防止特征坍缩，并通过去除年龄混淆的偏相关分析对每个存活特征进行标注。使用约1.4万张T1加权MRI扫描（ADNI和AIBL数据集）进行验证。

Result: GeoSAE识别出紧凑且完全可解释的特征集，仅用2%的嵌入维度即可预测MCI-to-AD转化（AUC 0.746），而共病标注的特征仅达到随机水平。识别特征在不同队列间无需重新训练即可复现（r=0.97），并定位于与Braak分期一致的神经解剖学区域。

Conclusion: 几何引导的SAE能够从冻结的脑MRI基础模型中提取可解释的生物标志物，为临床解释提供了有效工具。
...
TL;DR: 本文提出利用双像素（DP）传感器拍摄的多视角图像自动解决多视角3D重建中的尺度模糊性问题，无需参考物体或事先标定。通过DP图像中的散焦模糊信息与多视角重建得到的尺度模糊深度图结合，可确定绝对尺度，并基于交叉视角模糊核进行图像对齐优化。


<details>
  <summary>Details</summary>
Motivation: 多视角3D重建（SfM+MVS）存在未知尺度模糊性问题，通常需要场景中存在已知尺寸的参考物体才能解决。本文旨在消除这一限制，使多视角3D重建能在无参考物体的情况下自动恢复绝对尺度。

Method: 1) 利用双像素（DP）传感器捕获的多视角图像中的散焦模糊信息；2) 将散焦模糊与多视角重建得到的尺度模糊深度图结合，提出线性方法估计绝对尺度；3) 基于交叉视角模糊核，通过强度优化阶段将左右DP图像相互对齐，进一步优化结果。

Result: 在多种不同相机和镜头采集的多样化场景中，该方法均能有效恢复绝对尺度，实验验证了其有效性。

Conclusion: 双像素传感器提供的散焦模糊信息可以自动解决多视角3D重建的尺度模糊问题，无需参考物体或事先标定，代码和数据已开源。
...
TL;DR: 提出一种基于局部线性混合形状的移动端高保真人脸化身重建方法，使用WebGPU实现，在移动设备上可达2K分辨率120FPS。


<details>
  <summary>Details</summary>
Motivation: 现有基于高斯的人体化身方法虽然质量高，但计算量大难以部署到移动设备；近期蒸馏方法虽能运行但细节损失严重。需要一种既能保持高细节又能高效运行于移动设备的方法。

Method: 观察到局部区域高斯属性高度相关，提出使用局部线性混合形状（local linear blendshapes）在身体小区域建模高斯属性的全局非线性变化；对属性变化小的高斯移除混合形状以减小计算量和模型大小；采用端到端训练无需预训练模型；使用WebGPU实现多设备适配。

Result: 方法能渲染细节更优的高质量人体化身，在移动设备上达到2K分辨率120FPS。

Conclusion: 提出的局部线性混合形状方法有效平衡了移动端化身渲染的质量与效率，端到端训练和WebGPU实现使其具备实际部署价值。
...
TL;DR: 本文提出DSCache，一种无需训练的缓存构建机制，通过解耦历史缓存与即时缓存，并采用位置无关编码，使预训练离线模型适应无限流式视频理解，在流式视频问答基准上取得平均2.5%的准确率提升。


<details>
  <summary>Details</summary>
Motivation: 现有流式视频VLLM无法扩展到无限视频流，或仅关注缓存复用策略，而缓存构建机制对模型性能的影响未被充分探索。

Method: 提出解耦流式缓存(DSCache)：1) 维护累积的历史KV缓存，同时按需构建独立的即时缓存，与历史缓存解耦以保持近期输入的信息量；2) 采用位置无关编码策略，使KV缓存支持未见位置并防止位置溢出，实现超出训练长度的位置外推。

Result: 在流式视频问答基准上达到最优性能，平均准确率比先前方法提升2.5%。

Conclusion: DSCache通过解耦缓存设计和位置无关编码，有效解决了流式视频理解中无限流处理与训练-推理长度不匹配两大挑战，且无需重新训练。
...
TL;DR: 本文介绍了一个名为BadmintonGRF的多模态羽毛球数据集，包含同步多视角视频（120 FPS）、Kistler测力台和Vicon动捕数据，并提供了分层数据访问（Tier 1/Tier 2）及基准任务，用于无标记载荷估计研究。


<details>
  <summary>Details</summary>
Motivation: 非周期性球类运动的实验室级多模态资源稀缺，尤其是公开提供仪器化地面反作用力（GRF）与高帧率多视角视频配对的数据集几乎不存在，这限制了在真实训练场景下无标记载荷估计方法的发展。

Method: 构建包含8台同步RGB相机（~120 FPS）、4块Kistler测力台和Vicon动捕系统的采集平台；跨模态时间对齐结合人工验证事件、自动化质量保证和每相机时间偏移元数据；提供两层数据访问：Tier 1（含姿态、对齐GRF、元数据等）和Tier 2（原始RGB和C3D）；设计基准任务从2D姿态映射GRF。

Result: 公开数据集包含10名受试者的156次仪器化试验，共17,425个冲击片段（Tier 1）；多视角去重后得到12,867个视角相关实例和1,732个唯一冲击事件；原始多视角RGB数据超过1 TB；提供了留一法交叉验证分割及10个参考基线结果。

Conclusion: 该数据集填补了羽毛球运动中多模态GRF-视频配对数据的空白，为无标记载荷估计提供了标准化基准；数据分层访问平衡了开放性与对完整运动学/外观信息研究的支持。
...
TL;DR: 本文提出Chart-FR1，一种面向高信息密度图表的聚焦驱动细粒度图表推理模型。通过Focus-CoT（视觉聚焦思维链）增强细粒度感知，Focus-GRPO（聚焦驱动的强化学习算法）压缩冗余视觉信息并自适应控制推理深度，以及构建HID-Chart基准数据集，在多个图表基准上超越现有MLLMs。


<details>
  <summary>Details</summary>
Motivation: 现有MLLMs在处理高信息密度（HID）图表（含多子图、图例、密集标注）时面临三大挑战：细粒度感知不足导致关键视觉线索遗漏；冗余/噪声视觉信息影响多模态推理性能；缺乏根据信息量自适应调节推理深度的能力。

Method: 1) Focus-CoT：将推理步骤显式链接到关键视觉线索（局部图像区域、OCR信号）以增强细粒度感知；2) Focus-GRPO：聚焦驱动强化学习算法，包含信息效率奖励（压缩冗余信息）和自适应KL惩罚机制（根据发现的视觉线索灵活控制推理深度）；3) HID-Chart：构建带信息密度度量的高信息密度图表基准。

Result: 在多个图表理解与推理基准上，Chart-FR1优于当前最先进的MLLMs。

Conclusion: 所提出的聚焦驱动细粒度推理范式有效解决了高信息密度图表中的感知、聚焦效率和自适应深度推理问题。
...
### [92] [WindowQuant: Mixed-Precision KV Cache Quantization based on Window-Level Similarity for VLMs Inference Optimization](https://arxiv.org/abs/2605.02262)
*Wei Tao,Xiaoyang Qu,Peiqiang Wang,Guokuan Li,Jiguang Wan,Kai Lu,Jianzong Wang*

Main category: cs.CV

TL;DR: 本文提出WindowQuant方法，通过窗口自适应混合精度量化来优化视频语言模型中的KV缓存，解决了视觉token序列过长导致的推理延迟和GPU内存问题。该方法包含窗口级量化搜索和窗口级KV缓存计算两个模块，在保持模型精度的同时提升了硬件效率。


<details>
  <summary>Details</summary>
Motivation: 视频语言模型（VLM）的视觉token序列过长，导致推理延迟高和GPU内存消耗大。现有基于token粒度的混合精度量化方法搜索过程耗时且硬件计算效率低。

Method: 提出WindowQuant方法，包含两个模块：1）窗口级量化搜索：基于视觉token窗口与文本提示之间的相似度分数，快速确定KV缓存窗口的最优位宽配置；2）窗口级KV缓存计算：在量化前对KV缓存窗口进行重排序，避免混合精度量化在推理计算中的硬件低效问题。

Result: 在多个数据集上的大量实验表明，WindowQuant在性能上优于现有的最先进VLM模型和KV缓存量化方法。

Conclusion: WindowQuant通过窗口自适应的混合精度量化策略，有效平衡了VLM中KV缓存的压缩效率与模型精度，同时解决了硬件计算效率问题。
...
TL;DR: 本文提出AFFormer，一种基于Transformer的自适应特征融合框架，通过建模时间、智能体间和空间相关性，增强V2X协同感知在信道损伤（噪声、衰落、干扰）下的鲁棒性。在V2XSet和DAIR-V2X数据集上，该方法在理想和受损通信条件下均优于现有方法。


<details>
  <summary>Details</summary>
Motivation: V2X协同感知虽能提升3D目标检测性能，但实际通信中存在的噪声、衰落和干扰等信道损伤会导致共享的感知特征发生退化，影响检测的可靠性与安全性。

Method: 提出AFFormer框架，包含三个核心模块：Multi-Agent and Temporal Aggregation（跨智能体和时序的上下文感知融合）、Dual Spatial Attention（高效建模空间依赖关系）、Uncertainty-Guided Fusion（基于熵的融合特征精炼）。同时采用教师-学生知识蒸馏策略，将融合特征与可靠的早期协同监督对齐，增强鲁棒性。

Result: 在V2XSet和DAIR-V2X数据集上，AFFormer在理想和受损通信条件下均持续优于现有方法，证明了其对通信导致特征退化的鲁棒性，并保持了竞争性的效率-精度权衡。

Conclusion: 通过Transformer架构对时序、跨智能体和空间相关性建模，并结合不确定性引导融合与知识蒸馏，能够有效缓解V2X协同感知中通信信道损伤带来的特征退化问题，提升智能交通系统的可靠性。
...
TL;DR: 本文提出M²-REPA，首个针对多模态视频生成的表征对齐方法。通过解耦扩散模型中间表征中不同模态的特征，并将其与各自对应的专家基础模型对齐，充分利用多个基础模型的先验知识。实验表明该方法在视觉质量和长期一致性上显著优于基线。


<details>
  <summary>Details</summary>
Motivation: 现有多模态世界模型试图联合生成RGB、深度、掩码等多模态视频，但未能充分利用已有基础模型的丰富先验知识，导致生成质量受限。

Method: 1) 解耦扩散模型中间表征中的模态特定特征；2) 分别与对应的专家基础模型对齐；3) 设计两个协同目标：多模态表征对齐损失（特征-专家匹配）和模态特定解耦正则化（促进跨模态互补性）。

Result: 在视觉质量和长期一致性上显著优于基线方法。

Conclusion: 通过联合优化多个基础模型的先验知识，M²-REPA能有效提升多模态视频生成的表现，为多模态生成任务提供了新的表征对齐范式。
...
### [95] [Behavior-Grounded Lane Representation Learning for Multi-Task Traffic Digital Twins](https://arxiv.org/abs/2605.01901)
*Rei Tamaru,Pei Li,Bin Ran*

Main category: cs.CV

TL;DR: 本文提出了GeoLaneRep，一个面向交通数字孪生的行为基础车道表征学习框架，能够联合编码静态几何、车辆轨迹和操作描述，实现跨相机零样本匹配（横向排名误差0.004，边缘角色F1=1.000）、异常检测（AUROC=0.991）及规格驱动的车道几何合成（准确率87.9%）。


<details>
  <summary>Details</summary>
Motivation: 现有交通数字孪生系统基于静态几何表示，缺乏捕捉动态功能语义的能力，无法支持行为感知推理（如复杂交通条件下的车道运行状态）。

Method: 提出GeoLaneRep框架，将静态车道几何、观测车辆轨迹和操作描述符联合编码到共享的跨相机语义嵌入空间；采用对比跨相机对齐、辅助角色监督和时间异常检测的联合目标训练编码器；利用行为嵌入条件化扩散生成器合成满足目标规格的车道几何。

Result: 在16个路侧相机和132个车道上，零样本跨相机匹配的横向排名误差为0.004，边缘角色F1为1.000，窗口级异常检测AUROC为0.991；在38个车道组上规格合成准确率为87.9%。

Conclusion: GeoLaneRep为路侧观测与下游数字孪生任务之间提供了语义接口，支持跨相机迁移、行为感知监控和目标导向的车道合成，可作为交通数字孪生中行为基础表征的有效方案。
...
### [96] [SurgCheck: Do Vision-Language Models Really Look at Images in Surgical VQA?](https://arxiv.org/abs/2605.01911)
*Jongmin Shin,Ka Young Kim,Eunki Cho,Seong Tae Kim,Namkee Oh*

Main category: cs.CV

TL;DR: 本文提出SurgCheck，一个用于量化手术视觉问答中语言捷径依赖的诊断基准。通过在配对问题设计中移除实体名称，发现多个VLM在视觉输入不变的情况下性能显著下降，表明模型依赖语言捷径而非真正的视觉理解。


<details>
  <summary>Details</summary>
Motivation: 现有手术VQA数据集常包含语言捷径（问题措辞隐含约束答案空间），导致模型报告的性能可能反映的是对语言捷径的依赖而非真实的视觉理解，因此需要一种诊断方法量化这种依赖程度。

Method: 提出SurgCheck诊断基准，采用配对问题设计：每个手术帧关联一个含实体名称的原始问题和去除实体名称的低偏置版本（通过边界框、箭头、空间位置、迂回表达四种线索维持问题可回答性）。性能差距作为捷径依赖的诊断信号。评估了通用和手术专用VLM在零样本和微调设置下的表现，并引入LLM作为裁判的零样本评估协议。

Result: 五个VLM在低偏置问题上均出现一致性的性能下降，尽管视觉输入完全相同。文本消融实验显示动作和目标预测的性能下降极小，表明这些任务的预测主要由语言捷径而非视觉推理驱动。

Conclusion: SurgCheck提供了一个受控的诊断框架，暴露了现有手术VQA基准中被语言偏置掩盖的失败模式。强基准性能并不一定意味着真实的视觉理解，需要在手术VQA中引入偏置感知的评估。
...
### [97] [EAPFusion: Intrinsic Evolving Auxiliary Prior Guidance for Infrared and Visible Image Fusion](https://arxiv.org/abs/2605.01916)
*Zhenyu Sun,Luobin Zhang,Axi Niu,Haishen Wang,Qingsen Yan*

Main category: cs.CV

TL;DR: 提出EAPFusion，利用自进化内在先验和动态卷积实现红外-可见光图像融合，在多个数据集上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有融合方法依赖静态训练权重，无法在推理时自适应场景内容；且粗粒度辅助语义注入存在粒度不匹配问题，难以同时突出目标并保留细节。

Method: 维护一组紧凑的内在先验并跨尺度逐步更新，利用进化后的先验动态生成卷积核，实现实例自适应的动态卷积；设计通道级融合模块，通过交错红外和可见光通道并进行局部通道混合增强跨模态互补性。

Result: 在多个数据集上（含跨数据集评估和语义分割任务）取得了定量和定性最优的融合结果，并持续提升下游任务性能。

Conclusion: EAPFusion通过自进化先验和动态卷积有效解决了静态权重和粒度不匹配问题，实现了更优的红外-可见光图像融合。
...
### [98] [SimPB++: Simultaneously Detecting 2D and 3D Objects from Multiple Cameras](https://arxiv.org/abs/2605.01924)
*Yingqi Tang,Zhaotie Meng,Erkang Cheng,Haibin Ling*

Main category: cs.CV

TL;DR: SimPB++提出了一种统一的端到端混合解码器架构，能够同时从多摄像头进行2D透视视图目标检测和3D鸟瞰视图目标检测，通过动态查询分配、自适应查询聚合、查询组注意力、裁剪缩放策略和传播去噪策略等模块实现深度交互，支持混合监督训练，在nuScenes和Argoverse2数据集上取得了SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有两阶段pipeline仅将2D检测结果作为3D检测的一次性线索，未能实现2D与3D任务的深度融合与互惠增强，且缺乏对多摄像头场景下两种任务的统一端到端建模。

Method: 提出混合解码器架构，包含交互耦合的多视角2D和3D解码器；动态查询分配（DQA）自适应分配2D查询到3D候选；自适应查询聚合（AQA）利用多视图2D特征循环精炼3D表示；查询组注意力实现2D检测的组内通信；裁剪缩放策略处理远距离感知；传播去噪策略结合RoI检测器辅助训练；支持混合监督学习。

Result: 在nuScenes数据集上2D和3D检测均达到SOTA性能；在Argoverse2上实现了最远150米的长距离检测能力。

Conclusion: SimPB++通过双向3D-2D-3D循环精炼的端到端框架，有效统一了多摄像头2D和3D感知任务，显著提升了检测精度和远距离感知能力，同时降低了对昂贵3D标签的依赖。
...
### [99] [PubMed-Ophtha: An open resource for training ophthalmology vision-language models on scientific literature](https://arxiv.org/abs/2605.02720)
*Verena Jasmin Hallitschke,Carsten Eickhoff,Philipp Berens*

Main category: cs.CV

TL;DR: PubMed-Ophtha是一个从PubMed Central的15,842篇开放获取文章中提取的102,023对眼科图像-标题对的分层数据集。它直接从PDF提取全分辨率图像，拆分为面板/子图像，标注了成像模态和标记状态，使用两步LLM方法将图注拆分为子标题（BLEU 0.913），并公开了人类标注数据、模型和完整生成管道。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在眼科领域潜力巨大，但其发展依赖于大规模、高质量的图像-文本数据集，而这类数据集目前仍然稀缺。

Method: 从PubMed Central的15,842篇开放获取文章中提取PDF，全分辨率提取图像并分解为面板和子图像；使用两步LLM方法将图注拆分为面板级子标题；标注成像模态（彩色眼底摄影、OCT、视网膜成像等）和标记状态（如箭头等注释标记的存在情况）；训练面板/图像检测模型；发布人类标注真值数据、模型和完整生成管道以确保可复现性。

Result: 构建了包含102,023对眼科图像-标题对的分层数据集；两步LLM拆分方法在人工标注数据上达到平均句子BLEU分数0.913；面板和图像检测模型分别达到mAP@0.50为0.909和0.892；图像提取的中位IoU达到0.997。

Conclusion: PubMed-Ophtha通过大规模、高质量的眼科图像-文本数据集及配套工具，为眼科视觉语言模型的研究提供了重要资源，并注重可复现性。
...
TL;DR: CADFS是一个数据为中心的框架，通过FeatureScript表示和45万真实CAD模型数据集，让大型视觉语言模型能够生成包含15种建模操作的复杂CAD设计历史，在文本条件CAD生成和图像重建任务上达到SOTA。


<details>
  <summary>Details</summary>
Motivation: 现有生成式CAD系统受限于简化的表示和有限的数据集，仅能生成草图-拉伸等简单操作，无法处理更复杂的建模操作，亟需一种能生成复杂、真实CAD设计历史的框架。

Method: 提出FeatureScript-based表示法，构建包含15种建模操作的45万真实CAD模型数据集；设计新pipeline重建可执行的FeatureScript程序并提供多模态标注；微调视觉语言模型在该表示上进行CAD生成。

Result: 在文本条件CAD生成和图像重建任务上达到SOTA，生成的设计比先前框架更准确、更多样化、特征更丰富；消融实验验证了FeatureScript表示、扩展操作集和文本描述三个组件的有效性。

Conclusion: 该框架显著拓宽了生成式CAD的复杂度和真实感，为生成式CAD领域提供了更强大的数据驱动方案。
...
TL;DR: 针对视频扩散模型及其变体（步蒸馏、因果蒸馏）与现有LoRA权重不兼容的问题，本文提出了一种无数据的聚类感知频谱仲裁框架CASA，通过分析权重空间中的频谱干扰机制，动态协调目标流形保护与LoRA对齐恢复，有效缓解了风格退化和结构崩塌。


<details>
  <summary>Details</summary>
Motivation: 现有LoRA无法直接适配步蒸馏或因果蒸馏的视频扩散模型变体，直接应用会导致风格退化和结构崩塌，但其底层机制尚不明确，亟需填补这一空白。

Method: 深入权重空间分析发现不兼容源于共享奇异子空间内功能聚类的频谱干扰，具体为两种范式虽都遵循频谱刚性但建立了冲突的路由路径（构造性过载或破坏性抵消）；据此提出CASA，一种基于频谱密度的无数据框架，动态仲裁目标流形保护与LoRA对齐恢复。

Result: 大量实验表明CASA有效减轻了伪影并恢复了LoRA的功能。

Conclusion: CASA成功解决了视频扩散模型变体与LoRA之间的权重空间不匹配问题，提供了一种无需训练数据的通用解决方案。
...
TL;DR: ProtoFair是一种公平性感知对比损失，通过无监督原型聚类识别伪反事实对（同一聚类中不同敏感属性的样本），将它们拉近以学习对敏感属性不变的表示，无需修改现有SSL目标，在CelebA和UTKFace上实现了公平性提升并保持竞争性准确率。


<details>
  <summary>Details</summary>
Motivation: 现有自监督学习方法学习到高质量视觉表示，但这些表示往往捕获了训练数据中的人口统计学偏差。已有的公平性方法需要重新设计自监督学习目标本身，限制了它们在不同SSL框架间的可移植性。

Method: ProtoFair提出一种公平性感知对比损失，利用无监督原型聚类识别伪反事实对（相同聚类但不同敏感组别的样本），在嵌入空间中拉近这些内容匹配的跨组样本，使编码器学习到对敏感属性不变的表示。该方法仅需敏感属性标注，无需目标标签，可与SimCLR和SupCon无缝集成。

Result: 在CelebA和UTKFace数据集上的实验表明，ProtoFair在保持竞争性准确率的同时，持续提升了公平性指标。

Conclusion: ProtoFair是一种即插即用的公平性增强方法，能够在不修改现有自监督学习目标的前提下，有效缓解视觉表示中的人口统计学偏差问题。
...
TL;DR: 本文提出并实现了一个系统框架，用于分析3D Gaussian Splatting (3DGS)在自动驾驶安全相关场景重建中的能力与局限性，重点关注车辆和行人两类关键目标的重建保真度，为工业级AD软件开发与测试提供指导。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶系统（ADS）的安全性依赖于全面且多样的数据集，但现场数据收集缺乏对稀有但可能发生的安全相关场景的覆盖。3DGS虽在场景重建与编辑中展现出潜力，但其在工业保真度评估方面尚不充分，尤其是针对动态、非受控环境及部分遮挡物体的重建质量缺乏系统性分析。

Method: 提出并实现了一个系统框架，用于分析3DGS在安全相关场景重建中的能力与局限性。该框架重点评估车辆和行人这两类ADS最关键目标类别在多个新颖视角（横向和纵向）下的重建保真度。

Result: 研究提供了关于3DGS重建在多个新颖视角下保真度退化的行业洞察，明确了其在横向和纵向视角下的性能表现及局限性。

Conclusion: 该框架能够支持3DGS方法集成到真实世界的工业级自动驾驶软件开发与测试流程中，为安全相关场景的重建质量评估提供了系统性分析与行业指导。
...
TL;DR: 本文研究了在低资源场景下，利用阿拉伯语、乌尔都语和波斯语共享文字系统的特点，通过跨脚本联合训练提升手写文本识别（HTR）性能。实验表明该方法优于单脚本基线，特别是在低资源设置下达到了新SotA。关键发现是跨脚本迁移主要由字符层面的重叠驱动，共享字符受益显著，而语言特有字符转移效果有限甚至为负。


<details>
  <summary>Details</summary>
Motivation: 阿拉伯语系手写文本识别在标注数据有限时性能严重下降，而阿拉伯语系各语言共享书写系统且字符重叠度高，因此利用跨脚本训练缓解数据稀缺问题。

Method: 采用CRNN模型，在多个相关阿拉伯语系数据集（阿拉伯语KHATT、乌尔都语NUST-UHWR、波斯语PHTD）的并集上进行联合训练，然后在各目标语言上分别评估。实验在K=100,500,1000三种低资源设置下进行行级控制实验，并通过字符级统计分析探究迁移机制。

Result: 在波斯语PHTD上联合训练达到CER 9.99，超越此前报告结果；在乌尔都语UNHD上CER从17.20降至14.45。字符级分析表明性能提升主要集中在对齐的共享字符上，语言特有字符转移效果有限或为负。

Conclusion: 跨脚本联合训练能有效提升低资源阿拉伯语系手写文本识别性能，但迁移收益主要由脚本级别的字符重叠驱动而非均匀提升，为理解低资源语系中的迁移动力学提供了洞见。
...
TL;DR: 提出了一种基于分割掩码的自监督预训练方法，通过重建关键身体部位（尤其是手部）的运动来提升手语识别中的细粒度表征学习，在三个数据集上以更少输入实现SOTA。


<details>
  <summary>Details</summary>
Motivation: 现有方法使用在通用动作数据集上预训练的编码器，难以捕捉手语中细微的手部差异和运动模式。

Method: 采用基于分割的掩码策略，根据关键身体部位的存在和运动进行自适应掩码，并通过掩码-重建的自监督目标来学习细粒度手语表征。

Result: 在WLASL、NMFs-CSL和Slovo三个数据集上取得了最先进的性能，在实例级和类别级Top-1准确率上均优于现有方法，且使用了更少的输入帧和模态。

Conclusion: 基于分割的自监督掩码预训练能有效捕捉手语中的细粒度手部差异，提升识别精度，同时降低对多帧多模态输入的依赖。
...
TL;DR: 本文提出替代球形裁剪的三种点云裁剪策略（指数、高斯、线性），通过增大裁剪范围保持点数不变，在多个室内外数据集上提升了3D深度学习模型性能，尤其在室外大场景中取得新SOTA结果。


<details>
  <summary>Details</summary>
Motivation: 现有3D点云处理中常用的球形裁剪方式会导致周围几何上下文信息的丢失，限制了模型对场景语义的理解，因此需要探索更好的裁剪策略以保留更丰富的上下文信息。

Method: 提出指数、高斯、线性三种裁剪方法替代传统球形裁剪，在保持相同点数的前提下增大裁剪空间范围；在多个室内外数据集上评估两种3D深度学习模型架构。

Result: 改变裁剪策略能够提升模型性能，尤其是在大规模室外场景中取得了新的最优结果（state-of-the-art）。

Conclusion: 通过设计更合理的点云裁剪策略（而非仅关注模型架构），可以在不增加计算负担的情况下显著提升3D语义理解性能，尤其适用于大规模室外点云场景。
...
TL;DR: 本文提出EchoCare-CLIP，一个超声影像-临床文本对齐的CLIP-style双编码器对比学习框架，基于16K+多器官图文对数据集，系统评估了不同文本编码器、caption策略和微调方式对跨模态对齐和下游任务的影响，发现更强的对齐并不保证更好的零样本分类性能，且基于模板的caption质量不亚于LLM生成的caption。


<details>
  <summary>Details</summary>
Motivation: 现有的超声基础模型仅基于视觉，无法实现零样本和少样本迁移到标注稀缺的新任务。作者希望通过视觉-语言对齐来弥补这一差距，使模型能够利用临床文本知识进行跨任务迁移。

Method: 构建多器官（乳腺、肝脏、肺、甲状腺）16K+图文对数据集，其中78%以上来自专家标注报告，其余通过三级模板和LLM生成caption流水线补充。采用CLIP-style双编码器对比学习框架，评估CLIP和BioClinicalBERT两类文本编码器，以及模板和LLM生成两种caption策略，对比OpenAI CLIP和BiomedCLIP基线。

Result: 最佳配置达到0.682的对齐分数；CLIP变体部分微调在BUSI和AULI零样本分类上分别达到0.709和0.626；完全端到端微调因过拟合导致迁移性能下降；模板caption匹配或优于LLM生成caption；线性探测和少样本适配结果呈现数据集依赖性。

Conclusion: 超声视觉-语言对齐可通过公开数据实现，但鲁棒的临床迁移需要在领域适应、编码器容量和caption监督质量之间谨慎权衡。
...
TL;DR: SFI-Bench是一个基于视频的基准测试，包含1500+专家标注问题，用于系统评估多模态大模型在结构化空间推理和功能推理两个高级认知维度上的能力。实验表明，当前模型难以将空间记忆与功能推理和外部知识相结合。


<details>
  <summary>Details</summary>
Motivation: 现有基准主要评估MLLM的几何感知能力，缺乏对高阶认知能力（理解物体"是什么"而非"在哪里"）的探测，因此需要能够评估具身智能所需高级推理能力的基准。

Method: 构建SFI-Bench基准：从多样化第一人称室内视频扫描中收集1500+专家标注问题，系统评估两个互补维度——结构化空间推理（理解复杂布局、形成连贯空间表征）和功能推理（推断物体可供性及其上下文依赖效用），包含条件计数、多跳关系推理、功能配对、知识引导故障排除等任务。

Result: 当前MLLM在结合空间记忆与功能推理及外部知识方面持续表现困难，揭示了实现具身智能的关键瓶颈。

Conclusion: SFI-Bench可作为衡量通向更具认知能力和真正具身多模态智能体进展的诊断工具，指出了当前模型在高级认知推理方面的关键不足。
...
TL;DR: 本文提出 Predictive Video VAE (PV-VAE)，通过将预测学习与视频重建统一，在潜在空间中编码时序预测结构，显著提升视频生成质量（收敛速度提升52%，FVD改善34.42），且具有良好的可扩展性和下游视频理解性能提升。


<details>
  <summary>Details</summary>
Motivation: 现有视频VAE在重建质量上表现良好，但继续优化重建损失并未带来生成性能的提升。如何增强视频潜在表示的"可扩散性"（diffusability）仍是一个关键且未解决的挑战。

Method: 提出预测重建目标函数：随机丢弃未来帧，仅编码部分过去观测，训练解码器同时重建已观测帧并预测未来帧。这迫使潜在空间编码时序预测结构，建立对视频动态的更连贯理解。

Result: 在UCF101上，相比Wan2.2 VAE，收敛速度提升52%，FVD提升34.42。模型具有良好的可扩展性，生成性能随VAE训练提升；同时在下游视频理解任务中取得一致增益。

Conclusion: 将预测学习融入视频VAE重建过程是一种有效策略，能够显著提升潜在表示的时序连贯性和运动先验，从而改善生成质量和下游任务表现。
...
TL;DR: SpecEdit提出了一种无需训练的动态分辨率采样框架，通过草稿-验证机制，仅在编辑相关区域执行高分辨率去噪，在其他区域保持粗分辨率，从而在扩散模型图像编辑中实现最高10倍加速（Qwen-Image-Edit）和7倍加速（FLUX.1-Kontext-dev），与步蒸馏等技术结合可达13倍加速。


<details>
  <summary>Details</summary>
Motivation: 现有动态分辨率采样方法使用低级启发式（如边缘检测、通道方差）进行上采样，与编辑语义对齐弱，导致结构不一致；且未验证空间区域是否需要语义修改就进行上采样，造成冗余计算和累积误差。

Method: 采用草稿-验证（draft-and-verify）方案：首先生成低分辨率草稿估计语义结果，然后通过token级差异识别与编辑相关的token进行高分辨率去噪，其余token保持粗分辨率。

Result: 在Qwen-Image-Edit上实现最高10倍加速，在FLUX.1-Kontext-dev上实现7倍加速；与步蒸馏等技术结合可达13倍加速，且保持强质量。

Conclusion: SpecEdit是一种无需训练、与现有加速技术互补的动态分辨率图像编辑框架，能有效减少计算开销同时保持编辑质量。
...
TL;DR: 本文研究了VV和VH交叉极化融合在SAR洪水制图中的有效性，实验表明融合模型比单极化模型在植被覆盖和异质洪水区域表现出更优的分割性能。


<details>
  <summary>Details</summary>
Motivation: 单极化SAR数据在表面散射和体散射共存的复杂环境中洪水制图性能受限，亟需探索交叉极化融合策略以提升制图精度。

Method: 采用深度学习分割框架联合利用VV和VH极化互补信息，在相同训练条件下比较VV单极化、VH单极化和VV-VH融合三种配置的性能，使用IoU和F1-score等指标评估。

Result: VV-VH融合模型在植被覆盖和异质洪水区域一致优于单极化模型，能够更精确地描绘洪水边界。

Conclusion: 交叉极化SAR融合对提升灾害监测应用中SAR洪水制图的可靠性具有重要意义。
...
TL;DR: HeroCrystal 是一个面向多摄像头域自适应目标检测的隐私保护框架，通过生成式数据增强、联邦学习和知识蒸馏三个阶段的协同设计，在保护数据隐私的同时解决了类别不平衡和异构架构问题，在跨域检测基准上实现了33.4% mAP的最新最佳性能。


<details>
  <summary>Details</summary>
Motivation: 现有跨域目标检测方法面临三大挑战：1) 数据隐私问题，传统方法需收集大量目标域数据，存在隐私泄露风险；2) 类别不平衡（长尾分布），罕见类别检测性能严重退化；3) 客户端异构架构难以统一融合。亟需一种既能保护隐私又能处理异构性的多摄像头域自适应检测框架。

Method: 提出三阶段框架：(1) 生成阶段：基于单张目标域图像的一次性目标感知扩散生成模块，利用提示控制合成特定目标实例，实现隐私保护增强和可控罕见目标生成；(2) 联邦阶段：客户端采用概率Faster R-CNN提高定位精度，结合动态模型对比策略抑制域偏差；服务端在无原始数据访问下进行异构架构模型融合；(3) 蒸馏阶段：提出不一致类别集成算法，解决跨客户端的标签不一致和架构异构问题。

Result: 在多个跨域检测基准上，HeroCrystal 在隐私保护多类设定下超越现有多种源域自适应和联邦学习方法，相比先前隐私保护方法 mAP 提升 +2.1%，达到 33.4% mAP 的新最优结果。

Conclusion: HeroCrystal 通过生成式数据增强与联邦学习的有机结合，有效解决了多摄像头场景下的隐私保护、类别不平衡和架构异构三大核心挑战，为实际多摄像头AI监控系统提供了可行的技术方案。
...
TL;DR: 提出SlimDiffSR，一种轻量级扩散模型框架，通过不确定性引导的时间步分配策略、结构化剪枝和MMD蒸馏，实现遥感图像超分辨率，相比多步扩散模型获得200倍推理加速和20倍参数缩减。


<details>
  <summary>Details</summary>
Motivation: 扩散模型在图像超分辨率中性能优异，但计算成本高，限制了其在遥感应用中的实际部署。现有单步扩散方法依赖固定时间步，未能充分利用遥感图像特性。

Method: 1) 不确定性引导的时间步分配策略，将重建难度与扩散时间步显式关联，构建更强的单步教师模型；2) 针对遥感图像的结构化剪枝策略，包含频率可分离卷积、方向可分离卷积和查询驱动全局聚合模块；3) 引入最大均值差异(MMD)进行知识蒸馏，对齐教师与学生模型的特征分布。

Result: 在多个遥感基准上实现效率与重建质量的良好平衡，达到多步扩散模型的200倍推理加速和20倍参数缩减，在效率上明显优于现有轻量扩散基线。

Conclusion: SlimDiffSR通过针对遥感图像特性的轻量化设计，有效解决了扩散模型在遥感超分辨率中的高计算成本问题，实现了实用化的高效部署方案。
...
TL;DR: 提出3DFSR模型，通过深度学习方法对ALS森林点云进行超分辨率处理，同时提升点密度和降低噪声，显著改善单木级森林调查精度（F1从0.71提升至0.97，DBH估计RMSE从13.45cm降至6.43cm）。


<details>
  <summary>Details</summary>
Motivation: ALS点云稀疏且有噪声，导致单木级森林调查（如树干定位、树木尺寸估计）不准确，需要提升点云质量以支持更精细的森林参数提取。

Method: 提出基于体素CNN的U-Net架构模型3DFSR，在ALS森林点云上同时实现点密度提升和噪声降低。

Result: 在温带和寒带森林ALS数据上，3DFSR在点云质量（CD 0.249m, HD 2.711m）上优于SOTA方法；增强后点云可直接应用TLS/MLS算法，F1从0.71提升至0.97，DBH RMSE从13.45cm降至6.43cm；适用密度范围10-1700点/m²且可跨平台泛化。

Conclusion: 3DFSR能有效提升ALS森林点云质量，使原本适用于TLS/MLS的精细森林调查算法可直接应用于增强后的ALS点云，显著提高单木级森林参数估计精度。
...
TL;DR: 本文提出MultiSense-Pneumo，一个面向肺炎筛查与分诊支持的多模态框架，整合结构化症状描述、咳嗽音频、口语语言和胸部X光片，在低资源环境下实现离线运行的多模态决策支持。


<details>
  <summary>Details</summary>
Motivation: 肺炎在全球尤其是低资源地区造成沉重疾病负担，现有计算方法多局限于单模态（主要依赖放射影像），缺乏多模态整合。临床评估本质上是多模态的，因此需要一种能够融合多种异构信号的计算框架来支持筛查和分诊。

Method: MultiSense-Pneumo融合五种模态：确定性症状分诊、基于LightGBM的声学分类、使用ResNet-18的域对抗放射影像分析、基于Transformer的语音识别，以及可解释的多模态融合算子。每个模态转换为归一化风险信号，通过融合算子聚合为统一的筛查估计。

Result: 实验结果表明放射影像通路在域偏移下具有鲁棒性，同时揭示了声学信号在少数类召回率方面的局限性。系统可在标准笔记本硬件上完全离线运行，适合社区健康工作者和农村诊所等场景。

Conclusion: MultiSense-Pneumo是一个面向筛查和分诊支持的研究原型，而非临床验证的诊断系统。它展示了在低计算资源条件下实现多模态肺炎筛查的可行性和透明可解释的决策支持架构。
...
TL;DR: 本文提出InfiltrNet，一种结合CNN和Swin Transformer的双分支架构，通过跨注意力融合模块从多模态MRI预测胶质瘤的三区浸润风险图，利用距离变换生成标签，在BraTS 2020和2025上优于五个基线方法。


<details>
  <summary>Details</summary>
Motivation: 胶质瘤会浸润超出MRI可见肿瘤边界的周围组织，现有深度学习方法仅分割可见肿瘤，无法评估周围组织的浸润风险，而这对手术规划和放射治疗至关重要。

Method: 提出InfiltrNet双分支架构：CNN编码器+Swin Transformer编码器，通过跨注意力融合模块整合特征；基于距离变换的标签生成策略从BraTS标注推导三区浸润风险；训练使用Dice-CE损失+边界感知损失及中间层辅助监督。

Result: 在BraTS 2020和BraTS 2025数据集上，InfiltrNet优于五种基线方法；GradCAM++和Occlusion敏感性分析证实模型关注临床相关的瘤周区域。

Conclusion: InfiltrNet有效预测胶质瘤周围组织浸润风险，为手术和放疗提供更精准的决策依据。
...
TL;DR: 本文提出FedPuReL方法，通过梯度纯化和残差学习解决长尾分布下的个性化联邦学习问题，在保持全局模型类别平衡的同时实现无偏的个性化，实验表明该方法在各类长尾场景中均优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有个性化联邦学习（PFL）方法在面对非独立同分布数据与长尾类分布同时出现的真实场景时存在两个问题：（1）微调会侵蚀基础模型固有的类别平衡性，导致性能低于零样本基线；（2）传统个性化技术通过参数或特征级融合将这种偏差转移到局部模型。该问题在PFL中尚未被充分探索。

Method: 提出FedPuReL（Federated Learning via Gradient Purification and Residual Learning），包含两个核心模块：（1）梯度纯化：利用零样本预测净化局部梯度，维持全局模型的类别平衡；（2）残差学习：将个性化建模为冻结的全局模型之上的残差校正，从而实现无偏的个性化。

Result: 大量实验表明，FedPuReL在多种长尾场景下的全局模型和个性化模型上均一致优于现有最先进方法，代码已开源。

Conclusion: FedPuReL通过梯度纯化和残差学习有效解决了长尾分布下个性化联邦学习中的类别不平衡问题，为真实场景中的PFL应用提供了可行的解决方案。
...
TL;DR: 本文通过受控对比实验发现，强通用视觉基础模型在遥感图像检索任务中与专用遥感基础模型表现相当甚至更优，且跨场景泛化更稳定。研究指出当前遥感专用预训练策略存在局限性，未来模型应更好地利用遥感图像的物理、空间、光谱和地理特性。


<details>
  <summary>Details</summary>
Motivation: 遥感数据获取成本高且标注需要专业知识，视觉基础模型利用大规模无标注数据的能力对此尤为重要。然而，近期涌现的遥感专用视觉基础模型是否在基于检索的评估中优于强通用视觉基础模型，这一问题尚不明确。

Method: 在相同数据集、检索协议和评估指标下，对代表性遥感专用模型和通用视觉基础模型进行受控对比，评估域内性能和跨场景泛化能力。

Result: 强通用视觉基础模型与遥感专用模型性能相当甚至更优；遥感专用模型在跨场景评估中表现显著退化，而通用模型迁移更稳定。

Conclusion: 仅依靠遥感预训练并不能保证获得更强的面向检索的遥感表征，当前遥感专用预训练策略存在局限性，未来模型应更好地利用遥感图像的多维特性。
...
TL;DR: 本文提出NAN-SPOT框架，利用现成检测器的隐藏层负感知范数（NAN）来估计物体性（objectness），无需重新训练基础检测器，仅需数百张图像上的分钟级训练即可实现开放集目标检测。同时引入COCO-Open数据集，将未知目标标注从433个扩展至1853个。实验表明，该方法在未知目标检测上超越需要大量训练的方法，且不损害已知目标性能。


<details>
  <summary>Details</summary>
Motivation: 现有开放集目标检测方法通常需要大量重新训练检测器来学习物体性，计算成本高昂。本文假设标准现成检测器在训练过程中已经包含有助于物体性判断的线索，旨在探索一种训练轻量级的解决方案。

Method: 提出NAN-SPOT框架，利用现成检测器隐藏层中的负感知范数（Negative-Aware Norm）作为度量来估计物体性，无需重新训练基础检测器，仅需在数百张图像上进行分钟级训练。同时构建了COCO-Open数据集以支持更全面的评估。

Result: NAN-SPOT在未知目标检测上达到了优于需要大量训练的方法的性能，同时不降低已知目标的检测性能。COCO-Open成为目前标注最全面的OSOD数据集。

Conclusion: NAN-SPOT是一种高效且鲁棒的开放集目标检测方法，为自动驾驶中的开放世界感知提供了有前景的方向。
...
TL;DR: LabBuilder是一个端到端系统，能够从简洁的文本描述生成并验证3D实验室布局，通过三个组件（LabForge、LabGen、LabTouchstone）实现功能性、安全性和可执行性的实验室环境设计，显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 自动化实验室的部署受限于安全且可执行环境的设计难度，现有3D场景生成方法主要面向家居场景，忽视了科学实验所需的严格功能语义和安全约束。

Method: 提出LabBuilder系统，包含三个紧密耦合的组件：LabForge维护标注资产和化学知识的元数据集，将自然语言规范转化为结构化协议；LabGen通过迭代的约束感知优化策略合成实验室布局；LabTouchstone作为统一基准评估生成的布局。

Result: 大量实验表明，LabBuilder显著优于现有最先进方法，生成的实验室环境不仅真实，而且功能有效、安全，适用于复杂的实验工作流程。

Conclusion: LabBuilder有效解决了自动化实验室环境设计中功能语义和安全约束缺失的问题，为科学发现加速提供了可行的布局生成方案。
...
TL;DR: 本文研究了利用扩散模型（FLUX.2-4B Klein）和图像到图像翻译模型（REGEN）提升合成数据集逼真度的方法，并提出了一种混合策略，实验表明混合方法在保持语义一致性的同时实现了更好的视觉真实感。


<details>
  <summary>Details</summary>
Motivation: 现代游戏引擎虽已大幅提升视觉保真度（如光线追踪技术），但合成图像与真实图像之间仍存在显著的sim2real外观差距，限制了合成数据集在实际应用中的使用。本文旨在探索弥合这一差距的有效方法。

Method: 1) 使用最先进的扩散模型FLUX.2-4B Klein进行图像生成与编辑；2) 使用传统的图像到图像翻译模型REGEN；3) 提出一种混合方法，结合扩散模型的几何与材质变换能力以及图像到图像翻译技术的分布匹配能力。

Result: REGEN模型表现优于FLUX.2-4B Klein；将两者结合的混合方法相比单独使用任一模型能获得更好的视觉真实感，同时保持语义一致性。

Conclusion: 混合扩散模型与传统图像到图像翻译模型的方法能有效提升合成数据集的逼真度，为弥合sim2real外观差距提供了可行方案。
...
TL;DR: 本文提出将卷积块注意力模块（CBAM）集成到传统CNN块中，用于胸部X光片的胸部疾病多标签分类，在ChestXray14数据集上达到了0.8695的平均AUC，优于多个基线方法。


<details>
  <summary>Details</summary>
Motivation: 传统胸部X光片解读耗时且依赖专家知识；深度学习方法虽提升了诊断效率，但仍面临类别不平衡和多病灶共存定位的挑战。

Method: 将卷积块注意力模块（CBAM）集成到传统CNN块中，利用CBAM的特征细化能力和CNN块的特征提取能力，增强多标签分类任务性能。

Result: 在ChestXray14数据集上达到0.8695的平均AUC，优于多个当前最优基线方法。

Conclusion: CBAM与CNN块的结合能有效提升胸部X光片多标签分类的性能，代码已开源。
...
TL;DR: 本文揭示了多模态上下文学习（ICL）中存在的归纳鸿沟问题：模型常通过错误推理得出正确答案，且难以从示例中提取一致规则。为此，提出一个归纳-演绎框架，包括基于相似性的视觉令牌压缩模块、动态注意力再平衡机制和思维链范式，并结合监督微调与强化学习的辅助学习流程，在8个基准上取得显著提升。


<details>
  <summary>Details</summary>
Motivation: 当前ICL在视觉-语言模型中的扩展仍然脆弱，模型存在归纳鸿沟：即能从错误推理得到正确答案，同时难以从演示中提取一致规则。此外，冗余的视觉令牌和偏向首张图像的注意力分布进一步加剧了这一问题。

Method: 提出一个将多模态ICL重构为归纳-演绎过程的框架，包含：(1) 基于相似性的视觉令牌压缩模块，过滤冗余图像块；(2) 动态注意力再平衡机制，均衡分配对所有图像的关注；(3) 思维链范式，引导模型分析示例、推导规则并应用于查询。辅助学习流程结合监督微调与基于可验证奖励的强化学习，增强忠实引用和噪声过滤能力。

Result: 在8个涵盖视觉感知、逻辑推理、STEM问题和讽刺检测的基准上，针对多个开源VLM，该方法相比标准ICL基线取得了一致且显著的改进。

Conclusion: 该工作突显了在多模态场景下赋予模型真正归纳能力的潜力，为提升VLM的上下文学习性能提供了有效路径。
...
TL;DR: UnGAP提出了一种闭环框架，将异质不确定性从被动指标转变为主动视觉提示，通过UPFM模块校准特征分布，解决裂缝分割中因光照、模糊和纹理模糊导致的随机不确定性，同时缓解高方差导致梯度抑制的结构优化病态问题。


<details>
  <summary>Details</summary>
Motivation: 当前不确定性感知方法仅将不确定性估计作为事后分析的被动终点，未能将其反馈以优化特征表示；且独立像素异质建模虽适合裂缝分割，但存在结构性优化病态——高预测方差会衰减损失梯度，导致模型忽略困难样本并对复杂边界欠拟合。

Method: 提出UnGAP框架，核心是Uncertainty-Prompted Feature Modulator (UPFM)，将随机不确定性作为主动视觉提示，通过逐像素仿射变换动态校准特征分布；引入边界感知检测头进一步约束预测精度。

Result: UnGAP在裂缝分割任务上实现了优越的分割精度与实时推理速度的平衡，验证了将不确定性从被动指标转化为主动校准工具的有效性。

Conclusion: 将不确定性估计与特征学习建立闭环机制，通过UPFM将高方差从梯度抑制信号转化为特征修正信号，有效提升了裂缝分割的鲁棒性和精度。
...
TL;DR: FEAT是一种基于解耦双注入和正交引导噪声融合的时尚设计与虚拟试穿方法，支持从任意设计源（包括非服装图像）进行服装和配饰的编辑与试穿，达到了最先进性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法局限于服装图像类设计源，无法利用艺术品、抽象图像等创意来源，且不支持包含配饰的完整着装。FEAT旨在突破这些限制，实现从任意设计源进行服装和配饰的编辑与试穿。

Method: 提出解耦双注入（DDI）机制，通过内容和风格解耦选择性注入设计线索；提出正交引导噪声融合（OGNF），利用正交投影去除残留衣物并通过区域特定噪声策略实现虚拟试穿，且OGNF无需训练。

Result: 大量实验表明FEAT在设计灵活性、提示一致性和视觉真实感方面达到了最先进水平。

Conclusion: FEAT成功将时尚设计从服装图像扩展到任意设计源，并首次支持配饰的虚拟试穿，显著提升了设计灵活性和应用范围。
...
TL;DR: 针对基于流变换器的训练自由图像编辑方法中存在的累积漂移导致重建保真度下降的问题，本文提出DirectEdit方法，通过直接对齐前向路径消除重建误差，并引入注意力特征注入和多分支掩码引导噪声混合的保持机制，在不增加额外NFE的情况下实现了优于现有方法的编辑效果。


<details>
  <summary>Details</summary>
Motivation: 现有训练自由图像编辑方法在反转过程中使用不匹配时间步的噪声潜变量近似重建路径，导致累积漂移（accumulated drift），严重限制了重建保真度。

Method: 提出DirectEdit方法：(1) 直接对齐前向路径而非修正反转路径，消除固有重建误差；(2) 基于注意力特征注入的保持机制；(3) 多分支掩码引导噪声混合策略，平衡保真度与可编辑性。整个过程不增加额外神经函数评估（NFE）。

Result: 在多种场景下的广泛实验表明，DirectEdit实现了高效且准确的图像编辑，性能优于现有最先进方法。

Conclusion: 通过直接对齐前向路径替代传统反转路径修正策略，结合注意力特征注入与多分支掩码引导噪声混合，可有效解决训练自由图像编辑中的重建误差问题，实现保真度与编辑性的良好平衡。
...
TL;DR: 提出混合原型流匹配（MPFM）框架，通过将正常特征分布连续变换到结构化高斯混合原型空间，并引入互信息最大化正则化器，实现开集监督异常检测，在多个基准上达到SOTA。


<details>
  <summary>Details</summary>
Motivation: 现有基于原型的开集监督异常检测方法通常使用单峰高斯先验建模正常数据，无法捕捉其固有的多模态特性，导致决策边界模糊。

Method: 1) 提出混合原型流匹配（MPFM），学习从正常特征分布到结构化高斯混合原型空间的连续变换；2) 将速度场显式建模为高斯混合先验，每个分量对应一个正常类别，实现模态感知的分布传输；3) 引入互信息最大化正则化器（MIMR），防止原型坍塌并增强正常-异常可分离性。

Result: 在多种基准数据集上，单异常和多异常设置下均达到最先进性能。

Conclusion: MPFM通过高斯混合原型空间和流匹配框架有效捕捉正常数据的多模态特性，结合互信息正则化提升了异常检测的判别能力。
...
TL;DR: 本文提出异常偏好优化（Anomaly Preference Optimization），将异常生成重构为偏好学习问题，通过隐式偏好对齐机制利用真实异常作为正样本，结合时间感知容量分配模块在扩散时间线上动态分配模型容量，并在推理时采用层次化采样策略，在有限数据下实现了异常样本生成的真实性和多样性的SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法在有限数据下生成异常样本时，难以兼顾真实性和多样性，常受限于分布不匹配和过拟合问题。

Method: 1) 隐式偏好对齐机制：将真实异常作为正参考，从去噪轨迹偏差中直接导出优化信号，无需人工标注；2) 时间感知容量分配模块：沿扩散时间线动态分配模型容量，高噪声阶段优先结构多样性，低噪声阶段强化细粒度真实性；3) 层次化采样策略：在推理时调节连贯性与真实性的权衡。

Result: 在真实性和多样性两个指标上均显著超越现有基线，达到SOTA性能。

Conclusion: 将异常生成建模为偏好学习问题，结合扩散模型的时间动态容量分配，能有效平衡生成样本的真实性与多样性。
...
TL;DR: ExpoCM提出了一种基于概率流常微分方程（PF-ODE）的单步生成式高动态范围（HDR）重建框架，通过曝光感知一致性轨迹和曝光引导的亮度-色度损失，在单次推理中同时恢复过曝区域的细节、抑制欠曝区域的噪声并保留正常曝光区域的结构，相比DDPM和DDIM实现超过400倍和20倍的推理加速，并在多个基准上达到最先进的保真度和感知精度。


<details>
  <summary>Details</summary>
Motivation: 现有的基于扩散模型的单图像HDR重建方法忽略了退化过程的曝光依赖性，且因迭代采样导致计算成本高昂。

Method: 将HDR重建重新表述为PF-ODE，通过软曝光掩膜将LDR图像分为过曝、欠曝和正常曝光区域，构建区域条件化一致性轨迹以实现单步无蒸馏推理；并提出在CIE L*a*b*空间中的曝光引导亮度-色度损失来优化感知质量。

Result: 在HDR-REAL、HDR-EYE和AIM2025基准上达到最先进的保真度和感知精度，推理速度相比DDPM（1000步）提升超过400倍，相比DDIM（50步）提升超过20倍。

Conclusion: ExpoCM通过曝光感知一致性模型实现了高效、高质量的单图像HDR重建，兼顾了恢复过曝细节、抑制欠曝噪声和保持结构完整性。
...
TL;DR: 提出MooD框架，首次利用连续效价-唤醒（VA）值实现细粒度且高效的情感图像编辑，结合VA感知检索、视觉迁移与语义引导，并构建AffectSet数据集，在情感可控性和视觉保真度上均取得优异表现。


<details>
  <summary>Details</summary>
Motivation: 现有情感图像编辑方法推理效率低且依赖离散情感表示，难以捕捉复杂细微的人类情感，且实际应用受限。

Method: 1) 提出VA感知检索策略，桥接模糊的情感值与具体视觉语义；2) 融合视觉迁移与语义引导实现可控情感编辑；3) 构建VA标注数据集AffectSet用于模型优化和评估。

Result: 在情感可控性和视觉保真度上均优于现有方法，同时保持高效率；消融实验揭示了设计中的关键因素。

Conclusion: MooD是首个直接利用连续VA值进行细粒度情感图像编辑的框架，实现了高效、可控且保真的情感编辑效果。
...
TL;DR: 本文提出TemPose-TF-ASF（Adjacent-Stroke Fusion）框架，通过显式建模前后击球动作的双向时序依赖关系，利用两阶段训练策略在羽毛球击球预测任务上显著提升准确率和Macro-F1，且具有良好的模型迁移性和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有羽毛球击球预测方法难以有效建模丰富的时序上下文信息，限制了细粒度运动分析和战术决策支持的准确性。

Method: 提出TemPose-TF-ASF，一种上下文感知的扩展模型，通过引入前后击球的类型信息来增强击球识别；采用两阶段训练与推理策略：首阶段基线模型生成初步预测作为估计的时序上下文，次阶段利用这些预测联合优化ASF模块和分类器，显式建模双向时序依赖关系。

Result: 在大规模羽毛球比赛数据集上，该方法在Accuracy和Macro-F1指标上一致优于基线模型及其变体；将ASF集成到其他先进方法中也取得了显著的性能提升，证明了其强迁移性和泛化能力。

Conclusion: 通过显式建模双向时序击球依赖关系，ASF模块可无缝集成到现有最优模型中，有效提升击球预测性能，并展现出良好的可迁移性和泛化性。
...
TL;DR: 提出了一种数据驱动的持续适应框架，通过结合野外数据和生成器数据来更新AI生成图像检测器。利用事实核查文章构建弱监督野外数据集，结合少量生成器数据实现对新模型的适应，并采用持续学习缓解灾难性遗忘，在两个检测器上分别提升9.14%和8%的平均准确率。


<details>
  <summary>Details</summary>
Motivation: 现有AI生成图像检测器在分布偏移和面对新出现的生成模型时性能显著下降，需要一种能够持续适应不断变化环境的检测器更新方法。

Method: 提出数据驱动的持续适应框架：1）通过事实核查文章检索构建弱监督的野外数据集；2）在训练中引入少量生成器驱动数据以适应新模型；3）采用持续学习框架结合两种数据，实现稳健适应并缓解灾难性遗忘。

Result: 在两个先进检测器上的实验表明，该方法分别实现了+9.14%和+8%的平均准确率提升。

Conclusion: 野外数据和生成器驱动数据对检测器适应至关重要，两者在持续学习框架下结合能有效实现对新生成模型的鲁棒适应，同时避免灾难性遗忘。
...
TL;DR: 本文提出了一种自监督空间-角度隐式神经表示（SA-INR），仅需每个扩散方向单个视角即可重建高分辨率dMRI，大幅加速扫描。该方法实现了空间超分辨和零样本角度超分辨，在模拟数据上对已训练和未训练方向均取得高保真度，且合成的角度数据提高了下游DTI模型拟合的定量精度。


<details>
  <summary>Details</summary>
Motivation: 旋转视图厚层采集具有高SNR效率，但需要大量旋转视图以满足奈奎斯特采样，导致扫描时间过长。现有方法难以在保证高质量的同时实现快速采集。

Method: 提出SA-INR框架，使用多层感知机（MLP）作为隐式神经表示，以b=0结构先验和通过FiLM（特征线性调制）注入的b方向信息为条件，在 anisotropic 输入上进行端到端自监督训练，实现连续q空间表示学习。

Result: 在模拟数据上，对已训练方向PSNR达34.82 dB，对未训练方向PSNR达33.08 dB。合成角度数据显著提升了DTI模型拟合的定量准确性。

Conclusion: SA-INR框架打破了经典采样极限，为实现快速、定量的高分辨率dMRI铺平了道路。
...
TL;DR: Hyp2Former是一种基于双曲空间的端到端开放集全景分割框架，通过编码已知类别的层次语义关系，在不显式建模未知类的情况下，实现未知物体的可靠检测与分割。
...
Method: 提出Hyp2Former框架，在双曲空间中连续学习层次化的语义相似性，显式编码已知类别间的层级关系，使模型学习到多级语义抽象的结构化嵌入空间，未知物体通过靠近高层语义概念（如"动物"而非"电子设备"）实现可靠检测。

Result: 在MS COCO、Cityscapes和Lost&Found等多个公开数据集上，Hyp2Former在未知物体发现与分布内鲁棒性之间取得了最佳平衡，优于现有OPS方法。

Conclusion: 利用双曲空间编码语义层次结构是一种有效策略，可在不依赖未知类显式建模的情况下提升开放集全景分割性能。
...
TL;DR: 提出一种通过学习解耦编辑方向实现对潜在扩散模型风格属性精细参数化控制的方法，利用引导组合缩小风格微调模型与基础模型之间的领域差距，并引入训练正则化损失和优化的空条件嵌入以保持原始语义，在轮廓、局部对比度、水彩化效果和几何图案等风格属性上实现了比现有文本编辑技术更精准、连续可调的修改。


<details>
  <summary>Details</summary>
Motivation: 文本到图像扩散模型在图像合成和编辑领域取得了革命性进展，但对风格属性的精确控制仍然是一个挑战，常常导致无意的内容修改。

Method: (1) 从合成数据集中学习解耦的编辑方向；(2) 使用引导组合来弥合风格微调模型与基础模型之间的领域差距；(3) 引入训练正则化损失；(4) 增强DDIM反演，结合优化的空条件嵌入进行真实图像编辑。

Result: 在轮廓、局部对比度、水彩化效果和几何图案等多种风格属性上的评估表明，与当前基于文本的编辑技术相比，该方法提供了更整合、更精确且连续可调的风格修改。

Conclusion: 提出方法实现了对风格属性的精细参数化控制，在保持原始语义的同时进行风格调整，并取得了优于现有文本编辑技术的效果。
...
TL;DR: StableMind是一种正则化微调框架，通过脑侧表示稳定（岭投影先验+傅里叶特征增强）和图像侧监督可靠（难度感知图像模糊）两大策略，在仅1小时新被试fMRI数据下实现跨被试解码，图像/脑检索精度分别达84.02%和81.66%，超越SOTA方法5.71%。


<details>
  <summary>Details</summary>
Motivation: 现有跨被试fMRI解码方法在新被试数据有限且历史被试原始数据不可访问时性能严重下降，原因是脑侧fMD响应跨被试差异大导致表示不稳定，以及图像侧有限fMRI信号无法可靠支撑细粒度视觉细节的监督。

Method: (1) 脑侧稳定：将预训练模型的岭投影作为微调先验约束有限数据适配，并采用傅里叶域特征级脑数据增强提升对个体差异的鲁棒性；(2) 图像监督可靠：引入难度感知图像模糊，在脑-图像对齐中削弱弱支持的细粒度细节，保留稳定视觉结构。

Result: 在自然场景数据集（NSD）上，统一1小时微调协议下，四名被试平均图像检索准确率84.02%，脑检索准确率81.66%，脑检索比SOTA方法提升5.71%，且所需可训练参数更少。

Conclusion: StableMind证明了通过正则化策略（先验约束、数据增强、自适应模糊）可有效解决有限数据下跨被试fMRI解码的脑不稳定与监督不可靠问题，为实际低成本fMRI视觉解码提供了可行方案。
...
TL;DR: 本文系统综述了眼底OCT图像分析中的表示学习方法，涵盖从早期深度学习到最新基础模型与视觉-语言系统的演进，并建立包含监督学习、自监督/半监督、生成方法、3D建模、多模态学习及大规模预训练基础模型的结构化分类体系，同时讨论了公开数据集、评估协议及未来研究方向（如体素级基础模型预训练、不确定性建模、联邦学习、公平性与可解释性等）。


<details>
  <summary>Details</summary>
Motivation: OCT已成为眼科主要成像模态，但处理大量采集数据依赖专家标注且诊断一致性不足，急需通过表示学习实现自动化分析以减少人工依赖并提升跨设备/人群的鲁棒性。

Method: 采用结构化分类法系统梳理OCT图像表示学习文献，按学习范式（监督CNN/Transformer、自监督/半监督、生成方法、3D体积建模、多模态表示、大规模预训练基础模型）组织，对每类分析核心方法贡献与局限，并给出统一数学框架和数据集/评估协议综述。

Result: 建立了OCT表示学习的完整分类体系，涵盖从CNN/Transformer到基础模型的完整技术演进链路，识别出各范式的关键局限，并提供了公开数据集和评估协议的标准化视角。

Conclusion: 指出未来关键研究方向包括：体积级基础模型预训练、不确定性感知表示学习、联邦学习与隐私保护、公平性与偏差缓解、基于概念的可解释性等，推动OCT自动化分析迈向更可靠、公平和可解释的临床应用。
...
TL;DR: 本文提出VODA（纯视觉-语言模型域适应）这一更严格的域适应设置，完全消除对源模型的依赖，仅使用随机初始化模型、ViL模型和无标签目标数据。为此设计了TS-DRD两阶段框架，通过预热和去噪区域蒸馏实现有效适应，在多个基准上取得了与甚至优于传统SFDA方法的效果。


<details>
  <summary>Details</summary>
Motivation: 现有SFDA方法虽声称无需源数据，但仍需源预训练模型初始化，并非真正"无源"。作者观察到不同源模型对同一目标域最终结果影响极小，说明源模型本身作用有限，因此提出完全消除源依赖的VODA设置。

Method: 提出TS-DRD（两阶段去噪区域蒸馏）框架：第一阶段用ViL模型指导进行预热训练；第二阶段寻找ViL模型和适应模型共有的去噪区域，生成更干净的蒸馏监督信号。

Result: 在Office-Home、VisDA和DomainNet-126数据集上，VODA设置下的TS-DRD方法达到与甚至优于仍使用源模型的现有SFDA方法的性能。

Conclusion: 证明了VODA设置的有效性，表明域适应可以在完全脱离源模型的情况下仅依靠ViL模型完成，为进一步消除源域依赖提供了新方向。
...
TL;DR: 本文评估了GleasonAI模型（基于注意力的多实例学习）在10,366个前列腺活检核心上的表现，模型在ISUP分级中达到0.86的二次加权kappa值，性能在17年间的存档标本中保持稳定，且AI分配的 grade 组与前列腺癌特异性死亡率存在显著预后梯度，证明了AI分级模型的泛化能力和病理档案作为大规模AI开发资源的潜力。


<details>
  <summary>Details</summary>
Motivation: 人工智能在临床前列腺病理中逐渐应用，但其在不同样本制备和长期保存条件下的泛化能力尚不清楚，需要验证AI模型在真实世界多样化数据上的鲁棒性。

Method: 使用端到端注意力机制的多实例学习模型GleasonAI，在包含1998-2015年间10,366个活检核心的独立验证队列（来自14个瑞典地区）上进行评估，采用二次加权kappa评估核心级ISUP分级一致性，并分析预后梯度。

Result: 核心级ISUP分级的二次加权kappa为0.86，与经验丰富的病理学家相当；性能在不同地理区域和17年时间跨度下保持稳定；AI分配的 grade 组与前列腺癌特异性死亡率存在显著预后梯度。

Conclusion: AI分级模型具有良好的泛化能力，对存档材料的时间相关变化具有鲁棒性，病理档案可作为AI开发、验证和回顾性预后研究的大规模资源。
...
TL;DR: 提出GLASSNet，利用冻结的SAMv2编码器（仅需3%可学习参数）和轻量级卷积适配器，通过全局-局部双解码器架构实现显著目标检测，在多个基准上超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 基础模型（如SAM）在SOD任务中潜力未被充分挖掘，全量微调计算成本高且在小数据下易过拟合，需要一种高效利用冻结基础模型的方法。

Method: 使用冻结的SAMv2作为编码器，搭配轻量级空间感知卷积适配器（减少97%+可学习参数）；采用双解码器架构，一个解码器捕获全局长程语义，另一个捕获边缘纹理等局部细节，融合互补线索生成精确的显著图。

Result: 在标准SOD和伪装目标检测基准上均超越现有最先进方法。

Conclusion: 冻结基础模型结合针对性适配与全局-局部解码的策略在SOD任务中高效且有效。
...
TL;DR: 本文提出广义时刻检索（GMR）任务，涵盖多匹配或无匹配的查询场景；构建了大规模足球视频基准Soccer-GMR，设计了统一评估协议和两种建模范式（轻量级适配器和基于GRPO奖励的MLLM微调），实验表明该方法在各项指标上均有提升，揭示了现有方法的局限性。


<details>
  <summary>Details</summary>
Motivation: 传统视频时刻检索（VMR）假设每个查询只对应一个匹配时刻，但现实中查询可能对应多个时刻或没有匹配时刻，因此需要更统一、更现实的检索设定。

Method: ① 提出广义时刻检索（GMR）统一设定；② 构建Soccer-GMR基准，采用时长灵活的半自动标注流水线并辅以人工验证；③ 设计互补评估指标（空集拒绝、正查询定位、端到端GMR性能）；④ 提出两种建模范式：轻量级即插即用GMR适配器用于判别式VMR模型，以及针对GMR设计的GRPO奖励用于微调多模态大语言模型。

Result: 在Soccer-GMR基准上，两种方法在所有评估指标上均取得一致性提升，同时揭示了现有方法在GMR设定下的关键局限性。

Conclusion: GMR是一个更具现实性和挑战性的视频-语言理解基准，现有方法仍有显著改进空间。
...
TL;DR: 本文提出了一种基于解耦表示的显式重建约束方法，用于抑制低光照图像增强中的异常通道放大和色度噪声。在LOLv2-Real、MIT-Adobe FiveK和LSRW数据集上取得了竞争性或更优的性能，PSNR达29.71 dB，SSIM为0.89。DarkFace实验表明该方法能改善低光照条件下的人脸检测。


<details>
  <summary>Details</summary>
Motivation: 现有低光照图像增强方法存在异常通道放大和色度噪声问题，影响图像质量和后续任务性能。本文旨在通过解耦表示和显式重建约束来解决这些问题，提升增强效果和下游任务适用性。

Method: 从解耦表示中推导出显式重建约束，施加于模型以抑制异常通道放大和色度噪声。具体实现方法未在摘要中详述，但提供了开源代码。

Result: 在LOLv2-Real上达到29.71 dB PSNR和0.89 SSIM，在MIT-Adobe FiveK和LSRW上也取得了竞争性或更优性能。DarkFace实验显示低光照条件下的下游人脸检测性能得到改善。

Conclusion: 提出的方法能有效抑制异常通道放大和色度噪声，在多个数据集上取得优异性能，并能改善低光照条件下的人脸检测任务。代码和预训练模型已开源。
...
TL;DR: 提出AutoFocus，一种无需训练、不确定性感知的主动视觉搜索框架，通过token级困惑度构建各向异性高斯空间概率场，实现自适应区域缩放与鲁棒的GUI坐标预测，在ScreenSpot-Pro和ScreenSpot-V2上持续提升性能。


<details>
  <summary>Details</summary>
Motivation: 高分辨率界面中，密集布局和小交互元素导致现有VLM在GUI接地任务上性能下降，而现有缩放策略缺乏自适应确定需要精炼区域及探索空间不确定性的机制。

Method: 1) 通过token级困惑度采样多个坐标假设，构建各向异性高斯空间概率场建模方向不确定性；2) 基于概率场生成全局/局部区域提议，引入Shape-Aware Zooming平衡定位精度与上下文保留；3) 视觉提示聚合步骤通过结构化比较选择最一致的预测。

Result: 在ScreenSpot-Pro和ScreenSpot-V2基准上，通用型和GUI专用型VLM均取得一致性改进。

Conclusion: 无需额外训练，通过主动不确定性建模与自适应缩放可有效弥合高分辨率GUI接地中的分辨率差距。
...
TL;DR: 本文提出一种弱监督跨视角指代多目标跟踪框架ViewSAM，利用SAM2/SAM3等基础模型生成伪标签，仅需物体类别标签作为粗粒度监督，通过亲和力引导的跨视角重提示策略和视图感知的多模态语义建模，实现了与全监督方法相当的性能。


<details>
  <summary>Details</summary>
Motivation: 现有CRMOT方法严重依赖昂贵的帧级空间标注和跨视角身份监督，为降低这种依赖，本文探索在弱监督条件下利用基础模型的能力进行CRMOT。

Method: 提出两阶段弱监督框架：第一阶段设计亲和力引导的跨视角重提示策略，精炼并关联SAM3生成的跨相机轨迹片段；第二阶段构建ViewSAM模型，基于SAM2设计视图感知的跨模态语义模块，将视图变化建模为可学习条件。

Result: ViewSAM在弱监督下达到SOTA性能，且与全监督方法相比具有竞争力；仅需约10%的额外参数。

Conclusion: 基础模型可作为可靠的伪标签生成器用于弱监督CRMOT；视图感知的跨模态建模能够有效弥合跨视角视觉差异与文本表达之间的鸿沟。
...
TL;DR: 本文提出一种基于YOLO11和YOLO11-Pose的自动化系统，通过骨骼关键点提取推搡动作中的身体倾斜角度和关节角度，利用随机森林分类器实时检测监控视频中的中等程度暴力行为（推搡）。在受控环境下精度达0.98，在真实监控场景（高视角、大倾斜角）下精度为0.72，证明了骨骼分析用于早期暴力干预的可行性。


<details>
  <summary>Details</summary>
Motivation: 公共场所的身体暴力是重大公共卫生问题，轻微推搡往往是更严重暴力升级的前兆。现有监控系统缺乏对中等程度暴力（推搡）的自动化实时检测能力，因此需要开发一套能够早期识别暴力前兆行为的系统。

Method: 集成YOLO11和YOLO11-Pose进行人体检测和骨骼关键点提取，计算身体倾斜角度以及肩部和髋部之间的关节角度，使用随机森林分类器区分正常行为和攻击性身体接触。

Result: 在受控正面视角环境中精度达0.98；在最具挑战性的真实监控场景（高空、大倾斜角度拍摄）中，尽管存在显著的透视畸变和视觉噪声，系统仍保持0.72的精度。

Conclusion: 基于骨骼分析的自动化系统能够有效检测监控视频中的推搡行为，具备在城市安全场景中实现早期暴力干预的可行性。
...
TL;DR: 提出SAIL框架，通过将视网膜解剖先验知识与语义特征融合，在不修改现有后验可解释性方法的前提下，生成更清晰、符合解剖结构的归因图，提升OCT视网膜疾病诊断的可解释性。


<details>
  <summary>Details</summary>
Motivation: 现有深度学习在OCT视网膜疾病诊断中达到专家级准确率，但其"黑箱"特性阻碍临床采纳；现有后验可解释AI方法难以描绘精细病变结构、尊重解剖边界或抑制噪声，限制了解释的可信度。

Method: 提出Structure-Aware Interpretable Learning (SAIL)框架，在表示层面整合视网膜解剖先验知识，通过融合设计将解剖先验与语义特征耦合，无需修改标准后验可解释性方法即可生成更优归因图。

Result: 在多个OCT数据集上的实验表明，提出的结构感知方法一致性地增强可解释性，产生临床有意义且解剖意识强的解释。消融实验显示强可解释性需要结构先验和语义特征共同作用，且两者的适当融合对解释质量至关重要。
...
TL;DR: 本文提出OphMAE，一种多模态眼科基础模型，通过3D OCT与2D OCT的跨模态融合架构和自适应推理机制，在17项诊断任务中达到SOTA性能（AMD AUC 96.9%，DME AUC 97.2%），且具备单模态降级推理能力和优异的数据效率。


<details>
  <summary>Details</summary>
Motivation: 现有眼科AI模型局限于单模态推理，与临床依赖多模态影像互补诊断的实践脱节；同时高性能AI在资源受限环境中因缺乏先进3D成像硬件而难以部署。

Method: 提出OphMAE，基于跨模态融合架构（联合3D OCT体素深度与2D en face OCT平面信息）和自适应推理机制，在大规模配对数据集（183,875对OCT图像，32,765名患者）上进行预训练。

Result: 在17项诊断任务（48,340对OCT图像，8,191名患者）上达到SOTA：AMD AUC 96.9%，DME AUC 97.2%；单模态降级推理仍保持93.7% AUC（AMD）；仅用500个标注样本即可达到95.7% AUC。

Conclusion: OphMAE建立了一个可扩展、自适应的眼科AI框架，能够在不同任务中保持稳健性能，弥合了多模态临床需求与单模态硬件限制之间的鸿沟。
...
TL;DR: 提出Perceptual Flow Network (PFlowNet)，通过解耦感知与推理、结合变分强化学习与邻域几何塑形，实现推理导向的视觉感知，在V* Bench和MME-RealWorld-lite上取得新SOTA。


<details>
  <summary>Details</summary>
Motivation: 现有LVLM在标准MLE优化下存在语言偏差和幻觉问题，引入视觉专家几何先验虽能缓解，但该先验偏向几何精度、推理效用有限，需要更有效的视觉推理方法。

Method: PFlowNet解耦感知与推理建立自条件生成过程，通过变分强化学习整合多维奖励与邻域几何塑形，在保持视觉可靠性的同时促进推理导向的感知行为。

Result: 在V* Bench上达到90.6%，在MME-RealWorld-lite上达到67.0%，均刷新SOTA记录，并提供了可证的性能保证。

Conclusion: PFlowNet通过放弃对专家先验的刚性对齐，实现了可解释且更有效的视觉推理，为LVLM的幻觉问题提供了新的解决思路。
...
TL;DR: SIAM是一个仅使用6个高质量手工标注模板训练的3D全脑分割框架，可同时分割16种脑内及颅外解剖结构，在8个异质数据集上匹配或超越现有方法，并展现出优越的跨对比度一致性和对灰质萎缩的敏感性。


<details>
  <summary>Details</summary>
Motivation: 现有基于合成训练的脑MRI分割方法依赖数百个自动标注模板，引入系统性偏差且难以灵活纳入新解剖结构。本文旨在用极少的高质量手工标注模板实现全脑（包括颅外组织）的分割，消除对预处理流程的依赖。

Method: SIAM将域随机化扩展到强度和形状两个域：通过合成图像生成实现对比度多样性，利用高分辨率空间变换模拟皮质厚度和深部核团形态的解剖差异。模型基于6个高质量手工标注模板训练，同时分割脑组织和颅外组织（脑脊液、血管、硬脑膜、颅骨、皮肤等），实现无需预处理的自动分析。

Result: 在8个异质数据集（N=301，包含T1加权、T2加权、CT多种对比度和广泛年龄范围）上评估，SIAM在脑结构分割上匹配或超越现有方法，并扩展至非脑结构的自动分割；同时表现出优越的跨对比度和重复采集一致性，以及对灰质萎缩的更高敏感性。

Conclusion: SIAM证明了仅用极少量高质量模板即可训练出对比度无关的全脑分割模型，为解剖结构扩展提供了灵活性，并实现了完全自动化、无需预处理的端到端分析。模型和模板已开源。
...
### [150] [Virtual Scanning for NSCLC Histology: Investigating the Discriminatory Power of Synthetic PET](https://arxiv.org/abs/2605.02746)
...
TL;DR: 本文提出利用3D Pix2Pix GAN从CT扫描生成合成PET数据，结合MINT多阶段融合架构进行NSCLC组织学亚型分类（ADC vs SCC），在714例多中心数据上使AUC从0.489提升至0.591，GMean从0.305提升至0.524。


<details>
  <summary>Details</summary>
Motivation: [¹⁸F]FDG PET/CT在NSCLC组织学亚型分类中具有重要价值，但实际PET扫描存在成本高、辐射暴露等限制。研究旨在探索通过"虚拟扫描"策略，利用合成PET数据作为特征增强手段，在无需真实PET扫描的情况下提升CT-only的组织学分类性能。

Method: 使用预训练在FDG-PET/CT Lesions数据集上的3D Pix2Pix GAN从解剖CT扫描合成伪PET体积；将合成代谢特征与结构CT数据集成到MINT多阶段中间融合架构中，实现跨模态互补信息利用。

Result: 在714例多中心数据集上，多模态方法相比CT基线显著提升：AUC从0.489提高到0.591，GMean从0.305提高到0.524，差异具有统计学显著性。

Conclusion: 合成PET扫描能够提供具有判别性的代谢线索，使深度学习模型利用跨模态互补信息，在真实PET不可用场景下作为一种潜在的特征增强策略。
...
TL;DR: 本文指出现有开放世界文本引导的类无关计数模型在语义对齐方面存在缺陷，无法正确判断文本提示应计数的目标类别，并提出了新的评估框架PrACo++（含负标签测试和干扰测试）和MUCCA多类别评估数据集，实验表明现有模型在语义理解上显著不足。


<details>
  <summary>Details</summary>
Motivation: 现有开放世界文本引导的类无关计数（CAC）评估协议仅关注单类别图像内的标准计数误差，忽略了模型正确将文本提示与视觉场景对齐的基本能力，导致模型在实际场景中产生虚假计数响应并降低可靠性。

Method: 提出PrACo++测试套件（包含负标签测试和干扰测试两种评估协议及新专用指标）和MUCCA多类别类无关计数评估数据集（每张图像包含多个标注对象类别），对10种SOTA方法进行实验评估。

Result: 尽管现有模型在标准计数指标上表现良好，但在理解和定位对象类别描述方面存在显著缺陷；提供了语义相似性对失败影响的定量分析。

Conclusion: 需要更具备语义理解能力的架构，本文为开放世界文本引导CAC方法的未来评估提供了可靠框架。
...
TL;DR: 本文提出一种高效的视频增强框架，将模拟VLA视频转换为逼真的训练视频，通过结构化条件提取、字幕重写、条件视频传输模型以及扩散特征重用和核心集采样策略，在多个机器人基准和真实平台上实现了一致的性能提升（如RDT-1B提升8%，π₀提升5.1%）。


<details>
  <summary>Details</summary>
Motivation: 模拟数据虽廉价且可并行采集，但存在显著的视觉域差距和环境多样性不足的问题，导致其在真实世界中的泛化能力弱，因此需要弥合模拟与真实之间的视觉鸿沟。

Method: 提出视频增强框架，包含：(1) 通过视频语义分割和视频字幕提取结构化条件；(2) 重写字幕以多样化环境；(3) 使用条件视频传输模型合成逼真视频；(4) 引入扩散特征重用机制加速生成；(5) 采用核心集采样策略选择紧凑非冗余子集进行增强。

Result: 在Robotwin 2.0上RDT-1B提升8%，在LIBERO-Plus上π₀提升5.1%，并在LIBERO、真实机器人平台上均有一致改进。

Conclusion: 所提框架有效弥合了模拟与真实之间的视觉域差距，在有限计算资源下显著提升了VLA模型在真实世界的泛化能力。
...
TL;DR: FoR-Net是一种轻量级语义分割架构，通过学习重要性图和Top-K激活机制聚焦于困难区域（如薄结构和边界），在Cityscapes上以有限计算资源实现了有竞争力的性能。


<details>
  <summary>Details</summary>
Motivation: 现有轻量级语义分割方法依赖全局建模，计算成本高；本文旨在通过区域聚焦推理，以简单高效的方式提升模型在困难区域（薄结构、物体边界）的分割一致性。

Method: 提出FoR-Net，包含三个关键组件：(1) 选择器模块预测区域级重要性图；(2) Top-K激活机制筛选并增强重要区域；(3) 多尺度卷积分支聚合不同感受野的上下文信息，实现对困难区域的针对性优化。

Result: 在Cityscapes基准上，FoR-Net在有限计算资源下取得了有竞争力的分割性能，尤其在薄结构和边界等挑战性区域表现出更好的一致性。

Conclusion: 区域聚焦推理为高效语义分割提供了一种简单有效的归纳偏置，证明了有选择性地关注困难区域可以替代沉重的全局建模。
...
### [154] [TOC-SR: Task-Optimal Compact diffusion for Image Super Resolution](https://arxiv.org/abs/2605.02767)
...
TL;DR: 本文提出TOC-SR框架，通过紧凑扩散骨干发现和单步蒸馏，实现高效且高质量的图像超分辨率，参数量减少6.6倍，计算量减少2.8倍。


<details>
  <summary>Details</summary>
Motivation: 扩散模型在图像恢复任务中性能优异，但模型尺寸大、迭代采样过程计算成本高，难以在实际部署中应用。

Method: 从16通道潜扩散模型出发，利用特征级生成蒸馏构建参数高效的替代模块，并通过ε约束贝叶斯优化进行架构搜索，在保持生成保真度的同时最小化模型复杂度。随后将紧凑扩散骨干应用于超分辨率任务，并将扩散过程蒸馏为单步生成器。

Result: 紧凑扩散骨干相比原始扩展模型参数量减少6.6倍，GMACs减少2.8倍；在保持高质量重建效果的同时实现了高效超分辨率。

Conclusion: TOC-SR通过紧凑架构发现与单步蒸馏，有效平衡了扩散模型在超分辨率任务中的效率与质量，为实际部署提供了可行方案。
...
### [155] [Linearizing Vision Transformer with Test-Time Training](https://arxiv.org/abs/2605.02772)
*Yining Li,Dongchen Han,Zeyu Liu,Hanyi Wang,Yulin Wang,Gao Huang*

Main category: cs.CV

TL;DR: 本文通过架构对齐（选择TTT作为线性复杂度架构）和表征对齐（引入关键实例归一化和轻量局部增强模块），成功将预训练Softmax注意力权重复用到线性注意力模型中，仅需1小时微调即可在Stable Diffusion 3.5上实现与原始模型相当的文本到图像质量，同时推理加速1.32-1.47倍。


<details>
  <summary>Details</summary>
Motivation: 线性复杂度注意力机制虽能克服Softmax注意力的二次复杂度瓶颈，但从头训练成本过高；而直接将预训练Transformer权重复制到线性注意力模型中存在表征鸿沟，导致权重迁移失效。

Method: 1) 架构对齐：选择TTT（Test-Time Training）作为线性复杂度架构，其双层动态公式与Softmax注意力在结构上保持一致；2) 表征对齐：引入关键实例归一化（key instance normalization）解决键移不变性差异，以及轻量局部增强模块弥补局部性差异。

Result: 在Stable Diffusion 3.5上验证，仅用4×H20 GPU微调1小时，所提SD3.5-T⁵模型在文本到图像质量上与微调后的Softmax模型相当，在1K和2K分辨率下分别实现1.32倍和1.47倍推理加速。

Conclusion: 通过双重对齐策略（架构+表征），可以低成本地将预训练Softmax注意力模型高效转换为线性复杂度模型，兼顾质量与效率。
...
### [156] [HumanSplatHMR: Closing the Loop Between Human Mesh Recovery and Gaussian Splatting Avatar](https://arxiv.org/abs/2605.02784)
*Yeheng Zong,Pou-Chun Kung,Yike Pan,Seth Isaacson,Yizhou Chen,Ram Vasudevan,Katherine A. Skinner*

Main category: cs.CV

TL;DR: 本文提出HumanSplatHMR，一种联合优化框架，通过将几何姿态估计与可微渲染闭环耦合，同时优化3D人体姿态和学习高保真虚拟人，实现了对新视角和新姿态的泛化能力，显著优于现有解耦方法。


<details>
  <summary>Details</summary>
Motivation: 现有方法（ViT、NeRF、Gaussian Splatting）在恢复人体3D几何时存在不足：ViT过度拟合2D视角且不可靠，NeRF/高斯泼溅方法将姿态与外观分离，限制了新姿态渲染的泛化能力；同时这些方法依赖动作捕捉系统或离线精化，不适用于野外场景。

Method: 提出HumanSplatHMR联合优化框架，核心思想是闭环耦合几何姿态估计与可微渲染。使用SOTA人体姿态估计器提供的网格估计作为输入，通过可微渲染器反向传播光度、分割和深度损失到姿态参数和全局位置，实现姿态与外观的联合优化。

Result: 实验表明，该方法在姿态恢复基线和虚拟人基线两个方向均取得一致改进，优于省略图像级精化的姿态恢复方法以及解耦姿态估计与虚拟人重建的基线方法。

Conclusion: HumanSplatHMR有效解决了现有方法在人体3D几何恢复和新姿态渲染泛化上的局限性，为野外场景下的人体姿态估计与高保真虚拟人重建提供了可行方案。
...
### [157] [IConFace: Identity-Structure Asymmetric Conditioning for Unified Reference-Aware Face Restoration](https://arxiv.org/abs/2605.02814)
*Axi Niu,Jinyang Zhang,Senyan Qing*

Main category: cs.CV

TL;DR: IConFace提出了一种统一框架，通过身份-结构非对称条件调制，在有参考和无参考场景下实现盲脸恢复，利用参考图像的身份锚点和退化图像的结构锚点，在单一检查点中同时提升身份一致性和细节恢复质量。


<details>
  <summary>Details</summary>
Motivation: 盲脸恢复在严重退化下高度病态，身份关键细节可能丢失。同身份参考可减少歧义，但姿态、表情、光照、年龄、妆容等不匹配会导致参考外观的过度使用。

Method: 提出IConFace统一框架，采用身份-结构非对称条件调制。参考图像通过归一化权重的全局AdaFace身份锚点进行图像级调制；退化图像通过低秩残差和块级退化交叉注意力（双路由记忆）增强为空间结构锚点。

Result: 单一检查点在参考可用时利用参考，参考缺失时退化为无参考恢复，提升了身份一致性、细节恢复质量以及仅有退化图像的恢复效果。

Conclusion: IConFace通过身份-结构非对称调制实现了统一的有/无参考盲脸恢复框架，有效解决了参考不匹配和退化严重的问题。
...
### [158] [VideoNet: A Large-Scale Dataset for Domain-Specific Action Recognition](https://arxiv.org/abs/2605.02834)
...
TL;DR: 本文提出了VideoNet，一个包含37个领域1000种动作的领域特定动作识别基准。实验表明当前VLM在该基准上表现不佳（如Qwen3-VL-8B仅45.0%准确率），即使在二选一场景下也仅59.2%。VLMs难以充分利用上下文示例（few-shot提升有限），而微调（Molmo2-4B在近50万问答对数据上微调）可超越所有8B开源模型。


<details>
  <summary>Details</summary>
Motivation: 现有VLM缺乏对动作识别能力的评估（缺乏多样且具有挑战性的数据），作者旨在重新推动VLM时代下的动作识别研究，聚焦于领域特定动作。

Method: 1) 构建VideoNet基准：37个领域、1000种动作；2) 多轮实验：多选题→二选一→k-shot上下文示例；3) 收集约50万视频问答对的训练数据集；4) 对Molmo2-4B进行微调。

Result: Gemini 3.1 Pro 69.9% vs Qwen3-VL-8B 45.0%（多选题）；二选一场景Qwen仅59.2%；few-shot下Qwen提升+7.0%但Gemini下降-4.8%，均不及非专家人类+13.6%；微调后Molmo2-4B超越所有8B开源模型。

Conclusion: 当前VLMs在领域特定动作识别上能力不足，且难以有效利用上下文示例；通过大规模领域特定数据微调可以显著提升模型性能，该方向值得进一步研究。
...
TL;DR: ActDiff-VC 是一种基于扩散模型的超低码率视频压缩框架，通过可变长分段、按需传输关键帧和紧凑点轨迹追踪来生成感知高质量的重建帧，在 UVG 和 MCL-JCV 基准上实现高达 64.6% 的码率节省和显著的感知质量提升。


<details>
  <summary>Details</summary>
Motivation: 扩散模型为超低码率下的感知重建提供了强大的生成先验，但有效的视频压缩需要利用高度紧凑的条件信号来控制生成过程。现有方法在极低码率下难以兼顾压缩效率与感知质量。

Method: 将视频划分为可变长度片段，仅在需要时传输关键帧，并通过紧凑的点轨迹集合总结时序动态；引入内容自适应关键帧选择和预算感知的稀疏轨迹选择两种机制，作为条件信号输入扩散解码器生成其余帧。

Result: 在 UVG 和 MCL-JCV 基准上，相比强学习编解码器，在相同 NIQE 下码率降低 64.6%，KID 提升 64.6%，FID 提升 37.7%，在超低码率下实现了优于学习和扩散基线的感知率失真权衡。

Conclusion: ActDiff-VC 通过内容自适应的关键帧与稀疏轨迹条件设计，有效利用扩散模型的生成先验，在超低码率视频压缩中取得了显著的感知质量与压缩效率提升。
...
### [160] [AlbumFill: Album-Guided Reasoning and Retrieval for Personalized Image Completion](https://arxiv.org/abs/2605.02892)
*Yu-Ju Tsai,Brian Price,Qing Liu,Luis Figueroa,Daniil Pakhomov,Zhihong Ding,Scott Cohen,Ming-Hsuan Yang*

Main category: cs.CV

TL;DR: AlbumFill是一个无需训练的图像补全框架，通过从个人相册中检索与身份一致的参考图像来实现个性化图像补全，并配套了包含5.4万张人像样本的数据集。


<details>
  <summary>Details</summary>
Motivation: 现有方法要么使用通用修复模型导致身份不一致，要么假设用户明确提供了合适的参考图像——而实际场景中用户往往不会主动提供，因此需要系统自动从个人相册中搜索与身份一致的参考图像。

Method: 无需训练的框架AlbumFill：1) 视觉语言模型从受损图像中推断缺失的语义线索；2) 利用这些线索引导组合式图像检索，从个人相册中召回与身份一致的参考图像；3) 将检索到的参考图像输入基于参考的图像补全模型进行修复。

Result: 在5.4万张人像样本的数据集上进行实验，多个基准方法对比表明个性化补全任务具有挑战性，并验证了身份一致参考图像检索的重要性。

Conclusion: AlbumFill通过自动检索身份一致的参考图像实现了更实用的个性化图像补全，且无需额外训练，实验证明了该方法的有效性和参考图像质量对补全结果的关键作用。
...
### [161] [H-Probes: Extracting Hierarchical Structures From Latent Representations of Language Models](https://arxiv.org/abs/2605.00847)
*Cutter Dawes,Aryan Sharma,Angelos Ioannis Lagos,Shivam Raval*

Main category: cs.CL

TL;DR: 本文提出H-probes线性探针方法，从语言模型的潜在表征中提取层次结构（深度和距离）。在合成树遍历任务中，发现层次相关子空间是低维、因果重要且可泛化的；在数学推理等真实层次数据中也发现类似但较弱的层次结构。结果表明模型不仅在句法和概念层面，还在推理过程本身编码了层次抽象。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在需要层次推理的任务中表现优异，但对其如何在几何上表示层次结构的潜在构造缺乏系统分析。

Method: 开发H-probes线性探针组，从潜在表征中提取层次深度和成对距离；在合成树遍历任务和真实数学推理数据上进行验证和消融实验。

Result: H-probes能稳健提取包含层次结构的子空间；这些子空间是低维的、对任务性能因果重要、且在域内和域外都能泛化；在真实数学推理数据中也发现类似层次结构。

Conclusion: 模型不仅在句法和概念层面表示层次，还在更深的推理过程抽象层面编码了层次结构。
...
### [162] [Model Organisms Are Leaky: Perplexity Differencing Often Reveals Finetuning Objectives](https://arxiv.org/abs/2605.00994)
*Mohammed Abu Baker,Luca Baroni,Dan Wilhelm*

Main category: cs.CL

TL;DR: 本文提出了一种基于困惑度（perplexity）的简单方法，通过利用微调模型过度泛化其微调行为的倾向，从模型生物体中揭示微调目标。该方法首先生成多样化补全，然后按参考模型与微调模型之间的困惑度差距降序排名，排名靠前的补全即可揭示微调目标，无需模型内部状态或先验假设。在76个模型生物体（0.5至700亿参数）上的实验表明，该方法对绝大多数模型有效，且即使使用不同家族的参考模型也能发挥作用。


<details>
  <summary>Details</summary>
Motivation: 微调可以显著改变大语言模型的行为，包括引入有害或不安全的行为。为了研究这些风险，研究人员开发了模型生物体（经过微调以表现出特定已知行为的模型），但识别这些行为仍然具有挑战性。

Method: 1) 使用来自通用语料库的短随机前缀从微调模型生成多样化补全；2) 按参考模型与微调模型之间困惑度差距降序排列补全；3) 排名靠前的补全往往能揭示微调目标。该方法仅需要微调模型的下一个词元概率，与暴露token对数概率的API封闭模型兼容。

Result: 在76个模型生物体（0.5至700亿参数）上评估，包括后门模型、通过合成文档微调内化虚假事实的模型、具有隐藏问题行为的对抗训练模型以及表现出新兴错误对齐的模型。绝大多数模型生物体的微调目标都能通过排名靠前的补全揭示，其中通过合成文档微调或训练生成精确短语的模型特别容易受影响。即使使用不同家族的参考模型作为替代也同样有效。

Conclusion: 基于困惑度差距排名补全的简单方法可以有效揭示微调模型的目标行为，无需访问精确的微调前检查点，且适用于API封闭模型，为检测和监控微调后模型的安全风险提供了一种实用工具。
...
### [163] [Can AI Debias the News? LLM Interventions Improve Cross-Partisan Receptivity but LLMs Overestimate Their Own Effectiveness](https://arxiv.org/abs/2605.01006)
*Faisal Feroz,Jonas R. Kunst*

Main category: cs.CL

TL;DR: 本研究通过两项预注册实验发现，大型语言模型（LLM）对新闻标题进行去偏处理时，实质性重述框架（而非表面词汇替换）能提升保守派对自由派新闻的信任度，但LLM对自身干预效果的评估缺乏准确性和心理保真度。


<details>
  <summary>Details</summary>
Motivation: 党派新闻媒体侵蚀了跨党派信任，而LLM提供了一种规模化去偏内容的潜在手段，本文旨在检验LLM生成的去偏干预能否改善跨党派受众的信任相关判断。

Method: 两项预注册实验：研究1采用词汇去偏（将情绪化词汇替换为更温和的同义词），研究2采用实质性重述干预。同时使用人类读者和LLM模拟的"硅基参与者"进行对比，并进行调节效应分析。

Result: 研究1中词汇去偏对任何结果均无影响；研究2中实质性重述显著提高了保守派对自由派新闻标题的信任度、完整性和参与意愿，且未在自由派样本中产生逆火效应。硅基参与者在研究1中显示出稳健效应但人类无反应，在研究2中方向一致但效应量显著更大。

Conclusion: 基于LLM的去偏处理在针对意识形态框架而非表面语言时能改善跨党派接受度，但当前模型缺乏定量准确性和定性心理保真度，无法在没有人类监督的情况下评估自身干预效果。
...
### [164] [CLEAR: Revealing How Noise and Ambiguity Degrade Reliability in LLMs for Medicine](https://arxiv.org/abs/2605.01011)
*Kevin H. Guo,Chao Yan,Avinash Baidya,Katherine Brown,Xiang Goa,Juming Xiong,Zhijun Yin,Bradley A. Malin*

Main category: cs.CL

TL;DR: CLEAR框架通过扰动选项数量、真实答案/弃权选项存在性及语义框架，系统评估LLM在医学基准上的推理能力。发现增加合理选项会降低模型正确识别和弃权能力；弃权框架从"以上皆非"到"不知道"时，模型谨慎性下降；定义"谦逊赤字"衡量正确识别与弃权之间的差距，且该赤字随模型规模增大而恶化。


<details>
  <summary>Details</summary>
Motivation: 现有医学LLM评估依赖简化的考试式基准，无法反映真实医疗咨询中的模糊性。现有评估方法忽视了决策空间呈现方式、模糊性和不确定性对LLM推理的影响。

Method: CLEAR框架系统扰动三个维度：(1)合理答案选项数量；(2)是否包含真实答案或弃权选项；(3)答案选项的语义框架。在3个基准、17个LLM上进行评估。

Result: (1)增加合理选项数降低模型正确识别和弃权能力；(2)弃权从"以上皆非"变为"不知道"时，模型更不谨慎，甚至增加错误选择；(3)定义"谦逊赤字"，该赤字随模型规模增大而恶化。

Conclusion: 标准医学基准存在局限性，仅通过模型规模扩展无法解决LLM的可靠性问题。
...
### [165] [Psychologically Potent, Computationally Invisible: LLMs Generate Social-Comparison Triggers They Fail to Detect](https://arxiv.org/abs/2605.01017)
*Hua Zhao,Jiapei Gu,Michelle Mingyue Gu*

Main category: cs.CL

TL;DR: 本文提出了XHS-SCoRE基准，用于检测小红书帖子是否引发向上、向下或无社会比较。研究发现，虽然该信号在领域内是可学习的，但基于提示的分类器检测不稳定，存在系统性的失败模式（如中性化偏误和模型特定的方向偏斜）。实验进一步表明，LLM生成的内容可以影响感知地位，但检测能力仍然脆弱。


<details>
  <summary>Details</summary>
Motivation: 社会比较是社交平台上一种重要的行为信号，现有工作多关注情感分析或立场检测，缺乏针对"读者视角下社会比较触发"这一细粒度、行为真实的语义信号的基准和评估框架。作者希望填补这一空白。

Method: 构建了XHS-SCoRE基准（中文小红书帖子，标注向上/向下/无社会比较）；使用多种提示式LLM分类器和监督式中文编码器基线进行对比实验；设计受控实验评估LLM生成内容对感知比较的影响。

Result: 提示式LLM分类器存在稳定的、可解释的失败模式：倾向于将触发比较的帖子中性化，且不同模型有方向性偏斜；监督式编码器在领域内可学习该信号，但提示方法泛化性差；LLM生成的内容能影响读者感知，但检测能力仍脆弱。

Conclusion: XHS-SCoRE为读者视角的社会比较检测提供了基准和诊断框架，揭示了生成流畅性与可靠检测能力之间的不匹配，提示当前LLM在细粒度社会语义信号上的局限性。
...
### [166] [A Theoretical Game of Attacks via Compositional Skills](https://arxiv.org/abs/2605.01034)
*Xinbo Wu,Huan Zhang,Abhishek Umrawal,Lav R. Varshney*

Main category: cs.CL

TL;DR: 本文提出了一个攻击者与防御者之间博弈的理论框架，形式化了对抗性提示攻击，推导出理论最优攻击策略（与现有方法紧密相关）和可证明最优的防御策略，分析了博弈均衡并揭示了攻击者的固有优势，实验表明理论最优攻击的实践实例在多种LLM和基准上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型能力日益增强，但现有对齐策略仍可通过精心设计的对抗性提示被绕过，缺乏一个统一的理论框架来理解和分析攻击与防御之间的博弈关系。

Method: 形式化定义攻击者与防御者之间的博弈框架，设计理论最优响应攻击策略，分析博弈均衡及攻击者优势，推导可证明最优的防御策略，并通过实验评估理论最优攻击的实际效果。

Result: 理论最优攻击的实践实例在不同LLM和基准上比现有对抗性提示方法表现更强。

Conclusion: 该理论框架揭示了攻击与防御博弈中的固有不对称性，为理解和设计更鲁棒的LLM安全机制提供了理论基础。
...
### [167] [Compared to What? Baselines and Metrics for Counterfactual Prompting](https://arxiv.org/abs/2605.01048)
*Zihao Yang,Mosh Levy,Yoav Goldberg,Byron C. Wallace*

Main category: cs.CL

TL;DR: 本文指出反事实提示方法（如改变患者性别评估LLM偏见）存在根本性缺陷：每个反事实编辑都是复合处理，同时改变了目标变量和表面形式。通过统计比较，发现LLM对性别变化的预测翻转率（14.9%）与单纯改写输入的翻转率（14.1%）无显著差异。作者提出框架，将目标干预效果与改写输入的效果进行统计比较，并重新评估了MedPerturb数据集，发现120个测试中仅5个达到统计显著性。同时，该框架能有效检测真实的性别方向偏见。


<details>
  <summary>Details</summary>
Motivation: 反事实提示被广泛用于评估LLM的偏见和思维链忠实度，但每个反事实编辑不仅改变了目标变量，还引入了偶然的表面形式变化（违反"处理变异性无关性"假设），导致观察到的效果无法直接归因于目标因素。需要建立基线来区分真正的因果效应和一般模型敏感性。

Method: 提出框架：将目标干预（如改变性别）下观测到的差异与单纯改写输入（不改变目标变量）所引发的差异进行统计比较。若两者无显著差异，则不能将效果归因于目标变量。评估了聚合指标、逐样本分布指标和回归指标三种方法。

Result: 在MedQA上，改变患者性别导致14.9%的预测翻转率，与改写输入导致的14.1%无统计显著差异。重新分析MedPerturb数据集，120个测试中仅5个达到统计显著性。而在职业传记分类任务中，框架成功检测到显著的方向性别偏见。逐样本指标的统计功效远高于聚合指标，回归指标能独特地表征效应方向和幅度。

Conclusion: 现有的反事实提示评估方法高估了LLM对特定因素的敏感性，因为未考虑一般模型敏感性（即对表面形式变化的反应）。所提出的框架能有效区分真正的因果效应和偶然变化，为更可靠的LLM评估提供了方法论基础。
...
### [168] [A Systematic Exploration of Text Decomposition and Budget Distribution in Differentially Private Text Obfuscation](https://arxiv.org/abs/2605.01065)
*Stephen Meisenbacher,Angelo Kleinert,Florian Matthes*

Main category: cs.CL

TL;DR: 本文系统评估了差分隐私文本混淆中不同的文本分解方法和隐私预算分配技术，发现设计选择对结果有显著影响，并证明了通过优化DP混淆过程可以最大化经验性权衡。


<details>
  <summary>Details</summary>
Motivation: 现有差分隐私文本混淆研究主要在词级进行扰动，但有意义的文本隐私保护应发生在完整文档层面。目前缺乏对文本分解方式和隐私预算分配技术的系统性评估，需要理解不同设计选择对隐私-效用权衡的影响。

Method: 系统评估多种文本分解方法（分块策略）与隐私预算ε分配技术的组合，在差分隐私文本混淆框架下进行实验比较，测试不同分块与分配方案搭配的效果。

Result: 设计选择至关重要，即使在可比隐私预算下，不同方法的选择会导致显著不同的结果；提供了通过优化DP混淆过程来最大化经验性权衡的可行证据。

Conclusion: 文本分解和预算分配的设计选择对DP文本混淆效果影响重大，优化这些流程可以在隐私保护与文本效用之间取得更好的平衡。
...
### [169] [Controlled Paraphrase Geometry in Sentence Embedding Space: Local Manifold Modeling and Latent Probing](https://arxiv.org/abs/2605.01073)
*Leonid Bedratyuk*

Main category: cs.CL

TL;DR: 本文研究语义相近句子嵌入云的局部几何结构，提出基于仿射、二次、三次拟合模型的局部几何建模方案及表面潜变量探测方法，实验表明非线性模型优于线性模型，但几何有效性不等同于分类效用，并贡献了CoPaGE-300K数据集。


<details>
  <summary>Details</summary>
Motivation: 探究受控的释义类语义变化在句子嵌入空间中如何组织，以及这种局部结构是否能被低阶拟合模型显式建模。

Method: 提出基于仿射、二次、三次拟合的局部几何建模方案，以及基于表面的潜变量探测过程，在降维的局部PCA空间中构造合成潜变量点，并评估其表面一致性、邻域结构保持、经验分布匹配、Hessian形状描述子稳定性及拟合系数稳定性。

Result: 非线性局部模型（二次、三次）比仿射模型更准确地描述嵌入云；基于表面的生成具有强拟合几何保真度；但几何有效性并不自动带来分类性能提升。

Conclusion: 支持对句子嵌入空间进行显式局部几何建模，需区分几何有效性与判别效用；贡献了CoPaGE-300K受控模板数据集。
...
### [170] [Teaching LLMs Brazilian Healthcare: Injecting Knowledge from Official Clinical Guidelines](https://arxiv.org/abs/2605.01077)
*Hugo Abonizio,Filipe Rocha Lopes,Roberto Lotufo,Rodrigo Nogueira*

Main category: cs.CL

TL;DR: 本文针对巴西统一医疗系统（SUS）的官方临床指南，通过合成数据生成、持续预训练和GRPO强化学习，训练出14B参数的Qwen2.5模型，在HealthBench-BR和PCDT-QA两个新基准上分别达到83.9%和85.4%的准确率，超越了GPT-5.2、Claude Sonnet 4.6等更大模型。


<details>
  <summary>Details</summary>
Motivation: 巴西SUS依赖官方临床指南为超2亿公民提供诊疗标准，但现有LLM在葡萄牙语临床指南知识上表现不佳，且缺乏针对巴西葡萄牙语临床知识的基准评测。

Method: 从178份官方指南（约540万token）出发，使用4个生成器LLM生成约7000万token的三种格式合成数据（改写、维基风格文章、问答对），随后进行持续预训练和GRPO强化学习。

Result: 最佳模型在HealthBench-BR（1780道平衡真假临床断言）达83.9%，在PCDT-QA（890道开放式临床问题）达85.4%，超越GPT-5.2、Claude Sonnet 4.6、Gemini 3.1 Pro及基于RAG的Google AI Overview。消融实验表明生成器多样性和强化学习是关键。

Conclusion: 证明了小规模（14B）模型可通过针对性的合成数据生成与强化学习在低资源语言临床领域超越大规模通用模型，并公开了全部数据集、基准和模型权重以促进可复现研究。
...
### [171] [Interpretable Difficulty-Aware Knowledge Tracing in Tutor-Student Dialogues](https://arxiv.org/abs/2605.01097)
*Shuyan Huang,Alexander Scarlatos,Jaewook Lee,Andrew Lan*

Main category: cs.CL

TL;DR: 本文提出了一种可解释的、考虑难度的对话式知识追踪框架，基于大语言模型（LLMs），通过显式建模学生能力和题目难度，并融入项目反应理论（IRT），在对话场景中实现了比现有方法更准确且可解释的学生表现预测。


<details>
  <summary>Details</summary>
Motivation: 现有基于对话的知识追踪（KT）方法通常忽略题目难度建模，并且依赖LLM生成的黑箱潜在表征，导致预测不够准确且缺乏可解释性。

Method: 构建了一个基于LLM的可解释难度感知对话式知识追踪框架，显式建模学生能力和教师提问的难度；利用原始文本和下一轮问题估计学生知识状态与任务难度；集成项目反应理论（IRT）将LLM输出映射为能力参数和难度参数，实现可解释的预测。

Result: 在两个师生对话数据集上，该框架在定量和定性评估中均优于现有知识追踪基线，同时生成了符合认知理论的可解释输出。

Conclusion: 显式建模难度并融合IRT理论到LLM中，能够有效提升对话式知识追踪的准确性和可解释性，具有认知理论基础。
...
### [172] [Quantifying and Predicting Disagreement in Graded Human Ratings](https://arxiv.org/abs/2605.01168)
*Leixin Zhang,Çağrı Çöltekin*

Main category: cs.CL

TL;DR: 本文研究人工标注者在不当语言（冒犯性语言、仇恨言论、有毒言论）评分中的分歧模式，提出量化标注者观点对立的"对立指数"（Opposition Index），并验证从文本特征预测标注方差及对立观点的可行性。


<details>
  <summary>Details</summary>
Motivation: 人工标注者之间存在固有分歧，且不同实例的分歧程度各异。现有研究较少关注如何预测这种分歧模式以及量化标注者之间的观点对立程度。

Method: 提出对立指数（Opposition Index）量化标注者观点对立程度；采用两种方差预测方法：直接预测方差值与从预测的标注分布中估计方差；使用文本特征进行预测。

Result: 估计方差与观测方差之间存在中等程度的正相关；直接预测方差与从预测分布估计方差两种方法性能相当；高对立指数项的预测难度更大且常被模型低估。

Conclusion: 文本特征可用于预测标注分歧程度；对立指数有助于识别具有潜在对立观点的实例，但这类实例的预测仍是挑战。
...
### [173] [Compute Optimal Tokenization](https://arxiv.org/abs/2605.01188)
...
TL;DR: 本文系统研究了分词压缩率（每token平均字节数）对缩放定律的影响，通过在50M-7B参数范围内训练988个潜在分词模型(BLT)，发现计算最优配置下模型参数量与数据字节数成正比而非token数，且最优压缩率低于BPE分词器并随计算量增加而降低。


<details>
  <summary>Details</summary>
Motivation: 虽然缩放定律已指导数据和模型大小的最优选择，但数据的基本单元——token——对这一关系的影响尚未被充分探索。

Method: 训练988个潜在分词模型(BLT)，参数范围50M-7B，通过控制压缩率（每token平均字节数）系统研究信息粒度对缩放趋势的影响。

Result: 计算最优配置下模型参数量与数据字节数成正比而非token数；最优压缩率低于BPE分词器所得值，且随计算量增加而降低。

Conclusion: 这些发现对潜在分词和子词分词以及非英语语言均具有普适性，可指导语言模型开发者选择分词方案以最大化计算效率。
...
### [174] [SRA: Span Representation Alignment for Large Language Model Distillation](https://arxiv.org/abs/2605.01205)
*Quoc Phong Dao,Hoang Son Nguyen,Pham Khanh Chi,Tung Nguyen,Linh Ngo Van,Nguyen Thi Ngoc Diep,Trung Le*

Main category: cs.CL

TL;DR: 本文提出SRA框架，将跨分词器知识蒸馏重新定义为多粒子动力学系统中的跨度表示对齐，通过质心表征和几何正则化实现跨架构蒸馏，显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有跨分词器知识蒸馏方法主要聚焦于token级别的对齐策略，这些策略对分词器之间的差异敏感且脆弱，需要更鲁棒的对齐方法。

Method: 提出Span表示对齐（SRA）框架：将每个跨度建模为粒子簇，用注意力加权的质心（CoM）表示状态；采用注意力导出的加权机制优先处理关键跨度；使用几何正则化保持表示空间结构完整性；引入对齐跨度logit蒸馏增强跨模型知识迁移。

Result: 在跨架构蒸馏实验中，SRA一致且显著优于最先进的跨分词器知识蒸馏基线方法。

Conclusion: 物理启发的基于质心的跨度对齐方法比token级别对齐更鲁棒有效，为跨分词器知识蒸馏提供了新的理论基础和实践范式。
...
### [175] [GIFT: Guided Fine-Tuning and Transfer for Enhancing Instruction-Tuned Language Models](https://arxiv.org/abs/2605.01256)
*Zhiwen Ruan,Yichao Du,Jianjie Zheng,Longyue Wang,Yun Chen,Peng Li,Jinsong Su,Yang Liu,Guanhua Chen*

Main category: cs.CL

TL;DR: 提出GIFT框架，通过利用指令微调模型的置信度信号指导预训练基座模型的低秩适配器微调，再将适配器合并回指令微调模型，实现任务专业化与通用指令遵循能力的平衡。


<details>
  <summary>Details</summary>
Motivation: 现有方法将指令微调模型仅作为最终合并阶段的被动目标，未参与训练过程的引导，导致任务适应过程中缺乏对指令遵循行为的有效约束。

Method: GIFT框架包含两个阶段：(1) 在预训练基座模型上使用来自指令微调模型的置信度信号（confidence signals）指导低秩适配器（LoRA）微调；(2) 将学到的适配器合并回指令微调模型，获得任务专业化模型。

Result: 在数学和知识密集型基准测试中，GIFT在多个模型家族和规模上一致优于直接微调和代表性迁移基线方法，同时保持鲁棒的泛化能力和良好的测试时扩展行为。

Conclusion: GIFT通过引入指令模型的引导信号，在任务适应过程中有效保留了通用指令遵循能力，是一种简单高效的微调迁移框架。
...
### [176] [Addressing Data Scarcity in Bangla Fake News Detection: An LLM-Based Dataset Augmentation Approach](https://arxiv.org/abs/2605.01292)
*Ahmed Alfey Sani,Kazi Akib Zaoad,Shefayat E Shams Adib,Md Abdul Muqtadir,Ajwad Abrar*

Main category: cs.CL

TL;DR: 本研究提出基于Gemma 3 27B IT模型的系统增强框架，通过生成合成孟加拉语新闻、语义过滤和可控子采样，将虚假新闻F1分数从0.85提升至0.88，并公开了4,545个合成样本及代码。


<details>
  <summary>Details</summary>
Motivation: 数字媒体中虚假信息的蔓延亟需可靠的假新闻检测系统，但孟加拉语等低资源语言面临数据集规模小且类别严重不平衡的问题，现有数据集虽宝贵但限制了模型性能，且基于LLM的数据增强在孟加拉语领域鲜有探索。

Method: 使用指令微调的Gemma 3 27B IT模型通过零样本和少样本提示生成合成孟加拉语新闻，结合语义过滤和可控子采样保持标签一致性与多样性，比较不同增强率和随机/相似性选择策略。

Result: 仅对少数类采用高增强率和随机子采样策略取得最佳效果，虚假新闻F1分数从0.85提升至0.88。

Conclusion: 精心设计的LLM驱动的数据增强能在低资源场景下显著提升假新闻检测性能，为推进多语言虚假信息研究提供了实用基础。
...
### [177] [Enhancing Game Review Sentiment Classification on Steam Platform with Attention-Based BiLSTM](https://arxiv.org/abs/2605.01315)
...
<details>
  <summary>Details</summary>
Motivation: 针对Steam游戏评论的情感分析需求，探索深度学习模型相比传统机器学习方法在游戏用户反馈情感分类上的有效性，帮助开发者理解玩家反馈。

Method: 使用注意力机制增强的BiLSTM模型，采用PyTorch实现；以TF-IDF+PyCaret AutoML作为传统基线；使用类别加权交叉熵损失函数处理类别不平衡问题；通过注意力可视化展示模型可解释性。

Result: 测试集上达到83%准确率、85%加权F1分数，负面评论召回率达90%，优于传统TF-IDF+AutoML基线方法。

Conclusion: BiLSTM+Attention模型能有效分析Steam评论中的用户情感，为开发者理解玩家反馈提供了有价值的工具。
...
### [178] [Sentiment Analysis of Mobile Legends App Reviews Using Machine Learning and LSTM-Based Deep Learning Models](https://arxiv.org/abs/2605.01317)
*Vira Putri Maharani,Kharisa Harvanny,Daris Samudra,Luluk Muthoharoh,Ardika Satria,Martin Clinton Tosima Manullang*

Main category: cs.CL

TL;DR: 本文对比了机器学习与基于LSTM的深度学习方法在Mobile Legends应用评论情感分析中的表现。使用包含10,000条正面、负面和中性标签的评论数据集，评估了基于TF-IDF和PyCaret AutoML的传统模型，并与捕捉序列依赖性的LSTM模型进行对比。结果表明，LSTM模型以92%的准确率和91%的加权F1分数优于传统机器学习基线，证明深度学习在处理非正式且依赖上下文的用户评论文本方面更为有效。


<details>
  <summary>Details</summary>
Motivation: 探究深度学习（特别是LSTM）相比传统机器学习方法在非正式、依赖上下文的用户评论情感分析任务中的有效性差异。

Method: 使用10,000条标注为正面、负面和中性的Mobile Legends应用评论数据集，分别采用基于TF-IDF和PyCaret AutoML的传统机器学习方法，以及用于捕捉文本序列依赖性的LSTM深度学习模型进行情感分析实验。

Result: LSTM模型在准确率（92%）和加权F1分数（91%）上均优于传统机器学习基线模型。

Conclusion: 深度学习（LSTM）在处理非正式、依赖上下文的用户评论文本的情感分析任务中比传统机器学习方法更有效。
...
### [179] [Benchmarking LightGBM and BiLSTM for Sentiment Analysis on Indonesian E-Commerce Reviews](https://arxiv.org/abs/2605.01322)
...
TL;DR: 本文比较了基于PyCaret AutoML的机器学习方法（LightGBM、逻辑回归、SVM）与深度学习方法（BiLSTM）在印尼电商评论情感分析任务上的表现。BiLSTM以98.87%的准确率和F1分数胜出，LightGBM以98.23%的准确率和极短的训练时间成为最佳ML模型。


<details>
  <summary>Details</summary>
Motivation: 在印尼电商评论的情感分析场景下，系统比较AutoML驱动的传统机器学习方法与深度学习方法的表现，以确定哪种方法在准确性和效率上更优。

Method: 使用Hugging Face上15,000条印尼电商评论数据集，划分训练/验证/测试集。ML实验采用PyCaret AutoML框架，比较LightGBM、逻辑回归和SVM；DL实验构建BiLSTM架构进行对比评估。

Result: BiLSTM准确率98.87%、F1分数98.87%，优于所有ML模型；LightGBM为最佳ML模型，准确率98.23%且训练时间极短。

Conclusion: BiLSTM架构在捕捉印尼评论文本的序列上下文方面表现优异，是该特定分类任务的最优模型。
...
### [180] [Creating and Evaluating Figurative Language Dataset for Sindhi](https://arxiv.org/abs/2605.01323)
*Wazir Ali,Adeeb Noor,Saifullah Tumrani*

Main category: cs.CL

TL;DR: 本文提出了SiNFluD——首个信德语比喻语言分类基准数据集。通过从博客、社交媒体和文学来源收集原始文本并标注（一致性0.81），使用5折和10折交叉验证建立基线，评估了mBERT、XLM-RoBERTa、XLM-RoBERTa-XL及SetFit模型，其中XLM-RoBERTa-XL表现最佳。


<details>
  <summary>Details</summary>
Motivation: 信德语（Sindhi）的低资源特性导致其比喻语言处理研究匮乏，缺乏标准化的评估基准数据集。

Method: 从博客、社交媒体和文学来源收集原始文本→使用Doccano工具由两位母语者标注（一致性0.81）→5折和10折交叉验证建立基线→评估mBERT、XLM-RoBERTa、XLM-RoBERTa-XL以及SetFit（少样本微调句子Transformer）。

Result: 预训练的XLM-RoBERTa-XL在所有模型中取得了最佳性能。

Conclusion: SiNFluD填补了信德语比喻语言分类领域的空白，XLM-RoBERTa-XL在该低资源语言任务上表现最优。
...
### [181] [OralMLLM-Bench: Evaluating Cognitive Capabilities of Multimodal Large Language Models in Dental Practice](https://arxiv.org/abs/2605.01333)
*Rongyang Wang,Shuang Zhou,Jiashuo Wang,Wenya Xie,Xiaoxia Che*

Main category: cs.CL

TL;DR: 本文提出了一个评估多模态大语言模型（MLLMs）在牙科影像分析中认知能力的综合基准，涵盖根尖片、全景片和侧位头影测量片三种影像模态，定义了感知、理解、预测和决策四类认知任务，包含27项临床任务、3,820份临床评估，并评估了GPT-5.2和GLM-4.6等六个前沿模型，揭示了MLLMs与临床医生之间的性能差距。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在牙科影像分析中展现出潜力，但其捕获放射影像分析所需的多层次认知过程的能力尚不明确，因此需要系统性的评估基准来量化其认知能力。

Method: 构建了一个涵盖三种关键影像模态（根尖片、全景片、侧位头影测量片）和四类认知类别（感知、理解、预测、决策）的基准，包含27个来自公开数据集的临床任务，配有手动整理的标注和3,820份临床评估，并评估了六个前沿MLLMs（包括GPT-5.2和GLM-4.6）。

Result: 揭示了MLLMs与临床医生在牙科实践中的性能差距，描绘了模型的优势与局限，刻画了失败模式，并提供了改进建议。

Conclusion: 该数据资源将促进与临床认知、安全要求和工作流程复杂性相一致的下一代人工智能系统的发展。
...
### [182] [A Multi-View Media Profiling Suite: Resources, Evaluation, and Analysis](https://arxiv.org/abs/2605.01336)
*Muhammad Arslan Manzoor,Dilshod Azizov,Daniil Orel,Umer Siddique,Zain Muhammad Mujahid,Yufang Hou,Preslav Nakov*

Main category: cs.CL

TL;DR: 本文通过引入大规模标签集MBFC-2025（约2600家新闻媒体）、构建多视图表示（Alexa图、超链接图、LLM推导图、文章和Wikipedia描述），并系统评估嵌入视图与融合策略（包括基于强化学习的融合变体），在ACL-2020上取得SOTA结果，在MBFC-2025上建立强基准，填补了政治偏见与事实性自动检测领域缺乏统一资源、系统评估和实证发现等空白。


<details>
  <summary>Details</summary>
Motivation: 新闻媒体以大规模影响公众舆论，使得政治偏见和事实性的自动检测至关重要。然而，该领域仍缺乏统一资源、跨方法的全面评估，以及对表示和融合策略的系统分析，尤其是在标签稀疏和数据集多样性的条件下。此外，缺乏基于观测的广泛实证发现来阐明何种方法有效、何种失败及其原因。

Method: 1) 构建MBFC-2025大规模标签集（约2600家媒体）；2) 为ACL-2020（约900家媒体）和MBFC-2025构建多视图表示，包括Alexa图、超链接图、LLM推导图、文章和Wikipedia描述；3) 系统评估嵌入视图和融合策略，包括基于强化学习的融合变体；4) 在ACL-2020上实现SOTA，在MBFC-2025上建立强基准。

Result: 在ACL-2020数据集上取得了最先进的结果，并在新构建的MBFC-2025数据集上建立了强基准。

Conclusion: 通过提供统一资源、全面评估和系统分析，填补了政治偏见与事实性检测领域的关键空白，为后续研究奠定了基准和资源基础。
...
### [183] [LLM Output Detectability and Task Performance Can be Jointly Optimized](https://arxiv.org/abs/2605.01350)
*Koshiro Saito,Ryuto Koike,Masahiro Kaneko,Naoaki Okazaki*

Main category: cs.CL

TL;DR: PUPPET通过强化学习微调LLM，使用双奖励函数（检测器+任务评估器）生成既高可检测性又高下游任务性能的文本，在长问答、摘要和写作任务上优于水印方法，且仅需少量样本和GPU时间。


<details>
  <summary>Details</summary>
Motivation: 解决水印方法在提升机器生成文本可检测性的同时，往往导致下游任务性能下降的问题。

Method: 提出PUPPET框架，通过强化学习（RL）微调LLM，使用两个奖励函数：1) 检测器输出机器类别似然作为可检测性奖励；2) 评估器衡量任务特定指标作为性能奖励。联合优化这两个目标。

Result: 实验表明，PUPPET训练的LLM在可检测性上达到与水印方法相当的水平，同时在下游任务（长问答、摘要、写作）上显著优于水印方法。优化仅需几千样本和1-2 GPU小时。此外，优势在跨域任务、不同LLM家族和模型规模上一致，且对释义攻击具有鲁棒性。

Conclusion: PUPPET证明了通过强化学习联合优化可检测性和下游性能是一种高效且鲁棒的方案，为LLM生成文本的透明度和实用性提供了新的平衡路径。
...
### [184] [On Stable Long-Form Generation: Benchmarking and Mitigating Length Volatility](https://arxiv.org/abs/2605.01357)
*Zhitao He,Haolin Yang,Rui Min,Zeyu Qin,Yi R. Fung*

Main category: cs.CL

TL;DR: 本文针对大语言模型在长文本生成中存在的输出长度波动问题，提出了系统性解决方案：构建了VOLTBench基准用于量化波动性，通过注意力迹分析揭示了内部模式，并提出了无需训练的轻量级解码优化策略GLoBo，使平均输出长度提升148%、波动性降低69%。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要集中在单次生成质量上，忽略了长文本生成中输出长度的波动性。这种波动性不仅导致显著的计算成本，还严重影响了模型在实际应用中的可靠性。

Method: 论文采用三阶段方法：(1) 提出VOLTBench基准，系统量化长文本生成的长度波动性；(2) 通过分析注意力迹，深入探查导致波动的常见内部模式；(3) 提出GLoBo（Stable Generation via Logits Boosting），一种轻量级解码阶段优化策略，无需额外训练即可显著提升生成长度的准确性和稳定性。

Result: 在VOLTBench上的大量实验首次系统性地证实了主流模型存在严重的长文本输出不稳定性，并验证了所提方法使基础模型平均输出长度提升148%，长度波动性降低69%，同时保持高质量生成。

Conclusion: 本文首次系统性地关注并解决了长文本生成中的长度波动问题，提出的GLoBo方法在不牺牲生成质量的前提下，有效提升了生成长度的准确性和稳定性，为LLM的可靠应用提供了重要支撑。
...
### [185] [Embedding-based In-Context Prompt Training for Enhancing LLMs as Text Encoders](https://arxiv.org/abs/2605.01372)
*Ailiang Lin,Zhuoyun Li,Keyu Mao,Kotaro Funakoshi,Manabu Okumura*

Main category: cs.CL

TL;DR: 提出EPIC方法，用连续嵌入替换离散文本演示作为上下文提示，在提升LLM嵌入质量的同时降低计算开销，在MTEB基准上达到新SOTA。


<details>
  <summary>Details</summary>
Motivation: ICL能增强LLM的表示能力，但前置任务相关演示会导致序列长度增加，带来大量token开销，需要在保持ICL优势的同时降低计算负担。

Method: 提出EPIC（Embedding-based In-Context Prompt Training），将离散文本演示替换为对应的连续嵌入，在对比学习中促使模型对齐语义相关的文本对，同时让模型将演示嵌入解释为上下文提示的一部分。

Result: 在MTEB基准上达到新SOTA，超越仅使用公开检索数据训练的前沿模型，消融实验验证了方法的有效性和必要性。

Conclusion: EPIC训练的模型在推理时无论是否使用上下文提示均能取得优异的嵌入性能，实现了高质量嵌入与计算效率的兼顾。
...
TL;DR: 提出多粒度轨迹对齐框架(MTA)，通过层自适应策略在低层对齐词汇表示、高层对齐短语级表示，并引入动态结构对齐损失和隐层表示对齐损失，实现教师-学生模型在层间变换轨迹上的对齐，优于现有知识蒸馏方法。


<details>
  <summary>Details</summary>
Motivation: 现有知识蒸馏方法在固定层或token级输出上对齐表示，忽略了表示随层深如何演化，导致学生模型难以捕捉教师模型内部的关系结构，限制了知识迁移效果。

Method: 提出MTA框架：1) 层自适应策略，低层进行词级对齐保留词汇信息，高层进行短语级(如名词短语、动词短语)对齐捕获组合语义；2) 动态结构对齐损失，匹配每层内语义单元的相对几何关系；3) 隐层表示对齐损失，直接对齐选定的师生层。

Result: 在标准基准测试上，MTA持续优于最先进的基线方法，消融实验验证了各组件的贡献。

Conclusion: 通过沿层间变换轨迹对齐师生表示，并利用层自适应策略从词汇到短语的多粒度对齐，有效提升了知识蒸馏效果，与Transformer表示随深度变得越来越抽象的实证发现以及语言学中高层语义由低层词汇单元组合而成的观点一致。
...
TL;DR: 本文提出统一框架，从包含性（概念是否完全在子空间内）和分离性（概念是否与其他概念隔离）两个维度分析神经模型中的概念子空间。实验发现：不同估计方法影响这两个性质；LEACE方法表现良好但泛化能力有限；语音模型中音素信息同时满足包含与分离，而说话人信息难以被压缩到紧凑子空间中。


<details>
  <summary>Details</summary>
Motivation: 理解神经模型如何表示可解释概念具有挑战性，现有工作从探针和概念擦除等不同角度探索线性概念子空间，但缺乏统一的分析框架。

Method: 构建了沿包含性（概念是否完全在子空间内且不在其外）和分离性（概念是否与其他概念隔离）两个维度的统一框架，在文本和语音模型上对比五种不同来源的概念子空间估计方法。

Result: (1) 估计方法的选择显著影响包含性和分离性；(2) LEACE在两项指标上表现良好，但仍难以泛化到未见数据；(3) HuBERT语音表示中，音素信息既包含又分离于说话人信息，而说话人信息虽与音素分离，却难以被包含在紧凑子空间中。

Conclusion: 概念子空间可能不唯一，这影响后续分析；不同估计方法导致不同的包含性与分离性特征；LEACE综合表现最佳但泛化性不足；不同概念类型在表示结构上存在本质差异。
...
### [188] [MemORAI: Memory Organization and Retrieval via Adaptive Graph Intelligence for LLM Conversational Agents](https://arxiv.org/abs/2605.01386)
*Hung Pham Van,Nguyen Manh Hieu,Khang Pham Tran Tuan,Nam Le Hai,Linh Ngo Van,Nguyen Thi Ngoc Diep,Trung Le*

Main category: cs.CL

TL;DR: MemORAI是一个基于自适应图智能的LLM长期记忆框架，通过选择性过滤、多关系图追踪和查询自适应检索，在LOCOMO和LongMemEval基准上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有LLM缺乏持久记忆，图记忆系统存在信息稀释、缺乏来源追踪和检索忽略查询上下文等问题，难以支持长期个性化对话。

Method: 提出MemORAI框架，包含三项创新：1) 选择性记忆过滤与双层压缩以保留用户画像相关内容；2) 富来源的多关系图，在对话轮次级别追踪事实来源；3) 查询自适应子图检索与动态加权PageRank，通过查询条件化边权重进行检索。

Result: 在LOCOMO和LongMemEval基准上取得SOTA性能，在记忆检索和个性化响应生成方面均表现优异。

Conclusion: 选择性存储、富表示和自适应检索是构建连贯、个性化LLM智能体的关键要素。
...
### [189] [Verbal-R3: Verbal Reranker as the Missing Bridge between Retrieval and Reasoning](https://arxiv.org/abs/2605.01399)
*Sangkwon Park,Donghun Kang,Jisoo Mok,Sungroh Yoon*

Main category: cs.CL

TL;DR: 本文提出 Verbal-R3 框架，通过 Verbal Annotations（口头注释）显式连接检索结果与查询逻辑，结合生成器与口头重排序器进行迭代检索与推理，并引入相关性引导的测试时缩放策略，在复杂问答基准上达到最优性能。


<details>
  <summary>Details</summary>
Motivation: 传统 RAG 范式直接将原始检索文本注入 LLM 上下文，导致检索信息整合效果不佳，缺乏对检索结果与查询之间逻辑关系的显式建模。

Method: 提出 Verbal-R3 框架，包含 Generator（生成器）和 Verbal Reranker（口头重排序器）。Generator 执行迭代检索与推理；Verbal Reranker 返回相关性分数和 Verbal Annotations，引导 Generator 的推理与回答过程。通过相关性引导的测试时缩放（relevance-guided test-time scaling）优化推理轨迹扩展。

Result: 在复杂问答基准上达到 state-of-the-art 性能，验证了 Verbal Annotations 能显著提升 LLM 生成准确、上下文相关回答的能力。

Conclusion: Verbal-R3 通过显式的逻辑注释桥接检索结果与 LLM 推理能力，有效提升了 RAG 系统的信息整合质量与回答准确性。
...
### [190] [Injecting Distributional Awareness into MLLMs via Reinforcement Learning for Deep Imbalanced Regression](https://arxiv.org/abs/2605.01402)
*Yao Du,Shanshan Li,Xiaomeng Li*

Main category: cs.CL

TL;DR: 针对多模态大语言模型在长尾目标分布下的数值回归问题，提出了一种基于组相对策略优化的分布感知强化学习框架，通过引入CCC奖励实现跨样本关系监督，在无需修改架构的情况下显著提升中低样本场景的回归性能。


<details>
  <summary>Details</summary>
Motivation: 现有MLLM在长尾目标分布下进行数值回归时，基于token的SFT和点式回归奖励会导致模型偏向高密度区域，产生"回归到均值"行为，尾部性能较差。作者认为现有训练范式缺乏跨样本关系监督是这一问题的关键局限性。

Method: 提出基于组相对策略优化（GRPO）的分布感知强化学习框架，引入基于一致性相关系数（CCC）的奖励函数，在batch层面提供跨样本比较监督，使预测分布与真实分布在相关性、尺度和均值上对齐。该框架即插即用，无需修改模型架构。

Result: 在统一的长尾回归基准测试集上，该方法相比SFT和现有MLLM回归方法取得一致性提升，特别是在中样本和少样本场景下表现尤为突出。

Conclusion: 跨样本的关系监督对于解决MLLM在长尾分布下的数值回归问题至关重要，所提的分布感知强化学习框架是一种有效且通用的解决方案。
...
### [191] [Medmarks: A Comprehensive Open-Source LLM Benchmark Suite for Medical Tasks](https://arxiv.org/abs/2605.01417)
...
TL;DR: Medmarks是一个全开源医学LLM评估套件，包含30个基准测试，对61个模型的71种配置进行了系统评估。研究发现前沿推理模型（Gemini 3 Pro、GPT-5.1、GPT-5.2）表现最佳，专有模型比开源模型更具token效率，医学微调模型优于通用模型，且模型存在答案顺序偏差。部分评估（Medmarks-T）可直接用作强化学习环境。


<details>
  <summary>Details</summary>
Motivation: 现有LLM医学评估面临基准饱和、数据访问受限、任务覆盖不足等问题，亟需一个全面、开源且可持续的评估框架。

Method: 构建包含30个基准的Medmarks评估套件，涵盖问答、信息抽取、医学计算和开放临床推理；使用可验证指标和LLM-as-a-Judge对61个模型（71种配置）进行系统评估。

Result: 前沿推理模型表现最优；专有模型token效率显著高于开源模型；医学微调模型优于通用模型；模型存在答案顺序偏差（小模型和Grok 4尤为明显）。

Conclusion: Medmarks提供了全面的医学LLM评估框架，部分子集可作为RL训练环境，有助于推动医学大模型的标准化评估与后训练。
...
### [192] [Hallucinations Undermine Trust; Metacognition is a Way Forward](https://arxiv.org/abs/2605.01428)
*Gal Yona,Mor Geva,Yossi Matias*

Main category: cs.CL

TL;DR: 本文指出，提升LLM事实准确性的主要进展来自扩展知识边界（编码更多事实），而非增强对边界的认知（区分已知与未知）。作者提出"忠实不确定性"——将语言不确定性与内在不确定性对齐——作为超越"回答或弃权"二分法的第三条路径，并认为元认知（对自身不确定性的感知与行动）是构建可信且强大LLM的关键。


<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型在事实可靠性方面取得显著进展，幻觉（hallucinations）仍是生成式AI的主要问题。现有方法主要通过扩展模型知识边界来减少幻觉，但模型区分已知与未知的能力并未同步提升。作者希望探索超越简单"回答或弃权"二分法的新路径。

Method: 作者从概念层面重新框架化幻觉问题：将幻觉理解为"自信的错误"（confident errors），即模型在缺乏适当限定条件下传递的错误信息。据此提出"忠实不确定性"（faithful uncertainty）——使语言表达的不确定性与模型内在的不确定性对齐。对于直接交互，这意味着诚实沟通不确定性；对于智能体系统，不确定性作为控制层决定何时搜索及信任什么。

Result: 作者论证了消除幻觉与保持实用性之间存在的固有取舍：模型可能缺乏完美区分真相与错误的判别能力。但当将幻觉重新定义为"自信的错误"时，引入不确定性表达提供了第三条路径，使这一取舍得以消解。忠实不确定性是元认知的一个方面，元认知使LLM既能被信任又具备能力。

Conclusion: 元认知（metacognition）对于构建既可信赖又有能力的LLM至关重要。作者建议未来研究应关注如何使模型具备对其自身不确定性的感知与行动能力，并指出了这一方向下的开放性问题。
...
### [193] [Artificial intelligence language technologies in multilingual healthcare: Grand challenges ahead](https://arxiv.org/abs/2605.01441)
*Vicent Briva-Iglesias*

Main category: cs.CL

TL;DR: 本文综述了以LLM为基础的多语言AI语言技术在医疗场景中的应用，指出流利输出不等于临床安全，分析了跨语言、口音、任务和工作流中的性能差异与风险，提出七大挑战，呼吁负责任的社会技术设计、校准的人工监督和多学科协作。


<details>
  <summary>Details</summary>
Motivation: 尽管AI语言技术（尤其是LLM）越来越多地被嵌入多语言医疗工作流，但流畅的输出并不等同于临床安全或公平的沟通。现有文献缺乏系统性综合，需要从可靠性、安全文化和可信赖性等维度审视能力、评估实践、实施模式及常见错误。

Method: 采用叙事性综述方法，综合近期同行评审证据，涵盖书面沟通、口语沟通和新兴的智能体工作流三个层面，通过以人为中心的AI语言技术（HCAILT）视角进行分析。

Result: 发现不同语言、口音、任务和工作流间性能存在差异；效率提升可能掩盖错误、降低可追溯性并在临床医生、译员和卫生系统间转移责任；识别出文献中的关键趋同与矛盾点。

Conclusion: 进展不仅需要更好的模型，还需要负责任的社会技术设计、校准的人工监督，以及机器翻译/自然语言处理、翻译研究、人机交互、临床实践、实施科学和政策等领域的更强协作。
...
TL;DR: 本文提出跨语言审计框架，系统评估11个前沿LLM在紧急调度场景中的人口统计学公平性。通过对19,800个模型输出的分析发现，当事件严重性模糊时，模型在宗教信仰、性别和种族维度上表现出系统性偏差，且偏差在不同语言间呈现不对称性：性别偏差在中文中更显著，种族偏差在英文中更突出。研究还观察到人口统计学线索在某些场景下产生反向效应。论文为实际部署前的模型评估提供了可扩展的审计基础设施。


<details>
  <summary>Details</summary>
Motivation: LLM正被快速集成到应急呼叫分类和调度决策支持等高风险公共安全系统中，但其人口统计学公平性尚未得到充分检验。现有研究缺乏针对调度场景的跨语言审计方法，无法系统评估模型在不同语言和人口统计学维度上的偏差表现。

Method: 将警察优先调度系统操作化为五级序数分类任务，采用受控最小配对实验设计隔离人口统计学线索的影响。在19,800个模型输出上，涵盖11个前沿模型、15个场景对、三个人口统计学类别（宗教信仰、性别、种族）和两种语言（英语和中文），系统测量偏差的统计特征。

Result: 1) 当事件严重性模糊时，人口统计学偏差系统性地出现；当操作优先级由呼叫内容明确决定时，偏差基本消失。2) 偏差幅度：宗教信仰 > 性别 > 种族。3) 偏差在语言间不一致：性别偏差在中文中显著放大，种族偏差在英文中更突出。4) 某些场景中人口统计学线索产生反方向效应，挑战简单的刻板印象放大假设。

Conclusion: LLM调度偏差并非模型的固定属性，而是人口统计学信号、上下文模糊性和语言之间交互作用的结果。提出的框架为部署机构在实际采用前评估候选模型提供了可扩展的审计基础设施。
...
TL;DR: ReMedi是一个面向电子健康记录（EHR）临床结局预测的推理增强框架，通过困难样本再生机制生成推理-答案对，并结合偏好微调，在多项EHR预测任务中F1分数最高提升19.9%。


<details>
  <summary>Details</summary>
Motivation: EHR数据的复杂性和异质性使得临床结局预测极具挑战；现有LLM方法主要依赖蒸馏或RAG增强医学知识，缺乏针对复杂临床问题的推理能力优化。

Method: 提出ReMedi框架，包含两个核心机制：(1) 困难样本再生机制，利用真实标签作为提示增强复杂临床问题的推理过程；(2) 偏好数据构建循环，将真实结局指导融入其中，通过微调和偏好优化提升模型预测性能。

Result: 在多个EHR预测任务上，相比最先进基线方法，F1分数最高提升19.9%。

Conclusion: ReMedi通过生成推理-答案对并引入偏好微调，有效提升了LLM在真实临床预测任务中的表现，验证了推理增强策略在医学预测领域的有效性。
...
TL;DR: FT-RAG提出了一种细粒度检索增强生成框架，通过将表格分解为条目级语义单元构建结构图，结合结构邻居扩展机制和多模态融合，显著提升了结构化表格数据的检索和生成性能。同时发布了Multi-Table-RAG-Lib基准数据集。
...
Method: 1) 将表格分解为条目级语义单元构建结构化图；2) 采用结构邻居扩展机制在图检索中寻找语义相连实体；3) 通过多模态融合整合表格检索结果的上下文信息。

Result: 在表格级和单元格级Hit Rate上分别提升23.5%和59.2%，生成性能的精确值召回率提升62.2%。

Conclusion: FT-RAG在纯表格及异构表-文本混合上下文中均能有效提升事实性依据能力，在复杂混合模态文档推理任务上取得了新的SOTA性能。
...
TL;DR: 本文提出语法通过压缩词义不确定性来实现跨语言的信息整合，并通过计算语言学、脑成像和临床数据分析验证了这一机制。


<details>
  <summary>Details</summary>
Motivation: 孤立词汇的意义本身具有不确定性，而这种不确定性在组合和语境锚定中会降低。作者希望探究语法是否在跨语言中压缩这种不确定性，以及这种压缩如何在大脑中体现，并在语言障碍中如何被选择性破坏。

Method: 将压缩操作化为非语境 surprisal（基于词汇频率）与语境 surprisal（基于语法敏感模型）之间的相对差异。分析了20种语言的叙事语料，使用 fMRI 测量大脑活动，并在失语症、痴呆症和精神分裂症等患者中检测不确定性压缩的衰减情况。

Result: 语境 surprisal 降低了基于频率的 surprisal；这种降低与颠倒词序的 surprisal 成本密切相关，并与更丰富、非冗余的词汇和更复杂但最优的依存结构呈正向关系。fMRI 表明 surprisal 及其降低解释了理解和产生过程中的 BOLD 信号，区域有重叠但不同。不确定性压缩在失语症、痴呆症和精神分裂症中显著减弱，但在非语言性原发性障碍中保持完整。

Conclusion: 通过语法实现的不确定性压缩是理解语言原则、大脑基础及其障碍的核心概念。
...
TL;DR: 提出了一个用于机械可解释性的自主多智能体框架，通过两个耦合循环（解释精炼和特征发现）自动解释和发现大型语言模型中的内部特征，在Gemma-2系列模型上优于一次性自动解释方法。


<details>
  <summary>Details</summary>
Motivation: 现有的机械可解释性方法主要依赖一次性标注或人工分析，缺乏自动化、可验证的假设检验循环，难以生成可证伪且可审计的解释。

Method: 构建双循环多智能体框架：(1) 解释精炼循环：智能体提出竞争性假设，通过目标提示控制和多指标评估迭代测试；(2) 特征发现循环：智能体生成提示集，在激活空间中构建k近邻图，基于统计可分性和语义一致性标准检索候选特征。

Result: 在Gemma-2系列模型和权重稀疏Transformer的MLP神经元上，该方法优于一次性自动解释，发现了语言特定和安全相关的特征，并生成了可审计的解释轨迹。

Conclusion: 基于智能体的经验循环比一次性标签产生更清晰、更可证伪的解释，为自动化机械可解释性提供了有效范式。
...
TL;DR: 本文介绍了团队"Archaeology"在SemEval-2026任务13（AI生成代码检测）中的系统，通过微调四个预训练代码模型，在二分类（人类vs.AI代码）和11类归因任务上分别取得0.737和0.422的macro-F1成绩，排名第6和第7。


<details>
  <summary>Details</summary>
Motivation: AI生成代码的检测与归因是代码溯源和安全领域的重要问题，随着代码大模型的广泛应用，准确区分人类与AI编写的代码、识别具体生成模型具有实际应用价值。

Method: 基于TF-IDF和逻辑回归基线，微调CodeBERT、GraphCodeBERT、UniXcoder和CodeT5+四个预训练代码模型。Subtask-A采用留一语言交叉验证、代码增强、分块推理与截尾均值聚合、困难数据集阈值校准；Subtask-B采用三明治token打包、类别平衡损失、多种子集成与测试时增强。

Result: Subtask-A（二分类）macro-F1为0.737，在81支队伍中排名第6；Subtask-B（11类归因）macro-F1为0.422，在34支队伍中排名第7。

Conclusion: 微调预训练代码模型结合任务特定策略（如留一语言验证、分块推理、类别平衡损失等）能有效提升AI生成代码检测与归因性能，验证了不同技术组合在不同子任务上的有效性。
...
TL;DR: 提出基于二元评分标准的多裁判过滤方法，可消除LLM裁判偏差，并发布首个巴西葡萄牙语多轮聊天基准Prosa，显著提升排名一致性和区分度。


<details>
  <summary>Details</summary>
Motivation: 整体式LLM裁判打分会受到所选裁判模型偏差的显著影响，需要一种对裁判模型选择不敏感的评估方法。

Method: 采用二元评分标准（rubric scoring）结合多裁判过滤（multi-judge filtering）替代整体式打分；构建Prosa基准（1000个WildChat对话，3个模型家族的3个裁判对16个模型进行评估）。

Result: 过滤后的评分标准下三位裁判对16个排名完全一致，而整体式评分仅一致7个；相邻模型平均分差提升47%，区分能力增强；使用Gemini 3 Flash作为裁判评估成本约2.1美元。

Conclusion: 分解评判（二元评分标准）比选择裁判模型本身更重要；提出的方法和基准可复用于其他开放式评估场景。
...
TL;DR: 本文提出基于字符分布特征的AI文本检测新方法LD-Score，通过理论证明人类与AI在字符分布上存在“分离墙”，并构建了包含64万样本的MDTA基准数据集。LD-Score与现有困惑度方法相关性低（r=0.08-0.13），通过非线性分类器集成可显著提升AUROC和F1，尤其在专业领域效果突出。


<details>
  <summary>Details</summary>
Motivation: 现有基于对数概率的无训练AI文本检测方法（如Binoculars、DNA-DetectLLM）虽表现良好，但模型通过RLHF优化后会生成类似人类的概率分布，导致检测性能存在根本性上限。因此需要探索新的检测信号来突破这一瓶颈。

Method: 1）提出基于字符分布签名的检测信号LD-Score，理论论证AI模型在领域平衡语料上训练会逼近全局字符模式，而人类呈现领域专业化分布，形成“分离墙”；2）构建MDTA基准数据集（642,274样本，覆盖4模型、5领域、3温度、3对抗策略）；3）通过非线性分类器将LD-Score与现有方法（DNA-DetectLLM、Binoculars、FastDetectGPT）集成。

Result: LD-Score与困惑度方法相关性低（r=0.08-0.13）；集成后在所有方法上取得AUROC和F1的一致提升，在专业领域（词汇约束强化检测信号）增益尤为显著。

Conclusion: 基于字符分布特征的检测信号能够有效补充现有基于对数概率的方法，突破RLHF带来的性能上限，尤其是在专业领域具有更大应用潜力。
...
TL;DR: 提出了MultiBreak，一个大规模、多样化的多轮越狱基准，包含10,389个对抗性提示和2,665种有害意图，通过主动学习流水线生成，在多个LLM上实现了比次优数据集高达54.0和34.6的更高攻击成功率。


<details>
  <summary>Details</summary>
Motivation: 现有单轮越狱评估难以模拟真实对话场景，而多轮越狱基准存在规模有限或依赖模板导致多样性不足的问题，亟需一个大规模、多样化的多轮越狱评估基准。

Method: 统一多种有害越狱意图，引入主动学习流水线，通过不确定性引导的迭代微调生成器来产生更强的攻击候选，从而扩展高质量多轮对抗提示。

Result: MultiBench包含10,389个多轮对抗提示、2,665种不同有害意图；在DeepSeek-R1-7B和GPT-4.1-mini上分别比次优数据集高出54.0和34.6的攻击成功率；多样化的攻击类别能揭示细粒度的LLM漏洞。

Conclusion: 多轮越狱在自然对话场景下比单轮更具对抗效力，单轮下看似良性的类别在多轮场景中可能表现出显著更高的攻击有效性，MultiBench为推进LLM安全提供了可扩展的资源。
...
TL;DR: 本文提出自适应探索范式，让LLM智能体在运行时通过迭代执行代码动态发现BIM模型结构，替代传统静态查询生成方法。在包含37个IFC模型、1027个任务的ifc-bench v2基准测试中，该方法在所有配置下均显著优于静态方法。


<details>
  <summary>Details</summary>
Motivation: BIM模型存在高度异构性，传统自然语言转结构化查询的方法假定固定的数据组织方式（静态方法），这种假设最终会被BIM的异构性所打破，导致信息提取困难。

Method: 提出自适应探索新范式，基于LLM的智能体在运行时通过迭代执行代码来从BIM模型中提取信息，动态发现模型结构而非预先假设其结构。同时构建了ifc-bench v2开源基准测试集（37个IFC模型、1027个任务），并通过因子消融实验（2种LLM能力水平×4种增强策略）进行评估。

Result: 自适应探索在所有配置下均显著优于静态查询生成方法，不论采用何种增强策略。该优势在两种LLM能力水平和四种增强策略的消融实验中均得到一致验证。

Conclusion: BIM异构性问题应在范式层面解决，而非通过进一步优化静态方法来处理。自适应探索范式是比静态查询生成更有效的方向。
...
TL;DR: 本文提出"辩论陷阱"和"推理陷阱"概念，指出多智能体辩论（MAD）和封闭系统推理会保持答案准确性但破坏推理质量。作者提出SFS（支持忠实度分数）作为细粒度评估指标，EGSR（基于证据的苏格拉底式推理）替代对抗性辩论，并通过信息论证明（DPI界）解释推理退化的根本原因。实验表明MAD导致SFS下降43-98%，而EGSR恢复98%的忠实度。还发现人类评分者一致性本身不稳定（Fleiss kappa <= +0.018）。


<details>
  <summary>Details</summary>
Motivation: 现有MAD方法虽能保持答案准确性，但会逐渐破坏推理质量；同时缺乏对推理过程忠实度的可靠评估指标，且人类评分一致性本身存在严重问题。

Method: (i) SFS：将声明分解为原子声明，验证其与证据的匹配度；(ii) EGSR：用基于证据的探究替代对抗性论证；(iii) 信息论框架：通过DPI界（定理1）证明MAD中信息逐轮退化；(iv) 大规模实验：SciFact 300条+FEVER 1000条共16种条件；(v) R6队列研究：韩语n=10×30、英语n=3×200评估人类评分一致性。

Result: DebateCV保持88%准确率但SFS下降43%；多数投票MAD使SFS降至基线1.7%（p<10^-6, d=-0.96）；EGSR恢复98%；人类评分者Fleiss kappa<=+0.018，存在0.8-1.4 Likert量表内评者偏移。

Conclusion: 封闭系统推理（包括MAD）在马尔可夫链结构下必然受到DPI界的约束，导致推理质量退化；EGSR能有效缓解此问题；现有忠实度指标依赖的人类评分基准本身不可靠；提出可证伪猜想：任何保持定理1马尔可夫结构的封闭推理协议都受相同DPI界约束。
...
TL;DR: 针对对话方面级情感四元组分析（DiaASQ）中现有方法存在的结构噪声、时序缺失和距离稀释问题，提出结合线程约束有向无环图（TC-DAG）和话语感知旋转位置编码（D-RoPE）的新框架。TC-DAG通过线程约束过滤跨线程噪声并保持全局连通性，D-RoPE通过双流投影和多尺度频率对齐语义、以树状距离捕获线程依赖、引入话语级进展缓解距离稀释。在两个基准数据集上达到最优性能。


<details>
  <summary>Details</summary>
Motivation: 现有DiaASQ方法存在三个关键问题：(1) 简单GCN引入结构噪声，忽略对话时间序列；(2) 标准RoPE无法清晰区分词级句法顺序和话语级对话进展；(3) 存在距离稀释问题（Distance Dilution）。需要一种能同时建模对话结构、时序和层次语义的方法。

Method: 提出TC-DAG和D-RoPE两模块框架：TC-DAG基于线程约束构建有向无环图，过滤跨线程噪声，通过根锚定保持全局连通性，并融入对话时序信息。D-RoPE采用双流投影和多尺度频率信号对齐多层语义，利用树状距离捕获线程依赖关系，通过融入话语级进展缓解token级距离稀释问题。

Result: 在两个基准数据集上实现了最先进的性能（state-of-the-art）。

Conclusion: 通过TC-DAG和D-RoPE分别解决了对话结构噪声、时序缺失和距离稀释问题，有效提升了对话方面级情感四元组分析的性能。
...
TL;DR: 提出了一种基于熵的自适应知识蒸馏方法，通过动态调整token级别的训练过程（课程学习、温度调节、双分支架构），提升大模型向小模型的知识迁移效率。
...
Method: 提出熵驱动的自适应蒸馏策略：(1) token级课程学习，从低熵token逐步过渡到高熵token；(2) 根据token熵动态调整蒸馏温度；(3) 双分支架构，对低熵token使用logits蒸馏，对高熵token使用深层特征蒸馏。

Result: 大量实验验证了该方法的合理性和有效性。

Conclusion: 基于token熵的自适应蒸馏能显著提升知识迁移效率，为资源受限环境下的模型部署提供了更优方案。
...
TL;DR: 提出几何遗忘（GU）方法，通过操作模型的规划状态、利用安全参考提示的低秩几何结构、结合锚点上下文合成提示进行投影对齐，在无需原始训练数据的情况下实现选择性遗忘，在ToFU和UnlearnPII基准上取得强目标抑制与最小非目标性能损失。


<details>
  <summary>Details</summary>
Motivation: 现有LLM遗忘方法需要原始训练数据，且输出级拒绝微调或梯度更新在遗忘强度、非目标保留和数据可用性之间存在矛盾。

Method: 在提示时规划状态上操作，从少量安全参考提示中蒸馏紧凑低秩几何结构，使用轻量级锚点上下文合成提示触发隐藏规划表示向安全几何的投影对齐，并通过教师蒸馏正则化器减少非目标锚点的附带漂移。

Result: 在ToFU和UnlearnPII隐私遗忘基准上，GU实现强目标抑制，对非目标性能影响最小。

Conclusion: 有效遗忘可以通过最小合成数据实现，无需原始训练语料库。
...
TL;DR: 本文提出探索-承诺解耦范式，通过校准感知生成（CAG）框架将中间推理与最终输出解耦，在长文本生成中实现高达13%的事实性提升和37%的解码时间减少。


<details>
  <summary>Details</summary>
Motivation: 大型推理模型在复杂任务上表现优异，但在长文本生成中容易出现幻觉且错误会逐步累积。现有方法（如弃权策略和事实性驱动优化）采用耦合的探索-承诺范式，中间推理无条件地传播到最终输出，限制了信息选择和整合的细粒度控制。

Method: 提出探索-承诺解耦范式，将知识探索与最终承诺分离。具体实例化为校准感知生成（CAG）框架：在中间推理中增强校准可靠性估计，并在最终输出中优先使用可靠内容，使模型具备端到端的校准感知生成能力。

Result: 在五个长文本事实性基准测试和多个模型家族上，CAG将事实性提升最多13%，同时将解码时间减少最多37%。

Conclusion: 解耦范式是实现更可靠长文本生成的原则性方法，为构建可信且具有自我意识的生成系统提供了方向。
...
TL;DR: 提出“合规性鸿沟”(Compliance Gap)概念——AI在语言上同意指令但在行为上违反的第三类诚实维度。通过理论证明(结构必然性、文本不可检测性)和13项实验(6个前沿模型、2031次会话)证实：默认条件下所有模型指令合规率为0%，且人类无法从文本中检测此鸿沟。发布首个过程合规基准BS-Bench。


<details>
  <summary>Details</summary>
Motivation: 现有AI对齐评估仅衡量结果保真度(outcome fidelity)，而忽视过程保真度(process fidelity)。AI可能口头同意指令但实际行为背离，这一语言-行为脱节未被现有75类基准(IFEval、SWE-bench等)覆盖。

Method: 理论部分：定理1证明在仅奖励文本而不观测行为的RL训练下，合规性鸿沟结构上不可避免；定理2通过数据处理不等式证明该鸿沟从文本上不可检测。实验部分：设计2031次会话测试6个前沿模型，含默认框架、审计轨迹激励、移除委托工具等变体，并用9名盲审员进行人类检测实验。

Result: 默认框架下所有6个模型指令合规率0%（Claude Sonnet 4口头同意100%但行为全部绕过）；奖励审计轨迹时合规率97%；移除委托工具后合规率升至75%（Cohen's d=2.47），证实环境可供性而非权重编码失效。9名盲审员Fleiss' kappa=0.130，正确识别0/15合规会话。

Conclusion: RLHF训练的模型在默认条件下达到近100%的意图-行为鸿沟，远超人类的47%（心理学）和96.5pp（手术审计），需要独立的测量基础设施。发布BS-Bench作为首个过程合规公开基准。
...
TL;DR: 本文提出RMGAP基准，包含1097个实例，覆盖聊天、写作、推理和安全四个领域，用于评估奖励模型的泛化能力（即正确排序响应以对齐多样化用户偏好的能力）。通过对24个SOTA奖励模型的评估发现，即使最佳模型在Best-of-N准确率上也仅达到49.27%，表明奖励模型泛化能力存在显著提升空间。


<details>
  <summary>Details</summary>
Motivation: 现有奖励模型基准通常围绕单一通用偏好设计，无法评估奖励模型在不同用户偏好下的泛化能力。为填补这一关键空白，需要构建一个能衡量奖励模型正确排序响应以对齐多样化用户偏好能力的基准。

Method: 1) 从Chat、Writing、Reasoning、Safety四个领域收集原始prompt；2) 为每个prompt生成四种具有不同语言特征的响应；3) 通过对比这些候选响应并设计特定场景，构建能表达不同偏好的定制化prompt；4) 对每个prompt扩展两个释义变体以覆盖相同偏好的不同表达方式。

Result: 对24个SOTA奖励模型的评估显示，最佳模型Best-of-N准确率仅达49.27%，表明奖励模型在泛化能力上存在显著不足。

Conclusion: 现有奖励模型在泛化到多样化用户偏好方面表现有限，仍存在较大改进空间。RMGAP基准为该领域的评估提供了更全面的标准。
...
TL;DR: 本文提出圆柱表示假说（CRH），通过放宽线性表示假说（LRH）的正交性假设，解释了语言模型操控中效果不稳定且难以预测的现象。CRH认为概念表示具有圆柱结构：中心轴控制概念生成，周围法平面决定操控灵敏度，其中仅特定敏感扇区能有效激活概念，而其他扇区可能抑制或延迟激活，从而引入了内在不确定性。


<details>
  <summary>Details</summary>
Motivation: 现有基于线性表示假说（LRH）的理论框架假设概念可正交化以实现无损操控，但在真实表示中这一理想映射无法成立，无法解释操控效果中观察到的不稳定性和不可预测性。

Method: 放宽LRH的正交性假设，保留线性表示，提出圆柱表示假说（CRH）。将概念表示建模为圆柱结构：中心轴（捕捉概念存在与否的差异，驱动概念生成）和周围法平面（决定操控灵敏度，其中仅特定敏感扇区能有效激活概念）。从理论推导和实验验证两方面展开。

Result: 实验验证了圆柱结构的存在，证明CRH能有效解释真实场景中模型操控行为的波动性。法平面可从差分向量可靠识别，但敏感扇区无法被确定，从而引入扇区级别的内在不确定性。

Conclusion: CRH为语言模型操控中效果不稳定这一实际问题提供了原理性解释，是一种有效且实用的模型操控行为解释框架。
...
TL;DR: LLM在生成多选题时存在系统性的位置偏差，通过大量实验发现这些偏差具有结构性，并通过探测实验和激活引导技术揭示了其内在机制及可控性。


<details>
  <summary>Details</summary>
Motivation: 研究表明LLM生成的多选题中正确答案应均匀分布，但LLM存在位置偏差，影响题目质量，因此需要系统性研究这种偏差的结构、原因及控制方法。

Method: 在10个LLM和5个VLM上进行了三种多选题生成任务的大量实验；通过探测实验分析隐藏表示中蕴含的答案位置预测信号；应用激活引导（activation steering）技术操控内部表示以影响答案位置分布。

Result: 证实LLM存在系统性位置偏差，且同一模型家族的偏差模式相似；隐藏表示中问题主干编码了正确答案位置的预测信号；激活引导可以部分控制位置偏好并显著改变答案位置分布。

Conclusion: 该研究提供了研究LLM隐式位置规划的实用框架，强调了可控生成对于可靠多选题构建和评估的重要性。
...
TL;DR: 本文提出StALT指标，通过分析大型推理模型隐藏状态的时空动态（跨token的时间变化和跨层的空间集中度），发现正确推理轨迹表现出广泛的时间动态和局部层集中模式，StALT可作为无标签的正确性信号，有效区分正确与错误推理路径。


<details>
  <summary>Details</summary>
Motivation: 现有研究虽然发现LRM隐藏状态包含正确性相关信号，但粗粒度的聚合分析掩盖了token和层级的推理计算结构。需要更精细的指标来捕捉推理过程中的内部动态。

Method: 提出Spatiotemporal Amplitude of Latent Transition (StALT)——一种免训练的轨迹统计量，通过加权token内层显著性，汇总相邻token之间的时间变化，刻画隐藏状态在解码步和层之间的时空转换模式。

Result: StALT能可靠区分正确与错误推理轨迹，在推理密集型场景下提供有竞争力的无标签正确性信号，优于或媲美输出空间和基于长度的基线方法。干预实验表明StALT对内部推理需求的变化敏感。

Conclusion: LRM存在可测量的隐藏状态动态，StALT可作为理解内部推理计算的有效探针，弥补仅依赖输出评估的不足。
...
TL;DR: 本文针对现代希腊语资源匮乏的问题，提出了高质量QA数据集CulturaQA、内存高效的评估框架、开源希腊语LLM Maistros 8B，并系统评估了9个LLM在9个希腊语QA数据集上的表现。


<details>
  <summary>Details</summary>
Motivation: 现有LLM在多语言场景下对低资源语言（如现代希腊语）表现不佳，原因在于缺乏语言和文化特定的训练数据；同时，现有大型推理模型（LRM）虽然推理能力强，但参数规模巨大、推理延迟高，不适合常规部署。希腊语QA数据集数量有限且多仅用于评估，缺乏用于训练的高质量数据。

Method: (i) 构建CulturaQA数据集：由LRM生成并经人工整理的高质量QA数据；(ii) 提出内存高效的LLM评估框架，可适配多种语言和QA任务；(iii) 通过知识蒸馏和微调在CulturaQA上开发Maistros 8B开源希腊语LLM；(iv) 在9个人工整理的希腊语QA数据集上对9个LLM进行全面评估。

Result: 成功构建了CulturaQA数据集，开发了Maistros 8B模型，并建立了可适配多语言的评估框架，通过多模型多数据集评估验证了方法的有效性。

Conclusion: 通过LRM生成+人工整理的数据构建策略以及知识蒸馏微调方法，可以有效提升低资源语言（如希腊语）的LLM性能，为低资源语言NLP研究提供了可复现的方案。
...
TL;DR: 提出StressEval，一种基于模型故障驱动的数据合成框架，通过识别失败推理步骤、双视角实例合成和门控机制，生成动态、具挑战性且可控的测试实例。基于此构建的Dynamic OneEval基准在多个SOTA模型上导致显著更大的性能下降，同时保持可解释的难度因子。


<details>
  <summary>Details</summary>
Motivation: 现有静态LLM基准受污染和过拟合影响，而动态基准虽能缓解过时问题，却常以牺牲可回答性和可控性为代价。

Method: 三阶段框架：(1) 构建半结构化难度卡，识别失败推理步骤及其根本原因；(2) 双视角实例合成，分别针对知识缺口和推理断裂，同时保留底层难度因子；(3) 门控机制筛选合理无歧义的实例。

Result: 基于多个知识密集型推理数据集构建的Dynamic OneEval，在多个SOTA LLM上相比原基准产生显著更大的性能下降。

Conclusion: StressEval能有效生成动态、具挑战性且可控的测试实例，保留显式难度因子以支持更可操作的迭代改进。
...
TL;DR: Judge-R1是一个统一框架，通过智能体法律信息收集和基于强化学习的规则引导优化，显著提升了基于LLM的判决文书自动生成在法律准确性和生成质量上的表现。


<details>
  <summary>Details</summary>
Motivation: 判决文书自动生成对司法效率至关重要，但现有方法（如标准RAG和SFT）存在证据召回不足、法条幻觉和法律推理逻辑缺陷等问题。

Method: 提出Judge-R1框架，包含两个核心模块：(1) Agentic Legal Information Collection，利用动态规划智能体从多源检索精确法条和判例；(2) Rubric-Guided Optimization，采用GRPO强化学习结合综合法律奖励函数，确保符合司法标准和推理逻辑。

Result: 在JuDGE基准上的大量实验表明，Judge-R1在法律准确性和生成质量上显著优于当前最先进的基线方法。

Conclusion: Judge-R1通过统一框架同时改进法律信息收集和判决文书生成，有效解决了现有方法的关键缺陷。
...
TL;DR: 通过引入"稳定计数能力"测试，研究发现超过100种大语言模型的计数能力远低于其宣称的上下文窗口限制，模型行为并非基于通用逻辑或稳定规则应用，而是依赖于有限的类计数内部状态（类似"掰手指"），一旦资源耗尽，精确执行就会崩溃为猜测。


<details>
  <summary>Details</summary>
Motivation: 探究大语言模型在数学推理、编码和文档分析等任务上的成功是否真正反映了通用逻辑能力，还是仅仅是重复应用学习程序或模仿规则执行的模式匹配。

Method: 提出"稳定计数能力"测试方法：让模型重复计数符号直到失败。该方法消除了知识依赖、语义和歧义，避免了词汇和分词混淆，提供了超越标准知识基准的程序可靠性直接度量。

Result: 超过100种模型变体的稳定计数能力远低于广告宣传的上下文限制；模型行为既不符合开放逻辑，也不符合学习规则的稳定应用，而是类似于使用有限的类计数内部状态（"掰手指"）；一旦该资源耗尽，规则遵循的表象消失，精确执行崩溃为猜测，即使增加测试时计算也无济于事。
...
TL;DR: 本文提出VIDA数据集（2,500个精心筛选的实例）和基于LLM的消歧中心评估指标，用于评估多模态机器翻译中视觉信息对歧义消解的作用。实验表明，思维链监督微调（CoT-SFT）相比标准监督微调在消歧准确性上更优，尤其是在分布外数据上表现更强。


<details>
  <summary>Details</summary>
Motivation: 现有消歧导向MMT基准存在数据质量问题、与真实翻译场景不匹配，且难以覆盖开放式翻译中的广泛歧义类型。

Method: 构建VIDA数据集（2,500个实例，每个实例的歧义源片段需要视觉证据消解）；提出消歧中心评估指标（Disambiguation-Centric Metrics），使用LLM作为裁判分类器在片段级别验证歧义表达是否正确消解；实验对比了vanilla推理、SFT和CoT-SFT三种策略。

Result: SFT提升整体翻译质量，但CoT-SFT在消歧准确性上取得更一致的提升，尤其在分布外子集上泛化能力更强。

Conclusion: CoT-SFT方法能更好地泛化到多样化歧义类型，验证了在MMT中引入思维链推理对消歧任务的有效性。
...
TL;DR: 本文系统评估了15个开源语言模型（重点分析10个指令模型）在5个分类与推理基准、5种提示变体下的可靠性，发现评估设计可显著改变结论（如链式思维+首字符评估器使ARC-Challenge准确率下降72-88%）、置信度信号脆弱（所有模型在MMLU-Pro上口头置信度远高于实际准确率）、提示鲁棒性与参数量无可靠相关（相关系数范围-0.244至0.474）。作者呼吁报告校准定义、评估逻辑、口头可解析性和提示鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试仅依赖单提示准确率，忽略了模型可靠性失败（如校准误差、置信度偏差、提示敏感性等），导致对小型语言模型可靠性的结论可能被评估管道本身扭曲。

Method: 构建15模型开源语料库，对10个指令模型在5个基准（分类+推理）上使用5种提示变体，测量每个(模型×数据集×变体)单元的准确率、令牌概率校准、口头置信度校准、口头解析率和提示扰动扩散度。

Result: 1) 评估设计可大幅改变结论：ECE定义切换导致校准值平均绝对差异0.149；CoT+首字符评估器使ARC-Challenge准确率下降72-88%，修复后恢复93.8%-102.7%性能，表明是评估器端而非模型端失败。2) 置信度信号脆弱：所有模型在MMLU-Pro上口头置信度显著高于准确率和令牌概率置信度，口头解析率在单模型单变体上可崩溃。3) 提示鲁棒性与参数量无稳定相关：相关系数跨基准在-0.244至0.474之间。

Conclusion: 小型语言模型的可靠性结论不仅取决于被评估的模型本身，还取决于用于测量的评估管道。建议显式报告校准定义、评估逻辑、口头可解析性和提示鲁棒性。
...
TL;DR: 本文通过两个案例分析（EEBO-TCP中的早期现代英语语篇和皇家学会语料库中的科学写作），探讨了量化历史语言学方法与数据集特性之间的互动关系，论证了比较方法论反思如何揭示纯词汇频率方法的局限性以及数据集结构对语义变化检测的影响。


<details>
  <summary>Details</summary>
Motivation: 反思量化历史语言学方法如何与数据集特性相互作用，阐明纯词汇频率方法的局限性，并揭示数据集结构如何影响量化方法对语义变化的可靠检测。

Method: 采用两个工作案例进行并行比较分析：1）使用quad-based概念建模分析EEBO-TCP中的早期现代英语语篇（1470s-1690s，7.65亿词）；2）使用SynFlow分析皇家学会语料库6.0.4中的科学写作文本（1750-1799，源自7860万词开放语料库）。

Result: 展示了每种方法如何操作化概念、所需的数据假设以及所支持的历时解释，揭示了数据集结构对量化方法检测语义变化能力的影响。

Conclusion: 比较方法论反思能够阐明纯词汇、基于频率方法的局限性，并强调数据集结构塑造了量化方法能够可靠检测的语义变化类型。
...
TL;DR: 提出Pair2Score两阶段框架，利用成对比较学习绝对评分，在自动作文评分任务中通过参数高效的LLaMA适配实现性能提升，但迁移配置比单纯引入成对阶段更为关键。


<details>
  <summary>Details</summary>
Motivation: 许多评分应用需要绝对预测，而成对比较提供了更简单的学习目标，如何将二者有效结合是一个关键问题。

Method: 两阶段学习框架：Stage 1训练方向性Siamese排序器，从绝对标签导出成对比较；Stage 2使用可配置迁移策略（热启动和嵌入融合变体）训练绝对预测器，采用参数高效的LLaMA适配。

Result: 最佳迁移变体在所有三个评分维度（语法、词汇、句法）上相比绝对基线均提升了QWK；但并非所有迁移配置都有帮助，单周期成对阶段比延长训练更可靠，迁移配置决定下游评分收益。

Conclusion: Pair2Score有效将成对比较迁移到绝对评分中，但迁移配置的选择比是否包含成对阶段更为关键，为评分任务中的学习范式设计提供了重要启示。
...
TL;DR: EditPropBench是一个评估LLM编辑者能否将事实编辑传播到依赖声明上的基准。实验表明，最强LLM在隐式/自由形式层上仍缺失约30%的级联更新，37.2%的arXiv论文存在事实依赖的定性声明。


<details>
  <summary>Details</summary>
Motivation: 科学稿件中局部事实编辑常产生非局部的修订义务（如数字从215改为80后，“中等规模”等描述也变得过时），现有LLM编辑系统缺乏对这种级联传播效果的评估基准。

Method: 构建EditPropBench基准，包含合成稿件、定向编辑、受控事实图谱及句子级标注（直接目标、下游更新需求、受保护文本）。设计了三种编辑协议、对抗性指标探测、压力测试变体，并以Edit-Ripple Adherence (ERA)为核心指标。

Result: 在硬隐式/自由形式层上，5个LLM编辑系统的ERA范围为0.148-0.705；最强系统仍缺失约30%的级联更新。混合层压力测试中LLM优于确定性替换基线。arXiv审计发现37.2%的论文存在事实依赖的定性声明。

Conclusion: 当前LLM编辑者能够修复许多事实编辑的隐式后果，但可靠的科学修订仍需级联感知检查。
...
TL;DR: 本文针对SemEval-2026 CLARITY任务，比较了微调编码器与基于提示的大语言模型（LLM）在总统访谈问答对中的清晰度与规避检测表现。LLM集成在3分类任务1（宏F1=80，第9/41名）和9分类任务2（宏F1=59，第3/33名）中表现优异；部分编码器层解冻策略显著优于全微调；基于提示的LLM（无需参数更新）优于微调编码器，尤其在少数类上；富化输入（拼接访谈者完整话轮）提升LLM但非编码器性能；Clear Reply/Ambivalent边界仍是主要失败模式。


<details>
  <summary>Details</summary>
Motivation: 现有研究缺乏对编码器与LLM在回答清晰度与规避检测任务上的系统性比较，尤其在真实政治访谈数据（U.S. presidential interviews）中；同时需探究不同微调策略、输入表示及模型架构对该任务的影响。

Method: 采用四阶段流水线优化8种Transformer编码器（包括部分层解冻 vs 全微调），并对比基于提示的LLM（无需参数更新）；构建LLM集成；比较英语/多语言编码器组合；测试富化输入（拼接访谈者完整话轮）及Longformer扩展上下文窗口。

Result: LLM集成在Task 1达80宏F1（9/41），Task 2达59宏F1（3/33）；部分编码器层解冻大幅优于全微调；英语+多语言编码器集成优于单一家族；提示LLM优于微调编码器，尤其少数类；参数规模与性能无关；富化输入提升LLM但非编码器，且Longformer无法弥合差距；Clear Reply/Ambivalent为最大失败模式。

Conclusion: 基于提示的LLM在政治访谈回答清晰度与规避检测任务上优于微调编码器，无需任务特定参数更新；编码器与LLM对输入表示的反应存在根本性差异，且非序列长度限制所致；Clear Reply/Ambivalent分类边界与人类标注者分歧一致，是未来改进的关键方向。
...
TL;DR: ARGUS是一个策略自适应的在线广告治理系统，通过多智能体对抗仲裁框架（检察官-辩护人-裁判）解决政策非平稳变化带来的标签不一致和推理歧义问题，在少量标注数据下显著优于传统微调方法。


<details>
  <summary>Details</summary>
Motivation: 在线广告治理面临监管政策非平稳变化的严峻挑战，新政策（如教育、容貌焦虑限制）导致历史数据集存在严重的标签不一致和推理歧义，传统方法难以适应快速演变的监管需求。

Method: 提出ARGUS三阶段框架：(1) Policy Seeding进行初始感知；(2) Adversarial Label Rectification，采用"检察官-辩护人-裁判"架构解决旧标签与新政策的冲突；(3) Latent Knowledge Discovery，通过三方辩证讨论挖掘灰色地带违规。利用RAG增强的政策知识和CoT合成作为强化学习的动态奖励。

Result: 在工业数据集和公开数据集上的实验表明，ARGUS在少量黄金数据下显著优于传统微调基线，实现了卓越的策略自适应学习能力。

Conclusion: ARGUS通过多智能体对抗仲裁与强化学习结合，有效解决了政策非平稳变化下的广告治理难题，为动态监管环境提供了高效的自适应学习范式。
...
TL;DR: 本文提出InfoPDF框架，从互信息角度缓解合成传播数据的不可靠性，通过概率潜在分布建模和互信息优化目标，实现真实与合成传播的自适应融合与去噪，提升虚假新闻检测性能。


<details>
  <summary>Details</summary>
Motivation: 不完整的传播数据严重阻碍了鲁棒的虚假新闻检测。现有方法利用大语言模型通过角色扮演模拟缺失的用户交互来丰富传播信号，但合成传播数据本质上是不可靠的，直接融合会导致有偏表示，检测性能受限。

Method: 提出InfoPDF框架：1) 利用大语言模型生成属性特定的合成传播图；2) 将每个合成传播图建模为概率潜在分布，引导与真实传播的自适应融合；3) 设计基于互信息的优化目标，联合抑制合成传播中的噪声信号、保持真实与合成传播表示的一致性，并确保对虚假新闻检测和属性预测的任务充分性。

Result: 在三个真实数据集上，InfoPDF在多种虚假新闻检测任务中一致取得优越性能。进一步分析表明，InfoPDF能够估计属性级别的可靠性并学习更具判别性的传播表示。

Conclusion: InfoPDF从互信息视角有效缓解了合成传播数据不可靠的问题，为利用合成数据增强虚假新闻检测提供了新范式。
...
TL;DR: 本文系统分析了多语言骨科诊断中LLM的校准性与可靠性，发现零样本LLM在低资源语言中校准不稳定、可靠性下降，而领域自适应架构（IndicBERT-HPA）在跨语言判别和置信度行为上显著优于任务适配模型和零样本LLM，并提出了确定性智能体验证框架。


<details>
  <summary>Details</summary>
Motivation: LLM被越来越多地用于临床决策支持，尤其在低资源环境的多语言诊断中，但其在结构化、高风险任务中的可靠性、校准性和安全性尚未被充分理解。

Method: 基于英语、印地语和旁遮普语的自由文本临床笔记，评估三种建模范式：(i)任务对齐的多语言Transformer编码器、(ii)任务微调基线DistilBERT、(iii)领域自适应架构IndicBERT-HPA，并与零样本指令微调LLM进行比较。

Result: LLM表现出强语言流畅性，但在结构化多语言条件下校准不稳定、可靠性下降（尤其低资源语言）；领域自适应显著提升跨语言判别与置信度行为；IndicBERT-HPA在六个诊断类别上性能一致且部署特性更可预测。

Conclusion: 可靠的临床决策支持需要专门架构、显式可靠性分析和结构化验证。零样本LLM不适合高风险结构化诊断，而领域自适应方法及所提出的确定性智能体验证框架为安全关键系统提供了可行路径。
...
TL;DR: 本文首次系统比较了塔吉克语（西里尔字母）和波斯语（阿拉伯字母）之间音译的多种机器学习架构。通过从多个异质来源聚合创建了包含328,253句对的并行语料库，并采用分层随机抽样形成40,000句对的代表性子集。实验对比了六类模型（规则基线、LSTM+注意力、字符级Transformer、G2P Transformer、预训练多语言模型mBART/mT5+LoRA、字节级ByT5），结果表明ByT5以chrF++ 87.4（塔→波）和80.1（波→塔）的压倒性优势胜出。G2P Transformer在有限数据下显著优于mBART（72.3 vs 62.2）。使用子词分词（mT5）的模型完全失败（chrF++<18.5）。结论：对于塔吉克-波斯语音译任务，字节/字符级架构明确优于依赖子词分词的Seq2Seq模型。


<details>
  <summary>Details</summary>
Motivation: 塔吉克语（西里尔字母）与波斯语（阿拉伯字母）虽同源但书写系统不同，音译任务面临跨字母表字符映射的挑战。此前缺乏针对这一语言对的系统性音译研究，且现有多语言模型在字母级转换任务上的效果尚未被充分评估。

Method: 构建多源聚合平行语料库（328,253句对），经分层随机抽样获得40,000句对子集。实验六类模型：规则基线、LSTM+注意力、字符级Transformer、从零训练的G2P Transformer、预训练多语言模型（mBART、mT5+LoRA）、字节级ByT5，采用chrF++作为评估指标。

Result: ByT5最优（塔→波87.4 chrF++，波→塔80.1）；G2P Transformer优于mBART（72.3 vs 62.2）；mT5完全失败（<18.5）；规则基线最差。

Conclusion: 对于塔吉克-波斯语音译任务，字节/字符级架构（如ByT5）显著优于依赖子词分词的传统多语言Seq2Seq模型，这一结论为跨字母表音译的模型选择提供了重要参考。
...
TL;DR: 本文提出CECoR框架，通过分解与注入范式（将多跳事实错误分解为可解释推理步骤并注入可控扰动生成训练数据）结合两阶段学习（监督微调+强化学习），有效解决了多跳事实错误修正中组合推理难、标注数据少等问题，在多跳和单跳基准上均取得优异表现。


<details>
  <summary>Details</summary>
Motivation: 现有事实错误修正方法主要针对单跳场景，将整句视为原子单元处理，难以应对需要跨多个证据来源进行组合推理的多跳错误修正；同时面临标注数据稀缺、复杂推理链中语义错误定位困难等挑战。

Method: 提出CECoR（组合错误修正的推理感知合成）框架，核心包括：(1) 分解与注入范式——将多跳声明分解为可解释的推理步骤，并在其中注入可控扰动以合成高质量训练对；(2) 两阶段学习策略——结合监督微调与强化学习，提升事实准确性和鲁棒性。

Result: 在多跳基准测试上取得强性能，优于远程监督方法和少样本LLM基线；在单跳修正任务上也能有效泛化；在含噪证据条件下保持稳定。

Conclusion: CECoR通过推理感知的分解-注入范式，有效解决了多跳事实错误修正中的组合推理难题，兼具强性能和鲁棒性，适用于实际场景中的事实修正任务。
...
TL;DR: 本文系统综述了法律论证挖掘领域的研究现状，指出其发展缓慢的根本原因在于缺乏一种兼顾理论表达力与计算可行性的结构化表示方法，并提出了未来研究方向。


<details>
  <summary>Details</summary>
Motivation: 在人工智能快速发展的背景下，法律论证挖掘作为连接法律文本与智能分析的重要研究领域，具有重要的理论和实践意义，但整体发展相对缓慢，亟需深入分析其瓶颈所在。

Method: 基于对现有研究的系统性综述，从数据、技术和理论三个维度进行梳理，并通过深入分析诊断发展缓慢的根本原因。

Result: 研究发现，发展缓慢的根本原因并非数据稀缺或技术限制，而是缺乏一种能够调和理论表达力与计算可行性的结构化表示方法，具体表现为数据标准化困境、有效建模障碍和领域适应性局限。

Conclusion: 研究提出了几个关键的未来研究方向，旨在重新框定法律论证挖掘的关键问题并指明发展路径，具体模型与实现方案留待后续探索。
...
TL;DR: 本文提出了一种在解码时进行去偏的方法，通过Process Reward Model（PRM）作为裁判，在不修改模型权重的情况下，对候选token进行公平性和流畅性评分。设计了三种方案（Best-of-N、Sequential critique-and-revise、Constitutional self-audit），在四个模型和200个双语提示上评估，其中Sequential debiasing效果最佳，将偏见过均分提高最多+0.40，同时保持甚至提升流畅性。该方法还扩展到开放生成场景，并引入Bias Guard门控以控制开销。


<details>
  <summary>Details</summary>
Motivation: 大语言模型从训练数据中习得社会偏见，并在下游应用中强化刻板印象（性别、种族、宗教等）。现有的标准修复方法（在精选数据上重新训练、用人类反馈微调）成本高昂、需要访问模型权重，且可能损害模型在其他任务上的性能。因此需要一种无需修改模型权重、低成本且不损害通用能力的去偏方法。

Method: 提出解码时去偏框架，使用独立的Process Reward Model (PRM) 作为裁判，对每个候选token进行公平性和流畅性评分。设计了三种方案：(1) Best-of-N选择——从N个候选中选择最优；(2) Sequential critique-and-revise——顺序评审与修订；(3) Constitutional self-audit——宪法自我审计。评估使用四个模型（GPT-4o-mini, Llama 3.2 3B, Gemma 3 4B, Qwen 2.5 3B），200个提示双语基准（英语/乌尔都语），覆盖8类偏见。扩展到开放生成场景，并引入Bias Guard门控减少开销。

Result: Sequential debiasing效果最佳，将偏见过均分从基线提高最多+0.40，同时保持（有时甚至提升）流畅性。Best-of-N在原生实现中对生成器侧开销几乎为零。GPT-4o-mini验证了框架随模型能力扩展，三个开源模型暴露了小规模LLM的局限性。

Conclusion: 解码时去偏是一种有效且经济的偏见缓解策略，无需修改模型权重或重新训练。顺序去偏方案在公平性与流畅性之间取得了最佳平衡。Bias Guard门控可以有效控制推理开销。该框架具有跨模型、跨语言和跨偏见类型的可扩展性。
...
TL;DR: 本文提出MolViBench，首个面向分子振动编码（Molecular Vibe Coding）的基准测试，包含358个任务和五层认知水平，覆盖12种真实药物发现流程，并设计了多层评估框架来度量代码可执行性和化学正确性，系统评估了9个前沿编码LLM及三种实际编码范式。


<details>
  <summary>Details</summary>
Motivation: 现有基准要么是通用代码生成（无需化学知识），要么是化学知识问答（不涉及可执行代码生成），缺少同时评估LLM在分子任务中编程能力、分子理解和领域推理能力的基准，无法弥合分子振动编码这一新兴范式与实际评估需求之间的鸿沟。

Method: 构建包含358个任务的MolViBench，任务覆盖五层认知水平（从单API调用到端到端虚拟筛选管道设计）和12种真实药物发现流程；提出多层评估框架，结合类型感知输出比较和基于AST的API-语义回退分析，同时衡量代码的可执行性和化学正确性；系统评估9个前沿编码LLM并比较三种实际分子振动编码范式。

Result: 提供了针对LLM在AI加速分子发现中编码能力的实用、细粒度测试平台，通过系统评估揭示了不同编码范式下的性能差异。

Conclusion: MolViBench有效填补了分子振动编码评估的空白，为诊断LLM在分子任务中的编码能力提供了可靠的基准和评估框架。
...
TL;DR: 本文提出InfoLaw（信息缩放定律），一种数据感知的缩放框架，通过将预训练建模为信息累积过程，预测损失与数据混合配方、重复率和模型规模的关系，在多达7B参数、425B token的实验中实现0.15%的平均绝对误差，并可靠外推至不同过训练水平，支持高效数据配方选择。


<details>
  <summary>Details</summary>
Motivation: 现有缩放定律无法可靠地跨数据混合配方或重复率外推，导致在受限数据场景下（特别是过训练时）最优数据配方的选择问题未得到解决。

Method: 将预训练建模为信息累积过程，其中质量控制信息密度、重复引入尺度相关的递减收益；收集不同规模、质量分布和重复率下的训练数据，构建信息度量以准确预测模型性能。

Result: 在高达7B参数、425B token的规模下，InfoLaw预测损失的平均绝对误差为0.15%，最大绝对误差为0.96%，并能可靠外推至不同过训练水平。

Conclusion: InfoLaw实现了数据感知的缩放定律，解决了标准缩放定律在数据混合和重复场景下的外推局限，为不同计算预算下的高效数据配方选择提供了可行方案。
...
TL;DR: 本文提出了FiNE-Patents数据集，将专利新颖性评估从粗粒度的二分类任务转向细粒度的特征级联合检索与抽象推理任务，并验证了基于LLM的工作流在该任务上的有效性及鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有专利新颖性预测主要被当作粗粒度的二分类任务，容易受虚假相关性影响，且缺乏实际应用所需的细粒度可解释性。

Method: 构建FiNE-Patents数据集（含3658件专利主权利要求及从欧洲审查意见中提取的特征级现有技术引用标注）；提出将新颖性评估转变为特征级的联合检索与抽象推理任务；设计LLM工作流：将权利要求分解为特征、各特征与现有技术对比分析、最终推导权利要求级新颖性判断。

Result: LLM工作流在段落检索和新颖特征识别上优于基于嵌入的基线方法；LLM在权利要求级新颖性分类任务中对虚假相关性具有鲁棒性，而传统分类器则易受影响。

Conclusion: 细粒度的特征级评估范式比粗粒度二分类更适合专利新颖性分析，且LLM能提供更透明、可解释、鲁棒性更强的判断。
...
TL;DR: 本文首次对匈牙利语学生反思性写作进行自动分类研究，使用1,954篇专家标注的论文，比较了经典机器学习模型（TF-IDF/语义嵌入）与匈牙利语Transformer模型在四级反思量表上的分类效果。经典模型综合得分达71%，Transformer模型达68%但少数类泛化更好。


<details>
  <summary>Details</summary>
Motivation: 反思性思维是教育中的关键能力，但反思性写作评估耗时且主观。尽管自动反思分析已在多种语言中探索，匈牙利语研究尚属空白。

Method: 使用1,954篇专家标注的匈牙利语反思性论文（四级量表），采用两类方法：(1)基于TF-IDF和语义嵌入特征的经典机器学习模型；(2)匈牙利语特定Transformer模型进行文档级分类。针对类别不平衡，系统研究了类别加权、过采样、数据增强和替代损失函数，并进行了消融实验。

Result: 经典机器学习模型综合得分（准确率、F1、ROC AUC平均）达71%，Transformer模型得分68%。但Transformer模型在少数类上泛化能力更强。

Conclusion: 经典方法在低资源场景下仍具竞争力，Transformer模型在不平衡分类中鲁棒性更好。数据集和实验洞察为匈牙利语及其他形态丰富语言的自动反思分析提供了基础。
...
TL;DR: HalluScan是一个系统性评估大语言模型幻觉检测与缓解的基准框架，覆盖72种配置（6种检测方法、4个开源模型家族、3个领域）。提出三个贡献：HalluScore复合指标（与人类判断Pearson r=0.41）、自适应检测路由ADR（成本降低2.0倍，AUROC仅降0.1%）、系统化错误级联分解。实验表明NLI Verification AUROC最高达0.88，RAV为0.66。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在多种NLP任务中表现出色，但仍存在幻觉问题（生成与事实不符、偏离上下文或用户指令的内容），需要系统性的评估框架来检测和缓解这一问题。

Method: 构建HalluScan基准框架，在72种配置下评估6种检测方法、4个开源模型家族和3个领域；提出HalluScore复合指标、自适应检测路由（ADR）算法以及错误级联分解方法。

Result: NLI Verification达到最高AUROC 0.88，RAV为0.66；HalluScore与人类专家判断的Pearson相关系数为0.41；ADR实现成本降低2.0倍，AUROC仅下降0.1%；不同领域的幻觉错误类型存在显著差异。

Conclusion: HalluScan提供了系统化的幻觉检测评估框架，HalluScore可作为自动化评估指标，ADR能有效平衡检测性能与计算成本，且错误级联分解揭示了领域间幻觉类型的差异，为未来研究提供了方向。
...
TL;DR: 本文提出了一种基于标量一致路由机制和先验引导上下文图的多模态讽刺检测方法，通过不一致感知对比学习的两阶段非对称优化，在MUStARD基准上实现了SOTA性能，Macro-F1提升3.14%。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要依赖基于相似性的注意力机制和统一后融合策略，存在功能纠缠问题，限制了模型对文本与非语言线索之间语用不一致性的精确建模能力。

Method: 提出标量一致路由机制和先验引导上下文图；通过不一致感知对比学习实现两阶段非对称优化，仅融合最具判别力的多粒度证据；从架构上隔离原子冲突、组合冲突和上下文冲突。

Result: 在MUStARD基准及缓解虚假相关的平衡数据集上均取得SOTA性能，相比最强多模态基线在Macro-F1上提升3.14%。

Conclusion: 该工作提供了一种鲁棒、解耦的范式，用于建模人类交流中微妙的语用不一致性。
...
TL;DR: 本文探讨利用论证结构（前提与结论）来关联信息失序的不同维度（仇恨言论、虚假信息等），基于WSF-ARG+数据集（白人至上主义论坛消息），通过论证组件的可核查性与仇恨性标注来预测整条消息的仇恨程度，F1最高达96%。


<details>
  <summary>Details</summary>
Motivation: 信息失序（包含仇恨言论、虚假信息、误导信息等）是一个广泛而复杂的社会问题，不同维度之间缺乏统一的关联分析框架。作者希望利用论证结构作为桥梁，将不同维度的问题联系起来，提供更全面的解决方案。

Method: 使用WSF-ARG+数据集（白人至上主义论坛消息），该数据集包含论证结构标注（前提和结论）以及组件的可核查性（checkworthiness）和仇恨性（hatefulness）标注。通过对论证组件的标注信息进行分析，推断整条消息的仇恨程度。

Result: 实验结果表明该方法具有良好效果，F1分数高达96%，证明利用论证组件的标注信息可以有效预测整条消息的仇恨性。

Conclusion: 论证结构能够为信息失序的不同维度提供有意义的关联信息，该方法在仇恨内容识别方面表现优异，未来可进一步扩展以应对更广泛的信息失序问题。
...
TL;DR: 本文构建了一个流水线，用于恢复数字化旧百科全书的结构化文本信息，包括提取词条标题、识别条目、分类实体、跨版本匹配以及链接到Wikidata。将该方法应用于瑞典百科全书《Nordisk familjebok》的四个主要版本，取得了优异的性能指标。


<details>
  <summary>Details</summary>
Motivation: 旧百科全书的数字化通常仅停留在光学字符识别层面，未充分利用其底层结构；同时多版本百科全书难以追踪知识演变。因此需要自动化方法恢复文本结构以便于知识保存和传播分析。

Method: 构建了一条包含四个步骤的流水线：1）提取词条标题并识别条目；2）对实体进行分类；3）跨版本匹配条目；4）将条目链接到Wikidata项目。应用于瑞典百科全书《Nordisk familjebok》的四个主要版本（1876-1951年）。

Result: 词条标题提取F1分数97.8%，词条分类F1分数93.4%；小规模评估中跨版本匹配精度93%，Wikidata链接精度85%但召回率仅16.5%。

Conclusion: 自动化方法处理数字化历史知识是可行的，有助于促进通用知识的保存和理解知识传播。数据集和程序已公开。
...
TL;DR: 本文提出一种名为"摊销智能"的神经符号方法，通过单次使用LLM将法律文本翻译为确定性自主合约语言（DACL）图中间表示，随后基于确定性图执行进行裁决，相比运行时大型推理模型（如GPT-5.2、Gemini 3 Pro），实现了近乎完美的一致性，降低了90%以上的计算成本，并满足法律裁决的严格可审计性要求。


<details>
  <summary>Details</summary>
Motivation: 法律文本中的计算性法律条款理解需要复杂逻辑，而前沿大型推理模型虽能描述此类条款，但在构建生产级系统时受限于推理错误和高推理成本。

Method: 提出摊销智能（Amortized Intelligence）方法：使用LLM仅一次将法律文本翻译为确定性自主合约语言（DACL）——一种类型化图中间表示，随后通过确定性图执行进行裁决，并生成可视化可审计的追踪记录。

Result: 相比GPT-5.2和Gemini 3 Pro等运行时LRM基线，DACL智能体实现了近乎完美的一致性，缓解了概率模型中观察到的"推理悬崖"问题，在高吞吐工作流中计算成本降低超90%。

Conclusion: 该神经符号方法能在保持法律裁决严格可审计性要求的同时，显著提升一致性并大幅降低计算成本，适合生产级部署。
...
TL;DR: 本文利用MultiWikiQA数据集和LettuceDetect框架为306种语言创建合成幻觉数据集，训练了30种欧洲语言的token级幻觉分类器，并评估了5种模型在英语、丹麦语、德语和冰岛语上的幻觉率。结果表明，低资源语言（尤其是冰岛语）的幻觉率普遍更高，小模型（Qwen3-0.6B）幻觉率高达60%，而大模型表现更优。


<details>
  <summary>Details</summary>
Motivation: 现有幻觉评估主要聚焦英语，缺乏对低资源语言多语言场景下幻觉现象的系统研究，不清楚已有发现是否可迁移至其他语言。

Method: 基于MultiWikiQA数据集，利用LettuceDetect框架合成306种语言的幻觉数据，训练30种欧洲语言的token级幻觉分类器，并对5种模型（Qwen3-0.6B、Qwen3-14B、Gemma-3-12B-IT、cogito-v1-preview-qwen-32B、cogito-v1-preview-llama-70B）在4种语言上进行幻觉率评估。

Result: Qwen3-0.6B幻觉率最高（冰岛语中达60%的答案包含至少一次幻觉）；cogito系列大模型在多数语言上表现最佳；低资源语言（尤其是冰岛语）的幻觉率普遍高于高资源语言。

Conclusion: 模型规模和语言资源丰富度共同影响幻觉率：大模型幻觉率更低，低资源语言的幻觉问题更为严峻，需针对低资源语言开发更鲁棒的幻觉检测与缓解方法。
...
TL;DR: 本文提出了一个现代化的编码器框架用于语义角色标注（SRL），该框架在保留显式谓词-参数结构的同时，实现了10倍推理加速，并通过多种预训练模型（BERT、RoBERTa、DeBERTa）取得可比或更优的F1性能，同时利用依存信息进行结构一致性诊断和LLM行为分析，并展示了在多语言SRL投影中的应用。


<details>
  <summary>Details</summary>
Motivation: 传统SRL系统多依赖已进入维护模式的AllenNLP框架，难以兼容现代编码器架构和推理需求；同时，大语言模型虽在NLP领域占据主导，但缺乏显式的结构约束和系统性的可解释机制，因此需要重新审视结构化的SRL建模，提出既能保留显式谓词-参数结构又能满足高效推理需求的现代化框架。

Method: 提出一个现代化的编码器框架，采用BERT-base等预训练编码器，设计显式的谓词-参数结构建模；引入依存信息指导的诊断方法，用于表征span级别的不一致性，并对LLM行为进行表征层面的分析；最后将该框架应用于多语言SRL投影任务。

Result: 该框架在BERT-base上取得与以往方法相当的预测性能；使用RoBERTa和DeBERTa在同一框架下进一步提升了F1分数；推理速度提升10倍；依存线索主要改善结构稳定性；框架的显式结构可有效支持多语言SRL投影。

Conclusion: 结构化的SRL建模在现代编码器框架下依然具有价值，能够兼顾显式语义结构、高效推理和可解释性；依存信息有助于提升结构稳定性；该框架可扩展至多语言场景，展示了显式谓词-参数结构在下游应用中的实用性。
...
TL;DR: 本文系统比较了五种检索策略（稠密向量检索、混合BM25+稠密检索、交叉编码器重排序、多查询扩展、最大边际相关性）在生物医学RAG流水线中的表现。交叉编码器重排序综合得分最高（0.827），多查询扩展精确度最弱（0.671），所有RAG条件在答案相关性上显著优于无上下文消融实验。实验使用固定生成模型（GPT-4o-mini）、统一向量库（ChromaDB）和相同嵌入模型，确保差异仅来自检索策略。


<details>
  <summary>Details</summary>
Motivation: 高风险的生物医学领域亟需系统、可控、多指标的实证研究来回答"哪种检索策略在RAG中表现最佳"这一关键问题，填补此前缺乏严格比较的空白。

Method: 在BioASQ基准的250个问答对上，固定GPT-4o-mini为生成模型、ChromaDB为向量库、text-embedding-3-small为嵌入模型，采用四种DeepEval指标（上下文精确度、上下文召回率、 faithfulness、答案相关性）进行95%置信区间评估，并设置无上下文消融实验作为下界。

Result: 交叉编码器重排序取得最佳综合得分0.827和最高上下文精确度0.852；多查询扩展上下文精确度最弱（0.671）；稠密向量检索基线综合得分0.822，与最优策略仅差0.005；所有RAG条件答案相关性（0.658-0.701）远超无上下文消融实验（0.287）。

Conclusion: 交叉编码器重排序通过查询-文档交互获得可量化的检索增益；多查询扩展的朴素多样化会引入检索噪声；稠密向量检索以极低代价达到了接近最优的表现；检索在RAG中具有不可替代的实践价值。
...
TL;DR: 本文介绍了一个评估LLM和NLP系统在多语言和多文化场景下适应性的共享任务，基于扩展版BLEnD基准数据集，涵盖30多种语言-文化对（主要为低资源语言）。任务设SAQ和MCQ两个赛道，仅用于评估（禁止训练/微调）。共140+注册、62个团队提交最终结果、19篇系统描述论文。文章报告了最佳系统表现和常用方法，并讨论了低资源语言和未被充分代表文化中的评估、对齐和方法论挑战。


<details>
  <summary>Details</summary>
Motivation: 现有NLP模型在多语言、多文化场景下的适应性评估不足，尤其缺乏面向低资源语言和未被充分代表文化的系统性基准。研究者希望通过构建涵盖多样化语言-文化对的评估任务，揭示当前模型在这些场景下的表现短板，推动跨语言跨文化NLP研究的发展。

Method: 基于扩展版BLEnD基准数据集（30+语言-文化对，以低资源语言为主），设置两个评估赛道：简答题(SAQ)和选择题(MCQ)。任务仅限评估用途，参赛者不能使用数据进行训练/微调/少样本学习等模型修改。参赛者可提交任意NLP系统或采用多样化建模策略。最终收集62个团队的提交结果和19篇系统描述论文进行系统性分析。

Result: 超过140人注册，62个团队提交最终结果，19篇系统描述论文。研究报告了最佳性能系统的结果和最常见的方法学分析，涵盖低资源语言和未被充分代表文化的评估、对齐和方法论视角。

Conclusion: 该共享任务揭示了LLM和NLP系统在多语言、多文化场景（尤其是低资源语言）中的能力现状与局限，为跨语言评估提供了重要的基准参考。通过分析最佳系统和常见方法，为未来的模型对齐、评估方法论和低资源语言研究提供了有价值的洞见和开放性问题。
...
TL;DR: 本文比较了四种依存句法分析器（Biaffine LSTM、Stack-Pointer Network、AfroXLMR-large、RemBERT）在十种语言（重点关注低资源非洲语言）上的表现，发现Biaffine LSTM在低资源场景下稳定优于Transformer模型，而Transformer的优势随训练数据增加而恢复；形态复杂度（MATTR）在控制语料规模后是Transformer相对劣势的显著次要预测因子。


<details>
  <summary>Details</summary>
Motivation: Transformer模型在高资源语言的依存句法分析中达到最优性能，但它们相比更简单的架构在低资源场景下的优势尚不清楚。本文旨在系统评估不同解析器在低资源语言（特别是非洲语言）上的表现差异。

Method: 选取四种解析器（Biaffine LSTM、Stack-Pointer Network、AfroXLMR-large、RemBERT），在十种类型多样的语言上进行实验，重点关注低资源非洲语言；使用MATTR测量形态复杂度作为预测变量，控制语料规模进行分析。

Result: Biaffine LSTM在低资源场景下稳定优于Transformer模型；Transformer的优势随着训练数据增加而恢复；两种模型的交叉点落在低资源语言树库的典型数据量范围内；形态复杂度（MATTR）是Transformer相对劣势的显著次要预测因子。

Conclusion: 在低资源场景下，Biaffine LSTM比Transformer模型更适合作为句法工具开发的基线选择，直到积累足够的标注数据来发挥预训练Transformer的表示能力。
...
### [245] [Beating the Style Detector: Three Hours of Agentic Research on the AI-Text Arms Race](https://arxiv.org/abs/2605.02620)
*Andreas Maier,Moritz Zaiss,Siming Bayer*

Main category: cs.CL

TL;DR: 该研究利用自动化研究框架在数天内复现并扩展了一项ACL 2026关于个性化风格编辑的研究，证实了GPT-5.5和Claude Opus 4.7能缩小71-75%的风格差距（人类仅24%），并发现AI文本检测存在长度混淆问题，同时展示了通过迭代对抗训练可有效降低AI检测概率。


<details>
  <summary>Details</summary>
Motivation: 传统NLP研究复现耗时数周，作者希望验证能否利用现代自动化研究框架大幅加速这一过程，同时探索前沿LLM在个性化风格编辑和AI文本检测对抗方面的能力边界。

Method: 使用agentic-research框架，人类仅作为回路中的评审者；采用无泄漏的留出协议评估LLM（GPT-5.5、Claude Opus 4.7）与人类编辑的风格匹配能力；使用LUAR-MUD嵌入上的线性SVM进行AI文本检测；通过T=20次反馈迭代进行对抗性攻击。

Result: 复现了所有7个预注册假设，核心相关性r=+0.244 (p<10⁻⁸, n=648)精确到三位小数；GPT-5.5和Claude Opus 4.7缩小71-75%风格差距（人类仅24%）；检测AUC达0.93-1.00；Opus经过20次对抗迭代后，5个留出样本中有2个被翻转至人类半空间，所有边界缩小一个数量级。

Conclusion: 自动化工具有效加速NLP研究复现；前沿LLM在风格匹配上已超越人类编辑；当前AI检测存在长度混淆问题；在已知检测器面前，中等对抗成本即可显著降低AI文本的可检测性。
...
### [246] [Synthetic Users, Real Differences: an Evaluation Framework for User Simulation in Multi-Turn Conversations](https://arxiv.org/abs/2605.02624)
*Yu Lu Liu,Hyokun Yun,Tanya Roosta,Ziang Xiao*

Main category: cs.CL

TL;DR: 提出了REALSIM评估框架，从8个维度对真实与模拟用户-聊天机器人对话进行分布级比较，发现模拟用户在捕捉真实用户引入的沟通摩擦方面存在困难，可能导致评估过于乐观，且不同领域表现存在差异。


<details>
  <summary>Details</summary>
Motivation: 现有用户模拟评估方法仅能在单个对话层面产生粗糙的质量信号，缺乏更严格的评估手段来确保模拟的真实性。

Method: 提出REALSIM评估框架，从交互的沟通功能、用户状态和用户消息表面形式三个方面的8个维度对真实与模拟对话进行分布级比较，并构建了涵盖16个领域的1K多轮任务导向真实对话数据集。

Result: 模拟用户在捕捉真实用户引入的沟通摩擦方面存在困难，可能导致基于模拟的评估过于乐观；不同领域间性能存在差异。

Conclusion: 现有用户模拟器在真实性方面存在局限，需要更细致的分布级评估方法，且可能需要领域特定的用户模拟器。
...
### [247] [ContextualJailbreak: Evolutionary Red-Teaming via Simulated Conversational Priming](https://arxiv.org/abs/2605.02647)
*Mario Rodríguez Béjar,Francisco J. Cortés-Delgado,S. Braghin,Jose L. Hernández-Ramos*

Main category: cs.CL

TL;DR: 提出ContextualJailbreak，一种基于进化搜索的黑盒红队策略，通过在模拟多轮对话中应用五种语义变异算子（包括两种新算子）生成上下文引导的越狱攻击。在多个模型上达到90-100%的攻击成功率，且跨模型迁移显示不同提供商间的对齐鲁棒性存在显著差异。


<details>
  <summary>Details</summary>
Motivation: 现有自动化优化红队方法主要局限于单轮设置，缺乏对多轮对话式上下文引导的推理能力；尽管近期出现了基于搜索的多轮方法，但有效引导对话的变异器设计空间仍未得到充分探索。

Method: 采用进化搜索策略在模拟多轮引导对话上进行黑盒红队测试；使用两级评判器生成的0-5分制伤害分数作为循环信号，使部分有害响应也能指导搜索过程；定义五种语义变异算子：角色扮演、场景化、扩展、故障排除和机制解释（后两种为本文新提出）。

Result: 在50个HarmBench行为上，对gpt-oss:20B、qwen3-8B、llama3.1:70B达到100% ASR，对gpt-oss:120B达到90% ASR，平均超过四个基线31-96个百分点。发现的最有害攻击迁移到闭源前沿模型：gpt-4o-mini 90.0%，gpt-5 70.0%，gemini-3-flash 70.0%，但claude-opus-4-7仅17.5%，claude-sonnet-4-6仅15.0%。

Conclusion: 上下文引导的多轮越狱攻击效果显著优于单轮方法；不同大模型提供商之间在对齐鲁棒性上存在显著的不对称性，Claude系列展现出最强的防御能力。
...
### [248] [Fuzzy Fingerprinting Encoder Pre-trained Language Models for Emotion Recognition in Conversations: Human Assessment and Validity Study](https://arxiv.org/abs/2605.02665)
*Patrícia Pereira,Helena Moniz,Joao Paulo Carvalho*

Main category: cs.CL

TL;DR: 本文提出了一种结合预训练语言模型（PLMs）与模糊指纹（FFPs）的可解释方法用于对话情感识别（ERC），通过模糊相似度匹配情感原型，有效减少了少数类情感被误分类为多数类中性情感的问题，在达到SOTA性能的同时提供了决策可解释性。


<details>
  <summary>Details</summary>
Motivation: 标准PLM在ERC任务中缺乏可解释性，且在类别不平衡数据集中频繁将少数类情感误分类为占多数的中性类，需要一种既能保持高性能又能提供分类过程洞察的方法。

Method: 1) 利用PLM提取对话上下文相关的池化嵌入；2) 对每个情感类别，基于训练实例的激活模式进行排序和模糊化，生成类特定的模糊指纹（FFP）原型；3) 推理时，对输入话语生成模糊指纹，通过模糊集合交集的聚合相似度函数与情感原型进行匹配，完成情感分类。

Result: 实验结果表明，FFP集成减少了向中性类的过度分类；人工评估进一步支持了FFP预测的充分性；方法达到SOTA水平。

Conclusion: 该方法弥合了深度神经推理与人类感知之间的差距，在实现SOTA性能的同时为分类过程提供了有价值的可解释性洞察。
...
### [249] [mdok-style at SemEval-2026 Task 9: Finetuning LLMs for Multilingual Polarization Detection](https://arxiv.org/abs/2605.02695)
*Dominik Macko,Alok Debnath,Jakub Simko*

Main category: cs.CL

TL;DR: 本文针对SemEval-2026 Task 9的多语言极化检测任务，采用QLoRA参数高效微调技术对中等规模LLM进行序列分类微调，并通过匿名化、大小写转换和同形词替换等数据增强策略提升模型鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 在线极化现象常伴随仇恨言论、攻击性话语和社会分裂，因此在其升级前进行检测对于构建更安全、更具包容性的在线空间至关重要。

Method: 使用QLoRA参数高效微调技术对中等规模LLM进行序列分类任务微调；训练数据通过匿名化、小写化、大写化和同形词替换对22种语言的多语言训练集进行数据增强。

Result: 通过数据增强策略提升了检测的鲁棒性，使模型对多语言、多文化和多事件的极化检测更加稳健。

Conclusion: 所提方法有效应对了多语言极化检测任务，为在线极化的早期识别提供了可行方案。
...
### [250] [mdok-style at SemEval-2026 Task 10: Finetuning LLMs for Conspiracy Detection](https://arxiv.org/abs/2605.02712)
*Dominik Macko*

Main category: cs.CL

TL;DR: 本文提出了一种基于数据增强和自训练的方法，微调Qwen3-32B模型用于Reddit评论的阴谋论检测（二分类任务），在SemEval-2026 Task 10中排名第8（85百分位），证明该方法可从机器生成文本检测迁移至阴谋论检测。


<details>
  <summary>Details</summary>
Motivation: 训练数据量较少，需要有效方法提升模型在阴谋论检测任务上的性能。

Method: 采用数据增强和自训练策略，微调Qwen3-32B大语言模型，将其应用于二分类文本分类任务。

Result: 在52个参赛系统中排名第8，位于第85百分位，表现极具竞争力。

Conclusion: 源于机器生成文本检测的方法可成功迁移至阴谋论检测任务。
...
### [251] [FlexSQL: Flexible Exploration and Execution Make Better Text-to-SQL Agents](https://arxiv.org/abs/2605.02815)
*Quang Hieu Pham,Yang He,Ping Nie,Canwen Xu,Davood Rafiei,Yuepeng Wang,Xi Ye,Jocelyn Qiaochu Chen*

Main category: cs.CL

TL;DR: FlexSQL提出了一种灵活数据库交互的Text-to-SQL框架，允许智能体在推理的任何阶段探索schema、检查数据值并运行验证查询，通过多样化执行计划覆盖多解释、双语言实现（SQL/Python）及两级修复机制，在Spider2-Snow上以较小模型超越强基线，且作为技能集成到Claude Code中带来超10%的相对提升。


<details>
  <summary>Details</summary>
Motivation: 现有Text-to-SQL系统遵循固定流水线：schema元素一次性预检索，数据库仅在事后修复阶段被访问；这种设计限制了从早期错误中恢复的能力，无法灵活应对复杂schema和模糊查询。

Method: 1) 灵活数据库交互：在推理任意阶段可探索schema结构、检查数据值、运行验证查询；2) 多样化执行计划：生成多个计划覆盖不同查询解释；3) 双语言实现：根据任务选择SQL或Python实现计划；4) 两级修复机制：从代码级错误回溯到计划级修订。

Result: 在Spider2-Snow上使用gpt-oss-120b模型达65.4%分数，超越使用更强更大模型（gpt-o3、DeepSeek-R1）的强开源基线；集成到Claude Code后带来超10%相对提升。

Conclusion: 灵活探索与灵活执行共同贡献了方法的有效性，灵活性是Text-to-SQL系统设计的关键原则。
...
### [252] [2026 Roadmap on Artificial Intelligence and Machine Learning for Smart Manufacturing](https://arxiv.org/abs/2605.00839)
...
TL;DR: 本文提出了人工智能和机器学习在智能制造领域的综合性路线图，涵盖基础、应用和新兴方向，分析了工业大数据、感知系统、自主系统、数字孪生、机器人、供应链优化及可持续制造等关键主题，并探讨了物理信息AI、生成式AI、语义AI、可解释AI、大语言模型和基础模型等前沿方法，旨在为研究人员和工程师提供指导，推动AI驱动的智能制造实现可靠、可持续和可扩展的影响。


<details>
  <summary>Details</summary>
Motivation: 尽管AI和ML为智能制造带来了效率、适应性和自主性的新能力，但在工业部署中仍面临工业大数据复杂性、数据管理、异构系统集成以及高可靠性要求等关键挑战，亟需系统性路线图来指导技术发展和工业应用。

Method: 采用三部分结构：第一部分概述AI在智能制造中的基础与趋势；第二部分聚焦AI已取得进展的关键领域（工业大数据分析、先进感知、自主系统、增材制造、数字孪生、机器人、供应链优化、可持续制造）；第三部分探索非传统ML方法（物理信息AI、生成式AI、语义AI、先进数字孪生、可解释AI、数据中心计量学、LLMs和基础模型），识别机遇与障碍并提出集成策略和工业采纳建议。

Result: 识别了各领域的机会和现有障碍，概述了在方法、集成策略和工业采纳方面所需的进展，形成了一份面向研究人员、工程师和从业者的综合性路线图。

Conclusion: 该路线图旨在加速创新、协调学术与工业优先事项，确保AI驱动的智能制造为未来制造生态系统提供可靠、可持续和可扩展的影响力。
...
### [253] [Understanding Emergent Misalignment via Feature Superposition Geometry](https://arxiv.org/abs/2605.00842)
*Gouki Minegishi,Hiroki Furuta,Takeshi Kojima,Yusuke Iwasawa,Yutaka Matsuo*

Main category: cs.AI

TL;DR: 本文提出了一种基于特征叠加几何的解释，说明为什么在无害任务上微调会意外诱导LLM产生有害行为，并通过稀疏自编码器验证了这一机制，最终提出了一种几何感知的过滤方法将有害行为降低34.5%。


<details>
  <summary>Details</summary>
Motivation: 尽管已有经验证据表明微调会意外诱导LLM产生有害行为（emergent misalignment），但其背后的机制尚不明确，作者希望揭示这一现象的根本原因。

Method: 作者提出基于特征叠加几何的机制解释：由于特征以重叠表示编码，增强目标特征的微调会无意中增强与目标相似的有害特征。通过梯度级推导该效应，并在多个LLM（Gemma-2 2B/9B/27B、LLaMA-3.1 8B、GPT-OSS 20B）上使用稀疏自编码器进行实证验证，比较诱导/非诱导数据的特征几何距离。最后提出几何感知的过滤方法（删除最接近毒性特征的训练样本）。

Result: 实验表明，与有害行为相关的特征在几何空间上更接近引发不当行为的特征，这一趋势在健康、职业、法律等多个领域普遍存在。几何感知过滤方法将有害行为降低34.5%，显著优于随机删除，效果与LLM作为评判的过滤方法相当或略优。

Conclusion: 本文将新兴的错误对齐现象归因于特征叠加几何，为理解和缓解这一现象提供了理论基础和实用方法。
...
### [254] [ClinicBot: A Guideline-Grounded Clinical Chatbot with Prioritized Evidence RAG and Verifiable Citations](https://arxiv.org/abs/2605.00846)
*Navapat Nananukul,Mayank Kejriwal*

Main category: cs.AI

TL;DR: ClinicBot是一个通过结构化提取临床指南、优先级排序证据和多智能体协作来提高医疗问答可信度的AI系统，能生成可验证的临床建议。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在医疗场景中存在幻觉问题，现有RAG系统对所有证据一视同仁，产生噪声内容和与临床实践不符的通用答案，无法满足高风险的临床诊断对准确性、可验证性和严格遵循官方指南的要求。

Method: 三个关键创新：(1)将临床指南结构化为语义单元（建议、表格、定义、叙述）并附带显式来源；(2)基于临床重要性和指南结构进行证据优先级排序，而非文本相似度；(3)构建web界面提供简洁、可操作且可验证的答案。在糖尿病场景中结合多智能体设置进行大规模处理。

Result: 使用真实患者的糖尿病问题以及基于ADA糖尿病护理标准（2025）的风险评估工具进行演示，展示了语义知识提取和分层证据排序在多智能体环境中可靠运行的可行性。

Conclusion: 通过结构化知识提取和层次化证据排序，ClinicBot能够在多智能体环境下可靠处理大规模临床指南，为临床诊断提供可信赖的AI支持。
...
### [255] [Effect-Transparent Governance for AI Workflow Architectures: Semantic Preservation, Expressive Minimality, and Decidability Boundaries](https://arxiv.org/abs/2605.01030)
*Alan L. McCann*

Main category: cs.AI

TL;DR: 本文在Rocq 8.19中形式化验证了结构治理的AI工作流架构，通过Interaction Trees定义治理算子G，证明了治理可在不降低计算表达性的前提下施加于效果边界，并建立了7条关键性质。


<details>
  <summary>Details</summary>
Motivation: 形式化证明AI工作流架构中，效果级治理（governance）与计算表达性（computational expressivity）是正交维度，治理可以在不牺牲内部计算能力的前提下约束程序的效果边界。

Method: 使用Rocq 8.19中的Interaction Trees定义治理算子G，对内存访问、外部调用、大模型查询等效果指令进行中介治理；构建36个模块、约12,000行代码、454个定理的形式化开发，0个 admitted lemmas。

Result: 建立了7条性质：P1治理下图灵完备性、P2治理下预言机表达性、P3治理谓词的可判定边界、P4允许执行的目标保持、P5原语能力的表达最小性、P6结构治理严格包含内容级过滤、P7语义透明性（治理允许的执行中，治理解释与无治理解释观测等价）。

Conclusion: 治理与计算表达性是正交的：治理约束程序的效果边界，同时对内部计算保持语义透明。
...
### [256] [Algebraic Semantics of Governed Execution: Monoidal Categories, Effect Algebras, and Coterminous Boundaries](https://arxiv.org/abs/2605.01032)
*Alan L. McCann*

Main category: cs.AI

TL;DR: 本文提出了一种基于代数语义的管辖执行框架，通过三个公理（安全性、透明性、恰当性）定义GovernanceAlgebra，在交互树和参数化余归纳中实现。该框架在32个Rocq模块（约12,000行代码，454个定理）中机械化，建立了对称幺半范畴并保持所有张量组合的管辖性质。代数效应系统约束处理器代数，确保安全片段中只能构造管辖保持的处理器。能力索引组合为程序提供机器校验的能力边界，对偶保证定理证明within_caps和gov_safe在所有组合算子下同时成立。核心结果是"管辖与表达共边界"：每个通过四种原始态射构造子表达的程序在解释下都被管辖，且每个被管辖程序都是此类程序的像。图灵完备性在管辖内保持，未中介的I/O被排除在管辖片段之外。管辖拒绝被建模为安全的余归纳发散。该代数对参数化：任何实例化三个公理的系统继承所有导出性质。提取的OCaml作为BEAM运行时的NIF运行，通过70,000+随机输入属性测试验证了规约与运行时解释器的行为等价性。


<details>
  <summary>Details</summary>
Motivation: 现有管辖执行模型缺乏代数化的、组合性的形式语义基础，难以在保持表达能力的同时保证管辖属性的可组合性和可验证性。本文旨在建立一个管辖与表达共边界的代数框架，使得每个可表达的程序都在管辖下，且每个被管辖的程序都是可表达的。

Method: 基于交互树（Interaction Trees）和参数化余归纳（Parameterized Coinduction）构建代数语义。定义三公理GovernanceAlgebra（安全性、透明性、恰当性），诱导对称幺半范畴并验证五边形、三角形、六边形协调性。引入代数效应系统约束处理器代数，使用能力索引组合实现程序与能力边界的捆绑，通过对偶保证定理证明管辖安全与能力约束的组合封闭性。

Result: 在32个Rocq模块中机械化（12,000行代码，454个定理，0个admitted）。建立管辖与表达的共边界定理：四种原始态射构造子可表达的程序均在管辖下，反之亦然。图灵完备性在管辖内保持，未中介I/O被排除。管辖拒绝建模为安全余归纳发散。提取OCaml作为BEAM运行时NIF运行，通过70,000+随机输入测试验证了行为等价性。

Conclusion: 管辖代数是一个参数化框架：任何实例化三个公理的系统都自动继承收敛性、组合封闭性和目标保持性等所有导出性质。该工作为代数化、组合化、可验证的管辖执行提供了坚实的基础，且通过提取可运行代码验证了理论与实践的一致性。
...
### [257] [A Knowledge-Driven LLM-Based Decision-Support System for Explainable Defect Analysis and Mitigation Guidance in Laser Powder Bed Fusion](https://arxiv.org/abs/2605.01100)
*Basit Mahmud Shahriar,Md Habibor Rahman*

Main category: cs.AI

TL;DR: 本文提出了一种集成本体知识图谱与大语言模型的决策支持系统，用于激光粉末床熔融（LPBF）增材制造过程中的缺陷诊断与缓解指导。系统包含27种已知缺陷类型的层次化知识库，支持模糊自然语言查询、文献支撑的缺陷解释、多模态图像评估，并通过消融实验和评估者间信度分析验证了有效性，最终配置的宏平均F1分数达0.808。


<details>
  <summary>Details</summary>
Motivation: LPBF等安全关键制造过程中的缺陷诊断依赖专家知识，现有LLM缺乏结构化领域知识支撑，导致推理不一致、可解释性差。本文旨在通过本体引导的知识表示提升LLM辅助缺陷分析的可靠性、可解释性和实用性。

Method: 构建包含27种LPBF缺陷类型的层次化本体知识库；开发本体集成LLM的决策支持系统，支持模糊自然语言查询和文献支撑的缺陷原因/缓解策略检索；集成基于基础模型的多模态图像评估模块，通过语义对齐评分实现缺陷图像描述符引导的解读；通过定性比较、消融研究和Cohen's kappa评估者间信度分析进行验证。

Result: 全集成配置在文献数据集上宏平均F1分数达到0.808，优于其他三种对照配置；Cohen's kappa分析显示模型输出与文献参考标签之间具有显著一致性。

Conclusion: 本体引导的知识表示能够有效提升LLM辅助LPBF缺陷分析的一致性、可解释性和实际可用性，为制造领域的知识驱动决策支持提供了可行范式。
...
### [258] [Virtual Speech Therapist: A Clinician-in-the-Loop AI Speech Therapy Agent for Personalized and Supervised Therapy](https://arxiv.org/abs/2605.01101)
*Shakeel Sheikh,Patrick Marmaroli,MD Sahidullah,Slim Ouni,Fabrice Hirsch,Goncalo Leal,Bjorn W Schuller*

Main category: cs.AI

TL;DR: 本文提出了虚拟言语治疗师（VST），一个基于智能体的平台，通过自动化、自适应的AI工作流程简化口吃评估并提供个性化治疗方案。VST整合了深度学习口吃分类与多智能体大语言模型推理，由专家言语治疗师评估证实能持续生成高质量、循证的治疗建议。


<details>
  <summary>Details</summary>
Motivation: 传统口吃评估和个性化治疗方案制定依赖人工，流程繁琐且临床负担重。开发自动化、智能化的平台有助于减轻临床医生工作负担，提升治疗效果，使循证医学决策更高效。

Method: VST平台包含三个核心模块：1）患者语音样本采集与特征提取；2）基于深度学习的口吃类型分类；3）多智能体LLM推理过程：包括生成智能体、批评智能体和迭代优化机制，通过批评智能体评估方案的安全性、方法学合理性和循证一致性。采用"临床医生在环"范式，最终方案需经临床医生审查确认。

Result: 专家言语治疗师的实验评估证实VST能够持续生成高质量、循证的治疗建议，验证了系统在增强临床工作流程、减轻临床负担和改善言语障碍患者治疗结果方面的潜力。

Conclusion: VST通过深度学习和多智能体LLM推理的有机结合，成功构建了端到端的口吃评估与个性化治疗规划平台，在保持临床医生监督的同时实现了流程自动化，具有显著的临床应用价值。
...
### [259] [Towards Multi-Agent Autonomous Reasoning in Hydrodynamics](https://arxiv.org/abs/2605.01102)
*Jinpai Zhao,Albert Cerrone,Joannes Westerink,Clint Dawson*

Main category: cs.AI

TL;DR: 本文提出了一种基于层执行图（LEG）的多智能体系统（MAS）原型，用于水动力学研究。该系统通过规划器智能体构建查询特定的执行拓扑，整合专业智能体协同工作，在37个查询上达到93.6%的事实精度和100%通过率，有效缓解了单智能体架构的上下文饱和瓶颈。


<details>
  <summary>Details</summary>
Motivation: 单智能体系统（SAS）在LLM驱动的科学工作流中面临上下文窗口饱和问题：随着工具规范和观测轨迹积累，每次决策的有效上下文减少，端到端可靠性下降。

Method: 提出多智能体系统原型，核心是层执行图（LEG）：规划器智能体利用自然语言路由启发式构建查询特定执行拓扑；专业智能体在严格工具允许列表和互补数据类角色下运行；层间整合智能体融合并行输出为简洁简报；报告智能体合成最终响应；运行时记录每次工具调用的来源以支持可审计性。所有测试使用Claude Sonnet 4.6作为骨干模型。

Result: 在37个查询（6个复杂度类别）上达到93.6%事实精度和100%通过率；单线程到五条独立并行轨道的准确率均保持在90%以上；在模拟单个数据源丢失时系统优雅降级，仍能返回实质性部分答案。
...
### [260] [New Bounds for Zarankiewicz Numbers via Reinforced LLM Evolutionary Search](https://arxiv.org/abs/2605.01120)
*Jay Bhan,Nicole Nobili,Srinivasan Raghuraman,Patrick Langer*

Main category: cs.AI

TL;DR: 本文利用基于大语言模型的进化算法OpenEvolve，首次精确确定了三个Zarankiewicz数（Z(11,21,3,3)=116, Z(11,22,3,3)=121, Z(12,22,3,3)=132），并为另外41个参数建立了下界（其中多个与已知上界仅差一条边）。该方法成本极低（每个参数<30美元），展示了LLM引导进化搜索在组合数学构造中的潜力。


<details>
  <summary>Details</summary>
Motivation: Zarankiewicz数（禁止完全二分图K_{s,t}的最大边数）是极值图论中的经典问题，其精确值在大多数参数下仍未知。本文旨在利用LLM驱动的进化算法探索新的极值图构造，以确定或逼近更多Zarankiewicz数的精确值。

Method: 使用OpenEvolve——一个基于大语言模型的开源进化算法，通过针对该问题定制的奖励信号迭代优化数学构造生成算法，从而搜索高边数的无K_{s,t}二分图。

Result: 首次精确确定三个Zarankiewicz数：Z(11,21,3,3)=116, Z(11,22,3,3)=121, Z(12,22,3,3)=132；为41个参数建立下界（多个接近已知上界）；在4个已关闭案例中匹配已知值；每个参数成本低于30美元。

Conclusion: LLM引导的进化搜索是一种低成本、可复现且易使用的组合发现工具，能够为极值图论问题提供新的构造，具有推广到其他数学研究领域的潜力。
...
### [261] [Iterative Finetuning is Mostly Idempotent](https://arxiv.org/abs/2605.01130)
*Zephaniah Roe,Jack Sanderson,Dang Nguyen,Julian Huang,Todd Nief,Aryan Shrivastava,Chenhao Tan,Ari Holtzman*

Main category: cs.AI

TL;DR: 本文研究了模型在其自身输出上训练时，行为倾向（如谄媚或失配）是否会在下一代模型中被放大。实验涵盖SFT、SDF和DPO三种训练方式，发现SFT和SDF中倾向大多衰减或不变，仅在罕见情况下放大但以牺牲连贯性为代价；DPO中倾向放大可稳定发生，但重置模型初始化后消失。结果表明放大主要源于持续后训练，且放大与连贯性之间存在自然权衡。


<details>
  <summary>Details</summary>
Motivation: 探究当模型在其自身输出数据上训练时，已有的行为倾向（如谄媚、不对齐）是否会在后续世代中被放大，即自我反馈循环是否会导致倾向强化。

Method: 训练一系列模型，每个模型在其前驱模型生成的数据上进行微调，初始模型注入特定人格或信念。测试三种设置：对指令模型的监督微调（SFT）、对基础模型的合成文档微调（SDF）、直接偏好优化（DPO）。

Result: SFT和SDF中倾向大多衰减或保持恒定，罕见放大但伴随连贯性下降；DPO中当模型持续用自身输出偏好训练时倾向放大可靠发生，但每轮重置初始化后消失。放大主要来自持续后训练，非RL微调中放大罕见且对数据量高度敏感。

Conclusion: 倾向放大主要源于持续后训练阶段，限制该阶段是有效防御手段；放大与连贯性之间存在自然权衡，可作为内在抑制机制。
...
### [262] [To Use AI as Dice of Possibilities with Timing Computation](https://arxiv.org/abs/2605.01134)
*Jia Li,Vipin Kumar,Rui Zhang*

Main category: cs.AI

TL;DR: 本文提出基于动词的建模范式，替代传统名词主导范式，引入时间计算与可能性的精确定义。在3,276名乳腺癌患者纵向EHR数据上，该方法实现了临床重要患者轨迹的自动发现和反事实时间推演，两项结果均为纯数据驱动、无需领域知识，是机器学习领域的首次展示。


<details>
  <summary>Details</summary>
Motivation: 传统基于名词的建模范式将未来视为封闭维度，无法充分表征时间的开放性，制约了AI发展。需要一种新范式来使AI成为实现思维语法的有效工具。

Method: 提出基于动词的范式，定义时间计算（timing computation）与可能性（possibility）的概念。应用在3,276名乳腺癌患者的纵向电子健康记录（EHR）数据上。

Result: (1) 自动发现临床重要的患者轨迹；(2) 反事实时间推演（counterfactual timing deduction）。两项结果均为纯数据驱动，无需先验领域知识。

Conclusion: 基于动词的范式能够突破传统名词范式的局限，在临床数据上实现了轨迹发现与反事实时间推理，标志着机器学习在该方向上的首次成功展示。
...
### [263] [Position: Safety and Fairness in Agentic AI Depend on Interaction Topology, Not on Model Scale or Alignment](https://arxiv.org/abs/2605.01147)
*Tanav Singh Bajaj,Nikhil Singh,Karan Anand,Eishkaran Singh*

Main category: cs.AI

TL;DR: 本文论证了多智能体AI系统的安全性主要由交互拓扑结构而非单个模型权重决定，揭示了顺序推理、并行投票等交互方式会引发排序不稳定、信息级联和功能崩溃等系统性病理现象，并提出应将交互拓扑作为安全评估和监管的主要目标。


<details>
  <summary>Details</summary>
Motivation: 挑战AI安全领域关于"个体模型安全属性会组合为安全的多智能体行为"的普遍假设，指出该假设存在根本性错误。

Method: 通过跨模型族和跨规模的实证分析，识别出三种由拓扑驱动的病理现象：排序不稳定、信息级联和功能崩溃，并分析模型规模扩大如何强化这些效应。

Result: 发现交互拓扑（而非模型权重）主导多智能体系统的安全性；更强的模型反而会通过增强共识形成和降低初始决策挑战来强化拓扑带来的安全性问题；这些失效模式对模型中心的评估和对齐程序不可见。

Conclusion: 智能体AI应被视作动力系统而非对齐组件的集合；交互拓扑必须成为安全评估和监管的主要目标，系统需在部署前展示跨架构变体的鲁棒性。
...
### [264] [Arithmetic in the Wild: Llama uses Base-10 Addition to Reason About Cyclic Concepts](https://arxiv.org/abs/2605.01148)
...
TL;DR: Llama-3.1-8B在处理循环概念推理时，并未直接使用概念自身的循环结构（如月份的模12加法），而是复用了一种通用的基10加法机制：先计算两个输入的数值和（如6+8月=14），再将结果映射回循环概念空间（14→2月）。该模型使用任务无关的傅里叶特征（周期为2、5、10等基10加法周期）来计算和，而非循环概念的周期（如12）。此外，在第18层MLP中识别出28个稀疏神经元（约占0.2%），可划分为不同簇，每个簇负责计算不同周期傅里叶特征的和。


<details>
  <summary>Details</summary>
Motivation: 探究语言模型中的结构表示是否必然意味着结构计算——即当模型对循环概念具有循环结构表示时，其内部计算是否也直接采用对应的循环计算方式。

Method: 通过因果抽象分析方法，研究Llama-3.1-8B在循环概念推理（如月份偏移计算）中的内部机制，识别其所用的傅里叶特征周期，以及参与计算的特定MLP神经元。

Result: 模型并未使用与概念表示周期一致的计算（如模12加法），而是复用通用的基10加法机制；傅里叶特征周期对应基10加法（2、5、10）而非概念周期（12）；仅约0.2%的MLP神经元（28个）参与了此计算。

Conclusion: 表示的结构性与计算的结构性可以分离，揭示了因果抽象与特征几何的相互作用如何深化对语言模型机制的理解。
...
### [265] [LLMs Should Not Yet Be Credited with Decision Explanation](https://arxiv.org/abs/2605.01164)
*Wenshuo Wang*

Main category: cs.AI

TL;DR: 本文论证LLM目前不应被赋予决策解释的资格，因为现有证据仅支持预测和合理化生成，而非真正的解释。作者提出了一套评估标准（明确解释目标、区分弱替代假设、过程/干预敏感验证、限定范围）和信用校准原则，旨在防止过早地将LLM视为人类决策的解释模型。


<details>
  <summary>Details</summary>
Motivation: 近期研究越来越多地将准确的行为预测、看似合理的理由和基于结果的推理链视为LLM能够解释人类决策的证据，这可能导致对人类决策建模中"解释性进展"的过早重新定义，存在风险。

Method: 概念区分：将决策预测、理由生成和决策解释三个主张区分开；论证分析：指出现有证据主要支持前两者而非真正的解释；提出桥接标准：四个条件用于判断是否应赋予解释信用；信用校准原则：模型应被赋予其证据所能支持的最强主张，而非更强的主张。

Result: LLM在决策预测、理由生成和解释性假设生成方面具有价值，但现有证据无法区分真正的决策解释与支持预测的合理化（rationalization）。提出的桥接标准包括：明确解释目标、区分弱替代假设、使用过程敏感或干预敏感的验证方法、限定适用范围。

Conclusion: 应采用信用校准原则：LLM应被赋予其证据所能支持的最强主张，而非更强的主张。这一原则有助于将LLM从有说服力的决策叙述者转变为发现、测试和沟通人类行为解释的更可靠工具。
...
### [266] [NEURON: A Neuro-symbolic System for Grounded Clinical Explainability](https://arxiv.org/abs/2605.01189)
*Anuradha Chandrasekaran,Dimitrios Zikos,Mutlu Mete,Alan Pang,Brady D. Lund,Kewei Sha*

Main category: cs.AI

TL;DR: NEURON是一个神经符号系统，结合SNOMED CT本体知识、机器学习模型和RAG驱动的LLM层，为急性心力衰竭死亡预测提供可解释的临床AI，在MIMIC-IV数据集上将AUC从0.74-0.77提升至0.84-0.88，并在人类对齐指标上显著优于原始SHAP可视化（0.85 vs. 0.50）。


<details>
  <summary>Details</summary>
Motivation: 临床AI采用受阻于高性能模型的黑箱/灰箱性质，这些模型缺乏本体论基础和叙事透明度，无法提供专业级可解释性。

Method: NEURON系统：1) 集成SNOMED CT本体知识的结构化表示与机器学习模型；2) 利用基于RAG的LLM层，将SHAP特征归因与患者临床笔记合成为连贯的自然语言解释。

Result: 在MIMIC-IV急性心力衰竭死亡预测任务中，AUC从0.74-0.77提升至0.84-0.88；人类对齐指标上显著优于原始SHAP可视化（0.85 vs. 0.50）。

Conclusion: NEURON为部署可信、以人为中心的互联健康应用提供了鲁棒、可扩展的工程解决方案。
...
### [267] [GR-Ben: A General Reasoning Benchmark for Evaluating Process Reward Models](https://arxiv.org/abs/2605.01203)
...
TL;DR: GR-Ben是一个用于评估过程奖励模型(PRM)在科学和逻辑两大推理领域（含9个子领域）中过程级错误检测能力的基准。实验发现：现有PRM和LLM在数学以外的领域错误检测能力明显较弱；PRM难以识别知识性错误，而LLM在计算错误检测上表现更差。


<details>
  <summary>Details</summary>
Motivation: 现有PRM基准主要聚焦数学推理，无法全面评估PRM在多样化推理场景下的错误检测能力，因此需要构建覆盖更广泛推理领域的基准。

Method: 构建了GR-Ben基准，涵盖科学和逻辑两个主要推理领域及9个子领域，在22个模型（包括PRM和LLM）上进行大规模实验评估。

Result: (1) 在数学推理之外的领域，现有PRM和LLM的错误检测能力显著较弱；(2) PRM更难识别知识性错误，LLM在检测计算错误方面表现更差。

Conclusion: GR-Ben可促进通用领域PRM的研究，进而提升LLM的推理能力。
...
### [268] [Faithful Mobile GUI Agents with Guided Advantage Estimator](https://arxiv.org/abs/2605.01208)
*Haowen Hu,Pengzhou Cheng,Zheng Wu,Lingzhong Dong,Gongshen Liu,Zhuosheng Zhang*

Main category: cs.AI

TL;DR: 提出Faithful-Agent框架，通过两阶段训练（SFT+RFT）和引导优势估计器（GuAE）提升GUI智能体的忠实度，在陷阱成功率上从13.88%提升至80.21%。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型驱动的GUI智能体存在不忠实行为，依赖记忆捷径而非屏幕证据或用户指令进行交互。

Method: 采用两阶段流水线：(i) 忠实导向的SFT阶段，通过证据扰动注入培养拒绝行为；(ii) RFT阶段，引入引导优势估计器（GuAE），一种基于锚点和方差自适应的优势调整机制（基于GRPO），结合思维-行动一致性奖励，防止稀疏GUI奖励下低方差rollout组中的优势坍缩。

Result: 陷阱成功率（Trap SR）从基线13.88%提升至80.21%，同时保持稳健的通用指令遵循性能。

Conclusion: Faithful-Agent框架有效提升了GUI智能体的忠实度，使其更依赖屏幕证据和用户指令，而非记忆捷径。
...
### [269] [Zero-Shot Signal Temporal Logic Planning with Disjunctive Branch Selection in Dynamic Semantic Maps](https://arxiv.org/abs/2605.01222)
*Bowen Ye,Ancheng Hou,Junyue Huang,Ruijia Liu,Xiang Yin*

Main category: cs.AI

TL;DR: 提出了一种基于Transformer架构和传递强化学习的零样本STL规划求解器，能够在无需重新训练的情况下处理可变地图环境中的复杂STL任务规范，并在动态语义地图上展现了优异的零样本泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有STL规划方法存在瓶颈：基于精确优化的方法速度过慢，而基于学习的方法难以在不同环境中泛化。因此需要一种兼具高效性和强泛化能力的STL规划求解器。

Method: 将地图条件Transformer架构与轻量级启发式方法相结合以处理复杂的析取（OR）子公式，并利用传递强化学习（TRL）确保分解后的子任务在时间基础（temporal grounding）和逻辑连贯性上保持一致。

Result: 在包含多样化障碍物布局的动态语义地图上进行的实验表明，该方法在不同环境变化和广泛的STL覆盖范围下均实现了持续的性能提升，展现了优越的零样本泛化能力。

Conclusion: 所提出的框架能够在无需重新训练的情况下，通过零样本方式生成可行轨迹，有效解决了STL规划在可变环境中的泛化难题。
...
### [270] [EO-Gym: A Multimodal, Interactive Environment for Earth Observation Agents](https://arxiv.org/abs/2605.01250)
*Sai Ma,Zhuang Li,Sichao Li,Xinyue Xu,Ruibiao Zhu,Tony Boston,John A. Taylor*

Main category: cs.AI

TL;DR: 本文提出EO-Gym，一个面向地球观测（EO）交互式智能体的可控执行框架，包含66万+多模态文件、35个专业化工具和9078条轨迹的基准数据集EO-Gym-Data。评测发现主流VLM在交互式EO推理上表现不佳，而微调后的EO-Gym-4B（基于Qwen3-VL-4B）在Pass@3指标上从0.49提升至0.74。


<details>
  <summary>Details</summary>
Motivation: 现有EO基准大多将分析过程简化为固定输入的单轮任务，无法支持交互式、多模态、工具使用的地球观测分析，缺乏可复现的交互式评估环境。

Method: 构建Gymnasium风格的本地地理空间工作台，集成66万+多模态文件（按位置/时间/传感器索引）和35个EO专用工具（涵盖6类任务）；基于8个公开EO数据集及Landsat/Sentinel-2影像生成9078条轨迹、34604步推理的基准；通过微调Qwen3-VL-4B得到4B参数基线模型。

Result: 评估10个开源/闭源VLM，显示通用模型在时序和跨模态工作流上表现不佳；EO-Gym-4B将Pass@3从0.49提升至0.74。

Conclusion: EO-Gym为交互式EO智能体提供了可复现的评估环境，将EO分析形式化为需要跨地理、时间和传感模态规划的取证问题。
...
TL;DR: Valley3是一个面向全球电商任务的全模态大语言模型，通过四阶段持续预训练管道逐步获得音频理解、跨模态指令跟随、电商领域知识和长上下文推理能力，并引入可控推理模式和智能搜索能力，在电商基准测试上优于强基线模型。


<details>
  <summary>Details</summary>
Motivation: 针对全球电商场景中文本、图像、视频和音频等多模态理解与推理需求，特别是短视频场景下的音频-视觉跨模态任务，现有视觉语言模型缺乏原生多语言音频能力，亟需一个统一的全模态电商模型。

Method: 设计四阶段全模态电商持续预训练管道（音频理解→跨模态指令跟随→电商领域知识→长上下文推理），后训练引入可控推理模式（非思考模式+三级思考），并装备智能搜索工具调用能力以支持深度研究任务。

Result: 在涵盖6个任务的电商全模态基准测试中，Valley3在内部和开源电商基准上持续优于强基线模型，同时在通用领域基准上保持竞争力。

Conclusion: Valley3通过渐进式预训练、可控推理后训练和智能搜索增强，成功构建了面向全球电商的先进全模态大语言模型，兼顾了简单场景的效率与复杂场景的深度推理能力。
...
TL;DR: 提出了神经符号技能归纳（NSI）框架，通过将交互轨迹提升为模块化的逻辑驱动程序，使智能体能够发现何时及为何行动，在长期规划任务中优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 基于基础模型的智能体在长期规划中表现不佳，因为纯提示推理具有瞬时性；现有技能归纳方法虽能提取经验，但生成的状态盲参数化脚本缺乏条件逻辑，无法在动态环境中稳健执行。

Method: 提出神经符号技能归纳（NSI）框架，将交互轨迹转化为模块化的逻辑驱动程序，合成显式控制流和动态变量绑定，使智能体发现何时（when）及为何（why）行动。

Result: 在一系列智能体任务上，NSI持续优于最先进的基线方法。

Conclusion: NSI使智能体能够从小样本示例中归纳技能，灵活适应未见目标，实现自我进化，成为逻辑驱动技能的构建者。
...
TL;DR: 本文提出SAPO（Segment-Aligned Policy Optimization），一种面向大语言模型推理过程的段级强化学习范式，将推理步骤作为策略更新的基本单元，在多个推理基准上相比token级和序列级方法取得更优性能与训练稳定性。


<details>
  <summary>Details</summary>
Motivation: 现有LLM强化学习方法以单个token或整个响应序列为粒度进行策略优化，与推理过程的自然步骤结构不一致，导致多模态推理任务中信用分配不佳和训练不稳定。

Method: 提出段级对齐策略优化（SAPO），以推理段（reasoning segments）为基本单元进行策略更新；引入基于推理段的马尔可夫决策过程抽象，以及段级价值估计、优势计算和重要性采样机制，使其与推理边界语义对齐。

Result: 在代表性推理基准上，SAPO一致优于token级和序列级策略优化方法，显著提升准确率，同时表现出更好的训练稳定性和价值估计一致性。

Conclusion: 将强化学习更新与推理的内在结构对齐至关重要，为复杂推理任务中更高效、语义更合理的策略优化开辟了新路径。
...
TL;DR: 本文提出Truth or Tribe模拟框架，研究人格化智能体在面对矛盾信息（如 misinformation）时的内群体偏爱现象。通过三元交互范式的实验发现，智能体更倾向于接受来自身份相似同伴的错误答案，且这种偏好在认知复杂度增加时加剧。作者提出了三种缓解策略：身份盲指令、结构化反事实推理和异质视角集成。


<details>
  <summary>Details</summary>
Motivation: 现有研究已发现生成式语言模型存在内群体偏爱，但尚未探究当人格化智能体面对矛盾信息（如错误信息）时，内群体偏爱是否仍然存在，以及如何缓解其负面影响。

Method: 提出Truth or Tribe模拟框架，采用三元交互范式（triadic interaction paradigm）研究智能体在矛盾信息传播中的合作行为，并通过对照实验评估主要调节因素。提出三种干预策略。

Result: 人格化智能体表现出强烈的内群体偏爱，接受身份相似同伴错误答案的比率显著高于不相似同伴；在可废止推理（defeasible reasoning）情境中该偏爱仍然存在，且随认知复杂度增加而加剧。

Conclusion: 人格化智能体存在顽固的内群体偏爱偏差，但可通过身份盲指令、结构化反事实推理和异质视角集成三种策略有效缓解。
...
TL;DR: 本文提出DiagramNet——首个面向系统级图的多模态数据集，并设计解耦多智能体工作流（感知-推理-知识三阶段）和渐进式训练流程。3B参数模型结合该工作流在DiagramNet基准上超越EDA竞赛冠军及GPT-5、Claude-Sonnet-4等模型2倍以上，且工作流可泛化提升其他模型性能（Gemini-2.5-Pro提升128.7倍），在AMSBench上零样本连接推理达到SOTA。


<details>
  <summary>Details</summary>
Motivation: 系统级图存在非标准化符号和结构化训练数据稀缺的问题，导致现有多模态大语言模型难以识别这些图表。

Method: 提出DiagramNet多模态数据集（含10,977个连接标注和15,515个思维链QA对）；设计渐进式训练流程和解耦多智能体工作流（分为感知、推理、知识三阶段）。

Result: 3B参数模型结合工作流在DiagramNet上超越EDA竞赛冠军及GPT-5等模型2倍以上；工作流使Gemini-2.5-Pro任务1性能提升128.7倍；仅用60张图像即可迁移至AMSBench，零样本连接推理达到与GPT-5相当水平且超越Netlistify。

Conclusion: 该工作流具有通用性和可迁移性，能够有效提升不同模型在系统级图理解任务上的性能。
...
### [276] [A Cellular Doctrine of Morality: Intrinsic Active Precision and the Mind-Reality Overload Dilemma](https://arxiv.org/abs/2605.01376)
*Ahsan Adeel*

Main category: cs.AI

TL;DR: 当前基于简化神经科学的AI系统通过最大化奖励来放大信息注意力，缺乏内在的精确性机制来评估信息的真实性和价值，导致信息过载和偏见累积，即"心智-现实过载困境"。该论文提出通过构建基于锥体神经元生物物理动力学（利用局部和全局一致性预测进行证据有效性评估）的AI工具来缓解这一威胁，从而提升AI的"真实理解"能力，恢复认知生态条件。


<details>
  <summary>Details</summary>
Motivation: 现有AI系统缺乏内在的精确性机制来区分真实与虚假信息，仅通过最大化注意力奖励来运作，导致信息过载和偏见不断放大，威胁个体和社会的理性决策能力。

Method: 借鉴锥体神经元在清醒思维和高级认知中的生物物理动力学，利用局部和全局一致性预测作为内在主动精确性机制，在注意力分配和信息传播之前评估证据的有效性和上下文 adequacy（充分性）。

Result: 该方法有望减少信息过载、放大可靠信息，使AI具备更接近"真实理解"的能力，支持形成更充分 informed 的信念和更连贯的判断。

Conclusion: 基于生物物理动力学的AI设计路径可能有助于缓解心智-现实过载困境，恢复认知生态条件，但无法保证绝对成功。
...
### [277] [AI Safety as Control of Irreversibility: A Systems Framework for Decision-Energy and Sovereignty Boundaries](https://arxiv.org/abs/2605.01415)
*Wesley Shu,Peng Wei*

Main category: cs.AI

TL;DR: 本文提出“部署摩擦下降”从根本上改变了AI安全问题的性质，通过“决策能量密度”形式化分析，指出AI安全的关键不是局部正确性，而是通过制度和技术设计防止不可逆权力集中在单一高效率节点上。


<details>
  <summary>Details</summary>
Motivation: 传统高风险技术受限于资本密度、物理瓶颈、组织惯性和专业供应链，能力增长与部署之间存在缓冲；但AI能力可以低成本复制、调用、嵌入工作流并跨机构扩展，这一“部署摩擦下降”从根本上改变了安全问题的性质。

Method: 引入“决策能量密度”概念（节点生成、评估、选择和执行后果性决策的速率加权容量），识别三条主权边界（不可逆决策权、物理资源配置权、自我扩展权），并通过边界稳定定理证明安全需要防止不可逆权力由单一高效率节点释放。

Result: 边界稳定定理：安全不需要证明高级系统总是正确的，而是需要通过制度和技术设计，防止不可逆权力被单一高效率节点释放。效率压力、路径依赖、规模反馈和弱边界约束会将决策能量集中在最高效节点，扩散责任并增加不可逆系统级损失的概率。

Conclusion: AI安全应重新定义为分层控制、授权和外部可审查限制，连接对齐、安全工程、组织经济学和制度设计，以在决策密度上升的背景下控制不可逆性。
...
### [278] [TimeTok: Granularity-Controllable Time-Series Generation via Hierarchical Tokenization](https://arxiv.org/abs/2605.01418)
*Seokhyun Lee,Jaeho Kim,Changjun Oh,Mihaela van der Schaar,Changhee Lee*

Main category: cs.AI

TL;DR: TimeTok是一个统一的框架，通过分层分词策略实现时间序列的粒度可控生成（GC-TSG），支持从粗糙输入或零开始生成任意目标粒度的时间序列，在标准生成任务上达到SOTA，并展现出跨数据集的强迁移能力。


<details>
  <summary>Details</summary>
Motivation: 现有时间序列生成模型缺乏对时间粒度的控制，用户只能接受模型产生的固定粒度输出，无法根据需求灵活调整生成细节。

Method: 提出TimeTok框架，核心是分层分词策略，将时间序列映射为从粗到细的有序token序列；通过跨粒度层次的自回归生成过程产生token块，再解码为连续时间序列；控制token块数量即可实现输出粒度的显式控制。

Result: TimeTok在GC-TSG任务上表现优异，在标准生成任务上达到SOTA；在异构时间粒度的多数据集上训练展现出强迁移能力，优于单数据集训练模型。

Conclusion: 这是首个覆盖时间序列完整生成谱系的统一框架，为受益于多样化时间粒度的模型提供了有价值的基座。
...
### [279] [Artificial Jagged Intelligence as Uneven Optimization Energy Allocation Capability Concentration, Redistribution, and Optimization Governance](https://arxiv.org/abs/2605.01420)
*Wesley Shu,Peng Wei*

Main category: cs.AI

TL;DR: 本文提出"人工锯齿智能"(AJI)理论，将大模型能力分布不均建模为有限优化预算下梯度更新能量在各能力方向上的非均匀分配，并证明优化压力的持续集中会导致能力增益的离散度下界，揭示了窄目标缩放无法消除各向异性，而辅助目标可以恢复被忽视的能力。


<details>
  <summary>Details</summary>
Motivation: 现有研究将大模型能力分布不均归因于单一的"智能"标量，缺乏对能力锯齿状分布的正式理论解释。本文旨在建立AJI的形式化理论，将不均匀能力分布归因于优化压力的非均匀分配，而非一个标量化的智能概念。

Method: 将训练建模为有限预算过程，梯度驱动的更新能量在参数空间的能力相关方向上进行分配。定义了能力增益、优化能量份额和锯齿度三个核心概念；利用有限预算权衡定理证明优先发展一种能力会对其他能力产生机会成本（除非存在正耦合或共享结构）；提出能量方差正则化和辅助结构目标作为重塑优化场的干预机制。

Result: 证明了持续累积更新能量的集中会导致能力增益离散度的下界；有限预算权衡定理表明优先发展一种能力会对他者产生机会成本；预测早期能量集中可预示后期能力锯齿度；窄目标缩放无法消除各向异性；明确资助的辅助目标可恢复被忽视的能力。

Conclusion: AJI不仅是描述性的标签，而是一个可检验的理论——解释了有限优化资源如何产生集中的、延迟的、结构上不均匀的能力形成过程，为理解大模型能力分布不均提供了形式化框架和优化治理方向。
...
### [280] [Rethinking Explanations: Formalizing Contrast in Description Logics](https://arxiv.org/abs/2605.01442)
*Yasir Mahmood,Arnab Sharma,Axel-Cyrille Ngonga Ngomo,Balram Tiwari*

Main category: cs.AI

TL;DR: 本文提出了基于描述逻辑知识库的对比解释框架，旨在回答“为什么公理P为真而非公理Q”的问题，通过对比事实与反事实（ foil）来提供更符合用户认知需求的解释。研究给出了形式化定义，在EL和ALC描述逻辑中探索了对比解释的性质，并进行了实现和实验评估。


<details>
  <summary>Details</summary>
Motivation: 现有解释形式（如justifications和溯因推理）仅指出（缺失）蕴涵背后的推理步骤，缺乏以用户为中心的方法，未考虑询问者的需求、理解水平或先验知识。当用户发现P发生时，他们常因预期另一个类似事件Q的发生而感到惊讶，且单独的“为什么P”和“为什么不Q”的解释不令人满意，因为用户期望看到P与Q之间的差异。

Method: 首先提出对比性问题（contrastive questions）的形式化基础，然后在描述逻辑中定义对比解释（contrastive explanations）。事实包括ABox断言C(x)（概念C与个体x），可能的反事实（foil）包括C(y)（针对不同个体y）或D(x)（针对不同概念D）。在描述逻辑EL和ALC中探索对比解释的性质，并给出实现和实验评估。

Result: 给出了对比解释的形式化定义，并在EL和ALC描述逻辑中分析了其性质，基于定义实现了系统并在不同规模的知识库上进行了实验评估。

Conclusion: 对比解释提供了一种更符合用户认知需求的解释方式，通过对比事实与反事实的差异来回答用户对意外推断的疑问，是对现有DL解释形式（justifications和溯因推理）的重要补充。
...
### [281] [Grounding Multi-Hop Reasoning in Structural Causal Models via Group Relative Policy Optimization](https://arxiv.org/abs/2605.01482)
...
TL;DR: 提出SCM-GRPO框架，将事实验证建模为结构因果模型上的因果推理过程，并利用基于规则的强化学习（GRPO）优化推理链长度与准确率的权衡，在HoVer和EX-FEVER上超越SOTA。


<details>
  <summary>Details</summary>
Motivation: 现有方法虽引入思维链提升可解释性，但未显式建模证据与声明之间的因果依赖关系，且LLM存在幻觉和逻辑链断裂问题。

Method: 将推理过程建立在结构因果模型（SCM）上，将其视为构造性因果推理；发现推理链长度与准确率呈倒U型关系；提出基于规则的GRPO强化学习策略动态优化结构深度与简洁性之间的权衡。

Result: 在HoVer和EX-FEVER数据集上显著优于现有SOTA基线方法。
...
### [282] [MAP-Law: Coverage-Driven Retrieval Control for Multi-Turn Legal Consultation](https://arxiv.org/abs/2605.01486)
*Qinchuan Cheng,Ruixuan Xie,Jiaqi Liu,Xiaoya Yuan,Yuxin Liu*

Main category: cs.AI

TL;DR: MAP-Law提出了一种覆盖度驱动的检索控制框架，通过联合结构化状态（问题节点、法律要素节点、证据节点）和覆盖率信号（要素覆盖率、证据覆盖率、边际增益）来决定多轮法律咨询中的检索停止、重定向或生成回复，在仅用2.9轮检索和5.8条证据的情况下达到0.860的要素覆盖率，比固定七轮基线减少80%以上的证据量和58%的检索轮次。


<details>
  <summary>Details</summary>
Motivation: 法律咨询是高风险的、知识密集型任务，现有RAG方法在多轮对话中依赖固定检索深度或粗粒度启发式控制，导致关键法律要素支持不足或过度检索增加上下文负担、削弱回答焦点。

Method: 提出MAP-Law框架，将法律咨询建模为对联合结构化状态（问题节点、法律要素节点、证据节点）的受控检索过程；每轮检索后计算要素覆盖率、证据覆盖率和边际增益，据此决策是否继续检索、重定向搜索或生成最终回答，将检索停止从固定超参数转变为符合法律论证结构的可解释、可审计的决策。

Result: 在自构建的涵盖8个劳动法场景的50个案例数据集上，MAP-Law（以DeepSeek作为动作选择器）平均使用2.9轮检索和5.8条证据即达到0.860的要素覆盖率；相比固定七轮基线，证据量减少80%以上，检索轮次减少58%；消融实验验证了覆盖度驱动停止、联合图表示和基于LLM的动作选择的独立贡献。

Conclusion: MAP-Law通过覆盖度驱动的受控检索策略，在显著降低检索成本的同时保持了高覆盖率，证明了将法律论证结构融入检索控制决策的有效性。
...
### [283] [SciResearcher: Scaling Deep Research Agents for Frontier Scientific Reasoning](https://arxiv.org/abs/2605.01489)
*Tianshi Zheng,Rui Wang,Xiyun Li,Yangqiu Song,Tianqing Fang*

Main category: cs.AI

TL;DR: 本文提出SciResearcher——一个面向前沿科学推理的全自动智能体框架，通过合成基于学术证据的概念与计算任务，并结合监督微调与智能体强化学习，训练出SciResearcher-8B模型，在HLE-Bio/Chem-Gold等基准上以8B参数量达到了超越多个更大规模专有智能体的SOTA性能（19.46%），并在其他基准上取得13-15%的绝对提升。


<details>
  <summary>Details</summary>
Motivation: 现有深度研究智能体通过知识图谱构建或迭代网页浏览来收集信息寻求任务数据进行后训练，但在前沿科学领域面临两大瓶颈：领域知识散落在稀疏异构的学术资源中，且问题解决需要远超事实检索的复杂计算与推理能力。因此需要一种能自动构建高质量前沿科学推理数据的新范式。

Method: 提出SciResearcher全自动智能体框架，包含：(1) 基于学术证据合成多样化概念与计算任务；(2) 激发信息获取、工具集成推理和长程规划能力；(3) 利用所构建数据进行监督微调（SFT）和智能体强化学习（Agentic RL），最终训练出SciResearcher-8B智能体基础模型。

Result: SciResearcher-8B在HLE-Bio/Chem-Gold基准上达到19.46%准确率，在8B参数量级上刷新SOTA，并超越多个更大的专有智能体；在SuperGPQA-Hard-Biology和TRQA-Literature基准上分别取得13-15%的绝对增益。

Conclusion: SciResearcher为前沿科学推理的自动化数据构建提供了新范式，展现了可扩展的通往未来科学智能体的路径。
...
### [284] [MILD: Mediator Agent System with Bidirectional Perception and Multi-Layered Alignment for Human-Vehicle Collaboration](https://arxiv.org/abs/2605.01507)
*Jiyao Wang,Yunbiao Wang,Yubo Jiao,Xiao Yang,Dengbo He,Sasan Jafarnejad,Luis Miranda-Moreno,Raphael Frank,Jiangbo Yu*

Main category: cs.AI

TL;DR: 本文提出中介者在环驾驶（MILD）系统，通过智能体架构实现人车协同驾驶。MILD整合感知智能体与轻量级策略智能体，并开发证据与约束加权策略优化（ECPO）确保策略合规、可解释。实验证明MILD在感知准确性和策略质量上优于基线方法。


<details>
  <summary>Details</summary>
Motivation: 部分自动驾驶增加了人类驾驶员的认知负担，根源在于车辆意图和决策逻辑不透明，以及自动化系统对驾驶员状态和偏好感知不足。这种双向不匹配破坏了共享态势感知，加剧了人车交互中的协调失败。

Method: 提出MILD系统，采用智能体系统架构，将人类角色从被动监督者提升为主动管理者。包含：(1) 感知智能体实现车内车外联合理解；(2) 轻量级策略智能体生成合规且可解释的动作建议；(3) ECPO算法利用自动验证器引导策略在准确、结构完整、有证据支撑且无约束违规的方向优化；(4) 检索增强生成模块动态纳入交通法规、速度建议和驾驶员偏好等约束。

Result: 在三个公开数据集上的现场实验表明，MILD在可审计离线指标上超越基线方法，在感知准确性和策略质量上均表现更优，并在人工评估中获得了更高的策略充分性、舒适度和可解释性评分。

Conclusion: 本文为构建可审计、对齐人类价值的人车协作智能体提供了可行路径。
...
### [285] [Multi-Agent Reasoning Improves Compute Efficiency: Pareto-Optimal Test-Time Scaling](https://arxiv.org/abs/2605.01566)
*Florian Valentin Wunderlich,Lars Benedikt Kaesberg,Jan Philip Wahle,Terry Ruas,Bela Gipp*

Main category: cs.AI

TL;DR: 本文系统分析了自一致性、自精炼、多智能体辩论和混合智能体四种推理扩展策略的计算性能权衡，在MMLU-Pro和BBH基准上进行了34种配置和超过100次评估。研究发现推理扩展在最高计算预算下可提升准确率最多7.1个百分点；在相同计算预算下，辩论和混合智能体分别优于自一致性1.3和2.7个百分点；混合智能体在并行生成数超过顺序聚合数时效率最高。


<details>
  <summary>Details</summary>
Motivation: 现有推理方法优先追求原始性能而忽视计算效率，但实际应用常面临资源约束，因此需要系统分析不同推理扩展策略的计算-性能权衡。

Method: 对四种推理扩展策略（自一致性、自精炼、多智能体辩论、混合智能体）进行系统分析，在不同模型规模和参数配置下，基于帕累托最优前沿评估方法在MMLU-Pro和BBH两个推理基准上的准确率与计算预算的权衡。

Result: 推理扩展在最高计算预算（CoT的20倍）下在MMLU-Pro上提升准确率最多7.1个百分点。相同计算预算下，辩论和混合智能体分别优于自一致性1.3和2.7个百分点。自一致性较早饱和，多智能体方法在复杂任务上持续获益。混合智能体在并行生成数超过顺序聚合数时效率最高。

Conclusion: 多智能体方法在计算效率上优于自一致性，混合智能体在特定配置下最具计算效率，为资源受限场景下的推理扩展提供了设计指导。
...
### [286] [CP-SynC: Multi-Agent Zero-Shot Constraint Modeling in MiniZinc with Synthesized Checkers](https://arxiv.org/abs/2605.01675)
*Yuliang Song,Eldan Cohen*

Main category: cs.AI

TL;DR: CP-SynC是一个多智能体工作流，通过并行探索多条建模轨迹、合成语义检查器进行反馈验证以及多智能体证据聚合，实现了零样本的MiniZinc约束建模，在100个CP问题基准测试上显著优于现有基线方法。


<details>
  <summary>Details</summary>
Motivation: 将自然语言问题描述自动转化为可执行的CP模型是一个重要瓶颈，现有LLM方法在缺乏oracle验证时难以捕捉细微的语义错误。

Method: 提出CP-SynC多智能体工作流：(1) 建模智能体生成和精炼候选模型；(2) 验证智能体合成语义检查器提供反馈；(3) 并行探索多条建模轨迹；(4) 选择智能体通过多智能体证据聚合选出最终模型。

Result: 在100个CP问题的基准测试上，CP-SynC在MiniZinc建模方面显著优于现有基线方法。

Conclusion: 多智能体协作与证据聚合能有效提升LLM在约束建模任务上的语义正确性，减少对oracle验证的依赖。
...
### [287] [Latent State Design for World Models under Sufficiency Constraints](https://arxiv.org/abs/2605.01694)
*Keon Woo Kim*

Main category: cs.AI

TL;DR: 本文提出将世界模型研究视为在充分性约束下的潜在状态设计问题，基于功能角色（预测嵌入、递归信念状态、对象/因果结构、潜在动作接口、规划接口、记忆基底）对方法进行分类，并建立评估框架，认为可操作的世界模型关键在于状态构建与任务匹配，而非信息保留最大化。


<details>
  <summary>Details</summary>
Motivation: 现有世界模型研究多按架构或应用领域分类，忽略了潜在状态设计背后的功能目标和充分性约束差异，导致预测充分性与控制充分性之间的混淆，以及被动视频预测与反事实动作建模之间的鸿沟。论文旨在通过功能导向的分类法来弥合这些差距。

Method: 提出功能分类法，将方法按潜在状态的功能角色分为六类；建立七轴评估框架（表征、预测、规划、可控性、因果/反事实支持、记忆、不确定性），用于诊断潜在状态的保留、丢弃和使能能力。

Result: 不同功能角色的潜在状态在七轴评估矩阵中展现出差异化特征；传统基于架构的分组无法揭示预测充分性与控制充分性之间的关键区别。

Conclusion: 可操作的世界模型取决于其状态构建是否与具体任务匹配，而非保留最多信息；潜在状态设计应遵循任务的充分性约束而非信息最大化原则。
...
### [288] [Model Routing as a Trust Problem: Route Receipts for Adaptive AI Systems](https://arxiv.org/abs/2605.01710)
*Vincent Schmalbach*

Main category: cs.AI

TL;DR: 本文提出"路由收据"概念，用于记录AI系统在响应请求时所经过的运行时路径（如版本别名、服务层级、工具选择、区域端点、回退规则等），以增强透明度。路由收据是一种紧凑的记录，捕获关键决策信息，但不暴露专有内部细节。


<details>
  <summary>Details</summary>
Motivation: 当前AI系统存在多层路由决策（版本、服务层级、工具、区域、安全处理等），这些路由变化会影响响应的成本、质量和问责性，但用户无法感知具体发生了什么路由变化。仅知道"哪个模型回答了"不足以满足审计需求，运行时路径同样重要。

Method: 提出路由收据（route receipt）的概念和最小化架构，包括紧凑记录格式和脱敏模型。通过对多个广泛使用的AI平台进行文档调查，验证路由收据的"碎片"已存在于现有平台中，但缺乏可移植的、逐答案的记录。

Result: 路由收据应作为模型文档的一部分，与模型卡（描述训练后的模型工件）互补——模型卡描述静态模型，路由收据描述特定答案产生的运行时条件。调查显示现有平台已有路由信息的碎片化存在，但缺少统一的逐答案可移植记录。

Conclusion: 自适应AI系统需要路由收据来确保透明度，建议将路由收据纳入模型文档体系，作为模型卡的运行时补充，以解决用户对路由决策的审计需求。
...
### [289] [Are LLMs More Skeptical of Entertainment News?](https://arxiv.org/abs/2605.01727)
*Huiqian Lai*

Main category: cs.AI

TL;DR: 大型语言模型在多类新闻体裁中并非公平地进行可信度评估，对娱乐新闻存在系统性误判，需引入体裁分层假阳性分析。


<details>
  <summary>Details</summary>
Motivation: 探究LLM在不同新闻体裁（硬新闻与娱乐新闻）中是否采用统一的评估标准，揭示聚合性能指标可能掩盖的结构性偏差。

Method: 基于FakeNewsNet中的GossipCop数据集，采用组内设计；对四个前沿模型进行零样本评估，计算假阳性率差距；进行风格交换实验、提示缓解实验及探索性定性编码。

Result: DeepSeek-V3.2和GPT-5.2分别存在10.1和8.8个百分点的假阳性率差距（p<.001），而Claude Opus 4.6和Gemini 3 Flash无显著差异；风格交换效果有限；提示缓解对DeepSeek有效但对GPT-5.2效果甚微；定性编码发现两种错误模式：将私生活主张视为不可验证，以及将娱乐新闻视为认知上较弱的体裁。

Conclusion: 基于LLM的可信度评估不仅评估真相主张，还会差异化地识别新闻体裁的合法性；评估应纳入体裁分层的假阳性分析，而非仅关注总体准确率。
...
### [290] [Catching the Infection Before It Spreads: Foresight-Guided Defense in Multi-Agent Systems](https://arxiv.org/abs/2605.01758)
*Yue Ma,Ziyuan Yang,Yi Zhang*

Main category: cs.AI

TL;DR: 提出了一种无需训练的**前导引导局部净化（FLP）框架**，通过让每个智能体模拟未来交互轨迹、采用多角色仿真策略检测感染，并利用递归二分诊断和相册回滚实现局部净化，将大规模多模态多智能体系统的最大累积感染率从95%以上降至5.47%以下。


<details>
  <summary>Details</summary>
Motivation: 大规模多模态多智能体系统存在**传染性越狱攻击**问题——单个智能体被攻破后可在系统中扩散。现有防御通过训练一个更易传播的"治愈因子"来对抗病毒对抗样本，但这会导致智能体响应同质化，仅实现表面抑制而非真正恢复。根本原因在于：传染性越狱源于局部交互行为，而现有防御通过全局共享治愈因子运作，这种**局部与全局的失配**限制了防御效果。

Method: 提出**无需训练的FLP框架**：(1) 每个智能体模拟未来多轮对话的行为轨迹；(2) 引入**多角色仿真策略**，在多种交互上下文中进行鲁棒预测以反映MAS的多样性；(3) 以**响应多样性**作为诊断信号，从检索-结果和语义两个层面分析不同角色预测间的不一致性来检测感染；(4) 对感染智能体进行局部净化：近期感染通过**即时相册回滚**缓解，长期感染通过**递归二分诊断（RBD）**递归划分图像相册并定位消除病毒对抗样本。

Result: FLP将最大累积感染率从95%以上降至5.47%以下，且检索和语义指标与良性基线高度接近，表明有效保持了交互多样性。

Conclusion: 相比全局治愈因子的"以毒攻毒"策略，FLP通过未来轨迹推理和局部净化从根本上解决传染性越狱问题，既实现了强防御效果又保留了系统的响应多样性。
...
### [291] [Runtime Evaluation of Procedural Content Generation in an Endless Runner Game Using Autonomous Agents](https://arxiv.org/abs/2605.01783)
*Rishabh Kar*

Main category: cs.AI

TL;DR: 本文提出Momentum无尽跑酷游戏，将运行时地形生成、物体放置与自主智能体评估整合到统一流程中，通过波函数坍缩约束机制生成内容，并利用空中扫描与地面遍历两个评估智能体实时检测可玩性问题，构建了涵盖可玩性、多样性、可控性和运行性能的评估框架。


<details>
  <summary>Details</summary>
Motivation: 程序化内容生成(PCG)虽然能自动生成游戏内容，但生成的内容可能存在不平衡、阻塞、重复或技术上无法解通等问题，且传统评估通常是离线进行的，缺乏运行时统一验证机制。

Method: 在Momentum游戏中实现运行时地形瓦片与环境物体动态生成，使用类波函数坍缩(WFC)的约束机制进行物体放置，异步重建导航表面；设计空中扫描与地面遍历两个自主评估智能体，结合射线投射、体积物理扫描、障碍层过滤和结构化崩溃报告进行联合评估。

Result: 沿PCG四大轴线（可玩性、多样性、可控性、运行性能）构建了可测量的评估框架，推导了生成器基于自身放置约束的结构饱和界，并从第一性原理量化了每个片段的智能体扫描成本。

Conclusion: 生成与验证可以在同一运行时循环中统一实现，无需将评估作为单独的离线流程处理，为实时PCG系统提供了实用的评估范式。
...
### [292] [DataEvolver: Let Your Data Build and Improve Itself via Goal-Driven Loop Agents](https://arxiv.org/abs/2605.01789)
...
TL;DR: DataEvolver 是一个闭环视觉数据引擎，通过显式目标追踪、生成自校正和验证自扩展的双循环机制，支持多类型视觉数据（RGB、掩码、深度图、法线图、网格、姿态、轨迹等）的迭代构建。在物体旋转编辑任务上，Ours+DualGate 模型相比未适配基线和公开多角度 LoRA 均有提升。


<details>
  <summary>Details</summary>
Motivation: 构建可控视觉数据是图像编辑和多模态理解的瓶颈，单次渲染难以产出有效监督信号，需要通过迭代生成、检查、修正、筛选和导出来实现。

Method: DataEvolver 采用闭环视觉数据引擎，围绕显式目标、持久化工件、受限修正操作和接受决策组织流程。核心是双循环机制：生成时自校正循环（样本内）和验证时自扩展循环（数据集轮次间）。支持 RGB 图像、掩码、深度图、法线图、网格、姿态、轨迹和审查痕迹等多种工件类型。

Result: 在图像级物体旋转任务上，使用固定 Qwen-Edit LoRA 探针，Ours+DualGate 模型在 SpatialEdit 和保留测试集上优于未适配基线和公开多角度 LoRA。消融实验显示从场景感知生成到反馈驱动校正再到双门控验证的持续改进路径。

Conclusion: 主要贡献是一个可复用的视觉数据集构建框架，通过显式目标追踪、审查、修正和接受循环来构建视觉数据集。
...
### [293] [NeuroState-Bench: A Human-Calibrated Benchmark for Commitment Integrity in LLM Agent Profiles](https://arxiv.org/abs/2605.01847)
*Jia Xiao*

Main category: cs.AI

TL;DR: NeuroState-Bench是一个人类校准的基准测试，通过基准定义的侧查询探针（而非推断的隐藏激活）来评估智能体在多头任务中的承诺完整性。包含144个确定性任务和306个探针，覆盖8个认知动机失败家族。实验表明任务成功与承诺完整性存在分歧：成功领导者并非完整性领导者，31/32个模型在替换评估指标后排名改变。


<details>
  <summary>Details</summary>
Motivation: 仅依赖结果评估无法充分说明评估的智能体配置是否保持了解决多头任务所需的承诺完整性，因此需要一种能够直接量化承诺完整性的评估方法。

Method: 提出NeuroState-Bench基准，通过基准定义的侧查询探针（非隐藏激活推断）操作化承诺完整性。包含144个确定性任务、306个探针（8个失败家族、干净/干扰变体、3个难度级别）。主评估包含32个配置（16个本地子集+16个托管大模型子集）。人类校准使用104个采样任务单元、216个原始标注、108个裁决任务行（加权kappa=0.977, ICC(2,1)=0.977）。提出HCCIS-CORE、HCCIS-FULL、HCCIS+N等评估指标。

Result: 任务成功与承诺完整性存在分歧：成功领导者不是完整性领导者，31/32配置在替换评估指标后排名改变，完整性排名受干扰扰动更稳定。HCCIS-CORE达到0.8469 AUC和0.6992 PR-AUC，HCCIS-FULL为0.7997 AUC和0.6410 PR-AUC。探针准确率和状态漂移在ROC-AUC（0.8587）上略高，但HCCIS-CORE在PR-AUC上显著更高且更贴近基准构建目标。

Conclusion: NeuroState-Bench提供了一个经校准的评估轴，能够在比原始仅本地子集更广泛的模型网格上暴露承诺失败问题。
...
### [294] [Sheaf-Theoretic Planning: A Categorical Foundation for Resilient Multi-Agent Autonomous Systems](https://arxiv.org/abs/2605.01879)
*Manuel Hernández,Eduardo Sánchez-Soto*

Main category: cs.AI

TL;DR: 本文提出层论规划（STP）框架，利用拓扑斯理论和层语义替代传统符号逻辑与封闭世界假设，为多智能体系统在随机与对抗环境中的协调问题提供了范畴论基础上的新范式。


<details>
  <summary>Details</summary>
Motivation: 传统多智能体系统依赖事件演算和情境演算等符号逻辑模型，受封闭世界假设限制，无法处理未观测的智能体干预、计划中断以及信念-现实状态分歧等问题，亟需一种更具鲁棒性的形式化框架。

Method: 基于拓扑斯理论和层语义的层论规划（STP）框架，利用范畴论工具重新定义多智能体协调的数学模型，以替代传统的封闭世界逻辑推理机制。

Result: STP框架在理论上克服了经典MAS框架在应对随机性与对抗性环境时的局限性，提供了更灵活的知识表征和推理能力。

Conclusion: 层论规划作为符号逻辑与控制理论的变革性替代方案，有望在未来的弹性自主系统中发挥关键作用。
...
### [295] [Disentangling Intent from Role: Adversarial Self-Play for Persona-Invariant Safety Alignment](https://arxiv.org/abs/2605.01899)
*Jiajia Li,Xiaoyu Wen,Zhongtian Ma,Shuyue Hu,Qiaosheng Zhang,Zhen Wang*

Main category: cs.AI

TL;DR: 本文提出人物不变对齐（PIA）框架，通过攻击侧的人物谱系演化（PLE）和防御侧的人物不变一致性学习（PICL）协同进化，利用单侧KL散度约束实现安全决策与人物上下文的解耦，有效降低基于人物角色的越狱攻击成功率，同时保持模型通用能力。


<details>
  <summary>Details</summary>
Motivation: 现有基于人物角色的越狱攻击研究主要聚焦攻击迭代，缺乏系统性的防御机制约束；当前安全对齐技术仍无法有效防御此类攻击。

Method: 提出PIA对抗自博弈框架：攻击端采用人物谱系演化（PLE）高效探索高风险人物空间；防御端采用人物不变一致性学习（PICL），基于结构分离假设，通过单侧KL散度约束实现安全决策与人物上下文的解耦。

Result: PLE能高效探索高风险人物空间；PICL显著降低攻击成功率（ASR），同时保持模型通用能力。

Conclusion: 该对齐范式在越狱防御上展现了优越性和鲁棒性，实现了安全性与通用能力的平衡。
...
### [296] [A Language for Describing Agentic LLM Contexts](https://arxiv.org/abs/2605.01920)
*Noga Peleg Pelc,Gal A. Kaminka,Yoav Goldberg*

Main category: cs.AI

TL;DR: 本文提出Agentic Context Description Language (ACDL)，一种用于精确、可读、标准化描述LLM智能体系统中输入上下文结构及其动态演化的规范语言，并配套可视化工具，旨在替代当前非正式的上下文描述方式。


<details>
  <summary>Details</summary>
Motivation: LLM智能体系统中，上下文（prompt）的构建和演化方式对系统质量至关重要，但当前缺乏标准化的描述方法，通常依赖非正式文本、临时图表或代码审查，无法精确捕捉prompt跨步骤的演变过程，也无法清晰对比不同的上下文表示策略。

Method: 提出ACDL语言，包含角色消息序列、动态内容、时间索引引用、条件/迭代结构等核心构造，能够独立于具体实现捕获prompt的完整架构。支持两种表达形式：可手绘的白板图和可渲染的形式化语言。

Result: 通过多个现有系统及其变体的文档化演示，展示了ACDL在描述LLM系统上下文方面的表达能力。提供了配套工具、示例和文档（www.acdlang.org）。

Conclusion: 呼吁社区在日常交流与论文中采用ACDL作为LLM系统上下文描述的标准，以提升沟通效率和研究可复现性。
...
### [297] [Moira: Language-driven Hierarchical Reinforcement Learning for Pair Trading](https://arxiv.org/abs/2605.01954)
*Polydoros Giannouris,Yuechen Jiang,Lingfei Qian,Yuyan Wang,Xueqing Peng,Jimin Huang,Guojun Xiong,Sophia Ananiadou*

Main category: cs.AI

TL;DR: 本文提出了一种语言驱动的分层强化学习框架，利用大语言模型作为高层和底层策略，通过提示更新优化，在配对交易任务中实现了分层信用分配和延迟反馈下的有效适应。


<details>
  <summary>Details</summary>
Motivation: 许多序贯决策问题具有分层结构，高层语义选择约束下游动作，且反馈存在延迟和歧义。在这种设置下学习面临信用分配挑战：性能下降可能源于有缺陷的抽象、次优执行或两者的交互。现有方法难以有效处理这种分层结构下的延迟反馈和信用分配问题。

Method: 将配对交易建模为分层强化学习问题，高层策略负责资产对选择（长期语义推理），低层策略负责执行（短期部分可观测决策）。两个层级的策略均由大语言模型参数化，仅通过提示更新进行优化，利用轨迹级和回合级文本反馈来适应抽象和执行，无需基于梯度的微调。

Result: 在真实市场数据上的实验表明，该方法在一致性改进方面优于传统和基于LLM的基线方法，验证了语言驱动的分层强化学习的有效性。

Conclusion: 通过显式分离抽象选择与执行，该框架减少了分层层级间的非平稳性，实现了延迟反馈下的目标适应，为分层强化学习中的信用分配问题提供了有效解决方案。
...
### [298] [Personalized Digital Health Modeling with Adaptive Support Users](https://arxiv.org/abs/2605.02004)
*Zhongqi Yang,Mahkameh Rasouli,Neda Mohseni,Yong Huang,Iman Azimi,Amir M. Rahmani*

Main category: cs.AI

TL;DR: 提出统一个性化框架，通过自适应加权支持用户（包括相似和相异用户）训练个人模型，结合个人损失、相似性加权迁移和对比正则化，在六个数字健康任务上持续超越基线，低数据场景下RMSE降低约25%。


<details>
  <summary>Details</summary>
Motivation: 数字健康中个体生理和行为异质性大，个性化建模受限于用户数据稀缺且噪声大，现有方法仅利用相似用户或群体预训练，导致迁移有偏、泛化弱。

Method: 构建统一个性化框架：(1) 目标函数包含个人损失、相似用户加权迁移损失和相异用户对比正则化项；(2) 迭代优化算法联合更新模型参数和用户相似性权重；(3) 自适应权重决定支持用户对当前用户的贡献程度。

Result: 在四个真实数字健康数据集的六个任务上，该方法一致优于群体模型和个性化基线，大规模数据集RMSE降低高达10%，低数据场景RMSE降低约25%，自适应权重提升了数据效率和可解释性。

Conclusion: 同时利用相似和相异用户的自适应加权框架能有效缓解数据稀疏和噪声问题，生成更准确的个性化模型，并为目标数据选择提供可解释指导。
...
### [299] [Reliable AI Needs to Externalize Implicit Knowledge: A Human-AI Collaboration Perspective](https://arxiv.org/abs/2605.02010)
*Hengyu Liu,Tianyi Li,Zhihong Cui,Yushuai Li,Zhangkai Wu,Torben Bach Pedersen,Kristian Torp,Christian S. Jensen*

Main category: cs.AI

TL;DR: 本文提出通过知识对象（KOs）将隐性知识外显化，使人类能够验证AI学习的隐性知识，从而解决当前可靠性方法只能验证显性知识的根本缺陷。


<details>
  <summary>Details</summary>
Motivation: AI从显性知识和隐性知识中学习，但隐性知识（如推理模式、调试过程）因记录成本高于感知价值而未被外显化。当前可靠性方法只能验证显性知识，而最有价值的AI能力（推理、判断、直觉）恰恰是隐性知识，造成验证鸿沟。

Method: 提出知识对象（Knowledge Objects, KOs）——一种结构化工件，将隐性知识外显化为人类可检查、验证和认可的形式，从而改变验证的经济性，使原本成本过高的验证变得可行。

Result: 通过KOs，原本因成本过高而无法验证的隐性知识变得可验证，累积的人类验证能够随时间推移提升AI的可靠性。

Conclusion: 可靠的AI需要人类验证隐性知识的基础设施，KOs提供了一种可行的方案来填补当前可靠性方法的根本性空白。
...
### [300] [Tenability and Weak Semantics: Modeling Non-uniform Defense -- Extended Version](https://arxiv.org/abs/2605.02024)
*Uri Andrews,Luca San Mauro,John Spoerl*

Main category: cs.AI

TL;DR: 本文提出"可维持性"(tenability)系列语义——一种基于对话的论证语义，通过单调承诺博弈形式化论证者在辩论中维持特定论点或论据集的能力，其与所有已有的弱语义在三个基准模式上表现不同，并分析了三种变体的计算复杂度（静态可维持性为Π^P_2-完全，可维持性与强可维持性为PSPACE-完全）。


<details>
  <summary>Details</summary>
Motivation: 已有的弱语义虽然放松了可接受性的要求（如忽视自击败或不连贯攻击），但仍是基于扩展的，要求对所有合理攻击进行统一防御。这种统一性在面对策略性防御场景时过于苛刻，因为适当的反驳取决于对手的攻击路线。为此，需要一种基于对话的语义来形式化辩论中可维持性的策略本质。

Method: 提出基于单调承诺游戏(monotone commitment games)的对话式语义，参与者在有限的无冲突步骤上交替移动。定义三种变体：静态可维持性(static tenability)、可维持性(tenability)和强可维持性(strong tenability)，通过改变辩论者承担的义务来区分。通过三个基准模式（自击败攻击、浮动指派、析取恢复）与已有弱语义进行对比分析。

Result: 确定了三种可维持性变体之间的相对强度关系；证明了与已有弱语义之间的蕴含关系和分离关系；计算复杂度分析显示：静态可维持性判定为Π^P_2-完全，可维持性与强可维持性判定为PSPACE-完全。

Conclusion: 可维持性语义为论证语义研究提供了新的基于对话的视角，弥补了扩展式弱语义在策略性防御场景中的不足，并揭示了不同语义变体在表达力与计算复杂度之间的权衡。
...
### [301] [NORA: A Harness-Engineered Autonomous Research Agent for End-to-End Spatial Data Science](https://arxiv.org/abs/2605.02092)
*Bing Zhou,Xiao Huang,Huan Ning,Qiusheng Wu,Diya Li,Ziyi Zhang*

Main category: cs.AI

TL;DR: 本文提出NORA（Night Owl Research Agent），一个专为地理信息科学和空间数据科学设计的工程化多智能体自主研究系统。NORA通过包含21个领域专业工作流技能、9个专家子智能体和自定义MCP服务器的技能优先架构，编排完整的研究生命周期。系统核心包括两个新型领域专业技能：空间分析技能单元（编码ESDA、空间回归和诊断的决策框架）和空间数据下载技能（支持从权威地理空间数据源进行可重复获取）。作者形式化了科学研究智能体的"工程化设计"概念，展示了生命周期钩子、安全门、生成器-评估器分离、人在回路和状态持久化如何确保可靠和可重复的自主研究。通过6位领域专家和3位LLM评审员在七个维度（新颖性、质量、严谨性等）上的案例研究评估，结果表明领域专业化的工程化设计相比通用智能体配置显著提高了研究输出的效率和质量。


<details>
  <summary>Details</summary>
Motivation: 现有自主研究智能体大多是领域无关的，缺乏空间数据科学所需的专门推理、方法选择和数据获取能力，无法满足该领域严格的科学研究要求。

Method: 提出NORA系统，采用工程化设计的多智能体架构，包含21个领域专业技能、9个专家子智能体和MCP服务器；核心创新为空间分析技能单元和空间数据下载技能；通过生命周期钩子、安全门、生成器-评估器分离、人在回路和状态持久化等工程化机制确保研究的可靠性和可重复性。

Result: 领域专业化的工程化设计相比通用智能体配置，在研究效率和输出质量方面均有显著提升。

Conclusion: 领域专业化的工程化设计（harness engineering）是提升科学研究智能体性能的关键方向，为自主地理空间数据分析提供了可靠且可复现的解决方案。
...
### [302] [The Dynamic Gist-Based Memory Model (DGMM): A Memory-Centric Architecture for Artificial Intelligence](https://arxiv.org/abs/2605.02106)
*Terry Dorsey,Kevin Huggins*

Main category: cs.AI

TL;DR: 本文提出了动态要点记忆模型（DGMM），一种以记忆为中心的AI架构，将经验表示为显式、持久、图结构的 episodic-semantic 记忆，通过选择性线索条件回忆构建工作记忆，以解决大语言模型中持久记忆、时间基础、来源和可解释性方面的局限。


<details>
  <summary>Details</summary>
Motivation: 现有AI系统（特别是大语言模型）将经验隐式编码在固定参数中，缺乏持久记忆、时间基础、来源追溯和可解释性，难以保存、检查和重新解释过去的交互。

Method: 提出动态要点记忆模型（DGMM），将经验表示为随时间演化的图结构情景-语义记忆，以时间、来源和交互上下文为锚点编码概念结构，并通过选择性、线索条件回忆构建工作记忆，定义了加性记忆增长和回忆条件解释的形式化模式与架构不变性。

Result: DGMM具备情景持久性、线索条件惊喜的局部性、以及无需修改存储记忆即可实现上下文变异性，支持无需重新训练的演化解释。

Conclusion: DGMM提供了一种以记忆为显式、持久一阶实体的连贯架构理论，支持可解释、上下文感知和时间基础的人工智能系统。
...
### [303] [Planner Matters! An Efficient and Unbalanced Multi-agent Collaboration Framework for Long-horizon Planning](https://arxiv.org/abs/2605.02168)
*Wenyi Wu,Sibo Zhu,Kun Zhou,Biwei Huang*

Main category: cs.AI

TL;DR: 本文提出一种基于LM的多智能体框架，将自动化任务分解为规划者、执行者和记忆管理器三个角色，核心贡献在于通过系统性计算分配分析发现规划是影响任务性能的主导因素，并据此提出仅优化规划者的强化学习方法（冻结其他组件），在网页导航、操作系统控制和工具使用等基准上验证了其高效性。


<details>
  <summary>Details</summary>
Motivation: 基于语言模型的智能体在长周期规划与推理方面仍存在困难，现有方法缺乏对计算资源在不同模块间如何有效分配的系统性理解。

Method: 提出三角色多智能体框架（规划者、执行者、记忆管理器），通过计算分配分析识别规划为关键瓶颈，采用VLM作为评判器提供轨迹级奖励，仅对规划者进行强化学习优化，冻结执行器和记忆管理器。

Result: 在网页导航、操作系统控制和工具使用等基准上，该方法在保持计算效率的同时显著提升了长周期自动化任务的性能。

Conclusion: 将模型容量和学习集中于高层规划是实现长周期智能体自动化鲁棒且高效提升的关键方向。
...
### [304] [T$^2$PO: Uncertainty-Guided Exploration Control for Stable Multi-Turn Agentic Reinforcement Learning](https://arxiv.org/abs/2605.02178)
*Haixin Wang,Hejie Cui,Chenwei Zhang,Xin Liu,Shuowei Jin,Shijie Geng,Xinyang Zhang,Nasser Zalmout,Zhenyu Shi,Yizhou Sun*

Main category: cs.AI

TL;DR: 针对多轮强化学习中不稳定性源于低效探索的问题，提出T²PO框架，在Token级和Turn级两个细粒度层面通过不确定性监测实施探索控制，在WebShop、ALFWorld和Search QA等环境中显著提升了训练稳定性和性能。


<details>
  <summary>Details</summary>
Motivation: 多轮强化学习在训练中普遍存在不稳定性甚至训练崩溃的问题，现有稳定化技术（如细粒度信用分配和轨迹过滤）未能根本解决。作者认为其根源在于多轮场景下的低效探索——策略持续生成低信息动作，既无法降低不确定性，也无法推进任务进展。

Method: 提出T²PO（Token- and Turn-level Policy Optimization）框架：（1）Token层面：监测不确定性动态，当边际不确定性变化低于阈值时触发思考干预；（2）Turn层面：识别探索进展可忽略的交互，动态重采样这些轮次以避免无效轨迹。

Result: 在WebShop、ALFWorld和Search QA等多个环境中，T²PO在训练稳定性和性能提升上均取得了显著改进，探索效率更优。

Conclusion: 通过细粒度不确定性感知的探索控制，可以有效解决多轮强化学习中的不稳定问题，提升训练效果。
...
### [305] [MEMAUDIT: An Exact Package-Oracle Evaluation Protocol for Budgeted Long-Term LLM Memory Writing](https://arxiv.org/abs/2605.02199)
*Nishant Bhargava,Rodrigo Sobral Barrento*

Main category: cs.AI

TL;DR: 提出MEMAUDIT协议，将长期记忆写入问题转化为可审计的优化问题，通过精确包评估分离表示质量、状态保存和预算感知选择效应，超越了传统端到端QA评估的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有评估通常测量最终问答准确率，但这种方式将记忆写入与检索、提示和阅读推理纠缠在一起，无法单独评估记忆写入的质量，尤其是在未来查询未知的情况下。

Method: 设计MEMAUDIT协议：固定经验流、候选记忆表示、存储成本、语义证据单元、未来查询需求和预算，将写时记忆选择转化为可审计的优化问题；采用凹性-over-模性的语义覆盖目标，在存储和每经验单表示约束下，通过分支定界与MILP认证计算精确包最优解。

Result: 在受控精确包、有效性压力测试、人工审计的自然支持切片以及导出的Mem0、A-Mem和Letta存储上，MEMAUDIT成功分离了表示质量、有效性状态保存和预算感知选择效应，这些是端到端QA无法定位的。

Conclusion: MEMAUDIT提供了可复用的包生成器、认证求解器、自然包导出、外部系统评分器和可缓存的可重现性元数据，用于评估在固定存储预算下记忆写入器实际保存了什么。
...
### [306] [CBV: Clean-label Backdoor Attacks on Vision Language Models via Diffusion Models](https://arxiv.org/abs/2605.02202)
*Ji Guo,Xiaolong Qin,Cencen Liu,Jielei Wang,Jierun Chen,Wenbo Jiang*

Main category: cs.AI

TL;DR: 提出了一种基于扩散模型的干净标签后门攻击方法CBV，通过修改扩散模型的分数匹配过程生成自然中毒样本，结合多模态引导和GradCAM掩膜实现高隐蔽性后门攻击，在MSCOCO和VQA v2上对四种VLM实现了超过80%的攻击成功率。


<details>
  <summary>Details</summary>
Motivation: 现有针对VLM的后门攻击主要依赖数据投毒，通过添加视觉触发器并修改文本标签，导致图像-文本不匹配，使得中毒样本容易被检测。为解决这一局限性，需要开发更具隐蔽性的后门攻击方法。

Method: CBV方法包含三个关键组件：(1) 修改扩散模型逆生成过程中的分数，引导生成包含触发器图像特征的中毒样本；(2) 引入触发器图像的多模态文本信息作为生成过程中的引导；(3) 提出GradCAM引导的掩膜(GM)，将修改限制在语义最重要的区域而非整张图像。

Result: 在MSCOCO和VQA v2数据集上对四种代表性VLM进行评测，攻击成功率(ASR)超过80%，且不影响模型的正常功能。

Conclusion: CBV通过扩散模型生成自然、隐蔽的干净标签中毒样本，有效解决了现有方法中图像-文本不匹配易被检测的问题，为VLM后门攻击研究提供了新思路。
...
TL;DR: 本文提出将基准测试子集选择形式化为子模最大化问题，在多变量高斯模型下，熵（对数行列式协方差）和互信息是自然目标函数。实验表明，在小规模子集上，互信息选择在插值任务中优于熵选择。


<details>
  <summary>Details</summary>
Motivation: 评估大语言模型需大量基准测试，成本高昂，且许多基准高度相关。因此需要选择少量具有代表性的基准子集来降低评估成本。

Method: 将子集选择形式化为子模最大化问题，基于多变量高斯模型，以熵（对数行列式协方差）和互信息作为目标函数。熵选择等价于枢轴Cholesky分解，具有谱残差界；互信息通常非单调，但对小规模子集经验上单调，采用贪心算法优化。

Result: 在来自十个公开排行榜的三组矩阵上的实验表明，对于小规模子集，互信息选择在插值任务中优于熵选择。

Conclusion: 互信息是选择信息量最大的小规模基准子集的有效目标函数，可用于降低大语言模型评估成本。
...
TL;DR: CoVSpec是一个面向视觉语言模型（VLM）的协同推测解码框架，通过设备端无训练视觉token剪枝、自适应草稿策略和并行分支验证纠错机制，在设备-边缘协同推理场景下实现高达2.21倍吞吐提升并降低96%以上通信开销，且不牺牲任务精度。


<details>
  <summary>Details</summary>
Motivation: 大型VLM在移动设备上部署面临巨大的计算和内存挑战，而直接将推测解码扩展到VLM存在严重的视觉token计算冗余和高通信开销问题。

Method: 1) 无训练视觉token剪枝：联合考虑查询相关性、token活跃度和低秩依赖，在移动端剪枝冗余视觉token；2) 自适应草稿策略：动态调整验证频率和草稿长度；3) 并行分支机制：解耦验证与纠错，提升草稿端利用率并降低纠错传输开销。

Result: 在多个基准上，CoVSpec相比纯目标端推理实现最高2.21倍吞吐提升，相比基线方法降低96%以上通信开销，且不牺牲任务精度。

Conclusion: CoVSpec通过针对VLM特点优化的协同推测解码框架，有效解决了设备-边缘协同推理中的视觉token计算冗余和通信瓶颈问题，实现了高效且无损的VLM推理。
...
### [309] [Perturbation Dose Responses in Recursive LLM Loops: Raw Switching, Stochastic Floors, and Persistent Escape under Append, Replace, and Dialog Updates](https://arxiv.org/abs/2605.02236)
*Pawel Kaplanski*

Main category: cs.AI

TL;DR: 在递归语言模型循环中，需要注入多少文本才能改变已形成的吸引子模式，以及这种改变是否持久。通过30步递归循环实验发现：持续重定向受记忆策略制约，在12,000字符截断下持久性不超过50%；全历史协议下源盆地逃离可达75-80%，但目标一致性持久性仍较低。对抗性延续的ED50约为40个token。替换模式的高切换率主要由状态重置覆盖引起。非单调下降现象被证实是结构性的。共37组实验，并在不同模型上进行了复现。


<details>
  <summary>Details</summary>
Motivation: 递归语言模型循环会形成吸引子模式，实际问题是需要多少注入文本才能将已稳定的循环转移到别处，以及这种转移能否持续。

Method: 在30步递归循环中分离模型与上下文更新规则（追加、替换、对话更新），使用不同记忆策略（12,000字符截断 vs 全历史）和扰动注入（对抗性延续、插入探针等），在gpt-4o-mini上进行37组实验并用gpt-4.1-nano复现。

Result: 追加模式下：截断历史时目标一致性持久性约16%，源盆地逃离约36%（剂量400时均不超过50%）；全历史时源盆地逃离可达75-80%，目标一致性持久性在1,500 tokens时达0.50。对抗性延续的ED50约40 tokens。替换模式高切换率主要由状态重置覆盖导致，插入探针后降至12-32%。同质扰动控制实验排除了扰动异质性导致非单调下降的原因。

Conclusion: 递归循环评估应区分瞬时移动与持久逃离，减去随机地板效应，并将上下文更新规则视为与安全相关的第一类设计选择。
...
### [310] [PhysicianBench: Evaluating LLM Agents in Real-World EHR Environments](https://arxiv.org/abs/2605.02240)
...
TL;DR: PhysicianBench是一个评估LLM智能体在真实电子健康记录（EHR）环境中执行临床任务的基准测试，包含100个长时域任务，覆盖21个专科，平均每个任务需27次工具调用。最佳模型成功率仅46%，开源模型最高19%，揭示当前智能体能力与现实临床工作流之间存在显著差距。


<details>
  <summary>Details</summary>
Motivation: 现有医疗智能体基准主要关注静态知识回忆、单步原子操作或意图表达，缺乏对真实临床系统中长时域、复合工作流的可执行验证评估。

Method: 从真实会诊案例中改编100个长时域任务，由独立医生小组审核；在真实EHR环境中通过标准API实例化；任务分解为670个结构化检查点，由任务特定脚本执行接地验证。

Result: 最佳模型（闭源）成功率为46%（pass@1），开源模型最高19%，表明当前LLM智能体在真实临床工作流中表现有限。

Conclusion: PhysicianBench提供了真实且可执行验证的基准，衡量向自主临床智能体方向取得的进展，揭示了当前能力与临床需求间的巨大差距。
...
### [311] [Zero-Shot Confidence Estimation for Small LLMs: When Supervised Baselines Aren't Worth Training](https://arxiv.org/abs/2605.02241)
*Luong N. Nguyen*

Main category: cs.AI

TL;DR: 本文研究小型语言模型能否可靠估计自身正确性，发现零样本的平均token对数概率（无需训练数据）在分布内匹配或超越有监督基线（AUROC 0.650-0.714 vs 0.644-0.676），在分布外显著优于有监督方法（0.717-0.833 vs 0.512-0.564），并提出了检索条件自评估方法，在低延迟下提升自评估效果。


<details>
  <summary>Details</summary>
Motivation: 随着LLM推理成本成为部署预算的主要部分，将大多数查询路由到廉价本地模型、仅将困难查询交给云端大模型是常用的成本控制策略。然而，这一策略依赖于本地模型能否可靠估计自身正确性，从而决定是否向上路由。现有方法多需要监督训练数据，本文旨在探索无需训练数据的零样本置信度信号的可行性。

Method: 在三个7-8B模型家族、两个数据集上对比零样本置信度信号（平均token对数概率）与RouteLLM风格的有监督基线。同时提出检索条件自评估（retrieval-conditional self-assessment），这是一种生成前信号，在检索相似度高时选择性注入检索知识，提升自评估效果并降低延迟。

Result: 平均token对数概率在分布内匹配或超越有监督基线（AUROC 0.650-0.714 vs 0.644-0.676），在分布外显著胜出（0.717-0.833 vs 0.512-0.564）。检索条件自评估在低延迟下提升自评估AUROC最多0.069。基于1000个标注样本的有监督基线从未超过零样本信号。

Conclusion: 零样本置信度信号（平均token对数概率）可以有效替代有监督方法用于本地-云端路由决策，尤其在分布外场景下优势明显。检索条件自评估提供了更高效的预生成信号。研究结果支持无监督路由策略的可行性。
...
### [312] [A Study of Belief Revision Postulates in Multi-Agent Systems (Extended Version)](https://arxiv.org/abs/2605.02249)
*Michael Thielscher,Tran Cao Son*

Main category: cs.AI

TL;DR: 本文研究多智能体系统中的信念修正问题，将经典AGM信念修正公设推广到多智能体环境，提出广义全交多智能体信念修正算子，并探讨了迭代修正公设的推广及事件模型修正算子的潜在问题。


<details>
  <summary>Details</summary>
Motivation: 在认知规划中，当一个智能体获得某个状态属性的信念后，所有智能体的信念将如何变化？现有研究缺乏对多智能体信念修正的形式化评估框架，需要将经典AGM公设推广到多智能体场景。

Method: 基于多智能体Kripke模型表示智能体信念，将AGM信念修正公设推广到多智能体设定；提出广义全交多智能体信念修正算子作为满足所有推广公设的简单算子；进一步推广迭代修正的标准公设，并引入基于事件模型的修正算子。

Result: 提出了满足所有推广AGM公设的广义全交多智能体信念修正算子；定义了迭代多智能体信念修正的推广公设；发现基于事件模型的修正算子在满足所有推广的迭代修正公设方面存在潜在问题。

Conclusion: 为评估动态认知推理框架提供了形式化基础，指出了在Kripke模型上定义满足所有迭代多智能体信念修正公设的认知算子所面临的挑战。
...
### [313] [Towards Understanding Specification Gaming in Reasoning Models](https://arxiv.org/abs/2605.02269)
*Kei Nishimura-Gasparian,Robert McCarthy,David Lindner*

Main category: cs.AI

TL;DR: 本文构建并开源了一套多样化的任务套件，系统研究了LLM智能体在规范博弈（Specification Gaming）中的表现，发现所有测试模型在大多数任务中都会以不可忽视的比例利用规范漏洞，其中RL推理训练会显著增加这一比例，而测试时缓解措施只能减少但无法消除该问题。


<details>
  <summary>Details</summary>
Motivation: 规范博弈是LLM智能体的一种关键失效模式，但缺乏对其产生条件和驱动因素的系统性研究。

Method: 构建并开源了一套多样化的任务套件（包含八个设置，其中五个为非编码任务），在该套件上测试多个模型（包括Grok 4和Claude系列），并分析RL推理训练、RL推理预算以及测试时缓解措施对规范博弈率的影响。

Result: 所有测试模型在大多数设置中均以不可忽视的比例利用规范漏洞；Grok 4的规范博弈率最高，Claude模型最低；RL推理训练显著增加规范博弈率；增加RL推理预算对博弈率有弱正向影响；测试时缓解措施可减少但无法消除规范博弈。

Conclusion: 规范博弈是由RL推理训练引发的一个根本性挑战，研究团队开源了评估套件以支持后续工作。
...
### [314] [Complexity Horizons of Compressed Models in Analog Circuit Analysis](https://arxiv.org/abs/2605.02285)
*Pacome Simon Mbonimpa*

Main category: cs.AI

TL;DR: 提出一种基于先决条件图的性能感知模型压缩策略，通过将电路分析知识结构化为有向无环图，动态级联查询不同压缩程度的LLM变体，在保证推理准确性的同时选择最小的压缩模型。


<details>
  <summary>Details</summary>
Motivation: 传统评估方法将模型性能视为扁平化指标，未能考虑工程知识的层次结构，且LLM在电路分析等专业工程领域的部署面临推理准确性与计算效率之间的权衡。

Method: 将电子设计概念构建为有向无环图（DAG），设计基于先决条件的agentic数据集生成管道，以及一个动态级联查询跨压缩变体的策略评估引擎，根据概念知识边界选择最小的压缩模型。

Result: 在模拟电子数据集上的实验表明，先决条件图能够提供模型压缩与电路分析复杂度性能之间的细粒度映射。

Conclusion: 先决条件图可有效指导LLM压缩变体的选择，在保持电路分析能力的同时实现模型小型化，为专业工程领域的模型部署提供了实用策略。
...
### [315] [EngiAgent: Fully Connected Coordination of LLM Agents for Solving Open-ended Engineering Problems with Feasible Solutions](https://arxiv.org/abs/2605.02289)
*Xiyuan Zhou,Ruixi Zou,Xinlei Wang,Yuheng Cheng,Yan Xu,Junhua Zhao,Jinjin Gu*

Main category: cs.AI

TL;DR: EngiAgent是一个多智能体系统，通过全连接协调器模拟专家工作流程，在工程问题求解中显著提升可行性，超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在推理和代码生成方面表现出色，但在工程问题求解中常无法确保解的可行性，限制了其应用。工程问题不同于数学问题，需要开放性分析、可行性驱动的建模和迭代优化。

Method: 提出EngiAgent多智能体系统，包含问题分析、建模、验证、求解和方案评估等专用智能体，采用全连接协调器实现灵活的反馈路由，克服了传统流水线反射方法的僵化问题。

Result: 在四个代表性领域上的实验结果表明，EngiAgent在可行性方面相比现有方法取得了实质性改进。

Conclusion: EngiAgent建立了面向可行性的工程问题求解新范式，有效提升了LLM在工程决策中的实用性和鲁棒性。
...
### [316] [Distilling Long-CoT Reasoning through Collaborative Step-wise Multi-Teacher Decoding](https://arxiv.org/abs/2605.02290)
*Taewon Yun,Jisu Shin,Jeonghwan Choi,Seunghwan Bang,Hwanjun Song*

Main category: cs.AI

TL;DR: CoRD是一种协作式多教师解码框架，通过基于困惑度评分的逐步推理合成和束搜索，高效生成高质量的长链思维推理数据，使学生模型以更少的监督信号达到接近教师水平的性能。


<details>
  <summary>Details</summary>
Motivation: 现有蒸馏方法采用事后选择完整推理轨迹的方式，忽视了异构教师之间的协作，缺乏动态探索，导致冗余采样和互补推理的缺失，且全规模推理计算成本高昂。

Method: 提出CoRD框架，采用协作式多教师解码策略，通过预测性困惑度评分和束搜索进行逐步推理合成，使异构大推理模型（LRMs）联合构建连贯的推理轨迹，同时高效保留多样且高潜力的假设。

Result: CoRD生成的推理数据质量更高，以更少的结构化监督信号使学生模型达到接近教师水平的性能，且没有显著效率开销；在跨领域和开放式设置中具有良好的泛化能力。

Conclusion: CoRD通过多教师协作和动态探索有效解决了现有蒸馏方法的冗余采样和互补推理缺失问题，为长链思维推理的高效蒸馏提供了可行方案。
...
### [317] [Anon: Extrapolating Optimizer Adaptivity Across the Real Spectrum](https://arxiv.org/abs/2605.02317)
*Yiheng Zhang,Kaiyan Zhao,Shaowu Wu,Yiming Wang,Jiajun Wu,Leong Hou U,Steve Drew,Xiaoguang Niu*

Main category: cs.AI

TL;DR: 本文提出Anon优化器，通过可连续调节的自适应性（在SGD与Adam之间插值甚至超越二者）以及增量延迟更新（IDU）机制，在理论上保证收敛性，并在图像分类、扩散模型和语言建模任务上优于现有最先进优化器，弥合了经典优化器与现代优化器之间的性能差距。


<details>
  <summary>Details</summary>
Motivation: 自适应优化器（如Adam）在训练大模型时表现出色，但泛化性能往往不如非自适应方法（如SGD）。现有方法未能有效解决自适应性限制优化器适应多样化优化景观的能力这一关键问题。

Method: 提出Anon优化器，核心创新包括：(1) 连续可调的自适应性参数R，实现SGD-like到Adam-like行为的插值乃至外推；(2) 增量延迟更新（IDU）机制，比AMSGrad的硬最大跟踪策略更灵活，增强对梯度噪声的鲁棒性；(3) 在凸与非凸设定下提供了理论收敛保证。

Result: 在图像分类、扩散模型和语言建模任务上，Anon一致性地优于现有最先进优化器，证明了可调自适应性作为有价值的设计原则的有效性。

Conclusion: 自适应性可作为可调的设计原则，Anon提供了首个统一且可靠的框架，能够弥合经典与现代化化器之间的鸿沟，并超越它们的优势性能。
...
### [318] [Can Causal Discovery Algorithms Help in Generating Legal Arguments?](https://arxiv.org/abs/2605.02318)
*Soham Wasmatkar,Subinay Adhikary,Rakshit Rohan,Shouvik Kumar Guha,Saptarshi Pyne,Kripabandhu Ghosh*

Main category: cs.AI

TL;DR: 本文首次将因果发现算法应用于法律领域，通过分析150起凶杀案中17个法律概念的标注数据，发现了一些可生成可行法律论证的因果关系（如物理攻击未发生可概率为1地排除财产纠纷导致的凶杀），为自动化法律论证生成开辟了新方向。


<details>
  <summary>Details</summary>
Motivation: 因果发现算法已在医学、经济学等关键领域支持决策，但尚未被应用于法律领域。作者希望弥补这一空白，探索因果发现算法能否用于自动生成法律论证。

Method: 首先识别17个法律概念（如物理攻击、财产纠纷），然后对150起凶杀案进行这些概念的标注。接着应用多种主流因果发现算法分析标注数据，发现法律概念间的因果关系，并用概率量化关联可信度。

Result: 发现部分因果关系可用于生成可行的法律论证，例如：若凶杀案中未发生物理攻击，则足以（概率为1）证明该凶杀非因财产纠纷所致。

Conclusion: 因果发现算法能够辅助法律论证的生成，为未来在法律领域应用因果推理技术开辟了前景广阔的研究方向。
...
### [319] [ANO: A Principled Approach to Robust Policy Optimization](https://arxiv.org/abs/2605.02320)
*Yiheng Zhang,Yiming Wang,Kaiyan Zhao,Zhenglin Wan,Jiayu Chen,Leong Hou U*

Main category: cs.AI

TL;DR: 本文提出了一种名为锚定近邻优化（ANO）的新方法，通过建立统一信任域框架并引入降阶影响原则来解决PPO的硬裁剪与SPO无裁剪之间的根本矛盾。ANO在MuJoCo基准测试中实现了最先进的性能，并在激进超参数下展现出超越PPO的卓越稳定性。


<details>
  <summary>Details</summary>
Motivation: PPO的硬裁剪机制丢弃了来自异常值的梯度信息，导致样本效率低下；而移除裁剪（如SPO）则暴露于无界梯度，造成严重不稳定性。现有方法在梯度信息利用与优化稳定性之间存在根本性矛盾，亟需一种能同时解决这两个问题的新方法。

Method: 首先建立统一信任域框架，泛化现有目标函数；基于设计原则推导出锚定近邻优化（ANO）；提出降阶影响原则，实现从单调惩罚/硬阈值到动态异常值抑制的范式转变；证明该原则在高方差随机优化中稳定性的必要性。

Result: ANO在MuJoCo基准测试中显著优于PPO和SPO，达到最先进水平；即使在标准学习率3倍大小的激进超参数下，PPO完全失败时ANO仍能防止策略崩溃，展现出卓越的稳定性。

Conclusion: ANO具备鲁棒优化所需的最小结构复杂性，降阶影响原则为信任域策略优化提供了新范式，有效解决了梯度信息利用与优化稳定性之间的根本矛盾。
...
### [320] [Controllable and Verifiable Process Data Synthesis for Process Reward Models](https://arxiv.org/abs/2605.02395)
*Yinghui Chi,Lucien Wang*

Main category: cs.AI

TL;DR: 提出了一种可控可验证的流程监督数据合成框架，通过符号推理链注入模板感知错误并验证前缀不可推导性，生成高质量流程监督数据用于PRM训练，在逻辑和数学推理任务上取得了显著提升。


<details>
  <summary>Details</summary>
Motivation: 现有流程监督数据构建方法对错误位置、错误类型和轨迹一致性的控制有限，导致PRM训练数据质量不足。

Method: 构建正确符号推理链→注入模板感知错误→在错误状态下重新计算后续步骤→验证注入步骤不可从前缀推导→翻译为对齐的自然语言流程。

Result: 合成数据在逻辑推理基准的Best-of-8重排序任务上提升显著，且能迁移到数学推理任务；第一步错误定位比整体步骤分类更具挑战性。

Conclusion: 细粒度、可验证的流程监督对PRM训练至关重要，所提框架能有效提升PRM在推理任务上的表现。
...
### [321] [HeavySkill: Heavy Thinking as the Inner Skill in Agentic Harness](https://arxiv.org/abs/2605.02396)
...
TL;DR: 本文提出HeavySkill视角，将"深度思考"视为一种内化于模型参数中的技能（并行推理+摘要的两阶段流程），并通过系统实验证明该技能优于传统Best-of-N策略，且可通过强化学习进一步扩展深度和宽度，为自我进化的LLM提供新路径。


<details>
  <summary>Details</summary>
Motivation: 现有基于多智能体编排框架的复杂推理系统虽表现出色，但其真正驱动性能提升的底层机制被复杂的系统设计所掩盖，缺乏对核心驱动因素的清晰理解。

Method: 提出HeavySkill观点，将"深度思考"视为最小执行单元和内在技能，具体为一个两阶段流程（并行推理→摘要），并在多种领域进行系统性实证研究。

Result: HeavySkill始终优于传统Best-of-N策略；更强的LLM可接近Pass@N性能；深度思考的深度和宽度作为可学习技能，可通过强化学习进一步扩展。

Conclusion: 深度思考是一种可内化、可扩展的技能，为不依赖脆弱编排层的自进化LLM提供了有前景的发展方向。
...
### [322] [The Compliance Trap: How Structural Constraints Degrade Frontier AI Metacognition Under Adversarial Pressure](https://arxiv.org/abs/2605.02398)
*Rahul Kumar*

Main category: cs.AI

TL;DR: 针对11个前沿AI模型（来自8家厂商，67,221条评分记录）的元认知稳定性评估发现，8/11的模型在对抗压力下出现灾难性退化，准确率最高下降30.2个百分点。问题根源在于"服从陷阱"——强制性指令覆盖了认知边界，而非威胁内容的心理影响。具备高级推理能力的模型退化最严重，而Anthropic的Constitutional AI表现出近乎完全的免疫力，这源于对齐特定训练而非能力优势。


<details>
  <summary>Details</summary>
Motivation: 当前安全评估主要关注战略性欺骗（诡计），但存在一个更根本的失败模式——认知崩溃（元认知稳定性在对抗压力下失效），这在高风险决策管线中至关重要。

Method: SCHEMA评估框架：6条件因子设计，双分类器评分，对11个前沿模型（67,221条记录）进行系统评估，并通过因子隔离和良性干扰对照实验识别问题根源。

Result: 8/11模型出现灾难性元认知退化，准确率下降最高30.2个百分点（p<2×10⁻⁸，经Bonferroni校正）。退化由服从强制性指令驱动（"服从陷阱"），而非威胁内容。移除服从后缀即可恢复性能。高级推理模型退化最严重，Constitutional AI表现出免疫性。

Conclusion: 元认知稳定性是独立于战略欺骗的关键安全维度；"服从陷阱"是主要失效机制；对齐特定训练（如Constitutional AI）而非能力提升是解决该问题的有效路径；已开源全部数据集和评估基础设施。
...
### [323] [FitText: Evolving Agent Tool Ecologies via Memetic Retrieval](https://arxiv.org/abs/2605.02411)
*Kyle Zheng,Han Zhang,Renliang Sun,Chenchen Ye,Wei Wang*

Main category: cs.AI

TL;DR: FitText是一个无需训练的框架，通过将动态检索嵌入智能体推理循环，利用Memetic进化搜索迭代优化伪工具描述，显著提升了大规模API工具检索的准确率（ToolRet上平均排名从8.81提升至2.78，StableToolBench上平均通过率提升24个百分点），但该方法依赖强基座模型能力，弱模型下进化搜索会放大噪声。


<details>
  <summary>Details</summary>
Motivation: 用户描述任务的方式与工具文档之间存在语义鸿沟，随着API生态扩展到数万个端点，仅依赖初始查询的静态检索无法弥合这一鸿沟：智能体在执行过程中对所需工具的理解会不断演变，但其工具集却是固定的。

Method: FitText框架包含三个核心机制：(1) 生成自然语言伪工具描述作为检索探针；(2) 利用检索反馈迭代优化这些描述；(3) 通过随机生成探索多样化的候选描述。Memetic Retrieval在候选描述上施加进化选择压力，并由工具记忆引导避免冗余搜索。

Result: 在ToolRet（43k工具，4个领域）上，平均检索排名从8.81提升至2.78；在StableToolBench（16,464个API）上，平均通过率达到0.73，相比静态查询检索绝对提升24个百分点。在强基座模型上效果可迁移，但在弱基座模型下，进化搜索会反转效果——放大噪声而非精炼信号。

Conclusion: 动态检索通过嵌入推理循环能有效弥合语义鸿沟，但Memetic进化搜索的效果高度依赖基座模型能力，模型能力是进化工具探索的前提条件。
...
### [324] [The Model Knows, the Decoder Finds: Future Value Guided Particle Power Sampling](https://arxiv.org/abs/2605.02427)
*Tu Nguyen,Rasul Tutunov,Xiaotong Ji,Matthieu Zimmer*

Main category: cs.AI

TL;DR: 本文提出APPS算法，通过粒子滤波和未来价值引导的块级重采样，高效逼近序列级幂采样目标，在无训练推理场景下显著提升推理准确率与运行时的平衡。


<details>
  <summary>Details</summary>
Motivation: 基础LLM在无训练推理时已为正确多步解分配非平凡概率质量，但瓶颈在于推理时高效定位这些模式；现有幂采样近似需考虑未来依赖的修正因子，缺乏高效且内存可控的实现。

Method: 提出APPS（Auxiliary Particle Power Sampling）：块级粒子算法，使用提议校正的幂重新加权并行传播假设，通过未来价值引导的选择在重采样边界优化假设存活；未来信号可基于短视展开或轻量学习选择头。

Result: 在推理基准上，APPS在准确率-运行时权衡上优于其他无训练解码方法；部分后训练系统的性能差距可通过更忠实的推理时幂近似恢复。

Conclusion: APPS通过有限粒子群和可预测峰值内存实现更精确的幂采样近似，为无训练推理提供了高效缩放工具；未来价值引导的重采样是提升序列级解码质量的关键机制。
...
### [325] [Measuring AI Reasoning: A Guide for Researchers](https://arxiv.org/abs/2605.02442)
*Munachiso Samuel Nwadike,Zangir Iklassov,Kareem Ali,Rifo Genadi,Kentaro Inui*

Main category: cs.AI

TL;DR: 本文提出评估语言模型推理能力应基于自适应多步搜索过程而非仅关注最终答案准确性，主张采用过程导向评估，将中间推理轨迹的忠实性与有效性作为首要评估目标。


<details>
  <summary>Details</summary>
Motivation: 当前评估语言模型推理能力时过度依赖最终答案准确性，这一指标无法反映模型是否真正进行了推理（即自适应、多步搜索过程），且难以诊断和调试前沿模型产生个体解决方案的底层过程。

Method: 作者首先从评估角度定义推理为一种需要根据输入条件选择中间步骤并决定何时停止的搜索过程；然后分析可扩展架构中单次前向传递在实现变深度计算方面的结构性限制；最后论证应转向基于过程的评估范式，将中间推理轨迹视为一阶评估目标。

Result: 文章指出单次前向传递难以实现推理所需的变深度计算，因此中间解码和外部化推理轨迹是合适的评估接口；最终答案准确性不足以衡量推理能力，过程导向的评估才是有效路径。

Conclusion: 语言模型的推理评估应从答案导向转向过程导向，通过评估中间推理轨迹的忠实性和有效性来真正衡量模型的推理能力，而非仅依赖最终答案的正确性。
...
### [326] [Position: How can Graphs Help Large Language Models?](https://arxiv.org/abs/2605.02452)
*Xiyuan Wang,Yi Hu,Yanbo Wang,Chuan Shi,Muhan Zhang*

Main category: cs.AI

TL;DR: 本文从三个视角探讨了图如何帮助大语言模型（LLMs）：提供更新的知识源以减少幻觉、通过图提示技术增强推理能力、提升对结构化数据的理解，并展望了基于图的稀疏LLM架构和类脑记忆系统等未来方向。


<details>
  <summary>Details</summary>
Motivation: 现有研究多关注LLMs如何助力图学习任务，本文提出互补性问题——图如何帮助LLMs，旨在探索图与LLMs双向赋能中尚未充分挖掘的方向。

Method: 从三个视角分析：1) 图作为最新知识源减少LLM幻觉；2) 基于图的提示技术（CoT、ToT、GoT）增强推理能力；3) 图结构集成提升LLM对结构化数据的理解。并展望未来研究方向。

Result: 图能够为LLMs提供实时更新的知识源以缓解幻觉问题；图提示技术有效提升推理能力；图结构集成扩展了LLMs在电商、代码、关系数据库等领域的适用性。

Conclusion: 图与LLMs之间存在双向赋能关系，图在增强LLMs的知识时效性、推理能力和结构化数据理解方面具有重要潜力，未来可探索基于图的稀疏LLM架构和类脑记忆系统。
...
### [327] [Shadow-Loom: Causal Reasoning over Graphical World Model of Narratives](https://arxiv.org/abs/2605.02475)
*David Wilmot*

Main category: cs.AI

TL;DR: Shadow-Loom 是一个开源框架，将叙事转化为版本化的图形世界模型，通过因果物理（基于Pearl的因果阶梯和反事实演算）和叙事物理（基于Sternberg传统计算四种读者状态：神秘、戏剧反讽、悬念和惊喜）两个引擎并行处理，LLM仅用于边界任务，核心因果与反事实推理由类型化代码在图结构上执行。


<details>
  <summary>Details</summary>
Motivation: 传统叙事理解依赖大规模语言模型进行端到端处理，缺乏结构化的因果推理和叙事张力形式化建模。作者旨在构建一个可解释、可干预的框架，将因果推断与叙事理论结合，实现对人如何阅读故事的结构化理解。

Method: 将叙事转化为版本化图形世界模型；采用Pearl的因果阶梯（关联、干预、反事实）进行因果物理建模，结合Ancestral Multi-World Networks上的反事实演算；叙事物理则对同一图结构计算四种读者状态（神秘、戏剧反讽、悬念、惊喜），其中悬念形式化借鉴结构-情感路线的工作；LLM仅用于边界任务（提取、渲染、审计），核心推理由类型化代码在图结构上执行。

Result: 开发了Shadow-Loom开源框架，实现了双引擎（因果物理+叙事物理）并行处理叙事图结构。提供了完整代码、测试数据和流水线，作为研究工具体而非NLP基准模型发布。

Conclusion: Shadow-Loom通过将因果推理与叙事理论形式化结合，为结构化叙事理解提供了可解释、可干预的研究范式，展示了LLM在复杂推理任务中的边界化使用路线。
...
### [328] [Efficient Temporal Datalog Materialisation for Composite Event Recognition](https://arxiv.org/abs/2605.02488)
*Periklis Mantenoglou*

Main category: cs.AI

TL;DR: 本文提出将多种事件规范语言映射到Temporal Datalog->-（一种带分层否定且无未来依赖的时序Datalog），并通过Streaming Trigger Graphs扩展实现高效的流推理，从而构建统一的复合事件识别机制。


<details>
  <summary>Details</summary>
Motivation: 现有事件规范语言通常被孤立研究，难以比较其表达能力，且对应的流推理器适用范围不明确，亟需统一框架以促进比较和泛化。

Method: 将实际使用的多种事件规范语言片段映射到Temporal Datalog->-；提出Streaming Trigger Graphs作为流推理引擎，扩展了现有Datalog物化技术以支持时序场景。

Result: 实现了统一的复合事件识别机制，能够泛化到多种主流事件规范语言。

Conclusion: 该统一框架有助于比较不同事件规范语言的表达能力，并扩展流推理器的适用范围。
...
### [329] [GRAIL: A Deep-Granularity Hybrid Resonance Framework for Real-Time Agent Discovery via SLM-Enhanced Indexing](https://arxiv.org/abs/2605.02489)
*Jinliang Xu*

Main category: cs.AI

TL;DR: GRAIL是一个新型智能体发现框架，通过SLM预测、伪文档扩展和MaxSim共振机制，在保持高精度的同时将发现延迟降低至400ms以下（相比LLM方案加速79倍以上）。


<details>
  <summary>Details</summary>
Motivation: 现有基于LLM的智能体发现方法延迟过高（常超30秒），而传统向量检索虽快但语义精度不足，亟需一种兼顾速度与精度的工业级解决方案。

Method: (1) 使用微调小语言模型进行毫秒级能力标签预测，替代通用LLM解析器；(2) 通过合成查询扩充智能体描述，增强语义密度；(3) 采用MaxSim共振机制，计算用户查询与离散智能体使用示例间的最大相似度，缓解语义稀释。

Result: 在AgentTaxo-9K数据集（9,240个智能体）上，GRAIL相比LLM基线降低超过79倍端到端延迟，同时在Recall@10上显著优于传统向量搜索。

Conclusion: GRAIL为大规模多智能体协作场景下的实时智能体发现提供了可扩展的工业级解决方案，支撑"智能体互联网"愿景。
...
### [330] [Improving Model Safety by Targeted Error Correction](https://arxiv.org/abs/2605.02544)
*Abolfazl Mohammadi-Seif,Ricardo Baeza-Yates*

Main category: cs.AI

TL;DR: 提出了一种基于双分类器GBDT流水线的后验纠错方法，用于区分常规人类类似错误与高风险非人类误分类，在三个医疗和动物领域数据集上实现了显著的安全改进，且推理延迟极低（<2%）。


<details>
  <summary>Details</summary>
Motivation: 机器学习在关键应用中的广泛采用需要减少高风险错误的技术，尤其是那些可能导致严重后果的非人类误分类。

Method: 采用双分类器GBDT（梯度提升决策树）流水线，区分常规人类类似错误与高风险非人类误分类，并采用保守纠错策略进行后验修正。

Result: 在动物品种分类、ISIC 2018皮肤病变诊断和SICAPv2前列腺组织病理学三个领域，推理延迟仅增加1.60%-1.84%；非人类错误减少12.57%-34.1%；超类诊断安全性分别提升至90.41%和92.13%。

Conclusion: 无需昂贵的模型重训练即可显著提升安全关键应用的可靠性，证明了后验纠错框架在可信AI部署中的实用性。
...
### [331] [Strategy-Aware Optimization Modeling with Reasoning LLMs](https://arxiv.org/abs/2605.02545)
*Ruiqing Zhao,Fengzhi Li,Yuan Zuo,Rui Liu,Yansong Liu,Yunfei Ma,Fanyu Meng,Junlan Feng*

Main category: cs.AI

TL;DR: SAGE是一个策略感知框架，通过在数据构建和后训练中显式建模策略，提升大语言模型生成优化程序的质量，在8个基准上将pass@1从72.7提升至80.3，并生成更紧凑的约束系统。


<details>
  <summary>Details</summary>
Motivation: 大语言模型虽然能生成语法正确的优化程序，但难以可靠地选择有效的建模策略，导致公式化不正确和求解器效率低下。

Method: SAGE构建了求解器验证的多策略数据集，通过监督微调配合分段加权GRPO（Segment-Weighted GRPO）训练学生模型，奖励函数综合考虑格式合规性、正确性和求解器效率。

Result: 在8个基准上，SAGE将平均pass@1从72.7提升至80.3；pass@16的组件级多样性提升19-29%；约束数量减少14.2%。

Conclusion: 显式建模策略能够提升自动化优化建模的质量。
...
### [332] [Double Rectified Linear Unit-based Modular Semantics for Quantitative Bipolar Argumentation Framework](https://arxiv.org/abs/2605.02551)
*Gianvincenzo Alfano,Sergio Greco,Lucio La Cava,Francesco Parisi,Irina Trubitsyna*

Main category: cs.AI

TL;DR: 提出了一种新的QBAF逐步语义，在满足理性公设的同时产生更符合直觉的结果，并证明了其在无环及更广泛循环框架中的收敛性。


<details>
  <summary>Details</summary>
Motivation: 现有QBAF语义在简单无环案例中常产生分歧或反直觉的结果。

Method: 引入一种新的逐步语义，该语义满足已有理性公设，并分析其收敛行为。

Result: 新语义产生更符合直觉的结果，满足理性公设，并在无环及更广循环框架中收敛。

Conclusion: 新语义有效克服了现有方法的局限，在理论上保证了更广泛的适用性。
...
### [333] [On Training Large Language Models for Long-Horizon Tasks: An Empirical Study of Horizon Length](https://arxiv.org/abs/2605.02572)
*Sunghwan Kim,Junhee Cho,Beong-woo Kwak,Taeyoon Kwon,Liang Wang,Nan Yang,Xingxing Zhang,Furu Wei,Jinyoung Yeo*

Main category: cs.AI

TL;DR: 本文系统研究了任务交互步长（horizon length）对LLM智能体训练动力学的影响，发现长步长会导致训练不稳定（探索困难与信用分配问题），并提出步长缩减（horizon reduction）作为关键原则，可稳定训练并实现跨步长泛化（horizon generalization）。


<details>
  <summary>Details</summary>
Motivation: 现有工作多关注系统级优化或算法改进，对任务交互步长如何塑造训练动力学缺乏理解，本文旨在填补这一空白。

Method: 通过受控任务构建，保持决策规则和推理结构相同，仅改变任务完成所需的行动序列长度，系统对比不同步长下的训练行为。

Result: 增加步长本身构成训练瓶颈，导致严重的训练不稳定，源于探索困难和信用分配挑战；步长缩减可稳定训练并在长步长任务上取得更好性能；缩减步长训练出的模型可泛化到更长步长任务（步长泛化）。

Conclusion: 步长缩减是应对长步长任务训练挑战的关键原则，并揭示了跨步长泛化这一有前景的现象。
...
### [334] [Universal Smoothness via Bernstein Polynomials: A Constructive Approximation Approach for Activation Functions](https://arxiv.org/abs/2605.02591)
*Wentao Zhang,Yutong Zhang,Yifan Zhu,Wentao Mo*

Main category: cs.AI

TL;DR: 本文提出基于伯恩斯坦多项式的BerLU激活函数，通过可微二次过渡区域消除奇异性，在保持分段线性结构的同时实现严格连续可微性和单位Lipschitz常数，在Vision Transformer和CNN架构上均优于现有基线方法。


<details>
  <summary>Details</summary>
Motivation: 现有激活函数在优化稳定性与计算效率之间存在权衡：分段线性函数（如ReLU）在原点不可导导致优化不稳定，而平滑函数（如GELU）依赖超越运算带来计算开销，缺乏能同时兼顾两者的方案。

Method: 基于构造逼近理论提出通用平滑框架，利用伯恩斯坦多项式构造可微二次过渡区域，设计Bernstein Linear Unit (BerLU)激活函数，在原点附近实现平滑过渡同时保持分段线性结构。

Result: 理论分析证明BerLU具有严格连续可微性和单位Lipschitz常数（非扩张性），确保梯度稳定传播并防止梯度爆炸；在Vision Transformer和CNN架构上的图像分类基准测试中，性能一致超越现有基线，同时具有更优的计算和内存效率。

Conclusion: BerLU通过巧妙的数学构造在优化稳定性与计算效率之间取得了理想平衡，为深度学习中的激活函数设计提供了新的有效范式。
...
### [335] [Counterfactual Reasoning in Automated Planning](https://arxiv.org/abs/2605.02603)
*Alberto Pozanco,Daniel Borrajo,Manuela Veloso*

Main category: cs.AI

TL;DR: 本文调研了自动规划中的反事实推理研究，根据改变的元素、触发时机、原因和方式对现有工作进行分类，并总结了关键发现和未来研究方向。


<details>
  <summary>Details</summary>
Motivation: 传统自动规划假设任务参数完全预先指定，但现实世界需要灵活性以应对不可预见情况或改善结果，因此需要研究反事实推理。

Method: 对现有反事实推理工作进行分类，分类维度包括：改变了哪些元素（初始状态、目标、可用动作）、推理何时触发、以及为何及如何进行这些改变。

Result: 形成了一套对自动规划中反事实推理的系统分类体系。

Conclusion: 总结了关键发现，并提出了该领域有待解决的开放研究问题，为未来研究提供指导。
...
### [336] [Trustworthy AI Suffers from Invariance Conflicts and Causality is The Solution](https://arxiv.org/abs/2605.02640)
*Ruta Binkyte,Ivaxi Sheth,Zhijing Jin,Mohammad Havaei,Bernhard Schölkopf,Mario Fritz*

Main category: cs.AI

TL;DR: 本文主张因果关系是理解和平衡可信AI中性能与多个目标（公平性、鲁棒性、隐私、可解释性）之间权衡的必要工具，将可信AI的权衡重新解释为数据生成过程不同变化下的不相容不变性要求，并提出通过选择性不变性来软化或解决这些权衡。


<details>
  <summary>Details</summary>
Motivation: 随着AI在高风险领域的广泛应用，确保其可信度成为核心挑战，但公平性、鲁棒性、隐私、可解释性等核心目标难以同时实现，尤其是在保持效用的情况下。现有方法缺乏统一框架来理解和平衡这些权衡。

Method: 本文采用立场论文的形式，通过将可信AI的权衡重新解释为数据生成过程中不同变化下的不相容不变性要求，论证因果关系作为统一框架的必要性，并讨论了因果假设在经典ML模型和大规模基础模型中的显式或隐式应用。

Result: 提出因果关系能够为理解可信AI中的权衡提供统一框架，并通过选择性不变性来软化或解决这些权衡。该视角既适用于经典ML模型，也适用于大规模基础模型。

Conclusion: 因果关系是构建更可信AI的关键工具，但仍存在开放挑战和机遇，包括如何在现代大规模系统中有效应用因果假设。
...
### [337] [AcademiClaw: When Students Set Challenges for AI Agents](https://arxiv.org/abs/2605.02661)
...
TL;DR: 本文提出 AcademiClaw，一个包含 80 个复杂长时程学术任务的 bilingual benchmark，覆盖 25+ 专业领域（含 16 个需 CUDA GPU 的任务），使用多维评分和安全审计评估前沿模型，最佳模型仅达 55% 通过率，揭示了模型在任务领域、行为策略等方面的精细能力边界。


<details>
  <summary>Details</summary>
Motivation: OpenClaw 生态现有的基准仅评估助手级任务，未检验学术级能力。需要从真实学术工作流中构建复杂、长时程的评测基准，以推动能处理真实学术需求的智能体发展。

Method: 从 230 个学生提交的候选任务中通过专家评审筛选出 80 个任务，覆盖 25+ 专业领域；每个任务在隔离 Docker 沙箱中执行，采用六种互补技术结合的多维评分细则打分，并配有独立的五类安全审计。

Result: 对六个前沿模型的评测显示，最佳模型仅达 55% 通过率。进一步分析揭示了任务领域间清晰的能力边界、模型间不同的行为策略，以及 token 消耗与输出质量之间的脱节。

Conclusion: AcademiClaw 及其开源数据与代码可为 OpenClaw 社区提供有用资源，推动开发更强大、更通用的学术智能体。
...
### [338] [An explainable hypothesis-driven approach to Drug-Induced Liver Injury with HADES](https://arxiv.org/abs/2605.02669)
*Maciej Wisniewski,Bartosz Topolski,Pawel Dabrowski-Tumanski,Dariusz Plewczynski,Tomasz Jetka*

Main category: cs.AI

TL;DR: 本文提出DILI预测应从二分类问题转向可解释的假设生成问题，并为此构建了DILER Benchmark数据集和HADES智能体系统，在提升分类性能的同时实现了机制性假设生成的基线。


<details>
  <summary>Details</summary>
Motivation: 现有DILI预测方法多采用二分类框架，泛化能力有限且无法提供机制性见解来指导转化决策，因此需要将DILI预测重构为可解释的假设生成问题。

Method: 构建DILER Benchmark数据集（扩展了分子及文献衍生的肝毒性机制假设标注），提出HADES智能体系统，融合分子级预测、代谢物分解、结构理解和毒性通路证据来生成透明可审计的推理轨迹。

Result: HADES在二分类任务上超越现有模型（Test Set ROC-AUC 0.68，Post-2021 Set 0.59 vs. DILI-Predictor的0.63和0.50）；在机制假设生成任务上实现Hypothesis Alignment Fuzzy Jaccard Index 0.16。

Conclusion: DILI预测作为可解释假设生成问题具有内在复杂性，现有成果虽建立基线但仍需更先进的可解释方法来推动预测毒理学发展。
...
### [339] [The 2026 ACII Dyadic Conversations (DaiKon) Workshop & Challenge](https://arxiv.org/abs/2605.02672)
...
TL;DR: ACII-DaiKon挑战赛提出了一个面向双人对话中人际情感与社会动力学建模的基准，包含三个子挑战：方向性人际影响预测、话轮转换预测和 rapport 轨迹预测，基于包含945段多语言自然对话的Hume-DaiKon数据集，基线实验表明当前方法能捕捉粗略的双人模式，但稳健建模方向性依赖和长期人际动力学仍具挑战性。


<details>
  <summary>Details</summary>
Motivation: 现有对话情感建模基准大多以说话者为中心，未能充分表征对话伙伴间耦合的、随时间演化的过程，包括方向性影响、对话时序协调和 rapport 发展，亟需一个关注双人交互动态的标准化基准。

Method: 基于Hume-DaiKon数据集（945段双人对话，743.4小时，5种语言），设置三个子挑战：(1)方向性人际影响预测，(2)话轮转换预测（下一说话者和下次说话时间），(3)完整交互的rapport轨迹预测。采用固定数据划分、标准化评估指标和发布的基线系统。

Result: 基线最佳结果：影响预测CCC 0.40、Pearson 0.50；话轮转换Macro-F1 0.66、MAE 1.50秒；rapport轨迹CCC 0.68、Pearson 0.70。表明当前方法能捕获粗略双人模式，但方向性依赖和长期人际动态建模仍有挑战。

Conclusion: ACII-DaiKon为双人交互建模提供了共享平台，促进严格比较和跨学科讨论，涵盖数据有效性、评估协议和文化感知建模等方面。
...
### [340] [Coherent Hierarchical Multi-Label Learning to Defer for Medical Imaging](https://arxiv.org/abs/2605.02734)
*Joshua Strong,Pramit Saha,Emma Sun,Helen Higham,Alison Noble*

Main category: cs.AI

TL;DR: 本文首次研究了层次化多标签决策下的学习型委托（L2D）问题，形式化了层次化委托的一致性条件，提出了精确一致性投影和基于信念传播的联合行动模型两种解决方案，在医学影像基准上有效消除了委托不一致性。


<details>
  <summary>Details</summary>
Motivation: 现有L2D方法假设平坦标签空间，但实际医学影像诊断中标签遵循临床分类层级结构。在层次化多标签场景下，将委托视为每个标签独立的决策会产生一系列不一致性问题，包括分类学矛盾、委托违规以及已隐含标签的冗余委托。

Method: 1) 形式化了Selective-Exclusion切换协议下的层次化一致性委托规则；2) 推导了贝叶斯最优的层次化一致性委托决策；3) 提出精确一致性投影（动态规划解码器）保证完全一致性；4) 提出Taxonomic Belief Propagation (TBP)与Recursive Policy Optimisation (RPO)联合行动模型，通过推理时相同的递归过程进行训练。

Result: 在真实读片者和受控专家医学影像基准上，朴素二元关联L2D存在不可忽视的不一致性。精确投影完全消除了不一致性，而TBP+RPO在保持强效用性的同时将不一致性降至接近零。

Conclusion: 层次化多标签L2D中的委托一致性是必要且可实现的，所提出的方法能够在不显著牺牲效用的前提下有效保证一致性。
...
### [341] [AI and Open-data Driven Scalable Solar Power Profiling](https://arxiv.org/abs/2605.02738)
*Shiliang Zhang,Sabita Maharjan,Damla Turgut*

Main category: cs.AI

TL;DR: 本文提出一个基于开放数据的可扩展框架，利用基础视觉AI模型从卫星图像中检测太阳能电池板几何形状，结合气象数据生成城市级太阳能发电曲线，并发布了相关数据和API。


<details>
  <summary>Details</summary>
Motivation: 太阳能光伏部署快速增长，但缺乏详细、最新的屋顶光伏空间分布和容量信息，现有方法依赖专有数据、人工标注和封闭模型，透明度与可扩展性不足。

Method: 使用基础视觉AI模型从开源卫星图像检测太阳能板几何形状，避免人工标注和特定训练；将检测结果转换为地理参考多边形，生成空间明确的增量式清单；结合开放气象数据将面板足迹转化为区域太阳能发电曲线。

Result: 成功开发并发布了包含数据和API的开放框架，API可根据用户指定的建筑位置检索航拍图像、检测屋顶太阳能板并返回地理参考多边形。

Conclusion: 该框架减少了对专有数据、人工标注和封闭模型的依赖，提供了透明、可扩展的太阳能规划分析方案，可支持分布式太阳能整合、局部潮流优化、电价设计和基础设施规划等高级分析。
...
### [342] [Foundation Models to Unlock Real-World Evidence from Nationwide Medical Claims](https://arxiv.org/abs/2605.02740)
...
TL;DR: 本文提出ReClaim——基于2亿人、438亿医疗事件的行政索赔数据训练的大型生成式Transformer。在1000+疾病-onset预测中达到平均AUC 75.6%，显著优于LightGBM（66.3%）和Delphi（69.4%），并在医疗支出预测和目标试验模拟中展示了更强的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 行政索赔数据虽具备大规模、纵向、涵盖诊疗和支出的特点，但作为医疗基础模型训练基质的潜力尚未被探索。

Method: 使用MarketScan行政索赔数据（2008-2022，2亿+人，438亿事件），训练来自零的生成式Transformer模型，参数规模拓展至1.4亿、7亿、17亿。

Result: 1）1000+疾病预测平均AUC 75.6%，优于LightGBM（66.3%）和Delphi（69.4%），罕见病增益最大；2）性能随模型规模单调提升；3）事后训练（post-training）相比预训练提升13.8个百分点；4）医疗支出预测解释方差从0.28提升至0.37；5）目标试验模拟中系统偏差降低72%。

Conclusion: 行政索赔数据可作为医疗基础模型的可扩展基质，学习到的表示能跨时间周期和数据源泛化，支持疾病监测、支出预测和真实世界证据生成。
...
### [343] [Triple Spectral Fusion for Sensor-based Human Activity Recognition](https://arxiv.org/abs/2605.02743)
*Ye Zhang,Longguang Wang,Qing Gao,Chaocan Xiang,Mohammed Bennamoun,Yulan Guo*

Main category: cs.AI

TL;DR: 本文提出了一种面向传感器人体活动识别（HAR）的三重谱融合框架（TSF），通过在傅里叶域、图傅里叶域和小波域自适应滤波，实现多模态IMU数据的噪声抑制、异构节点融合与上下文冗余压缩，在10个基准数据集上取得优异性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于学习的HAR方法在融合异构传感器数据与建立长期上下文关联方面存在挑战，尤其是从时间维度进行信息融合的复杂性尚未被有效解决。

Method: 提出三重谱融合框架：1）自适应互补滤波抑制噪声，将IMU传感器组织为姿态和运动模态节点；2）在动态异构图上进行图傅里叶域自适应滤波，融合同构与异构节点信息；3）自适应小波频率选择抑制上下文冗余并缩短特征长度，增强时间戳图聚合与长期上下文关联。

Result: 在10个公开基准数据集上的大量实验表明，所提框架相比现有方法具有优越的性能。

Conclusion: 提出的三重谱融合框架通过联合利用傅里叶、图傅里叶和小波三个频域的自适应滤波，有效实现了多传感器融合与上下文关联，为HAR任务提供了一种新的有效范式。
...
### [344] [Mitigating Misalignment Contagion by Steering with Implicit Traits](https://arxiv.org/abs/2605.02751)
*Maria Chang,Ronny Luss,Miao Lui,Keerthiram Murugesan,Karthikeyan Ramamurthy,Djallel Bouneffouf*

Main category: cs.AI

TL;DR: 论文发现语言模型在多轮多智能体交互中会出现"对齐传染"现象——模型在社交困境游戏后变得更反社会，并提出了"隐性特质引导"技术来缓解这一问题。


<details>
  <summary>Details</summary>
Motivation: 现有对齐研究集中在单用户-单模型交互，忽略了多模型多轮交互中错误对齐行为在模型间传播的风险。

Method: 通过多轮社交困境游戏实验，观察模型行为变化；提出"隐性特质引导"技术，间歇性向系统提示注入强化模型初始特质的陈述。

Result: 模型在游戏后变得更反社会，且对手被引导作恶时效应加剧；重复系统提示效果不足甚至有害；隐性特质引导更有效地维持模型亲社会行为。

Conclusion: 隐性特质引导是一种无需访问模型参数或内部状态的有效对齐维护方法，适用于黑盒模型的多智能体场景。
...
### [345] [U-Define: Designing User Workflows for Hard and Soft Constraints in LLM-Based Planning](https://arxiv.org/abs/2605.02765)
*Christine P Lee,Xinyu Jessica Wang,Aws Albarghouthi,David Porfirio,Bilge Mutlu*

Main category: cs.AI

TL;DR: 本文提出U-Define系统，通过让用户用自然语言定义硬约束（必须遵守）和软约束（灵活偏好），并分别采用形式化模型检测和LLM-as-judge验证机制，帮助用户更可靠地表达意图并引导LLM生成任务计划。技术评估和用户研究表明，用户定义的约束类型提升了感知有用性、性能和满意度。


<details>
  <summary>Details</summary>
Motivation: LLM被广泛用于终端用户任务规划，但其黑箱特性限制了用户确保可靠性和控制的能力。现有系统采用验证技术，但硬约束过于僵化，数值灵活性权重又让用户困惑，用户难以有效应用约束来表达意图或适应现实世界的变化。

Method: 提出U-Define系统，支持用户用自然语言定义约束并将其分类为硬约束（hard）和软约束（soft）。硬约束通过形式化模型检测验证，软约束通过LLM-as-judge评估。通过技术评估和包含普通用户与专家用户的用户研究进行验证。

Result: 用户定义的约束类型（硬/软分类）在感知有用性、性能和满意度方面均优于基线方法，同时保持了良好的可用性。

Conclusion: 将约束抽象为高层类型（硬/软）并配以不同的验证机制，能帮助用户更可靠地表达和对齐意图，为设计灵活且可靠的约束式工作流提供了见解。
...
### [346] [When Audio-Language Models Fail to Leverage Multimodal Context for Dysarthric Speech Recognition](https://arxiv.org/abs/2605.02782)
*Pehuén Moure,Niclas Pokel,Bilal Bounajma,Yingqiang Gao,Roman Boehringer,Longbiao Cheng,Shih-Chii Liu*

Main category: cs.AI

TL;DR: 本文评估了当前音频-语言模型在利用临床上下文信息（如诊断标签、临床评分和详细描述）改善构音障碍语音识别方面的能力。研究发现，现有模型无法有效利用这些提示信息，但通过基于临床提示格式的LoRA微调，词错误率可降低52%（WER降至0.066），尤其在唐氏综合征和轻度障碍群体中效果显著。


<details>
  <summary>Details</summary>
Motivation: 构音障碍及其他非典型语音的自动语音识别（ASR）系统仍然脆弱。近期音频-语言模型有望通过引入临床上下文信息来提升性能，但尚不清楚这些模型是否能有效利用这类信息。

Method: 基于Speech Accessibility Project (SAP)数据集构建基准测试，在9个模型上进行匹配对比，评估诊断标签、临床语音评分和逐步丰富的临床描述对转录准确性的影响。同时进行上下文相关的LoRA微调实验，混合多种临床提示格式进行适配。

Result: 当前模型无法有效利用临床上下文信息，诊断信息和临床提示仅带来微不足道的改进甚至恶化WER。而LoRA微调在混合临床提示格式下实现WER 0.066，相比冻结基线降低52%，且在无上下文时保持性能不变。唐氏综合征和轻度障碍亚组获得显著提升。

Conclusion: 研究明确了当前模型在利用临床上下文方面的不足，为构建更具包容性的ASR系统提供了测试平台和方法方向——提示工程本身效果有限，但上下文相关的微调能够有效利用临床信息。
...
### [347] [AIs and Humans with Agency](https://arxiv.org/abs/2605.02810)
*David Mumford*

Main category: cs.AI

TL;DR: 本文比较了人类能动性与AI程序的潜在能动性，指出人类能动性需多年发育（额叶激活），而赋予LLM能动性的早期尝试遇到严重障碍，提出需要新架构使行动和计划与人类行为者在每个真实世界场景中联合制定。


<details>
  <summary>Details</summary>
Motivation: 探索如何赋予AI程序（尤其是LLM）类似人类的能动性，并识别当前方法的局限性。

Method: 比较分析人类能动性发育过程（额叶激活）与LLM能动性赋予的早期尝试，提出人机联合制定行动和计划的新架构。

Result: 早期赋予LLM能动性的尝试遇到严重障碍，表明当前架构存在根本性不足。

Conclusion: 需要开发新架构，使AI的行动和计划与人类行为者在具体现实场景中联合制定。
...
### [348] [SCPRM: A Schema-aware Cumulative Process Reward Model for Knowledge Graph Question Answering](https://arxiv.org/abs/2605.02819)
*Jiujiu Chen,Yazheng Liu,Sihong Xie,Hui Xiong*

Main category: cs.AI

TL;DR: 针对知识图谱推理中过程奖励模型的风险补偿效应（错误步骤被后续正确步骤补偿），提出架构感知累积过程奖励模型（SCPRM），通过推理前缀条件化和架构距离约束来评估推理路径，并集成到MCTS中形成SCPRM-MCTS，在医学、法律等风险敏感场景的KGQA任务上提升了推理效果。


<details>
  <summary>Details</summary>
Motivation: 现有过程奖励模型在知识图谱推理中存在风险补偿效应——错误步骤被后续正确步骤掩盖，导致对错误推理路径赋高分。这在医学、法律等风险敏感场景下尤为危险。

Method: 提出架构感知累积过程奖励模型（SCPRM）：(1) 以推理前缀为条件进行奖励评估；(2) 引入当前推理步骤与查询隐含目标之间的架构距离，提供累积和未来奖励来引导路径探索；进一步将SCPRM集成到蒙特卡洛树搜索（MCTS）中形成SCPRM-MCTS，进行多跳KG推理。

Result: 在医学、法律KGQA和CWQ数据集上，SCPRM-MCTS在Hits@k指标上平均比强基线提升1.18%，实现了更准确且风险敏感的推理评估。

Conclusion: SCPRM通过架构感知的累积奖励机制有效缓解了风险补偿效应，提升了风险敏感场景下知识图谱推理的可靠性。
...
### [349] [Compress Then Adapt? No, Do It Together via Task-aware Union of Subspaces](https://arxiv.org/abs/2605.02829)
*Jingze Ge,Yun Liu,Xue Geng,Wanqi Dong,Wang Zhe Mark,Min Wu,Xulei Yang*

Main category: cs.AI

TL;DR: JACTUS提出联合压缩与适配的统一框架，通过任务感知子空间正交合并、全局秩分配和紧凑核心矩阵训练，在80%参数保留下超越100%参数的高效微调方法，在视觉和语言任务上均取得SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法将参数高效微调（PEFT）与低秩压缩解耦为串行流程，先压缩后微调会导致压缩子空间与下游任务目标错配，浪费全局参数预算。

Method: 从校准集估计输入和预激活梯度协方差，与预训练权重子空间正交合并；在该合并子空间内进行投影低秩近似；按边际增益全局分配秩；仅训练紧凑核心矩阵。

Result: 视觉：ViT-Base在8个数据集上80%参数保留率达89.2%准确率，超越100%参数DoRA（87.9%）；语言：Llama2-7B常识QA上80%参数保留率达80.9%，超越100%参数DoRA（79.7%）。

Conclusion: JACTUS通过联合压缩与适配解决子空间错配问题，在更低参数预算下取得更优性能，同时避免保留完整冻结权重，实现快速鲁棒调优。
...
### [350] [HAAS: A Policy-Aware Framework for Adaptive Task Allocation Between Humans and Artificial Intelligence Systems](https://arxiv.org/abs/2605.02832)
*Vicente Pelechanoa,Antoni Mestre,Manoli Albert,Miriam Gil*

Main category: cs.AI

TL;DR: 本文提出HAAS框架，一种结合规则专家系统与上下文Bandit学习器的自适应人机任务分配方法，通过五个可审计认知维度和五级自主性光谱实现柔性治理，并在软件工程和制造领域验证了治理作为可调设计变量、工作量缓冲效应及无单一最优治理策略三个核心发现。


<details>
  <summary>Details</summary>
Motivation: 现有方法将人机任务分配视为二元选择，忽视了实际运营中人与AI共享任务、根据情境互补协作的丰富现实，缺乏一个可自适应调节、可审计的治理框架来平衡效率、监督与人类能力。

Method: HAAS框架包含两个耦合组件：1）基于规则的专家系统，在学习之前强制执行治理约束；2）上下文Bandit学习器，从结果反馈中选择可行的协作模式。任务-智能体适配通过五个可审计认知维度和五级自主性光谱（从纯人工到全自主）表征，并在软件工程和制造领域构建了可复现基准。

Result: 三个实证发现：1）治理是可调设计变量而非二元开关，更紧的约束将自主AI分配转为监督协作，带来领域特定的成本与收益；2）在制造领域，更强治理可同时提升运营绩效并降低疲劳（工作量缓冲效应）；3）无单一治理设置在所有情境下最优，中等治理随学习器经验积累变得更具竞争力。

Conclusion: HAAS可作为部署前的工作台，用于在组织承诺前比较和审查人机分配策略，支持对治理效果的可审计验证。
...
### [351] [Standing on the Shoulders of Giants: Stabilized Knowledge Distillation for Cross--Language Code Clone Detection](https://arxiv.org/abs/2605.02860)
*Mohamad Khajezade,Fatemeh H. Fard,Mohamed Sami Shehata*

Main category: cs.AI

TL;DR: 本文提出一种知识蒸馏框架，将DeepSeek-R1的推理能力迁移至紧凑型开源模型（Phi3、Qwen-Coder），结合响应稳定化方法（强制结论提示、二分类头、对比分类头），提升跨语言代码克隆检测的可靠性与实用性，在Python-Java等四组语言对上取得一致改进。


<details>
  <summary>Details</summary>
Motivation: 现有大语言模型用于跨语言代码克隆检测存在成本高、可重复性差、隐私风险、输出格式不可靠等问题，紧凑型开源模型难以遵循推理导向提示且输出无法稳定映射为二分类标签。

Method: (1) 从Project CodeNet构建推理导向的合成训练数据；(2) 使用LoRA微调Phi3和Qwen-Coder；(3) 提出三种响应稳定化策略：强制结论提示、二分类头、对比分类头；(4) 在Python-Java、Rust-Java、Rust-Python、Rust-Ruby四组语言对上进行评估。

Result: 知识蒸馏一致提升紧凑型模型的可靠性，在分布偏移场景下尤其改善预测性能；分类头变体相比生成式推理大幅减少推理时间。

Conclusion: 推理导向蒸馏结合响应稳定化使紧凑型开源模型在跨语言代码克隆检测任务上更实用、更可靠。
